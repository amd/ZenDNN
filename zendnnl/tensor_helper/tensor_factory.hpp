...
# *******************************************************************************/
#ifndef ZENDNNL_TENSOR_FACTORY_HPP
#define ZENDNNL_TENSOR_FACTORY_HPP
...
namespace zendnnl {
/** @namespace zendnnl::tensor_helper
 *  @brief Helpers for constructing test and example tensors.
 */
namespace tensor_helper {
using tensor_t = zendnnl::interface::tensor_t;
...
public:
    ...
    /** @brief uniform distributed tensor with aligned size */
    tensor_t uniform_dist_strided_tensor(const std::vector<index_type> size_,
            ...
            float range_, std::string tensor_name_ = "aligned uniform dist",
            tensor_t scale = tensor_t(), tensor_t zp = tensor_t());
