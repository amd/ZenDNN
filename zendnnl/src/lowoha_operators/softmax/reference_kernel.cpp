...
// (softmin(x) == softmax(-x)).
static void softmax_reference_impl(const void *input, void *output,
        const softmax_params &params, int num_threads) {
...
status_t softmax_reference_wrapper(
    ...
    int normalized_axis
            = params.axis >= 0 ? params.axis : params.ndims + params.axis;
    ...
    for (int i = 0; i < params.ndims; ++i) {
        if (i != normalized_axis) { params.batch *= params.shape[i]; }
    }
    params.axis_dim = params.shape[normalized_axis];

    log_info("Softmax Reference: ", params.ndims,
            "D tensor, flattened to batch=", params.batch,
