...
inline bool prepack_extras_metadata_undersized(
        ...
        const std::vector<bool> &is_weights_const, size_t total_matmul,
...
// ───────────────────────────────────────────────────────────────────────
status_t validate_group_matmul_direct_inputs(const std::vector<char> &layout,
    ...
    const size_t num_ops = framework_opt_in
    ...
    auto run_detect = [&](const std::vector<void *> &v, const char *name,
        ...
        const status_t st = detect_internal_alloc(v, num_ops,
                /*fused_moe_present=*/(fused_moe != nullptr),
                internal_alloc_mode::sweep_active, out_internal);
        if (st != status_t::success) {
            log_error("group_matmul_direct: fused_moe ", name,
                    " has a mixed "
    ...
    if (fused_moe != nullptr) {
        if (run_detect(fused_moe->dst_down, "dst_down", &fused_op2_internal)
                != status_t::success) {
            return status_t::failure;
        }
    }
    ...
    // `[0, num_ops)` only.
    const size_check_ctx size_ctx {/*num_ops=*/num_ops,
    ...
    if (size_bad(N.size()) || size_bad(K.size()) || size_bad(weight.size())
        ...
        log_error(
                "group_matmul_direct: vector size mismatch, num_ops=", num_ops);
        return status_t::failure;
    }
    ...
    // disabled.
    if (size_ctx.relaxed && params[0].total_matmul > params[0].active_matmul) {
        undersized_info first_failure {};
        if (prepack_extras_metadata_undersized(weight, K, N, ldb, transB,
                    is_weights_const,
    ...
    if (fused_op1_internal) {
        if (!dst.empty() && size_bad(dst.size())) {
            log_error(
                    "group_matmul_direct: fused_moe Op1 internal-alloc "
                    "requires "
    ...
    if (fused_op2_internal) {
        if (!fused_moe->ldc_down.empty()
            ...
            log_error(
                    "group_matmul_direct: fused_moe internal-alloc requires "
                    "ldc_down to be empty or sized to num_ops; got "
                    "ldc_down.size()=",
    ...
    if (moe_postop != nullptr && src.size() == 1 && !single_expert_parallel) {
        log_error(
                "group_matmul_direct: moe_postop is only supported in parallel "
                "mode");
        return status_t::failure;
    }
    if (gated_act != nullptr && gated_act->act != grp_matmul_gated_act_t::none
            && src.size() == 1 && !single_expert_parallel) {
        log_error(
                "group_matmul_direct: gated_act is only supported in parallel "
                "mode");
        return status_t::failure;
    }
    if (fused_moe != nullptr && src.size() == 1 && !single_expert_parallel) {
        log_error(
                "group_matmul_direct: fused_moe is only supported in parallel "
                "mode");
        return status_t::failure;
    }
    if (fused_moe != nullptr && fused_moe->down_weight.empty()) {
        log_error(
                "group_matmul_direct: fused_moe is non-null but down_weight is "
    ...
    if (src.size() == 1 && !single_expert_parallel) {
        ...
        for (size_t i = 0; i < num_ops; ++i) {
            if (weight[i] == nullptr || dst[i] == nullptr) {
                log_error("group_matmul sequential: null pointer at operation ",
                        i);
                return status_t::failure;
            }
            if (M[i] <= 0 || N[i] <= 0 || K[i] <= 0) {
                log_error(
                        "group_matmul sequential: invalid dimensions at "
                        "operation ",
                        i, ": M=", M[i], ", N=", N[i], ", K=", K[i]);
                return status_t::failure;
            }
        ...
        for (size_t i = 1; i < num_ops; ++i) {
            if (M[i] != M[0]) {
                log_error(
                        "group_matmul sequential: M must be constant across "
                        "layers, "
                        "M[0]=",
                        M[0], ", M[", i, "]=", M[i]);
                return status_t::failure;
            ...
            if (K[i] != N[i - 1]) {
                log_error(
                        "group_matmul sequential: dimension mismatch at layer ",
                        i, ": K[", i, "]=", K[i], " != N[", i - 1,
                        "]=", N[i - 1]);
                return status_t::failure;
            }
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        if (M[i] < 0 || N[i] <= 0 || K[i] <= 0) {
            log_error("group_matmul parallel: invalid dimensions at operation ",
                    i, ": M=", M[i], ", N=", N[i], ", K=", K[i]);
            return status_t::failure;
        }
        if (M[i] > 0) {
            if (src[i] == nullptr || weight[i] == nullptr) {
                log_error(
                        "group_matmul parallel: null src/weight at active "
                        "operation ",
                        i);
                return status_t::failure;
            }
            if (!fused_op1_internal && dst[i] == nullptr) {
                log_error(
                        "group_matmul parallel: null dst at active operation ",
                        i);
                return status_t::failure;
            }
        }
    ...
    if (run_gated_act) {
        ...
        if (act_dtype != data_type_t::f32 && act_dtype != data_type_t::bf16
            ...
            log_error(
                    "group_matmul_direct: gated_act requires f32, bf16, or f16 "
                    "dst");
            return status_t::failure;
        }
        for (size_t i = 1; i < num_ops; ++i) {
            if (params[i].dtypes.dst != act_dtype) {
                log_error(
                        "group_matmul_direct: gated_act requires uniform dst "
                        "dtype");
                return status_t::failure;
            }
        }
    ...
    if (run_gated_act || fused_moe != nullptr) {
        for (size_t i = 0; i < num_ops; ++i) {
            if (layout[i] != 'r' && layout[i] != 'R') {
                log_error(
                        "group_matmul_direct: gated_act/fused_moe requires "
    ...
    if (fused_moe != nullptr) {
        ...
        if (fused_op2_internal) {
            ...
            for (size_t i = 0; i < dd_sweep; ++i) {
                if (fused_moe->dst_down[i] != nullptr) {
                    log_error(
                            "group_matmul_direct: fused_moe Op2 internal-alloc "
        ...
        for (size_t i = 0; i < num_ops; ++i) {
            ...
            const grp_matmul_gated_act_t act_for_op2 = run_gated_act
                    ? gated_act->act
        ...
        if (moe_postop != nullptr) {
            for (size_t i = 1; i < num_ops; ++i) {
                if (fused_moe->N_down[i] != fused_moe->N_down[0]) {
                    log_error(
                            "group_matmul_direct: fused_moe + moe_postop "
                            "requires "
                            "uniform N_down, N_down[0]=",
                            fused_moe->N_down[0], " N_down[", i,
                            "]=", fused_moe->N_down[i]);
                    return status_t::failure;
                }
            }
    ...
    if (moe_postop != nullptr) {
        for (size_t i = 1; i < num_ops; ++i) {
            if (N[i] != N[0]) {
                log_error(
                        "group_matmul_direct: moe_postop requires identical N "
                        "across experts");
                return status_t::failure;
            }
            if (params[i].dtypes.dst != params[0].dtypes.dst) {
                log_error(
                        "group_matmul_direct: moe_postop requires identical "
                        "dst "
...
status_t group_matmul_direct(const std::vector<char> &layout,
    ...
    const size_t num_ops = (params[0].active_matmul > 0)
            ? static_cast<size_t>(params[0].active_matmul)
            : num_ops_input;
    const bool single_expert_parallel = (src.size() == 1 && num_ops == 1);
    ...
    if (any_f16_operand
        ...
        log_error(
                "group_matmul_direct: F16 data type is not supported on "
    ...
    if (src.size() == 1 && !single_expert_parallel) {
        if (moe_postop != nullptr) {
            log_error(
                    "group_matmul_direct: moe_postop is only supported in "
        ...
        if (gated_act != nullptr
                && gated_act->act != grp_matmul_gated_act_t::none) {
            log_error(
                    "group_matmul_direct: gated_act is only supported in "
                    "parallel mode; got a multi-op sequential chain "
                    "(src.size() == 1, num_ops > 1).  Either enable parallel "
        ...
        if (fused_moe != nullptr) {
            log_error(
                    "group_matmul_direct: fused_moe is only supported in "
                    "parallel mode; got a multi-op sequential chain "
...

    {
        ...
        detect_internal_alloc(dst, /*num_ops=*/no,
                /*fused_moe_present=*/(fused_moe != nullptr),
                internal_alloc_mode::quick_o1, &inline_op1_internal);
        if (fused_moe != nullptr) {
            detect_internal_alloc(fused_moe->dst_down, /*num_ops=*/no,
        ...
        if (inline_size_bad(N.size()) || inline_size_bad(K.size())
                || inline_size_bad(weight.size()) || inline_size_bad(lda.size())
                || inline_size_bad(ldb.size()) || inline_size_bad(layout.size())
                || inline_size_bad(transA.size())
                || inline_size_bad(transB.size())
                || inline_size_bad(alpha.size()) || inline_size_bad(beta.size())
                || inline_size_bad(bias.size())
        ...
        if (!inline_op1_internal
                && (inline_size_bad(dst.size())
                        || inline_size_bad(ldc.size()))) {
            return status_t::failure;
        }
    ...
    status_t val = op_instrumentation::validate([&]() {
        return validate_group_matmul_direct_inputs(layout, transA, transB, M, N,
                K, alpha, src, lda, weight, ldb, bias, beta, dst, ldc,
    ...
    if (src.size() == 1 && !single_expert_parallel) {
        ...
        for (size_t i = 0; i < num_ops; ++i) {
            ...
            matmul_algo_t kernel = kernel_select(exec_params[i], bp.Batch_A,
                    bp.Batch_B, 1, M[i], N[i], K[i], num_threads, bias[i],
                    is_weights_const[i], transB[i]);

            matmul_execute(layout[i], transA[i], transB[i], M[i], N[i], K[i],
                    alpha[i], cur_src, cur_lda, weight[i], ldb[i], bias[i],
                    beta[i], dst[i], ldc[i], is_weights_const[i],
                    size_of(exec_params[i].dtypes.src),
                    size_of(exec_params[i].dtypes.dst), num_threads, kernel,
                    exec_params[i], bp, auto_version);
        }
        ...
        const bool run_gated_act = (gated_act != nullptr
                && gated_act->act != grp_matmul_gated_act_t::none);
        const data_type_t act_dtype
                = run_gated_act ? params[0].dtypes.dst : data_type_t::none;
        ...
        if (fused_moe != nullptr) {
            ...
            if (fused_any_ggml) {
                const grp_matmul_gated_act_t fused_act = run_gated_act
                        ? gated_act->act
                        : grp_matmul_gated_act_t::none;
                ...
                for (size_t i = 0; i < num_ops; ++i) {
                    if (fused_params_unpacked[i].packing.pack_format_b != 1)
                        continue;
                    ...
                    const bool warmable = fused_weight_unpacked[i] != nullptr
                            && fused_moe_unpacked.down_weight[i] != nullptr
                            && is_weights_const[i] && transB[i];
                    if (!warmable) {
                        if (!active)
                            continue; // cold expert with no usable weight
                        // Active expert MUST be unpackable — surface the precise reason.
                        if (validate_ggml_packed_inputs(
                                    fused_params_unpacked[i],
                                    is_weights_const[i], 1, transB[i])
                                != status_t::success) {
                            return status_t::failure;
                        }
                    ...
                    status_t un1 = unpack_ggml_weights_and_cache(w1, N[i], K[i],
                            ldb[i], transB[i] ? 't' : 'n',
                    ...
                    status_t un2 = unpack_ggml_weights_and_cache(w2,
                            fused_moe_unpacked.N_down[i], k_down,
                            fused_moe_unpacked.ldb_down[i],
                            transB[i] ? 't' : 'n', w2_params,
                            fused_ggml_skip_reorder);
            ...
            status_t fused_st = group_matmul_fused_moe_execute(*fused_moe_eff,
                    run_gated_act ? gated_act->act
                                  : grp_matmul_gated_act_t::none,
                    act_dtype, layout, transA, transB, M_eff, N, K, alpha, src,
                    lda, *fused_weight_eff, ldb, bias, beta, dst, ldc,
                    is_weights_const, *fused_params_eff, num_threads,
                    &gemm_mode, moe_postop);
            if (fused_st != status_t::success) { return fused_st; }
        } else {
            ...
            if (any_ggml) {
                ...
                for (size_t i = 0; i < num_ops; ++i) {
                    ...
                    if (active) {
                        ...
                        const bool warmable = weight_unpacked[i] != nullptr
                    ...
                    status_t un = unpack_ggml_weights_and_cache(w, N[i], K[i],
                            ldb[i], transB[i] ? 't' : 'n', exec_params[i],
                            ggml_skip_reorder);
            ...
            const bool act_fused = group_matmul_run_parallel_dispatch(layout,
                    transA, transB, M_eff, N, K, alpha,
                    group_quantized ? quantized_src : src,
                    group_quantized ? quantized_lda : lda, *weight_eff, ldb,
                    bias, beta, dst, ldc, is_weights_const, exec_params,
