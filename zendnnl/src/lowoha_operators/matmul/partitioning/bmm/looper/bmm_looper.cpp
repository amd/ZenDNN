...
static void run_parallel_zendnnl(const void *src, const void *weight, void *dst,
        const BmmConfig &config, matmul_batch_params_t &batch_params,
        int M_block, const bmm_tile_callback_t &callback) {

    apilog_info("Using zendnnl_parallel_for");

    int64_t total_m_blocks = (config.M + M_block - 1) / M_block;
    int64_t total_work_items
            = static_cast<int64_t>(config.batch_count) * total_m_blocks;

    zendnnl_parallel_for(
            0, total_work_items, 1, [&](int64_t start_idx, int64_t end_idx) {
...
static void run_parallel_omp(const void *src, const void *weight, void *dst,
        const BmmConfig &config, matmul_batch_params_t &batch_params,
        int M_block, const bmm_tile_callback_t &callback) {
...
static void execute_partitioned(const void *src, const void *weight, void *dst,
        const void *bias, BmmConfig &config,
        matmul_batch_params_t &batch_params, matmul_params &params, char layout,
        char trans_input, char trans_weight, bool transA, float alpha,
        float beta, int lda, int ldb, int ldc, size_t src_type_size,
        size_t out_type_size, bool is_weights_const) {
    ...
    auto process_tile
            = [&](int batch_idx, int m_start, int m_len, const uint8_t *src_ptr,
                      const uint8_t *weight_ptr, uint8_t *dst_ptr) {
        bmm_tile_execute(batch_idx, m_start, m_len, src_ptr, weight_ptr,
                dst_ptr, ctx, params, batch_params);
    };

    apilog_info(
            "Executing BMM LOWOHA kernel with parallel partitioning, algo: ",
            static_cast<int>(config.kernel));
    ...
    if (plan.use_zendnnl_parallel) {
        ...
        run_parallel_omp(src, weight, dst, config, batch_params, plan.M_block,
                process_tile);
    }
...
void bmm_execute(const char layout, const bool transA, const bool transB,
    ...
    size_t weight_batch_stride_bytes
            = weight_batch_stride_elems * weight_type_size;
    ...
    if (kernel == matmul_algo_t::batched_sgemm) {
        ...
        matmul_batch_gemm_wrapper(layout, trans_input, trans_weight, M, N, K,
                alpha, src, lda, weight, ldb, beta, dst, ldc, params.dtypes,
                batch_count, batch_params.Batch_A, batch_params.Batch_B,
                params.mem_format_a, params.mem_format_b,
                src_batch_stride_bytes, weight_batch_stride_bytes,
                dst_batch_stride_bytes, params, bias, num_threads);
        return;
#endif
    }
    ...
    if (kernel == matmul_algo_t::onednn
        ...
        matmul_onednn_wrapper(trans_input, trans_weight, M, N, K, alpha, src,
                ...
                kernel, src_batch_stride_elems, weight_batch_stride_elems,
    ...
    if (num_threads > 1) {
        ...
        execute_partitioned(src, weight, dst, bias, config, batch_params,
    ...
    // ── Path 4: Single-thread fallback ──
        ...
        apilog_info(
                "Executing BMM LOWOHA kernel without zendnnl-partitioner, "
                "algo: ",
                static_cast<int>(kernel));
        ...
        for (int b = 0; b < batch_count; ++b) {
            ...
            uint8_t *dst_ptr
                    = static_cast<uint8_t *>(dst) + b * dst_batch_stride_bytes;
