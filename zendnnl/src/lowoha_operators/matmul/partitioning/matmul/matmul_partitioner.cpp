...
brgemm_blocking_params_t compute_brgemm_blocking(
    ...
    if (bp.weight_reuse) {
        bp.k_blocks_per_reduce
                = std::min(BRGEMM_K_BLOCKS_PER_REDUCE, bp.num_k_blocks);
    } else {
    ...
    bp.k_blocks_reduce_rem = (bp.k_blocks_per_reduce > 0)
            ...
            : 0;
    bp.loop_scheme
            = bp.weight_reuse ? LOOP_SCHEME_REUSE : LOOP_SCHEME_STREAMING;
...
static matmul_algo_t select_partition_kernel(const char trans_input,
    ...
    if (kernel == matmul_algo_t::libxsmm_blocked && !is_weights_const) {
        apilog_info(
                "LibXSMM blocked kernel is not supported for non-constant "
                "weights, falling back to LibXSMM");
    ...
    if (kernel == matmul_algo_t::libxsmm_blocked) {
        if ((config.N % BRGEMM_N_BLOCK != 0)
                || (config.K % BRGEMM_K_BLOCK != 0)) {
            apilog_info(
                    "LibXSMM blocked kernel requires N and K to be multiples "
                    "of nc and kc. Falling back to libxsmm.");
            return matmul_algo_t::libxsmm;
        }
    }
 ...
 */
static void apply_tile_postops(int m_start, int n_start, int m_len, int n_len,
        int ldc, void *C_tile, const matmul_params &params,
        ...
        if ((tile_po.po_type == post_op_type_t::binary_add
            ...
            if (tile_po.dims.size() == 2 && tile_po.dims[0] == 1) {
                ...
                offset = compute_postop_offset(
                        0, n_start, tile_po.leading_dim, tile_po.dtype);
            } else {
                offset = compute_postop_offset(
                        m_start, n_start, tile_po.leading_dim, tile_po.dtype);
            }
            tile_po.buff
                    = apply_offset(const_cast<void *>(tile_po.buff), offset);
        }

        if (dtypes.dst == data_type_t::f32) {
            libxsmm_postop<float>(m_len, n_len, ldc, C_tile, tile_po);
        } else {
            libxsmm_postop<libxsmm_bfloat16>(
                    m_len, n_len, ldc, C_tile, tile_po);
        }
    }
}
 ...
 */
static const void *libxsmm_weight_prepack(const void *weight, int M, int K,
        int N, int ldb, bool transB, int k_block_size, int n_block_size,
        data_type_t src_dtype, bool is_weights_const,
        blocked_brgemm_params_t &bp) {

    apilog_info("libxsmm_weight_prepack: K=", K, " N=", N,
            " k_block_size=", k_block_size, " n_block_size=", n_block_size,
            " is_weights_const=", is_weights_const);

    if (!is_weights_const) { return nullptr; }

    if (K % k_block_size != 0 || N % n_block_size != 0) { return nullptr; }

    int num_k_blocks = K / k_block_size;
    int num_n_blocks = N / n_block_size;

    if (num_k_blocks <= 0 || num_n_blocks <= 0) { return nullptr; }
    ...
    if (bp.weight_reuse) {
        bp.k_blocks_per_reduce
                = std::min(BRGEMM_K_BLOCKS_PER_REDUCE, num_k_blocks);
    } else {
        bp.k_blocks_per_reduce = num_k_blocks;
    }
    bp.k_blocks_reduce_rem = (bp.k_blocks_per_reduce > 0)
            ? (num_k_blocks % bp.k_blocks_per_reduce)
            : 0;
    bp.loop_scheme
            = bp.weight_reuse ? LOOP_SCHEME_REUSE : LOOP_SCHEME_STREAMING;
    ...
    Key_matmul key(transB, static_cast<unsigned int>(K),
            ...
            weight, static_cast<uint32_t>(matmul_algo_t::libxsmm_blocked));
...
static std::unordered_map<brgemm_cache_key_t, BrgemmCachedContext,
        brgemm_cache_key_t::hash>
        brgemm_ctx_cache;
static std::mutex brgemm_ctx_mutex;

static const BrgemmCachedContext &get_brgemm_context(char trans_input,
        char trans_weight, const matmul_partition_config_t &config) {
...

    {
        ...
        if (it != brgemm_ctx_cache.end()) { return it->second; }
    }

    BrgemmCachedContext ctx;
    ctx.bp = compute_brgemm_blocking(config);
    ctx.pd = predispatch_brgemm_kernels(trans_input, trans_weight,
            ctx.bp.m_block_size, ctx.bp.m_block_rem, ctx.bp.n_block_size,
            ctx.bp.n_block_rem, ctx.bp.k_block_size, ctx.bp.k_block_rem,
            ctx.bp.num_k_blocks, ctx.bp.k_blocks_per_reduce, config.lda,
            config.ldb, config.ldc, ctx.bp.stride_a, ctx.bp.stride_b,
            config.dtypes);
...
static const PreDispatchedBrgemm &get_brgemm_context_blocked(
...

    {
        std::lock_guard<std::mutex> lock(brgemm_ctx_mutex);
        auto it = brgemm_ctx_cache.find(key);
        if (it != brgemm_ctx_cache.end()) { return it->second.pd; }
    }

    bool vnni = (config.dtypes.src == data_type_t::bf16);
    unsigned long long blk_stride_a
            = static_cast<unsigned long long>(bp.k_block_size)
            * config.src_type_size;
    unsigned long long blk_stride_b
            = static_cast<unsigned long long>(bp.n_block_size) * bp.k_block_size
            * config.src_type_size;

    BrgemmCachedContext ctx {};
    ctx.pd = predispatch_brgemm_kernels('N', 'N', bp.m_block_size,
            bp.m_block_rem, bp.n_block_size, 0, bp.k_block_size, 0,
            bp.num_k_blocks, bp.k_blocks_per_reduce, config.lda,
            bp.n_block_size, config.ldc, blk_stride_a, blk_stride_b,
            config.dtypes, vnni);
    ...
    auto tile_body = [&](int k_off, int m_off, int n_off)
        ...
        if (k_off == 0) {
            const void *tile_bias
                    = compute_bias_offset(bias, n_off, config.dtypes.bias);
            init_output_tile(C_tile, tile_bias, m_len, n_len, config.ldc,
                    config.dtypes, beta);
        }

        bool is_last_k = (k_off + k_step >= k_full);
        int count = is_last_k
                ? (bp.k_blocks_reduce_rem > 0 ? bp.k_blocks_reduce_rem
                                              : bp.k_blocks_per_reduce)
                : bp.k_blocks_per_reduce;

        const void *A_base = get_matrix_block(src_ptr, m_off, k_off, config.lda,
                config.transA, config.src_type_size);

        const void *B_base;

        B_base = get_matrix_block(weight_ptr, k_off, n_off, config.ldb,
                config.transB, config.src_type_size);
        if (count > 0) {
            unsigned long long ull_count
                    = static_cast<unsigned long long>(count);
            run_brgemm(pd.main_ker[is_m_rem][is_n_rem], A_base, B_base, C_tile,
                    ull_count);
        }

        if (is_last_k && bp.k_block_rem > 0) {
            const void *A_tail = get_matrix_block(src_ptr, m_off, k_full,
                    config.lda, config.transA, config.src_type_size);
            const void *B_tail = get_matrix_block(weight_ptr, k_full, n_off,
                    config.ldb, config.transB, config.src_type_size);
            unsigned long long one = 1;
            run_brgemm(pd.tail_ker[is_m_rem][is_n_rem], A_tail, B_tail, C_tile,
                    one);
        }

        if (is_last_k && has_postops) {
            apply_tile_postops(m_off, n_off, m_len, n_len, config.ldc, C_tile,
                    params, config.dtypes);
        }
    };
...
void execute_brgemm_prepacked(const void *src, const void *blocked_weight,
    ...
    apilog_info("execute_brgemm_prepacked: M=", config.M,
            " num_k_blocks=", bp.num_k_blocks,
            " num_n_blocks=", bp.num_n_blocks,
            " k_block_size=", bp.k_block_size,
            " n_block_size=", bp.n_block_size,
    ...
    auto tile_body
        ...
        void *C_tile = dst_ptr
                + (static_cast<size_t>(m_off) * config.ldc
                          + nblk * n_block_size)
                        * out_elem;
        ...
        if (kblk == 0) {
            ...
            init_output_tile(C_tile, tile_bias, m_len, n_block_size, config.ldc,
                    config.dtypes, beta);
        }
        ...
        const void *A_base = src_ptr
                + (static_cast<size_t>(m_off) * config.lda
                          + kblk * k_block_size)
                        * src_elem;
        const void *B_base = blk_ptr
                + (static_cast<size_t>(nblk) * num_k_blocks + kblk)
                        * tile_bytes;

        if (count > 0) {
            unsigned long long ull_count
                    = static_cast<unsigned long long>(count);
            run_brgemm(pd.main_ker[is_m_rem][0], A_base, B_base, C_tile,
                    ull_count);
        }

        bool is_last_k = (kblk + bp.k_blocks_per_reduce >= num_k_blocks);
        if (is_last_k && has_postops) {
            apply_tile_postops(m_off, nblk * n_block_size, m_len, n_block_size,
                    config.ldc, C_tile, params, config.dtypes);
        }
    };

#if ZENDNNL_DEPENDS_PARLOOPER
    auto gemm_loop = ThreadedLoop<3>(
            {LoopSpecs {0L, (long)num_k_blocks, (long)bp.k_blocks_per_reduce,
                     false},
                    LoopSpecs {0L, (long)M, (long)bp.m_block_size},
...
#else
    if (bp.weight_reuse) {
        for (int kblk = 0; kblk < num_k_blocks;
                kblk += bp.k_blocks_per_reduce) {
#pragma omp parallel for collapse(2) schedule(static) \
        num_threads(config.num_threads)
...
void execute_partitioned_matmul(const char layout, const char trans_input,
    ...
    config.kernel = select_partition_kernel(trans_input, trans_weight, alpha,
            beta, config, params, is_weights_const);
#if ZENDNNL_DEPENDS_LIBXSMM
    if (config.kernel == matmul_algo_t::libxsmm_blocked) {
        blocked_brgemm_params_t bbp {};
        const void *blocked_wt = libxsmm_weight_prepack(weight, config.M,
                config.K, config.N, config.ldb, config.transB, BRGEMM_K_BLOCK,
                BRGEMM_N_BLOCK, config.dtypes.src, is_weights_const, bbp);
        if (blocked_wt) {
    ...
    if (config.kernel == matmul_algo_t::libxsmm) {
        ...
        execute_brgemm_std(trans_input, trans_weight, src, weight, dst, bias,
                config, params, beta);
        return;
    }
#endif
    ...
    if (config.kernel == matmul_algo_t::onednn
        ...
        apilog_info(
                ...
                "primitive, executing matmul LOWOHA kernel with OneDNN "
                "fallback, algo: ",
                static_cast<int>(config.kernel));
        matmul_onednn_wrapper(trans_input, trans_weight, config.M, config.N,
    ...
    apilog_info(
            "Given combination is not supported for matmul parallel primitive, "
            "executing matmul LOWOHA kernel with DLP fallback, algo: ",
            static_cast<int>(config.kernel));
    matmul_kernel_wrapper(layout, trans_input, trans_weight, config.M, config.N,
            config.K, alpha, src, config.lda, weight, config.ldb, beta, dst,
            config.ldc, params.dtypes, config.kernel, params.mem_format_a,
            params.mem_format_b, params, batch_params, bias, is_weights_const,
            config.num_threads);
}
} // namespace matmul
} // namespace lowoha
