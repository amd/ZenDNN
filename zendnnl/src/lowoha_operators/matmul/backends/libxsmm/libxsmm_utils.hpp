 ...
 */
static inline bool can_use_libxsmm(char transA, char transB, int M, int N,
        int K, float alpha, float beta, const matmul_params &lowoha_param,
        bool set_sizes_limit = true) {

#if ZENDNNL_DEPENDS_LIBXSMM

    // Early exit for unsupported scalar values
    if (alpha != 1.0f || (beta != 0.0f && beta != 1.0f)) { return false; }
    ...
    if (set_sizes_limit) {
        ...
        if (matrix_b_elements > 1000000
                || (M > 512 && matrix_b_elements > 1000000)) {
            return false;
        }
    }
 ...
 */
static inline size_t libxsmm_weight_block_size(
        int K, int N, data_type_t dtype) {
 ...
 */
static inline void libxsmm_weight_block(const void *weight, void *blocked_buf,
        int K, int N, int ldb, int k_block_size, int n_block_size,
        data_type_t dtype, bool transB = false) {

    int num_k_blocks = K / k_block_size;
    int num_n_blocks = N / n_block_size;

    log_info("libxsmm_weight_block: [", K, ",", N, "] -> [", num_n_blocks, ",",
            num_k_blocks, ",", n_block_size, ",", k_block_size,
...
#pragma omp parallel for collapse(2) schedule(static)
        ...
        const libxsmm_bfloat16 *src
                = static_cast<const libxsmm_bfloat16 *>(weight);
                ...
                for (int k_pair = 0; k_pair < k_block_size / VNNI; ++k_pair) {
                    for (int ni = 0; ni < n_block_size; ++ni) {
                        for (int v = 0; v < VNNI; ++v) {
                            ...
                            int k_idx = kblk * k_block_size + ki;
                            int n_idx = nblk * n_block_size + ni;
                            tile[k_pair * n_block_size * VNNI + ni * VNNI + v]
                                    = transB ? src[n_idx * ldb + k_idx]
                                             : src[k_idx * ldb + n_idx];
                        }
                    }
                }
...
inline static void libxsmm_postop(const int M, const int N, const int ldc,
    ...
    constexpr libxsmm_datatype IN_TYPE = std::is_same<T, float>::value
    ...
    switch (po.po_type) {
        ...
        case post_op_type_t::gelu_erf: {
            ...
            libxsmm_meltw_unary_shape shape = libxsmm_create_meltw_unary_shape(
                    N, M, ldc, ldc, IN_TYPE, OUT_TYPE, COMP_TYPE);
        ...
        case post_op_type_t::binary_add: {
            ...
            libxsmm_datatype po_buff_type = (po.dtype == data_type_t::bf16)
                    ? LIBXSMM_DATATYPE_BF16
                    : LIBXSMM_DATATYPE_F32;
            ...
            const bool bcast_col_in1 = po.dims.size() == 2 && po.dims[0] == 1;
            ...
            libxsmm_meltwfunction_binary kernel
                    = libxsmm_dispatch_meltw_binary(binary_type, s, bflags);
        ...
        case post_op_type_t::swish: {
            ...
            libxsmm_meltw_unary_shape sig_shape
                    = libxsmm_create_meltw_unary_shape(
                            N, M, ldc, N, IN_TYPE, OUT_TYPE, COMP_TYPE);

            libxsmm_meltwfunction_unary sig_kernel
                    = libxsmm_dispatch_meltw_unary(
                            LIBXSMM_MELTW_TYPE_UNARY_SIGMOID, sig_shape,
                            LIBXSMM_MELTW_FLAG_UNARY_NONE);

            if (!sig_kernel) {
                log_error(
                        "Failed to dispatch LIBXSMM sigmoid kernel for SiLU "
                        "post-op");
                break;
            }
            ...
            libxsmm_meltwfunction_binary mul_kernel
                    = libxsmm_dispatch_meltw_binary(
                            LIBXSMM_MELTW_TYPE_BINARY_MUL, mul_shape,
                            LIBXSMM_MELTW_FLAG_BINARY_NONE);

            if (!mul_kernel) {
                log_error(
                        "Failed to dispatch LIBXSMM binary-mul kernel for SiLU "
                        "post-op");
                break;
            }
        ...
        default: log_error("Unsupported post-op type"); break;
    }
}
...
inline static void libxsmm_bias(const int M, const int N, const int ldc,
    ...
    constexpr libxsmm_datatype OUT_TYPE = std::is_same<TA, float>::value
            ? LIBXSMM_DATATYPE_F32
            : LIBXSMM_DATATYPE_BF16;
    constexpr libxsmm_datatype BIAS_TYPE = std::is_same<TB, float>::value
            ? LIBXSMM_DATATYPE_F32
            : LIBXSMM_DATATYPE_BF16;
    constexpr libxsmm_datatype COMP_TYPE = LIBXSMM_DATATYPE_F32;
