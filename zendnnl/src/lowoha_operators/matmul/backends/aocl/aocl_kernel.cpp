...
void cvt_4bit_to_bf16(const int8_t *weights, bfloat16_t *wei_bf16, int k, int n,
        int ldb, bool is_transposed, data_type_t wei_dt, const void *scales,
        const std::vector<int64_t> &scale_dims, data_type_t scale_dt,
        const void *zp, const std::vector<int64_t> &zp_dims,
        data_type_t zp_dt) {
    const size_t scale_size
            = scale_dims.size() > 0 ? get_num_elements(scale_dims) : 0;
    const size_t zp_size = zp_dims.size() > 0 ? get_num_elements(zp_dims) : 0;

    // Cache per-tensor scale/zp values for efficiency
    const float per_tensor_scale = (scale_size == 1)
            ? read_and_cast<float>(scales, scale_dt, 0)
            : 0.0f;
    const float per_tensor_zp
            = (zp_size == 1) ? read_and_cast<float>(zp, zp_dt, 0) : 0.0f;
    bool is_float_domain_u4
            = zp_dt == data_type_t::bf16 && wei_dt == data_type_t::u4;
    ...
    if (scale_size > 1 && scale_size != static_cast<size_t>(n)) {
        ...
        if (num_groups > 0) { group_size = k / num_groups; }
    }
...
#pragma omp parallel for collapse(2)
    for (int row = 0; row < k; ++row) {
        for (int col = 0; col < n; ++col) {
            ...
            int8_t s4_value = extract_4bit_nibble(
                    weights[packed_byte_idx], is_low_nibble, wei_dt);
            ...
            if (scale_size == 1) {
                ...
                size_t scale_offset
                        = compute_quant_offset(row, col, scale_size);
                scale_value
                        = read_and_cast<float>(scales, scale_dt, scale_offset);
            }
...
void w4a8_cvt_and_cache_plain_s8(Key_matmul key, const int8_t *weights,
    ...
    if (cache.try_get(key, cached)) {
        apilog_verbose(
                "[W4A8.PLAIN HIT] plain s8 cache hit — reusing cached s4→s8 "
                "(K=",
                k, " N=", n, ")");
        s8_plain = cached;
        return;
    ...
    apilog_verbose(
            "[W4A8.PLAIN MISS] plain s8 cache miss — converting s4→s8 "
            "(K=",
            k, " N=", n, " size=", k * n, " bytes)");
...
void w4a8_populate_plain_s8_cache(const std::vector<const void *> &weight,
    ...
    for (int e = 0; e < num_ops; ++e) {
        ...
        if (cache.try_get(w4a8_key, cached_ptr)) {
            apilog_verbose(
                    "[W4A8.PLAIN HIT] plain s8 cache hit — reusing "
                    "cached s4→s8 (K=",
                    K[e], " N=", N[e], ")");
        ...
        apilog_verbose(
                "[W4A8.PLAIN MISS] plain s8 cache miss — converting "
                "s4→s8 (K=",
                K[e], " N=", N[e], " size=", static_cast<size_t>(K[e]) * N[e],
                " bytes)");
        const size_t alignment = 64;
        const size_t buf_size
...
bool reorderAndCacheWeights(Key_matmul key, const void *weights,
    ...
    // Out-of-place reordering
        ...
        if (!found_obj) {
            ...
            if (cached_ptr == nullptr) {
                apilog_info(
                        "Read AOCL cached weights WEIGHT_CACHE_OUT_OF_PLACE "
                        "(reusing user buffer)");
                reorder_weights = const_cast<void *>(weights);
            } else {
                apilog_info(
    ...
    // by inspecting whether the stored pointer is null.
        ...
        if (matmul_weight_cache.try_get(key, cached_ptr)) {
            if (cached_ptr == nullptr) {
                apilog_info(
                        "Read AOCL cached weights WEIGHT_CACHE_IN_PLACE "
                        "(reusing user buffer)");
                reorder_weights = const_cast<void *>(weights);
            } else {
                apilog_info(
        ...
        size_t plain_size
                = static_cast<size_t>(k) * static_cast<size_t>(n) * sizeof(T);
        size_t alignment = 64;
        size_t reorder_size
                = (b_reorder_buf_siz_req + alignment - 1) & ~(alignment - 1);
...
bool reorderAndCacheWeightsSymQuant(Key_matmul key, const void *weights,
    ...
    if (weight_cache_type == 0) {
        ...
        if (!found_obj) {
            ...
            if (cached_ptr == nullptr) {
                apilog_verbose(
                        "[AOCL.reorder symquant HIT] cached "
                        "(reusing user buffer)");
                reorder_weights = const_cast<void *>(weights);
            } else {
    ...
    // buffer just like the out-of-place branch.
        ...
        if (matmul_weight_cache.try_get(key, cached_ptr)) {
            if (cached_ptr == nullptr) {
                apilog_verbose(
                        "[AOCL.reorder symquant HIT] cached in-place "
                        "(reusing user buffer)");
                reorder_weights = const_cast<void *>(weights);
            } else {
                apilog_verbose(
...
void woqReorderAndCacheWeightsAocl(Key_matmul key, const int8_t *weights,
    ...
    if (!found_obj) {
        apilog_verbose(
                "[AOCL.reorder WOQ MISS] simulated WOQ reorder "
                "(weight_cache_type=",
                weight_cache_type, ")");
        size_t alignment = 64;
        ...
        apilog_verbose(
                "[AOCL.reorder WOQ HIT] simulated WOQ cache hit — "
                "reusing cached pack");
...
void w4a8ReorderAndCacheWeightsAocl(Key_matmul key, const int8_t *weights,
    ...
    if (src_dt != data_type_t::s8) {
        apilog_error(
                "[AOCL.reorder W4A8] supports only s8 source; "
    ...
    bool own_cvt_buf = false;
    {
        ...
        if (plain_cache.try_get(key, plain_hit) && plain_hit != nullptr) {
            ...
            apilog_verbose(
                    "[W4A8.REORDER] plain HIT — using cached plain s8 "
                    "(K=",
                    k, " N=", n, ")");
        }
    }
    ...
    if (!found_obj) {
        apilog_verbose(
                "[W4A8.REORDER MISS] blocked reorder cache miss "
                "(weight_cache_type=",
                weight_cache_type, ", src_dt=", static_cast<int>(src_dt), ")");
        size_t alignment = 64;

        if (cvt_weights == nullptr) {
            apilog_verbose(
                    "[W4A8.REORDER] plain MISS — allocating temp s4→s8 "
                    "(K=",
                    k, " N=", n, ")");
        ...
        apilog_verbose(
                "[W4A8.REORDER HIT] blocked reorder cache hit — "
                "reusing cached pack");
...
void run_dlp(char layout, char transA, char transB, int M, int N, int K,
    ...
    // BF16 kernels: bf16 source and weight or simulated WOQ S4 weights
        ...
        switch (dtypes.dst) {
            ...
            default:
                log_error(
                        "Unsupported output data type for bf16 source and s8 "
                        "weight");
                break;
        }
    } else if (dtypes.src == data_type_t::f32
            && dtypes.wei == data_type_t::s8) {
        switch (dtypes.dst) {
            case data_type_t::f32:
            ...
            default:
                log_error(
                        "Unsupported output data type for f32 source and s8 "
                        "weight");
                break;
        }
    }
    // INT8 kernels: u8 source
        ...
        const int8_t *weight_ptr = is_weight_blocked
                ? static_cast<int8_t *>(reordered_mem)
                : static_cast<const int8_t *>(B);
        switch (dtypes.dst) {
            case data_type_t::u8:
    ...
    // INT8 kernels: s8 source
        ...
        if (is_s8_sym_quant_scales) {
            switch (dtypes.dst) {
                ...
                default:
                    log_error(
                            "Unsupported output data type for sym_quant s8 "
                            "source");
                    break;
            }
        } else {
        ...
        switch (dtypes.dst) {
            case data_type_t::f16:
                aocl_gemm_f16f16f16of16(layout, transA, transB, M, N, K,
                        alpha_f16, static_cast<const uint16_t *>(A), lda,
                        mem_format_a,
...
#pragma omp parallel for num_threads(num_threads)
    for (int b = 0; b < batch_count; ++b) {
        a_ptrs[b] = static_cast<const uint8_t *>(A)
                + get_batch_idx(b, Batch_A) * src_stride;
        b_ptrs[b] = static_cast<const uint8_t *>(B)
                + get_batch_idx(b, Batch_B) * weight_stride;
    ...
    if (dtypes.src == data_type_t::f32 && dtypes.dst == data_type_t::f32) {
        ...
        aocl_batch_gemm_f32f32f32of32(&layout, &transA, &transB, &m_, &n_, &k_,
                &alpha, reinterpret_cast<const float **>(a_ptrs.data()), &lda_,
                reinterpret_cast<const float **>(b_ptrs.data()), &ldb_, &beta,
        ...
        aocl_batch_gemm_f16f16f16of16(&layout, &transA, &transB, &m_, &n_, &k_,
                &alpha_f16, reinterpret_cast<const float16 **>(a_ptrs.data()),
                &lda_, reinterpret_cast<const float16 **>(b_ptrs.data()), &ldb_,
                &beta_f16, reinterpret_cast<float16 **>(c_ptrs.data()), &ldc_,
                1, // single group
                &group_size, &mem_format_a, &mem_format_b, &metadata_array);
    } else if (dtypes.src == data_type_t::f16
            && dtypes.dst == data_type_t::f32) {
        ...
        aocl_batch_gemm_f16f16f16of32(&layout, &transA, &transB, &m_, &n_, &k_,
                &alpha_f16, reinterpret_cast<const float16 **>(a_ptrs.data()),
                &lda_, reinterpret_cast<const float16 **>(b_ptrs.data()), &ldb_,
                &beta_f16, reinterpret_cast<float **>(c_ptrs.data()), &ldc_,
                1, // single group
                &group_size, &mem_format_a, &mem_format_b, &metadata_array);
    } else {
