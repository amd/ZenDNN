    ...
    for (int kp = 0; kp < k_pairs; ++kp) {
      ...
      if (!transB) {
        ...
        for (int n = 0; n < nr_act; ++n) {
          d[n * VNNI_PAIR + 0] = (k0 < K) ? weight[k0 * ldb + (j0 + n)] : 0;
          d[n * VNNI_PAIR + 1] = (k1 < K) ? weight[k1 * ldb + (j0 + n)] : 0;
        }
      } else {
        // B transposed: B_logical[k][n] = weight[n * ldb + k]
        for (int n = 0; n < nr_act; ++n) {
          d[n * VNNI_PAIR + 0] = (k0 < K) ? weight[(j0 + n) * ldb + k0] : 0;
          d[n * VNNI_PAIR + 1] = (k1 < K) ? weight[(j0 + n) * ldb + k1] : 0;
        }
      }
