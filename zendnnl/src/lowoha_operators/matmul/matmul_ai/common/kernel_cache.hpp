...
#ifndef MATMUL_AI_KERNEL_CACHE_HPP
#define MATMUL_AI_KERNEL_CACHE_HPP
...
static constexpr int NR_PACK = 64;
...
struct PrepackedWeight {
  ...
  const float *get_panel(int pc, int panel_idx) const {
    return data + static_cast<size_t>(panel_idx) * K * NR_PACK + pc * NR_PACK;
  }
...
struct PrepackedWeightKeyHash {
  size_t operator()(const PrepackedWeightKey &k) const {
    ...
    h ^= std::hash<int>()(k.K) << 1;
    h ^= std::hash<int>()(k.N) << 2;
    h ^= std::hash<int>()(k.ldb) << 3;
    h ^= std::hash<bool>()(k.transB) << 4;
    return h;
  }
};
...
static constexpr int VNNI_PAIR = 2;
...
#endif // MATMUL_AI_KERNEL_CACHE_HPP
