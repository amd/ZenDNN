...
#include "lowoha_operators/matmul/matmul_ai/brgemm/kernel/bf16/bf16_brgemm_ukernel.hpp"
...
static constexpr int PF_DIST = 8;
...
void bf16_brgemm_ukernel(
    ...
    for (int pc = 0; pc < K; pc += BK) {
        ...
        for (; kk + 1 < k_full_pairs; kk += 2) {
            if constexpr (MR <= 4) {
                if (kk + PF_DIST < k_full_pairs) {
                    const char *pf = reinterpret_cast<const char *>(
                        b_off + (kk + PF_DIST) * b_stride);
