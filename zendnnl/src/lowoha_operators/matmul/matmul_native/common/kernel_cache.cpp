...
const PrepackedWeight *PrepackedWeightCache::get_or_prepack(
    ...
    if (auto *cached = cache_.find_and_touch(key)) return cached;
    ...
    for (int jp = 0; jp < np; ++jp) {
        ...
        if (!transB) {
            ...
            for (int kk = 0; kk < K; ++kk) {
                ...
                if (nr_act < NR_PACK)
                    std::memset(
                            d + nr_act, 0, (NR_PACK - nr_act) * sizeof(float));
            }
        } else {
        ...
        for (int kp = 0; kp < k_pairs; ++kp) {
            ...
            if (!transB) {
                ...
                for (int n = 0; n < nr_act; ++n) {
                    d[n * VNNI_PAIR + 0] = (k0 < K)
                            ? weight[static_cast<size_t>(k0) * ldb + (j0 + n)]
                            : 0;
                    d[n * VNNI_PAIR + 1] = (k1 < K)
                            ? weight[static_cast<size_t>(k1) * ldb + (j0 + n)]
                            : 0;
                }
            } else {
                // B transposed: B_logical[k][n] = weight[n * ldb + k]
                for (int n = 0; n < nr_act; ++n) {
                    d[n * VNNI_PAIR + 0] = (k0 < K)
                            ? weight[static_cast<size_t>(j0 + n) * ldb + k0]
                            : 0;
                    d[n * VNNI_PAIR + 1] = (k1 < K)
                            ? weight[static_cast<size_t>(j0 + n) * ldb + k1]
                            : 0;
                }
            }
...
const INT8KContiguousWeight *INT8KContiguousWeightCache::get_or_pack(
        const PrepackedWeightKey &key, const int8_t *weight, float src_scale,
        int32_t src_zp, const float *bias, const float *wei_scale,
        int wei_scale_count) {
    ...
    precompute_int8_dequant(cs_buf, bias, src_scale, src_zp, wei_scale,
            wei_scale_count, N, N_padded, cscale_buf, ebias_buf);
