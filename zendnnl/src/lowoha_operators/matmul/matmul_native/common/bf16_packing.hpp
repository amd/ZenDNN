...
static_assert(NR_PACK % 32 == 0,
        "NR_PACK must be a multiple of 32 for AVX-512 VNNI packing");

// BF16 A micro-panel packing: row-major A -> contiguous MR x KB BF16 blocks
inline void pack_a_bf16_block(const uint16_t *A_src, uint16_t *pack_buf, int ic,
        int pc, int M, int K, int lda, int mb, int kb, int MR) {
