...
inline fused_postop_t detect_fused_postop(const matmul_params &params) {
    for (const auto &po : params.postop_) {
        ...
        if (po.po_type == post_op_type_t::tanh) return fused_postop_t::tanh_op;
        if (po.po_type == post_op_type_t::swish) return fused_postop_t::swish;
    }
...
inline Int8QuantParams extract_int8_quant(const matmul_params &params) {
    ...
    if (params.quant_params.src_scale.buff)
        q.src_scale = *static_cast<const float *>(
    ...
    if (params.quant_params.src_zp.buff) {
        ...
        if (dt == data_type_t::s32)
            q.src_zp = *static_cast<const int32_t *>(
                    params.quant_params.src_zp.buff);
        else if (dt == data_type_t::s8)
            q.src_zp = static_cast<int32_t>(*static_cast<const int8_t *>(
                    params.quant_params.src_zp.buff));
        else if (dt == data_type_t::u8)
            q.src_zp = static_cast<int32_t>(*static_cast<const uint8_t *>(
                    params.quant_params.src_zp.buff));
    }

    if (params.quant_params.wei_scale.buff) {
        q.wei_scale = static_cast<const float *>(
                params.quant_params.wei_scale.buff);
        auto &dims = params.quant_params.wei_scale.dims;
        q.wei_scale_count = (!dims.empty() && dims.back() > 1)
                ? static_cast<int>(dims.back())
                : 1;
    }
    return q;
}
    ...
    *unfuseable = false;
    for (const auto &po : params.postop_) {
        ...
        if (*out_fused == fused_postop_t::none) {
            if (po.po_type == post_op_type_t::relu && po.alpha == 0.0f) {
                *out_fused = fused_postop_t::relu;
                continue;
            }
            if (po.po_type == post_op_type_t::gelu_tanh) {
                *out_fused = fused_postop_t::gelu_tanh;
                continue;
            }
            if (po.po_type == post_op_type_t::gelu_erf) {
                *out_fused = fused_postop_t::gelu_erf;
                continue;
            }
            if (po.po_type == post_op_type_t::sigmoid) {
                *out_fused = fused_postop_t::sigmoid;
                continue;
            }
            if (po.po_type == post_op_type_t::tanh) {
                *out_fused = fused_postop_t::tanh_op;
                continue;
            }
            if (po.po_type == post_op_type_t::swish) {
                *out_fused = fused_postop_t::swish;
                continue;
            }
        }
        *unfuseable = true;
...
inline int m1_gemv_cap_threads(int N, int request_threads, int min_cols = 64) {
    if (request_threads <= 1) return request_threads;
