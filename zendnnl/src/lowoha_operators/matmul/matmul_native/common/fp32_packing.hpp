...
inline void pack_a_block(
    ...
    if (!transA) {
        const float *A_block = A_src + static_cast<size_t>(ic) * lda + pc;
        ...
        const float *A_block = A_src + static_cast<size_t>(pc) * lda + ic;
        for (int ip = 0; ip < m_panels; ++ip) {
            const int i0 = ip * MR;
            const int mr = std::min(MR, m_actual - i0);
            float *dst = pack_buf + ip * MR * k_actual;
...
#endif // MATMUL_NATIVE_COMMON_FP32_PACKING_HPP
