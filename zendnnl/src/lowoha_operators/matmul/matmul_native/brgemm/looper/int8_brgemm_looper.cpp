...
static void pack_b_int8_vnni_panel(const int8_t *B, int ldb, int K, int N,
    ...
    for (int jp = 0; jp < np; ++jp) {
        ...
        int8_t *dst_panel
                = packed + static_cast<size_t>(jp) * k_quads * vnni_stride;
...
static void scale_tile_fp32(float *C, int ldc, int mr, int nr, float alpha) {
    ...
    for (int m = 0; m < mr; ++m) {
        ...
        for (; n + 15 < nr; n += 16)
            _mm512_storeu_ps(
                    row + n, _mm512_mul_ps(av, _mm512_loadu_ps(row + n)));
        for (; n < nr; ++n)
            row[n] *= alpha;
    }
}
    ...
    for (int m = 0; m < mr; ++m) {
        ...
        for (; n + 15 < nr; n += 16)
            _mm512_storeu_ps(dst + n,
                    _mm512_add_ps(_mm512_loadu_ps(dst + n),
        ...
        for (; n + 15 < nr; n += 16) {
            ...
            if (dst_dt == data_type_t::u8) {
                __m512i clamped = _mm512_max_epi32(
                        _mm512_min_epi32(i32, _mm512_set1_epi32(255)),
                        _mm512_setzero_si512());
                __m128i packed = _mm512_cvtusepi32_epi8(clamped);
                _mm_storeu_si128(
                        reinterpret_cast<__m128i *>(
                                static_cast<uint8_t *>(dst) + m * ldc_dst + n),
                        packed);
            } else {
                __m512i clamped = _mm512_max_epi32(
                        _mm512_min_epi32(i32, _mm512_set1_epi32(127)),
                        _mm512_set1_epi32(-128));
                __m128i packed = _mm512_cvtepi32_epi8(clamped);
                _mm_storeu_si128(
                        reinterpret_cast<__m128i *>(
                                static_cast<int8_t *>(dst) + m * ldc_dst + n),
                        packed);
            }
        }
        for (; n < nr; ++n) {
...
static void convert_fp32_to_bf16(const float *C_fp32, int ldc_fp32,
    ...
    for (int m = 0; m < mr; ++m) {
        ...
        for (; n + 15 < nr; n += 16) {
            ...
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_row + n), (__m256i)bf);
        }
        for (; n < nr; ++n) {
            uint32_t bits;
...
void int8_brgemm_execute(const GemmDescriptor &desc, const UarchParams &uarch,
    ...
    const bool need_fp32_buf = (alpha != 1.0f || beta != 0.0f || dst_is_int8
    ...
    if (!prepacked_b) {
        ...
        pack_b_int8_vnni_panel(
                B_raw, ldb, K, N, transB, owned_pack.get(), owned_cs.get());
...
#pragma omp for collapse(2) schedule(static)
            ...
            for (int jc = 0; jc < N; jc += NB) {
                ...
                for (int js = 0; js < n_strips; ++js) {
                    ...
                    const int8_t *pb = packed_ptr
                            + static_cast<size_t>(panel_idx) * k_quads
                                    * vnni_stride
                            + in_panel_off * 4;
                    ...
                    const float *tb = (has_bias && !need_fp32_buf)
                            ? bias_f + col
                            : nullptr;
                    fused_postop_t kernel_op
                            = need_fp32_buf ? fused_postop_t::none : fused_op;
                    ...
                    for (int mp = 0; mp < m_panels; ++mp) {
                        ...
                        if (need_fp32_buf) {
                            ...
                            c_fp32_dst = static_cast<float *>(dst) + row * ldc
                                    + col;
                            ldc_fp32 = ldc;
                        } else {
                        ...
                        if (mr_act == MR && nr_act == NR) {
                            hot_kernel(a_tile, lda, pb, vnni_stride, c_fp32_dst,
                                    ldc_fp32, K, BK, cs, effective_zp,
                                    src_scale, ws, ws_cnt, tb, kernel_op,
                                    c_bf16_dst, c_bf16_dst ? ldc : 0);
                        } else {
                            int8_brgemm_tail_kernel(a_tile, lda, pb,
                                    vnni_stride, c_fp32_dst, ldc_fp32, K, BK,
                                    mr_act, nr_act, cs, effective_zp, src_scale,
                                    ws, ws_cnt, tb, kernel_op, c_bf16_dst,
                                    c_bf16_dst ? ldc : 0);
                        }
                        ...
                        if (need_fp32_buf) {
                            ...
                            if (alpha != 1.0f)
                                scale_tile_fp32(
                                        tile_fp32, NR, mr_act, nr_act, alpha);
                            ...
                            if (beta != 0.0f) {
                                if (dst_is_fp32) {
                                    float *c_old = static_cast<float *>(dst)
                                            + row * ldc + col;
                                    beta_accumulate(tile_fp32, NR, c_old, ldc,
                                            mr_act, nr_act, beta);
                                } else if (dst_is_bf16) {
                                    // Read old bf16 C, convert to fp32, accumulate
                                    const uint16_t *old_bf
                                            = static_cast<const uint16_t *>(dst)
                                            + row * ldc + col;
                                    for (int m = 0; m < mr_act; ++m)
                                        for (int n = 0; n < nr_act; ++n) {
                                            uint32_t bits
                            ...
                            if (has_remaining_postops) {
                                apply_postops_tile(tile_fp32, NR, mr_act,
                                        nr_act, row, col, nullptr,
                                        remaining_postops);
                            }
                            ...
                            if (dst_is_int8) {
                                ...
                                requantize_tile(tile_fp32, NR, dst_row, ldc,
                                        mr_act, nr_act, desc.dst_dt, dst_scale,
                                        dst_zp);
                            } else if (dst_is_bf16) {
                                uint16_t *bf_dst = static_cast<uint16_t *>(dst)
                                        + row * ldc + col;
                                convert_fp32_to_bf16(tile_fp32, NR, bf_dst, ldc,
                                        mr_act, nr_act);
                            } else {
                                float *fp_dst = static_cast<float *>(dst)
                                        + row * ldc + col;
                                for (int m = 0; m < mr_act; ++m)
                                    std::memcpy(fp_dst + m * ldc,
                                            tile_fp32 + m * NR,
                                            nr_act * sizeof(float));
                            }
                        }
    ...
    if (s_log) {
        const char *dst_str = dst_is_fp32        ? "fp32"
                : dst_is_bf16                    ? "bf16"
                : desc.dst_dt == data_type_t::u8 ? "u8"
                                                 : "s8";
        apilog_info("Native INT8 BRGEMM looper: M=", M, " N=", N, " K=", K,
                " MB=", MB, " NB=", NB, " BK=", BK, " MR=", MR, " NR=", NR,
                " alpha=", alpha, " beta=", beta,
                " transA=", transA ? "true" : "false", " threads=", num_threads,
                " src=", src_is_u8 ? "u8" : "s8", " dst=", dst_str,
                " pack=", pack_source);
    }
}
