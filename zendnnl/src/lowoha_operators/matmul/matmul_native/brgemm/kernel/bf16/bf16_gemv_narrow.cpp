...
static inline __m128i load_b_pair_fast(
    ...
    const __m128i r1 = _mm_loadl_epi64(reinterpret_cast<const __m128i *>(
            B + static_cast<size_t>(2 * kp + 1) * ldb));
...
static inline __m128i load_b_pair_safe(
    ...
    const __m128i r0
            = _mm_loadl_epi64(reinterpret_cast<const __m128i *>(b_row0));
    const __m128i r1
            = _mm_loadl_epi64(reinterpret_cast<const __m128i *>(b_row1));
    return _mm_unpacklo_epi16(r0, r1);
}
...
static void bf16_gemv_narrow_impl(const uint16_t *__restrict A, int K,
    ...
    if (alpha != 1.0f) { acc = _mm_mul_ps(acc, _mm_set1_ps(alpha)); }
    ...
    switch (N) {
        ...
        default:
            ...
            log_error("bf16_gemv_narrow: out-of-range N=", N,
                    " (must be 1..kBf16GemvNarrowMaxN=", kBf16GemvNarrowMaxN,
                    "); upstream should gate on N. "
