...
static void bf16_thread_loop(
    ...
    if (M == 1 && num_threads <= 1) {
        ...
        const fused_postop_t tile_fop = can_fuse_epilogue
        ...
        for (int pc = 0; pc < K; pc += KB) {
            ...
            for (int col = 0; col < N; col += NR) {
                ...
                const float *tile_bias = (has_bias && fuse_this && bias_f)
                    ? (bias_f + col) : nullptr;
                const fused_postop_t fop = fuse_this
                    ? tile_fop : fused_postop_t::none;
                ...
                if (b_prepacked) {
                    ...
                    pack_b_vnni_strip(B_raw, ldb_raw, transB_raw,
                        col, std::min(static_cast<int>(NR_PACK), N - col),
                ...
                if (gemv_kernel && full_nr) {
                    gemv_kernel(At, lda, pb, pb_stride,
                                Ct, ldc_fp32, kb_act, beta_k,
                                tile_bias, fop, tile_bf16, tile_ldc_bf16);
                } else {
                    bf16_tail_kernel(At, lda, pb, pb_stride,
                                     Ct, ldc_fp32, kb_act,
                                     1, nr_act, beta_k,
                                     tile_bias, fop, tile_bf16, tile_ldc_bf16);
                }
            ...
            if (is_last_k) {
                if (alpha != 1.0f) {
                    ...
                    if (has_bias)
                        apply_postops_tile(C_fp32, ldc_fp32, 1, N,
                                           0, 0, bias_f, {});
                ...
                for (int ic = 0; ic < M; ic += MB) {
                    ...
                    for (int jr = 0; jr < nb_act; jr += NR) {
                        ...
                        const float *tile_bias =
                            (has_bias && can_fuse && bias_f) ? (bias_f + col) : nullptr;
