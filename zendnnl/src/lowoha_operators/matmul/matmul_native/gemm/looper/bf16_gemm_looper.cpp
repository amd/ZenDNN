...
// ============================================================================
static void bf16_thread_loop(const GemmDescriptor &desc, const BlockPlan &plan,
        const UarchParams &uarch, const void *src, const void *weight,
        void *dst, const void *bias, matmul_params &params,
            ...
            if (bias_changed || s_bias_cap < static_cast<size_t>(N)) {
                ...
                if (s_bias_fp32) {
                    ...
                    for (; n + 16 <= N; n += 16) {
                        ...
                        _mm512_storeu_ps(
                                s_bias_fp32 + n, _mm512_castsi512_ps(shifted));
                    }
                    // Scalar tail
                    for (; n < N; ++n) {
                        uint32_t bits = static_cast<uint32_t>(bias_bf16[n])
                                << 16;
    ...
    const bool can_direct_bf16
            = dst_is_bf16 && can_fuse_epilogue && !has_remaining_postops;
    ...
    // ── M=1 single-thread fast path ──────────────────────────────────────
    // Flat NR-panel scan: collapses the triple-nested K→N→M loop into a
    // single K→N loop. For GEMV (M=1), MB=1 and m_panels=1, so the
    // general loop structure has redundant bounds checks. This path
    // eliminates them for reduced per-shape overhead.
    if (M == 1 && num_threads <= 1) {
        {
            bf16_ukernel_fn_t gemv_kernel = select_bf16_ukernel(1, NR);
            const fused_postop_t tile_fop
                    = can_fuse_epilogue ? fused_op : fused_postop_t::none;
            ...
            for (int pc = 0; pc < K; pc += KB) {
                ...
                const bool fuse_this = is_last_k && can_fuse_epilogue;

                for (int col = 0; col < N; col += NR) {
                    const int nr_act = std::min(NR, N - col);
                    const bool full_nr = (nr_act == NR);

                    const float *tile_bias = (has_bias && fuse_this && bias_f)
                            ? (bias_f + col)
                            : nullptr;
                    const fused_postop_t fop
                            = fuse_this ? tile_fop : fused_postop_t::none;

                    uint16_t *tile_bf16 = nullptr;
                    int tile_ldc_bf16 = 0;
                    if (can_direct_bf16 && is_last_k) {
                        tile_bf16 = C_bf16_dst + col;
                        tile_ldc_bf16 = ldc;
                    }
                    ...
                    if (b_prepacked) {
                        ...
                        if (!s_otf) continue;
                        pack_b_vnni_strip(B_raw, ldb_raw, transB_raw, col,
                                std::min(static_cast<int>(NR_PACK), N - col), K,
                                K_padded_raw, pc, kb_act, s_otf);
                        pb = s_otf;
                        pb_stride = vnni_stride;
                    }

                    const uint16_t *At = A + pc;
                    float *Ct = C_fp32 + col;

                    if (gemv_kernel && full_nr) {
                        gemv_kernel(At, lda, pb, pb_stride, Ct, ldc_fp32,
                                kb_act, beta_k, tile_bias, fop, tile_bf16,
                                tile_ldc_bf16);
                    } else {
                        bf16_tail_kernel(At, lda, pb, pb_stride, Ct, ldc_fp32,
                                kb_act, 1, nr_act, beta_k, tile_bias, fop,
                ...
                if (is_last_k) {
                    if (alpha != 1.0f) {
                        scale_tile(C_fp32, ldc_fp32, 1, N, alpha);
                        if (has_bias)
                            apply_postops_tile(
                                    C_fp32, ldc_fp32, 1, N, 0, 0, bias_f, {});
        ...
        for (int pc = 0; pc < K; pc += KB) {
                    ...
                    for (int jr = 0; jr < nb_act; jr += NR) {
                        ...
                        for (int ip = 0; ip < m_panels; ++ip) {
                            ...
                            if (hot_kernel && full_nr && mr_act == MR) {
                                hot_kernel(At, lda, pb, pb_stride, Ct, ldc_fp32,
                                        kb_act, beta_k, tile_bias, tile_fop,
                                        tile_bf16, tile_ldc_bf16);
                            } else if (full_nr && mr_act >= 1 && mr_act <= 4) {
                                auto tail_uk = select_bf16_ukernel(mr_act, NR);
                                if (tail_uk) {
                                    tail_uk(At, lda, pb, pb_stride, Ct,
                                            ldc_fp32, kb_act, beta_k, tile_bias,
                                            tile_fop, tile_bf16, tile_ldc_bf16);
                                } else {
                                    bf16_tail_kernel(At, lda, pb, pb_stride, Ct,
                                            ldc_fp32, kb_act, mr_act, nr_act,
                                            beta_k, tile_bias, tile_fop,
                                            tile_bf16, tile_ldc_bf16);
                                }
                            } else {
                                bf16_tail_kernel(At, lda, pb, pb_stride, Ct,
                                        ldc_fp32, kb_act, mr_act, nr_act,
                                        beta_k, tile_bias, tile_fop, tile_bf16,
                                        tile_ldc_bf16);
                            }
                        }
                    }
            ...
            if (is_last_k) {
                if (alpha != 1.0f) {
                    ...
                    if (has_bias)
                        apply_postops_tile(
                                C_fp32, ldc_fp32, M, N, 0, 0, bias_f, {});
                    if (fused_op != fused_postop_t::none && fused_idx >= 0)
                        apply_postops_tile(C_fp32, ldc_fp32, M, N, 0, 0,
                                nullptr, {params.postop_[fused_idx]});
                }
                if (has_remaining_postops) {
                    apply_postops_tile(C_fp32, ldc_fp32, M, N, 0, 0, nullptr,
                            remaining_postops);
                }
            }
        }
                ...
                for (int pc = 0; pc < K; pc += KB) {
                    ...
                    for (int jr = 0; jr < nb_act; jr += NR) {
                        ...
                        for (int ip = 0; ip < m_panels; ++ip) {
                            ...
                            if (hot_kernel && full_nr && mr_act == MR) {
                                hot_kernel(At, at_stride, pb, pb_stride, Ct,
                                        ldc_fp32, kb_act, beta_k, tile_bias,
                                        tile_fop, tile_bf16, tile_ldc_bf16);
                            } else if (full_nr && mr_act >= 1 && mr_act <= 4) {
                                auto tail_uk = select_bf16_ukernel(mr_act, NR);
                                if (tail_uk) {
                                    tail_uk(At, at_stride, pb, pb_stride, Ct,
                                            ldc_fp32, kb_act, beta_k, tile_bias,
                                            tile_fop, tile_bf16, tile_ldc_bf16);
                                } else {
                                    bf16_tail_kernel(At, at_stride, pb,
                                            pb_stride, Ct, ldc_fp32, kb_act,
                                            mr_act, nr_act, beta_k, tile_bias,
                                            tile_fop, tile_bf16, tile_ldc_bf16);
                                }
                            } else {
                                bf16_tail_kernel(At, at_stride, pb, pb_stride,
                                        Ct, ldc_fp32, kb_act, mr_act, nr_act,
                                        beta_k, tile_bias, tile_fop, tile_bf16,
                                        tile_ldc_bf16);
                            }
                        }
                    ...
                    if (is_last_k) {
                        ...
                        if (alpha != 1.0f) {
                            ...
                            if (has_bias)
                                apply_postops_tile(Ctile, ldc_fp32, mb_act,
                                        nb_act, jc, ic, bias_f, {});
                            if (fused_op != fused_postop_t::none
                                    && fused_idx >= 0)
                                apply_postops_tile(Ctile, ldc_fp32, mb_act,
                                        nb_act, jc, ic, nullptr,
...
void bf16_gemm_execute(const GemmDescriptor &desc, const UarchParams &uarch,
    ...
    bf16_thread_loop(
            desc, bp.plan, uarch, src, weight, dst, bias, params, prepacked_b);
}
