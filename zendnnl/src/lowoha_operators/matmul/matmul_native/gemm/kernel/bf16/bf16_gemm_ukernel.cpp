...
// │      │ (NV=1)   │ (NV=2)   │ (NV=4)   │                              │
// ├──────┼──────────┼──────────┼──────────┼──────────────────────────────┤
// │   1  │  3 (9%)  │  5 (16%) │  9 (28%) │ UF=4, GEMV path              │
    ...
    for (; kk + UF - 1 < k_pairs; kk += UF) {
        for (int u = 0; u < UF; ++u) {
            ...
            for (int m = 0; m < MR; ++m) {
                uint32_t a_pair;
                std::memcpy(
                        &a_pair, &A[m * lda + 2 * (kk + u)], sizeof(a_pair));
                __m512bh a_bf16
...
void bf16_tail_kernel(const uint16_t *__restrict A, int lda,
    ...
    for (int m = 0; m < mr_act; ++m) {
        for (int v = 0; v < nv_full; ++v) {
            ...
            for (int kk = 0; kk < k_pairs; ++kk) {
                uint32_t a_pair = static_cast<uint32_t>(A[m * lda + 2 * kk])
                        | (static_cast<uint32_t>(A[m * lda + 2 * kk + 1])
                                << 16);
                __m512bh a_bf16
                        = (__m512bh)_mm512_set1_epi32(static_cast<int>(a_pair));
            ...
            if (k_rem) {
                uint32_t a_pair
                        = static_cast<uint32_t>(A[m * lda + 2 * k_pairs]);
                __m512bh a_bf16
                        = (__m512bh)_mm512_set1_epi32(static_cast<int>(a_pair));
                __m512bh b_bf16 = (__m512bh)_mm512_loadu_si512(
        ...
        if (nr_tail > 0) {
            ...
            __m512 acc = (beta != 0.0f)
                    ? _mm512_mul_ps(_mm512_maskz_loadu_ps(
                                            mask, C + m * ldc + col_off),
                              _mm512_set1_ps(beta))
                    : _mm512_setzero_ps();
            ...
            if (bias)
                acc = _mm512_add_ps(
                        acc, _mm512_maskz_loadu_ps(mask, bias + col_off));
            if (fused_op != fused_postop_t::none)
                acc = apply_fused_postop(acc, fused_op);
