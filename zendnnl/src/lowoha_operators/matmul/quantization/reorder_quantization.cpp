...
status_t reorder_quantization_wrapper(const void *&src, const int lda,
    ...
    status_t val_status = op_instrumentation::validate([&]() {
        if (params.quant_params.src_scale.dims.empty()
            ...
            log_error(
                    "Reorder quantization requires quant_params.src_scale "
        ...
        if (needs_zp
                && (params.quant_params.src_zp.dims.empty()
                        || params.quant_params.src_zp.dt
                                == data_type_t::none)) {
            log_error(
                    "Asymmetric (u8) quantization requires quant_params.src_zp "
                    "dims and dt to be set");
            return status_t::failure;
        }
        if (!is_dynamic) {
            if (!params.quant_params.src_scale.buff) {
                log_error(
                        "Static quantization requires "
                        "quant_params.src_scale.buff "
                        "to be provided");
                return status_t::failure;
            }
            if (needs_zp && !params.quant_params.src_zp.buff) {
                log_error(
                        "Static quantization requires quant_params.src_zp.buff "
    ...
    const bool try_native_bf16_f32_s8 = !is_w4a8 && is_dynamic && !needs_zp
            && quant_dtype == data_type_t::s8 && !is_batched
            && (orig_src_dtype == data_type_t::bf16
...
#pragma omp parallel for collapse(2)
                    for (int64_t m = 0; m < M_dim; ++m) {
                        for (int64_t g = 0; g < G_dim; ++g) {
                            std::memcpy(buf
                                            + static_cast<size_t>(m * G_dim + g)
                                                    * elem_size,
                                    tmp.data()
                                            + static_cast<size_t>(g * M_dim + m)
                                                    * elem_size,
                                    elem_size);
                        }
                    }
            ...
            apilog_info(
                    "Reorder quantization: per-token dynamic scales "
                    "(compute-only); "
                    "source left as ",
        ...
        log_error(
                "Reorder quantization: compute-only scale failed, "
...
#pragma omp parallel for collapse(2)
        ...
        if (apilog_info_enabled()) {
            ...
            apilog_info(is_dynamic ? "Dynamic" : "Static",
                    ...
                    dtype_info(orig_src_dtype), " to ", dtype_info(quant_dtype),
                    " (scale_elems=", ns, ", zp_elems=", nz, ")");
        }
    } else {
        log_error(
                "Reorder quantization of source failed, "
...
status_t group_reorder_quantization_wrapper(
    ...
    if (src.size() < num_ops || lda.size() < num_ops || transA.size() < num_ops
            || K.size() < num_ops || params.size() < num_ops) {
        return status_t::success;
    }
    ...
    auto fallback_per_expert = [&]() -> status_t {
        ...
        for (size_t i = 0; i < num_ops; ++i) {
            ...
            bp.Batch_A = 1;
            bp.Batch_B = 1;
            const status_t st = reorder_quantization_wrapper(src_i, lda[i],
                    reordered_lda, src_type_size, params[i], bp, transA[i],
                    M[i], K[i], num_threads, buffers.fallback_buf[i]);
    ...
    if (!requires_dynamic_quant) { return status_t::success; }
    if (src_dtype != data_type_t::bf16 && src_dtype != data_type_t::f32
            && src_dtype != data_type_t::f16) {
        return status_t::success;
    }
    if (compute_dtype != data_type_t::s8) { return fallback_per_expert(); }
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        ...
        if (M[i] < 0 || K[i] < 0) {
            log_error("Group reorder quantization: negative dimension at op ",
                    i, " (M=", M[i], ", K=", K[i], ")");
            return status_t::failure;
        }
        if (K[i] == 0 || lda[i] < K[i]) { return fallback_per_expert(); }
        if (lda[i] != K[i]) {
        ...
        if (gran == granularity_type_t::per_token
            ...
            if (g_i <= 1 || (K[i] % g_i) != 0) { return fallback_per_expert(); }
        } else {
    ...
    if (src_total > 0) {
        ...
        if (!new_src_arena) {
            log_error(
                    "Group reorder quantization: failed to allocate source "
                    "arena");
            return status_t::failure;
        }
    }
    if (scale_total > 0) {
        ...
        if (!new_scale_arena) {
            log_error(
                    "Group reorder quantization: failed to allocate scale "
                    "arena");
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        const size_t src_bytes = static_cast<size_t>(std::max(0, M[i]))
        ...
        if (src_bytes > 0) {
            ...
            if (src_off > src_total || src_bytes > src_total - src_off) {
                log_error(
                        "Group reorder quantization: source slice exceeds "
                        "arena");
                return status_t::failure;
            }
        ...
        if (!scale_buff) {
            ...
            const size_t scale_bytes = static_cast<size_t>(std::max(0, M[i]))
            ...
            if (scale_bytes > 0) {
                ...
                if (scale_off > scale_total
                        || scale_bytes > scale_total - scale_off) {
                    log_error(
                            "Group reorder quantization: scale slice exceeds "
                            "arena");
                    return status_t::failure;
                }
    ...
    if (src.size() != num_ops) {
        src_trimmed.assign(src.begin(),
                src.begin()
                        + static_cast<
                                std::vector<const void *>::difference_type>(
                                num_ops));
        src_arg = &src_trimmed;
    }
    if (K.size() != num_ops) {
        K_trimmed.assign(K.begin(),
                K.begin()
    ...
    const status_t st = zendnnl::lowoha::reorder::group_dynamic_quant(*src_arg,
            M, *K_arg, active_src_strides, dst, dst_strides, scale, gparams);
    if (st != status_t::success) { return st; }
