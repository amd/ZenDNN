...
private:
    ...
    std::unique_ptr<std::unordered_map<lru_key_t, timed_entry_t>>
            lru_cache_map_;
...
lru_cache_t<KEY_T, VALUE_T>::lru_cache_t(uint32_t capacity)
    : capacity_(capacity) {
    lru_cache_map_
            = std::make_unique<std::unordered_map<lru_key_t, timed_entry_t>>();
}
...
lru_cache_t<KEY_T, VALUE_T>::get_or_add(
        const lru_key_t &key, const value_t &value) {
    std::lock_guard<std::mutex> lock(mutex_);
...
template <typename KEY_T, typename VALUE_T>
void lru_cache_t<KEY_T, VALUE_T>::remove_if_invalidated(const lru_key_t &key) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = lru_cache_map_->find(key);
    if (it != lru_cache_map_->end()) { lru_cache_map_->erase(it); }
}
...
lru_cache_t<KEY_T, VALUE_T>::timed_entry_t::timed_entry_t(
        const value_t &value, size_t timestamp)
...
template <typename KEY_T, typename VALUE_T>
void lru_cache_t<KEY_T, VALUE_T>::add(
        const lru_key_t &key, const value_t &value) {
    std::lock_guard<std::mutex> lock(mutex_);
    evict(1);
    size_t timestamp = current_timestamp_++;
...
template <typename KEY_T, typename VALUE_T>
typename lru_cache_t<KEY_T, VALUE_T>::value_t lru_cache_t<KEY_T, VALUE_T>::get(
        const lru_key_t &key) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = lru_cache_map_->find(key);
    if (it != lru_cache_map_->end()) {
...
template <typename KEY_T, typename VALUE_T>
bool lru_cache_t<KEY_T, VALUE_T>::try_get(const lru_key_t &key, value_t &out) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = lru_cache_map_->find(key);
    if (it == lru_cache_map_->end()) { return false; }
    it->second.timestamp_ = current_timestamp_++;
