...
template <typename Elem, data_type_t Kind>
void moe_weighted_reduce_scalar(const group_matmul_moe_postop_params *postop,
        ...
        for (int t = 0; t < postop->num_tokens; ++t) {
            ...
            // k=0: initialize FP32 accumulator with w0 * src0.
            {
                ...
                for (; d < D16; d += 16) {
                    ...
                    __m512 vsrc = _mm512_castsi512_ps(_mm512_slli_epi32(
                            _mm512_cvtepu16_epi32(src_bf16), 16));
                    _mm512_storeu_ps(acc + d, _mm512_mul_ps(vw, vsrc));
                }
                for (; d < D; ++d)
                    acc[d] = w
                            * bfloat16_t::bf16_to_f32_val(
            ...
            // bfloat16_t::f32_to_bf16_avx512 in common/bfloat16.cpp.
            {
                ...
                for (; d < D16; d += 16) {
                    __m512i bits
                            = _mm512_castps_si512(_mm512_loadu_ps(acc + d));
                    __m512i lsb = _mm512_and_si512(
                            _mm512_srli_epi32(bits, 16), one);
                    ...
                    _mm256_storeu_si256(
                            reinterpret_cast<__m256i *>(out_row + d),
...
#pragma omp for schedule(static)
        for (int t = 0; t < postop->num_tokens; ++t) {
            ...
            // very large weighted sums must already bound their inputs.
            {
                ...
                for (; d < D16; d += 16) {
                    __m512 vacc = _mm512_loadu_ps(acc + d);
                    __m256i out_f16 = _mm512_cvtps_ph(vacc,
                            _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
                    _mm256_storeu_si256(
                            reinterpret_cast<__m256i *>(out_row + d), out_f16);
                }
                for (; d < D; ++d) {
...
status_t validate_group_matmul_moe_postop(
    ...
    if (postop == nullptr) { return status_t::success; }
    ...
    if (!postop->skip_weighted && postop->topk_weights == nullptr) {
        log_error(
                "group_matmul_direct: moe_postop requires topk_weights unless "
                "skip_weighted");
        return status_t::failure;
    }
    ...
    if (dst_elem != data_type_t::f32 && dst_elem != data_type_t::bf16
        ...
        log_error(
                "group_matmul_direct: moe_postop requires FP32, BF16, or F16 "
                "dst");
        return status_t::failure;
    }
    ...
    status_t slot_check = op_instrumentation::validate([&]() {
        const size_t total
                = static_cast<size_t>(postop->num_tokens) * postop->topk;
        for (size_t i = 0; i < total; ++i) {
            if (postop->row_ptrs[i] == nullptr) {
                log_error("group_matmul_direct: moe_postop row_ptrs[", i,
                        "] is null");
                return status_t::failure;
            }
        }
    ...
    if (slot_check != status_t::success) { return slot_check; }
...
status_t group_matmul_moe_postop_execute(
        const group_matmul_moe_postop_params *postop, const int D,
        const int num_threads, const data_type_t dst_elem) {
    ...
    if (dst_elem == data_type_t::bf16) {
        moe_weighted_reduce<uint16_t, data_type_t::bf16>(
                postop, D, num_threads);
        return status_t::success;
    }
