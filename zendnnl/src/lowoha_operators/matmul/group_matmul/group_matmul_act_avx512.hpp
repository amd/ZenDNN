...
static inline __m256i f32_to_bf16x16(__m512 f32) {
    ...
    return _mm512_cvtepi32_epi16(
            _mm512_srli_epi32(_mm512_add_epi32(i32, bias), 16));
}
...
static inline __m512 fast_exp_neg_avx512(__m512 x) {
    ...
    __m512 ti_clamped = _mm512_max_ps(
            _mm512_min_ps(ti, _mm512_set1_ps(127.0f)), _mm512_set1_ps(-126.0f));
...
static inline __m512 erf_avx512(__m512 x) {
    ...
    const __mmask16 x2_finite = _mm512_cmp_ps_mask(x2, fp_inf, _CMP_LT_OQ);
...
static inline __m512 swiglu_oai_avx512(__m512 gate, __m512 up) {
    ...
    return _mm512_mul_ps(_mm512_add_ps(one, up), _mm512_mul_ps(gate, sig));
}
