...
dlp_metadata_t *build_act_and_mul_metadata(post_op_type_t act_kind,
        void *up_base, int ldc, int m_rows, int dim, data_type_t dst_dtype) {
    ...
    mul_op.dims = {static_cast<int64_t>(m_rows), static_cast<int64_t>(dim)};
    lparams.postop_.push_back(mul_op);

    matmul_data_types dtypes = lparams.dtypes;
    return create_dlp_post_op(lparams, /*bias=*/nullptr, dtypes,
            /*N=*/dim, /*K=*/0, /*M=*/m_rows,
            /*zp_comp_acc=*/nullptr, /*zp_comp_ndim=*/0,
            matmul_algo_t::aocl_dlp,
            /*weight_ptr=*/
            nullptr); // weight_ptr is not used for MoE gated activations
}
...
bool apply_act_and_mul_dlp_impl(post_op_type_t act_kind, const char *op_name,
        void *dst, int row_start, int row_end, int N, int ldc,
    ...
    if (row_start >= row_end || dst == nullptr) { return true; }
    ...
    const int dim = N / 2;
    const size_t elem = (dst_dtype == data_type_t::f32) ? sizeof(float)
                                                        : sizeof(bfloat16_t);
    ...
    dlp_metadata_t *md = build_act_and_mul_metadata(act_kind,
            static_cast<void *>(up_base), ldc, m_rows, dim, dst_dtype);
    if (md == nullptr) {
    ...
    if (dst_dtype == data_type_t::f32) {
        aocl_gemm_eltwise_ops_f32of32('r', 'n', 'n', m_rows, dim,
                reinterpret_cast<const float *>(gate_base), ldc,
                reinterpret_cast<float *>(gate_base), ldc, md);
    } else {
        aocl_gemm_eltwise_ops_bf16obf16('r', 'n', 'n', m_rows, dim,
                reinterpret_cast<const int16_t *>(gate_base), ldc,
                reinterpret_cast<int16_t *>(gate_base), ldc, md);
    }
...
bool silu_and_mul_dlp(void *dst, int row_start, int row_end, int N, int ldc,
        data_type_t dst_dtype) {
    return apply_act_and_mul_dlp_impl(post_op_type_t::swish, "silu_and_mul_dlp",
            dst, row_start, row_end, N, ldc, dst_dtype);
}

bool gelu_and_mul_dlp(void *dst, int row_start, int row_end, int N, int ldc,
        data_type_t dst_dtype) {
    return apply_act_and_mul_dlp_impl(post_op_type_t::gelu_erf,
            "gelu_and_mul_dlp", dst, row_start, row_end, N, ldc, dst_dtype);
}

// Returns false when DLP cannot or did not apply the activation (caller
// must then fall back to apply_gated_act_inplace()).
bool apply_gated_act_inplace_dlp(grp_matmul_gated_act_t act, void *dst,
        int row_start, int row_end, int N, int ldc, data_type_t dst_dtype) {
    switch (act) {
        case grp_matmul_gated_act_t::none: return true;
        ...
        case grp_matmul_gated_act_t::swiglu_oai_mul: return false;
    }
    return false;
}
