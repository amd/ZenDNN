...
inline void offset_quant_by_row(matmul_quantization_params_t::matmul_quant_t &q,
...
inline void execute_m_tile(int e, int local_tid, int team_size,
        const std::vector<char> &layout, const std::vector<bool> &transA,
        const std::vector<bool> &transB, const std::vector<int> &M,
        const std::vector<int> &N, const std::vector<int> &K,
        const std::vector<float> &alpha, const std::vector<const void *> &src,
        const std::vector<int> &lda, const std::vector<const void *> &weight,
        const std::vector<int> &ldb, const std::vector<const void *> &bias,
        const std::vector<float> &beta, const std::vector<void *> &dst,
        const std::vector<int> &ldc, const std::vector<bool> &is_weights_const,
        const std::vector<matmul_params> &params, size_t src_elem,
        size_t dst_elem, matmul_algo_t algo) {
    ...
    offset_quant_by_row(
            slice_params.quant_params.src_scale, row_start, slice_M);
...
inline void execute_m_tile_act(int e, int local_tid, int team_size,
        const std::vector<char> &layout, const std::vector<bool> &transA,
        const std::vector<bool> &transB, const std::vector<int> &M,
        const std::vector<int> &N, const std::vector<int> &K,
        const std::vector<float> &alpha, const std::vector<const void *> &src,
        ...
        const std::vector<int> &ldc, const std::vector<bool> &is_weights_const,
        const std::vector<matmul_params> &params, size_t src_elem,
        size_t dst_elem, matmul_algo_t algo, grp_matmul_gated_act_t fused_act,
        data_type_t act_dtype) {

    execute_m_tile(e, local_tid, team_size, layout, transA, transB, M, N, K,
    ...
    if (fused_act != grp_matmul_gated_act_t::none) {
        ...
        if (row_start < row_end)
            apply_gated_act_inplace(fused_act, dst[e], row_start, row_end, N[e],
                    ldc[e], act_dtype);
    }
}
...
inline void execute_light_expert(int e, const std::vector<char> &layout,
        ...
        const std::vector<matmul_params> &params, matmul_algo_t algo,
...
inline void dqint8_compact_and_requant_slice(
    ...
    for (int m = 0; m < slice_M; ++m) {
        std::memcpy(compact_bf16_buf + static_cast<size_t>(m) * row_bytes,
    ...
    if (scale_dt == data_type_t::bf16) {
        ...
        for (int64_t i = 0; i < n; ++i) {
            ...
            std::memcpy(out_bytes + static_cast<size_t>(2 * i), &bf16,
                    sizeof(bf16));
        }
    }
}
...
void flat_m_tile(const std::vector<char> &layout,
    ...
    // will see the hint once and the user can ignore it.
    {
        ...
        if (s_hint_log && !s_hint_fired.load(std::memory_order_relaxed)
            ...
            if (max_M_for_hint == 1) {
                ...
                if (s_hint_fired.compare_exchange_strong(
                    ...
                    const int idle_pct
                            = (num_threads - active_ops) * 100 / num_threads;
                    apilog_info(
                            "[GRP_MATMUL.HINT] ALGO_2 (flat_m_tile) invoked on "
                            "a "
                            "decode-class call (max_M=1, active_ops=",
                            active_ops, ", num_threads=", num_threads,
                            ").  Approximately ", idle_pct,
                            "% of threads will idle on the Phase-2 join "
                            "barrier "
                            "because the M-tile slice plan cannot subdivide "
                            "M=1 "
                            "across threads.  If this routing was unintended, "
                            "set "
                            "ZENDNNL_GRP_MATMUL_AUTO_DECODE_ALGO=3 to keep "
                            "decode "
                            "calls on the wide-N parallel path, or raise the "
                            "per-expert M (larger batch / sequence) so the "
                            "M-tile planner has rows to subdivide.  Emitted "
                            "once "
                            "per process.");
                }
            }
    ...
    if (get_grp_matmul_m_tile_hybrid() == 0) {
        ...
        const bool gate_skew = (max_M >= kHybridMinMaxM) && (avg_M > 0)
        ...
        if (active_ops >= min_actives && gate_skew) {
            ...
            if (committed.ok) {
                ...
                if (test_api::s_capture_m_tile_path.load(
                ...
                for (int idx : heavy_exp) {
                    ...
                    ht_assign[idx] = std::min(M[idx],
                            static_cast<int>(std::max<int64_t>(1,
                                    (static_cast<int64_t>(M[idx]) + kSliceTarget
                                            - 1)
                                            / kSliceTarget)));
                ...
                if (ht_total_need <= heavy_pool) {
                    ...
                    while (surplus > 0) {
                        ...
                        int best_slice = 0;
                        for (int idx : heavy_exp) {
                            if (ht_assign[idx] <= 0 || ht_assign[idx] >= M[idx])
                                continue;
                            int cur_slice = (M[idx] + ht_assign[idx] - 1)
                                    / ht_assign[idx];
                    ...
                    int assigned = 0;
                    for (int idx : heavy_exp) {
                        ht_assign[idx] = std::max(1,
                                static_cast<int>(
                                        static_cast<int64_t>(heavy_pool)
                                        * M[idx] / heavy_M_sum));
                ...
                if (ht_assigned_final * 4 >= heavy_pool * 3) {
                    ...
                    for (int idx : heavy_exp) {
                        ...
                        for (int k = 0; k < t && next_tid < num_threads;
                                ++k, ++next_tid) {
...
#pragma omp parallel num_threads(num_threads)
                        ...
                        if (role == kRoleLight) {
                            for (int j = light_next.fetch_add(
                                    ...
                                    j = light_next.fetch_add(
                                            1, std::memory_order_relaxed)) {
                                execute_light_expert(light_exp[j], layout,
                                        transA, transB, M, N, K, alpha, src,
                                        lda, weight, ldb, bias, beta, dst, ldc,
                                        is_weights_const, params, algo,
                                        fused_act, act_dtype);
                            }
                        } else if (role == kRoleHeavy) {
                            const int e = mt_tid_to_expert[tid];
                            execute_m_tile_act(e, mt_tid_to_local[tid],
                                    mt_tid_to_team[tid], layout, transA, transB,
                                    M, N, K, alpha, src, lda, weight, ldb, bias,
                                    beta, dst, ldc, is_weights_const, params,
                                    src_elem, dst_elem, algo, fused_act,
                                    act_dtype);
                        }
                    }
                    return;
    ...
    const auto plan = plan_m_tile_single_tier_assignment(M, active_pos, num_ops,
            num_threads, active_ops, kSliceTarget, cores_per_ccd, num_ccds);
...
bool flat_m_tile_pipeline_bf16(const std::vector<char> &layout,
        ...
        // Op1 (W13) inputs
        const std::vector<int> &M, const std::vector<int> &N_w13,
        const std::vector<int> &K_in, const std::vector<float> &alpha_w13,
        const std::vector<const void *> &src, const std::vector<int> &lda,
    ...
    group_matmul_prepack::prepack_for_algo_2(
            group_matmul_prepack::build_prepack_params(weight_w13, K_in, N_w13,
                    ...
                    /*beta=*/&beta_w13));
    group_matmul_prepack::prepack_for_algo_2(
            group_matmul_prepack::build_prepack_params(weight_w2, K_w2, N_w2,
                    ldb_w2, transB, is_weights_const, params_w2, M,
                    get_grp_matmul_custom_kernel(), num_threads, /*nr_align=*/0,
    ...
    for (int tid = 0; tid < num_threads; ++tid) {
        ...
        if (team <= 0) continue;
        const int row_start
                = static_cast<int>(static_cast<int64_t>(M[e]) * local / team);
        const int row_end = static_cast<int>(
        ...
        if (regime == kRegDQINT8) {
            ...
            const size_t scale_b = static_cast<size_t>(slice_M)
                    * static_cast<size_t>(g2_e)
                    * sizeof(float); // {slice_M × G} f32
    ...
    if (regime == kRegDQINT8) {
        ...
        for (int e = 0; e < num_ops; ++e) {
            ...
            const status_t s = reorder_quantization_wrapper(src_e, lda[e],
                    reordered_lda, src_type_size, shadow, bp, transA[e], M[e],
                    K_in[e], num_threads, hoist_buffers_w13[e]);
            ...
            if (s != status_t::success) {
                ...
                log_error(
                        "flat_m_tile_pipeline_bf16 (DQ-INT8): pre-OMP src "
                        "hoist failed for expert ",
                        e, "; falling back to legacy two-pass.");
                return false;
            }
        ...
        for (int tid = 0; tid < num_threads; ++tid) {
            ...
            if ((cb > 0 && !pt.src_buf) || (ib > 0 && !pt.scale_buf)
                    || (sb > 0 && !pt.zp_buf)) {
                log_error(
                        "flat_m_tile_pipeline_bf16 (DQ-INT8): per-thread "
                        "Stage 2b alloc failed for tid ",
                        tid, " (compact=", cb, " int8=", ib, " scale=", sb,
...
#pragma omp barrier
        ...
        if (!alloc_fail.load(std::memory_order_relaxed) && e >= 0 && need > 0) {
            const int local = plan.tid_to_local[tid];
            const int team = plan.tid_to_team[tid];
            const int row_start = static_cast<int>(
            ...
            for (auto &po : w13_local.postop_) {
                if ((po.po_type == post_op_type_t::binary_add
                    ...
                    const bool is_broadcast_1d = (po.dims.size() <= 1)
                            || (po.dims.size() == 2 && po.dims[0] == 1);
                    if (!is_broadcast_1d) {
                        const int eff_ld = (po.leading_dim > 0) ? po.leading_dim
                                                                : N_w13[e];
                        const size_t po_elem = size_of(po.dtype);
                        po.buff = static_cast<uint8_t *>(po.buff)
                                + static_cast<size_t>(row_start) * eff_ld
                                        * po_elem;
                    }
                }
            }
            ...
            offset_quant_by_row(
                    w13_local.quant_params.src_scale, row_start, slice_M);
            offset_quant_by_row(
                    w13_local.quant_params.src_zp, row_start, slice_M);

            const size_t src_off = transA[e]
                    ? static_cast<size_t>(row_start) * src_elem_for_w13
                    : static_cast<size_t>(row_start) * lda_for_w13
                            * src_elem_for_w13;
            const void *src_slice
                    = static_cast<const char *>(src_for_w13) + src_off;

            execute_expert_slice(layout[e], transA[e], transB[e], slice_M,
            ...
            if (fused_act != grp_matmul_gated_act_t::none) {
                apply_gated_act_inplace(fused_act, sc.buf,
                        ...
                        /*N=*/N_w13[e], /*ldc=*/N_w13[e], act_dtype);
            }
            ...
            if (dst_w13_is_caller_alloc && !dst_w13.empty()
                    && dst_w13[e] != nullptr) {
                ...
                char *dst_w13_slice = static_cast<char *>(dst_w13[e])
                        + static_cast<size_t>(row_start) * ldc_w13[e]
                                * inter_elem;
                const char *sc_base = static_cast<const char *>(sc.buf);
                const size_t row_bytes
                        = static_cast<size_t>(spill_cols) * inter_elem;
                for (int m = 0; m < slice_M; ++m) {
                    std::memcpy(dst_w13_slice
                                    + static_cast<size_t>(m) * ldc_w13[e]
                                            * inter_elem,
                            sc_base
                                    + static_cast<size_t>(m) * N_w13[e]
                                            * inter_elem,
                            row_bytes);
                }
            }
            ...
            if (regime == kRegDQINT8) {
                ...
                const data_type_t op2_scale_dt
                        = (params_w2[e].quant_params.wei_scale.dt
                                  == data_type_t::bf16)
                        ? data_type_t::bf16
                        : data_type_t::f32;
                dqint8_compact_and_requant_slice(sc.buf, slice_M, K_w2[e],
                        /*row_stride_bf16_elems=*/N_w13[e], compact_bf16_buf,
                        int8_buf_for_w2, scale_f32_for_w2,
            ...
            offset_quant_by_row(
                    w2_local.quant_params.src_scale, row_start, slice_M);
            offset_quant_by_row(
                    w2_local.quant_params.src_zp, row_start, slice_M);
            ...
            char *dst_w2_slice = static_cast<char *>(dst_w2[e])
                    + static_cast<size_t>(row_start) * ldc_w2[e] * dst_elem_w2;

            execute_expert_slice(layout[e], transA_w2[e], transB[e], slice_M,
    ...
    if (alloc_fail.load(std::memory_order_relaxed)) {
        log_error(
                "flat_m_tile_pipeline_bf16: per-thread scratch alloc "
...
bool try_flat_m_tile_pipeline_bf16(const std::vector<char> &layout,
    ...
    auto classify_half
        ...
        if (p.dtypes.wei == data_type_t::s8) {
            ...
            if (!p.dynamic_quant) return kRegimeNone;
            if (p.dtypes.compute != data_type_t::s8)
                return kRegimeNone; // sym only
    ...
    const HalfRegime regime_w13 = classify_half(params_w13[0], M, num_ops_int);
    const HalfRegime regime_w2 = classify_half(params_w2[0], M, num_ops_int);
    ...
    if (regime_w13 == kRegimeDQINT8 && num_ops_int > 0) {
        const int g1
                = mtile_src_group_count(params_w13[0].quant_params.src_scale);
        const int g2
                = mtile_src_group_count(params_w2[0].quant_params.src_scale);
        if (g1 > 1 && K_in[0] % g1 != 0) return false;
        if (g2 > 1 && K_w2[0] % g2 != 0) return false;
    }

    const bool act_supported = (fused_act == grp_matmul_gated_act_t::none)
    ...
    return flat_m_tile_pipeline_bf16(layout, transA, transA_w2, transB, M,
            N_w13, K_in, alpha_w13, src, lda, weight_w13, ldb_w13, bias_w13,
            beta_w13, dst_w13, ldc_w13, dst_w13_is_caller_alloc, N_w2, K_w2,
            alpha_w2, weight_w2, ldb_w2, bias_w2, beta_w2, dst_w2, ldc_w2,
            fused_act, act_dtype, is_weights_const, params_w13, params_w2,
            num_threads);
}
