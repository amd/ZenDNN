...
status_t validate_w4a8_inputs(
        const matmul_params &params, int M, int N, int K) {
    ...
    if (G <= 0 || scale_N != static_cast<int64_t>(N)) {
        log_error(
                "W4A8 requires weight scale dims {G,N} "
                "(G=",
                G, ", scale_N=", scale_N, ", N=", N, ")");
        return status_t::failure;
    }
...
status_t validate_matmul_direct_inputs(const void *src, const void *weight,
    ...
    if (!src || !weight || !dst) {
        log_error("Null pointer input to matmul_direct: src=",
                ...
                ", weight=", static_cast<const void *>(weight),
    ...
    if (Batch_A <= 0 || Batch_B <= 0) {
        log_error("Invalid batch sizes: Batch_A=", Batch_A,
                ", Batch_B=", Batch_B);
        return status_t::failure;
    }
    ...
    auto validate_per_tensor
            = [](const void *buff, const std::vector<int64_t> &dims,
                      const char *param_name) -> bool {
        if (!buff) { return true; }

        int64_t nelems = std::accumulate(dims.begin(), dims.end(), int64_t {1},
    ...
    if (!is_w4a8 && params.quant_params.src_scale.buff) {
        int64_t nelems
                = quant_param_num_elements(params.quant_params.src_scale.dims);
        if (nelems != 1) {
            ...
            bool is_per_group = (nelems > static_cast<int64_t>(M))
                    ...
                    && (static_cast<int64_t>(K)
                                    % (nelems / static_cast<int64_t>(M))
                            == 0);
            if (!(is_supported_src && is_symmetric && is_supported_dst
                        && (is_per_token || is_per_group))) {
                log_error(
                        "Source quant scale: per-token/per-group requires "
                        "s8/bf16/f32 src, "
            ...
            int64_t wei_scale_nelems = params.quant_params.wei_scale.buff
                    ? std::accumulate(
                              params.quant_params.wei_scale.dims.begin(),
            ...
            if (wei_scale_supplied_externally && is_per_group) {
                ...
                int64_t expected_wei_nelems
                        = src_num_groups * static_cast<int64_t>(N);
                if (wei_scale_nelems != expected_wei_nelems) {
                    log_error(
                            "Per-group source scale requires per-group weight "
                            "scale "
                            "(expected ",
                            expected_wei_nelems, " elements, got ",
                            wei_scale_nelems, ")");
                    return status_t::failure;
                }
            }
    ...
    if (!validate_per_tensor(params.quant_params.src_zp.buff,
            ...
            || !validate_per_tensor(params.quant_params.dst_scale.buff,
                    params.quant_params.dst_scale.dims,
                    "Destination quant scale")
            || !validate_per_tensor(params.quant_params.dst_zp.buff,
                    params.quant_params.dst_zp.dims,
                    "Destination quant zero")) {
        return status_t::failure;
    }
    ...
    if (params.dtypes.wei == data_type_t::u4) {
        ...
        if (params.quant_params.wei_zp.dt != data_type_t::bf16
            ...
            log_error(
                    "Weights quant zero supports only bf16 or s8 data type "
                    "(mandatory) for u4 weights");
            return status_t::failure;
        }
    }
    ...
    if (is_f32_src && (is_bf16_out || is_f16_out)) {
        log_error(
                "Unsupported GEMM configuration: F32 source with BF16/F16 "
                "destination");
        return status_t::failure;
    }
    ...
    if (is_bf16_src && is_f16_out) {
        log_error(
                "Unsupported GEMM configuration: BF16 source with F16 "
                "destination");
        return status_t::failure;
    }
    ...
    if (is_f16_src && is_bf16_out) {
        log_error(
                "Unsupported GEMM configuration: F16 source with BF16 "
                "destination");
        return status_t::failure;
    }
    ...
    if (std::max(Batch_A, Batch_B) % std::min(Batch_A, Batch_B) != 0) {
        log_error(
                "Broadcasting is not compatible with given batch sizes: "
                "Batch_A=",
    ...
    for (size_t i = 0; i < params.postop_.size(); ++i) {
        ...
        // it to s8 and the call runs on that same sym_quant kernel.
            ...
            if (!dlp_int8_path_checked) {
                dlp_int8_path = is_sym_quant_config(params)
                        || is_dynamic_quant_config(params) || is_w4a8
                        || params.packing.pack_format_b == 1;
                dlp_int8_path_checked = true;
            }
            if (dlp_int8_path) {
                log_error("Post-op[", i,
                        "]: mish is not supported on AOCL INT8 "
...
const char *kernel_to_string(matmul_algo_t kernel) {
    switch (kernel) {
        ...
        case matmul_algo_t::auto_tuner: return "auto_tuner";
        case matmul_algo_t::native_gemm: return "native_gemm";
        case matmul_algo_t::native_brgemm: return "native_brgemm";
        case matmul_algo_t::moe_custom_kernel: return "moe_custom_kernel";
        default: return "none";
    }
}
...
unsigned int get_auto_tuner_ver() {
    ...
    if (auto_tuner_version_env) {
        ...
        if (version == 0 || version > 2) { return 1; }
        return version;
    }
...
matmul_algo_t kernel_select(matmul_params &params, int Batch_A, int Batch_B,
    ...
    if (batch_count > 1) {
        ...
        algo = params.lowoha_algo == matmul_algo_t::none
                ? (matmul_config.get_bmm_algo()
                                          == static_cast<int>(
                                                  matmul_algo_t::none)
                                  ? static_cast<int>(matmul_algo_t::libxsmm)
                                  : matmul_config.get_bmm_algo())
    ...
    if (matmul_config.get_weight_cache() == 2
        ...
        log_info(
                "weight_cache_type=2 is incompatible with "
    ...
    if (kernel == matmul_algo_t::auto_tuner
            && (Batch_A != 1 || Batch_B != 1 || !is_weights_const)) {
        kernel = matmul_algo_t::dynamic_dispatch;
    }
    ...
    if (kernel == matmul_algo_t::dynamic_dispatch) {
        if (batch_count > 1) {
            if (params.dtypes.wei == data_type_t::bf16) {
                kernel = select_algo_by_heuristics_bf16_bmm(
                        batch_count, M, N, K, num_threads);
            } else {
                kernel = matmul_algo_t::aocl_dlp;
            }
        } else if (is_woq) {
            kernel = select_algo_by_heuristics_woq_int4_mm(
                    M, N, K, num_threads);
        } else {
            if (is_weights_const == false && M >= 4096 && M <= 8192 && K == 1024
                ...
                if (params.dtypes.wei == data_type_t::bf16) {
                    if (M <= 8) {
                        ...
                        // when !transB).
                        kernel = native::bf16_gemv_best_algo(
                                M, N, K, num_threads, transB);
                    } else {
                    ...
                    const int kp = (K + 3) & ~3;
                    const int np = ((N + native::BKC_NR_PAD - 1)
                                           / native::BKC_NR_PAD)
                            * native::BKC_NR_PAD;
                    const size_t b_packed = static_cast<size_t>(kp) * np;
                    static const size_t l2 = static_cast<size_t>(
                            native::detect_uarch().l2_bytes);
    ...
    const bool is_bf16_f32_per_token_sym = params.dtypes.wei == data_type_t::s8
            && (params.dtypes.src == data_type_t::bf16
                    || params.dtypes.src == data_type_t::f32)
            && !params.quant_params.src_zp.buff && src_scale_nelems > 1
    ...
    if (is_f16) {
        ...
        bool f16_batched_sgemm_eligible
                = (batch_count > 1 && kernel == matmul_algo_t::batched_sgemm
    ...
    if (!is_weights_const && kernel == matmul_algo_t::onednn_blocked) {
        log_info(
                "onednn_blocked requires constant weights, falling back to "
                "onednn");
        kernel = matmul_algo_t::onednn;
    }
    ...
    bool is_aocl_f16_accum = (kernel == matmul_algo_t::aocl_dlp
            || kernel == matmul_algo_t::aocl_dlp_blocked
            || kernel == matmul_algo_t::batched_sgemm);
    matmul_config.set_accum_type((is_aocl_f16_accum && is_f16)
                    ? data_type_t::f16
                    : data_type_t::f32);
