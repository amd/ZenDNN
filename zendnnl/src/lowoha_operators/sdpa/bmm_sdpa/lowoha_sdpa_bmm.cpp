...
status_t bmm_based_sdpa(const void *query, const void *key, const void *value,
    ...
    if (scale == 0.0f) { scale = calculate_default_scale(params.head_dim); }
    ...
    size_t elem_size = (params.qkv_dt == data_type_t::bf16) ? sizeof(uint16_t)
                                                            : sizeof(float);
    ...
    if (attn_mask != nullptr) {
        ...
        mask_po.leading_dim
                = static_cast<int>(mask_po.dims[mask_po.dims.size() - 1]);
    ...
    const size_t scores_bytes
            = static_cast<size_t>(batch_heads) * seq_len * seq_len * elem_size;
    ...
    // =========================================================================
    {
        ...
        if (!mask_postops.empty()) { mm_params.postop_ = mask_postops; }
        ...
        status_t status = matmul::matmul_direct('r', // layout: row-major
                false, // transA: false
...
#pragma omp parallel for collapse(2) num_threads(num_threads)
    ...
    // =========================================================================
    {
        ...
        status_t status = softmax::softmax_direct(scores, // input (in-place)
