...
static int32_t get_conv_weight_cache_type() {
  static const int32_t weight_cache_type = []() -> int32_t {
    if (zendnnl::common::is_global_cache_off()) {
      return 0;
    }
...
bool reorderAndCacheWeights(
  ...
  if (weight_cache_type == 0 || is_weights_const == 0) {
    ...
    dnnl::reorder(src_weights_mem, dst_weights_mem).execute(eng_stream,
        src_weights_mem, dst_weights_mem);
  }
  else {
...
status_t conv_onednn_wrapper(
  ...
  try {
    ...
    auto src_md = dnnl::memory::desc(src_dims, dtype,
                                     dnnl::memory::format_tag::nhwc);

    // Weights: allow OneDNN to choose optimal format
    auto weights_md = dnnl::memory::desc(weights_dims, dtype,
                                         dnnl::memory::format_tag::any);

    // Output in NHWC format
    auto dst_md = dnnl::memory::desc(dst_dims, dtype,
                                     dnnl::memory::format_tag::nhwc);

    // Bias memory descriptor (if present)
    std::unique_ptr<dnnl::memory::desc> bias_md_ptr;
    if (bias != nullptr) {
      bias_md_ptr.reset(new dnnl::memory::desc(bias_dims, dtype,
    ...
    if (params.depthwise.is_depthwise) {
      ...
      auto weights_hwio_mem = dnnl::memory(weights_hwio_md, eng,
                                           const_cast<void *>(filter));

      // Reorder and cache weights
      reorderAndCacheWeights(cache_key, weights_hwio_mem, weights_mem, eng,
                             is_weights_const);
