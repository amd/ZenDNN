 ...
 *
 * Reads ZENDNNL_CACHE_OFF / JSON `global_cache_off` as a global kill switch, then
 * ZENDNNL_CONV_WEIGHT_CACHE as the conv-specific override:
...
static int32_t get_conv_weight_cache_type() {
    static const int32_t weight_cache_type = []() -> int32_t {
        if (zendnnl::common::is_global_cache_off()) { return 0; }

        bool conv_weight_cache_enabled = true;

        char *weight_cache_env = std::getenv("ZENDNNL_CONV_WEIGHT_CACHE");
        if (weight_cache_env) {
            bool parsed = conv_weight_cache_enabled;
            if (zendnnl::common::parse_cache_bool(weight_cache_env, parsed)) {
                conv_weight_cache_enabled = parsed;
            } else {
                apilog_warning("Unrecognized ZENDNNL_CONV_WEIGHT_CACHE value '",
                        weight_cache_env,
                        "'; expected 0/1, false/true, off/on or no/yes. "
                        "Keeping local cache default.");
            }
        }
        return conv_weight_cache_enabled ? 1 : 0;
    }();
...
bool reorderAndCacheWeights(const Key_conv &key, dnnl::memory &src_weights_mem,
    ...
    static lru_cache_t<Key_conv, dnnl::memory> conv_weight_cache(
    ...
    if (weight_cache_type == 0 || is_weights_const == 0) {
        ...
        dnnl::reorder(src_weights_mem, dst_weights_mem)
                .execute(eng_stream, src_weights_mem, dst_weights_mem);
    } else {
        ...
        if (conv_weight_cache.try_get(key, dst_weights_mem)) {
            apilog_info(
                    "Read onednn conv cached weights "
                    "WEIGHT_CACHE_OUT_OF_PLACE");
        } else {
            apilog_info(
                    "onednn conv reorder weights WEIGHT_CACHE_OUT_OF_PLACE");
            dnnl::stream eng_stream(eng);
            dnnl::reorder(src_weights_mem, dst_weights_mem)
                    .execute(eng_stream, src_weights_mem, dst_weights_mem);
...
status_t conv_onednn_wrapper(const void *input, const void *filter,
    ...
    try {
        ...
        if (params.depthwise.is_depthwise) {
            ...
            weights_dims = {static_cast<dnnl::memory::dim>(
                                    params.depthwise.groups), // groups = C_in
                    static_cast<dnnl::memory::dim>(
                            params.depthwise.depth_multiplier), // OC per group
            ...
            log_info("Conv OneDNN: Depthwise convolution with groups=",
                    params.depthwise.groups,
            ...
            weights_dims = {static_cast<dnnl::memory::dim>(dims.out_channels),
                    static_cast<dnnl::memory::dim>(dims.in_channels),
                    static_cast<dnnl::memory::dim>(dims.filter_height),
                    static_cast<dnnl::memory::dim>(dims.filter_width)};
        }
        ...
        auto weights_md = dnnl::memory::desc(
                weights_dims, dtype, dnnl::memory::format_tag::any);
        ...
        if (bias != nullptr) {
            bias_md_ptr.reset(new dnnl::memory::desc(
                    bias_dims, dtype, dnnl::memory::format_tag::x));
        }
        ...
        for (const auto &po : params.postop_) {
            switch (po.po_type) {
                case post_op_type_t::relu:
                    ...
                    pops.append_eltwise(
                            dnnl::algorithm::eltwise_relu, po.alpha, po.beta);
                    break;
                case post_op_type_t::clip:
                    log_info(
                            "Conv2D OneDNN: Adding ReLU6 post-op (clipped "
                            "relu)");
                    pops.append_eltwise(
                            dnnl::algorithm::eltwise_clip, po.alpha, po.beta);
                    break;
                ...
                case post_op_type_t::elu:
                    ...
                    pops.append_eltwise(
                            dnnl::algorithm::eltwise_elu, po.alpha, po.beta);
                    break;
                case post_op_type_t::gelu_tanh:
                    log_info("Conv2D OneDNN: Adding GELU-Tanh post-op");
                    pops.append_eltwise(dnnl::algorithm::eltwise_gelu_tanh,
                            po.alpha, po.beta);
                    break;
                case post_op_type_t::gelu_erf:
                    log_info("Conv2D OneDNN: Adding GELU-Erf post-op");
                    pops.append_eltwise(dnnl::algorithm::eltwise_gelu_erf,
                            po.alpha, po.beta);
                    break;
                case post_op_type_t::tanh:
                    log_info("Conv2D OneDNN: Adding Tanh post-op");
                    pops.append_eltwise(
                            dnnl::algorithm::eltwise_tanh, po.alpha, po.beta);
                    break;
                case post_op_type_t::sigmoid:
                    log_info("Conv2D OneDNN: Adding Sigmoid post-op");
                    pops.append_eltwise(dnnl::algorithm::eltwise_logistic,
                            po.alpha, po.beta);
                    break;
                case post_op_type_t::binary_add: {
                    log_info(
                            "Conv2D OneDNN: Adding Binary Add post-op "
                            "(residual connection)");
                ...
                default:
                    log_error("Conv2D OneDNN: Unsupported post-op type: ",
                            static_cast<int>(po.po_type));
                    break;
            }
            post_op_index++;
        }

        if (pops.len() > 0) { conv_attr.set_post_ops(pops); }
        ...
        if (bias != nullptr) {
            conv_pd.reset(new dnnl::convolution_forward::primitive_desc(eng,
                    dnnl::prop_kind::forward_inference,
                    dnnl::algorithm::convolution_direct, src_md, weights_md,
                    *bias_md_ptr, dst_md, strides, dilations, padding_l,
                    padding_r, conv_attr));
        } else {
            conv_pd.reset(new dnnl::convolution_forward::primitive_desc(eng,
                    dnnl::prop_kind::forward_inference,
                    dnnl::algorithm::convolution_direct, src_md, weights_md,
                    dst_md, strides, dilations, padding_l, padding_r,
                    conv_attr));
        }

        // Create hash for blocking format
        auto hash_blocking_desc
                = [](const dnnl::memory::desc &mem_desc) -> size_t {
            size_t hash_value = 0;
            ...
            for (int i = 0; i < inner_nblks; ++i) {
                hash_value = hash_value * prime
                        + std::hash<int64_t> {}(inner_blks[i]);
                hash_value = hash_value * prime
                        + std::hash<int64_t> {}(inner_idxs[i]);
            }
        ...
        if (params.depthwise.is_depthwise) {
            ...
            const int64_t groups
                    = static_cast<int64_t>(params.depthwise.groups);
            const int64_t dm
                    = static_cast<int64_t>(params.depthwise.depth_multiplier);

            dnnl::memory::dims hwio_strides_5d = {
                    dm, // stride for G (maps to I in HWIO)
                    static_cast<dnnl::memory::dim>(
                            1), // stride for O_per_group (maps to O in HWIO)
                    static_cast<dnnl::memory::dim>(
                            1), // stride for I_per_group (trivial, size=1)
                    filt_w * groups * dm, // stride for H
                    groups * dm // stride for W
            };

            auto weights_hwio_md
                    = dnnl::memory::desc(weights_dims, dtype, hwio_strides_5d);
            auto weights_hwio_mem = dnnl::memory(
                    weights_hwio_md, eng, const_cast<void *>(filter));

            // Reorder and cache weights
            reorderAndCacheWeights(cache_key, weights_hwio_mem, weights_mem,
                    eng, is_weights_const);

            apilog_info(
                    "Conv OneDNN: Depthwise weights reordered from TF "
                    "[H,W,I,dm] to OneDNN blocked format");
            ...
            dnnl::memory::dims hwio_strides_4d = {
                    static_cast<dnnl::memory::dim>(
                            1), // stride for O (innermost in HWIO)
                    out_ch, // stride for I
            ...
            auto weights_hwio_md
                    = dnnl::memory::desc(weights_dims, dtype, hwio_strides_4d);
            auto weights_hwio_mem = dnnl::memory(
                    weights_hwio_md, eng, const_cast<void *>(filter));
            ...
            apilog_info(
                    "Conv OneDNN: Standard weights reordered from TF [H,W,I,O] "
                    "to OneDNN blocked format");
        }
        ...
        if (bias != nullptr) {
            auto bias_mem
                    = dnnl::memory(*bias_md_ptr, eng, const_cast<void *>(bias));
...
status_t conv_onednn_wrapper(const void *input, const void *filter,
        const void *bias, void *output, conv_params &params) {
    log_error(
            "Conv: OneDNN backend not available (ZENDNNL_DEPENDS_ONEDNN not "
            "defined)");
    return status_t::failure;
}
