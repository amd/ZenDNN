...
inline float apply_postop(float val, const conv_postop &postop,
    ...
    switch (postop.po_type) {
        ...
        case post_op_type_t::sigmoid: return 1.0f / (1.0f + std::exp(-val));

        case post_op_type_t::tanh: return std::tanh(val);
 ...
 */
template <typename T>
void conv2d_reference_impl(const T *input, const T *filter, const T *bias,
        T *output, const conv_params &params, const int num_threads) {
    ...
    log_info("Conv2D Reference: batch=", batch, " in=[", in_h, ",", in_w, ",",
            in_c, "] out=[", out_h, ",", out_w, ",", out_c, "] kernel=[", kh,
            ",", kw, "]");
    log_info("Conv2D Reference: stride=[", stride_h, ",", stride_w, "] pad=[",
            pad_t, ",", pad_l, "] dilation=[", dilation_h, ",", dilation_w,
            "] groups=", groups);
            ...
            for (uint64_t ow = 0; ow < out_w; ++ow) {
                ...
                for (uint64_t oc = 0; oc < out_c; ++oc) {
                    ...
                    for (uint64_t kh_idx = 0; kh_idx < kh; ++kh_idx) {
                        for (uint64_t kw_idx = 0; kw_idx < kw; ++kw_idx) {
                            ...
                            int64_t ih = static_cast<int64_t>(oh * stride_h)
                                    + static_cast<int64_t>(kh_idx * dilation_h)
                                    - pad_t;
                            int64_t iw = static_cast<int64_t>(ow * stride_w)
                                    + static_cast<int64_t>(kw_idx * dilation_w)
                                    - pad_l;
                            ...
                            if (ih >= 0 && ih < static_cast<int64_t>(in_h)
                                ...
                                for (uint64_t ic = ic_start; ic < ic_end;
                                    ...
                                    if (is_depthwise) {
                                        ...
                                        filter_idx = group * kh * kw
                                                        * in_c_per_group
                                                        * out_c_per_group
                                                + kh_idx * kw * in_c_per_group
                                                        * out_c_per_group
                                                + kw_idx * in_c_per_group
                                                        * out_c_per_group
                                                + ic_offset * out_c_per_group
                                    ...
                                    float in_val
                                            = static_cast<float>(input[in_idx]);
                                    float filter_val = static_cast<float>(
                                            filter[filter_idx]);
                    ...
                    uint64_t out_idx = n * out_h * out_w * out_c
                            + oh * out_w * out_c + ow * out_c + oc;

                    for (const auto &postop : params.postop_) {
                        sum = apply_postop<T>(
                                sum, postop, postop.buff, out_idx);
                    }
...
status_t conv_reference_wrapper(const void *input, const void *filter,
    ...
    if (params.dtypes.input == data_type_t::f32) {
        conv2d_reference_impl<float>(static_cast<const float *>(input),
