...
struct depthwise_params {
    ...
    uint32_t
            groups; ///< Number of groups for grouped convolution (1 = standard conv)
    bool is_depthwise; ///< True if depthwise convolution (groups == in_channels)
    uint32_t
            depth_multiplier; ///< Depth multiplier for depthwise (output_channels_per_group)
