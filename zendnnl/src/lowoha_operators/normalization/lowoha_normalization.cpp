...
status_t normalization_kernel_wrapper(const void *input, void *output,
    ...
    const bool ln_f16_fma_eligible
            = rms_f16_fma_eligible && (!params.use_shift || beta_f16_fma_ok);
    ...
    if (can_use_f16_fma_kernel() && rms_f16_fma_eligible
        ...
        status_t status
                = rms_norm_avx512_fp16(input, output, residual, gamma, params);
    ...
    if (has_avx512f
        ...
        params.accum_type = data_type_t::f32;
        status_t status
                = rms_norm_avx512(input, output, residual, gamma, params);
...
status_t normalization_direct(const void *input, void *output,
    ...
    if ((src_is_f16 && dst_is_bf16) || (src_is_bf16 && dst_is_f16)) {
        log_error(
                "Normalization: f16/bf16 cross-mixing between src and dst "
                "is not supported");
