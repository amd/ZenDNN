...
static inline int simd_accum_lane(uint64_t i, uint64_t vec128) {
    if (i < vec128) { return static_cast<int>(i % F16_ACCUM_LANES); }
...
static inline float load_scalar(const void *base, data_type_t dt, uint64_t i) {
    switch (dt) {
        case data_type_t::f32: return static_cast<const float *>(base)[i];
...
static inline void store_scalar(
        void *base, data_type_t dt, uint64_t i, float v) {
    switch (dt) {
        case data_type_t::f32: static_cast<float *>(base)[i] = v; break;
        case data_type_t::bf16:
...
// ============================================================================
static void layer_norm_impl(const void *input, void *output, const void *gamma,
        const void *beta, const norm_params &params, int num_threads) {
...
// ============================================================================
static void batch_norm_impl(const void *input, void *output, const void *gamma,
        const void *beta, const float *running_mean, const float *running_var,
    ...
    for (uint64_t n = 0; n < N; ++n) {
        for (uint64_t c = 0; c < C; ++c) {
            ...
            for (uint64_t s = 0; s < spatial_size; ++s) {
                ...
                if (params.use_scale) { norm_val *= g; }
                if (params.use_shift) { norm_val += bt; }
...
// Where N = norm_size.
// ============================================================================
static void rms_norm_impl(const void *input, void *output, const void *gamma,
        const norm_params &params, int num_threads) {
    const uint64_t batch = params.batch;
    const uint64_t norm_size = params.norm_size;
    const float eps = params.epsilon;
...
// ============================================================================
static void fused_add_rms_norm_impl(const void *input, void *output,
        void *residual, const void *gamma, const norm_params &params,
        int num_threads) {
    const uint64_t batch = params.batch;
    const uint64_t norm_size = params.norm_size;
    const float eps = params.epsilon;
    ...
    for (uint64_t b = 0; b < batch; ++b) {
        ...
        if (use_f16_accum) {
            ...
            for (uint64_t i = 0; i < norm_size; ++i) {
                ...
                sq_lane[lane]
                        = to_f16_rounded(std::fmaf(sum, sum, sq_lane[lane]));
...
status_t normalization_reference_wrapper(const void *input, void *output,
    ...
    switch (params.norm_type) {
        case norm_type_t::LAYER_NORM:
            layer_norm_impl(input, output, gamma, beta, params, num_threads);
            return status_t::success;

        case norm_type_t::BATCH_NORM:
            batch_norm_impl(input, output, gamma, beta,
                    static_cast<const float *>(running_mean),
                    static_cast<const float *>(running_var), params,
                    num_threads);
            return status_t::success;

        case norm_type_t::RMS_NORM:
            rms_norm_impl(input, output, gamma, params, num_threads);
            return status_t::success;
