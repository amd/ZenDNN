...
static inline void layer_norm_row_fp16(const void *__restrict in_row,
    ...
    float total_sum = reduce_add_ph_to_fp32(sum0) + reduce_add_ph_to_fp32(sum1)
            + reduce_add_ph_to_fp32(sum2) + reduce_add_ph_to_fp32(sum3);
    float total_sq = reduce_add_ph_to_fp32(sq0) + reduce_add_ph_to_fp32(sq1)
            + reduce_add_ph_to_fp32(sq2) + reduce_add_ph_to_fp32(sq3);
    ...
    if (use_scale) {
        for (; i < vec128; i += 128) {
            __m512h g0 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i + 0), inv_std_h);
            __m512h g1 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i + 32), inv_std_h);
            __m512h g2 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i + 64), inv_std_h);
            __m512h g3 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i + 96), inv_std_h);

            __m512h d0 = _mm512_sub_ph(
                    f16x32_load_typed<InType>(in_p + i + 0), mean_h);
            __m512h d1 = _mm512_sub_ph(
                    f16x32_load_typed<InType>(in_p + i + 32), mean_h);
            __m512h d2 = _mm512_sub_ph(
                    f16x32_load_typed<InType>(in_p + i + 64), mean_h);
            __m512h d3 = _mm512_sub_ph(
                    f16x32_load_typed<InType>(in_p + i + 96), mean_h);
        ...
        for (; i + 31 < norm_size; i += 32) {
            __m512h g0 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i), inv_std_h);
            __m512h d0 = _mm512_sub_ph(
                    f16x32_load_typed<InType>(in_p + i), mean_h);
            if (use_shift) {
        ...
        if (i < norm_size) {
            ...
            __mmask32 mask = (__mmask32)((1ULL << tail) - 1ULL);
            __m512h g0 = _mm512_mul_ph(
                    f16x32_load_mask_typed<GammaType>(g_p + i, mask),
                    inv_std_h);
            __m512h d0 = _mm512_sub_ph(
        ...
        for (; i < vec128; i += 128) {
            __m512h d0 = _mm512_mul_ph(
                    _mm512_sub_ph(
                            f16x32_load_typed<InType>(in_p + i + 0), mean_h),
                    inv_std_h);
            __m512h d1 = _mm512_mul_ph(
                    _mm512_sub_ph(
                            f16x32_load_typed<InType>(in_p + i + 32), mean_h),
                    inv_std_h);
            __m512h d2 = _mm512_mul_ph(
                    _mm512_sub_ph(
                            f16x32_load_typed<InType>(in_p + i + 64), mean_h),
                    inv_std_h);
            __m512h d3 = _mm512_mul_ph(
                    _mm512_sub_ph(
                            f16x32_load_typed<InType>(in_p + i + 96), mean_h),
                    inv_std_h);
            ...
            if (use_shift) {
                d0 = _mm512_add_ph(
                        d0, f16x32_load_typed<BetaType>(b_p + i + 0));
                d1 = _mm512_add_ph(
                        d1, f16x32_load_typed<BetaType>(b_p + i + 32));
                d2 = _mm512_add_ph(
                        d2, f16x32_load_typed<BetaType>(b_p + i + 64));
                d3 = _mm512_add_ph(
                        d3, f16x32_load_typed<BetaType>(b_p + i + 96));
            }
        ...
        for (; i + 31 < norm_size; i += 32) {
            __m512h d0 = _mm512_mul_ph(
                    _mm512_sub_ph(f16x32_load_typed<InType>(in_p + i), mean_h),
                    inv_std_h);
            if (use_shift) {
        ...
        if (i < norm_size) {
            ...
            __mmask32 mask = (__mmask32)((1ULL << tail) - 1ULL);
            __m512h d0 = _mm512_mul_ph(
                    _mm512_sub_ph(f16x32_load_tail_typed<InType>(
                                          in_p + i, mask, tail),
                            mean_h),
                    inv_std_h);
            if (use_shift) {
                d0 = _mm512_add_ph(
                        d0, f16x32_load_mask_typed<BetaType>(b_p + i, mask));
            }
            f16x32_store_tail_typed<OutType>(out_p + i, d0, mask, tail);
        }
    }
...
// =============================================================================
// Fused Layer Norm + Add (native FP16) — single row.
//
//   mean    = (1/N) * Σ x[i]
//   var     = (1/N) * Σ x[i]² - mean²          (clamped to ≥ 0)
//   inv_std = 1 / sqrt(var + eps)
//   y[i]   = gamma[i] * (x[i] - mean) * inv_std + beta[i] + residual[i]
//
// Identical to layer_norm_row_fp16 plus a read-only residual (element type ==
// OutType, the output domain) added at each store boundary. The add is a pure
// epilogue after all reduction math, so it only incurs the usual single
// store-time rounding — no accumulation-precision concern, hence no build flag.
// =============================================================================

template <typename InType, typename OutType, typename GammaType,
        typename BetaType>
static inline void fused_layer_norm_add_row_fp16(const void *__restrict in_row,
        void *__restrict out_row, const void *__restrict res_row,
        const void *__restrict gamma, const void *__restrict beta,
        uint64_t norm_size, float inv_n, float epsilon, bool use_scale,
        bool use_shift) {
    const InType *in_p = static_cast<const InType *>(in_row);
    OutType *out_p = static_cast<OutType *>(out_row);
    const OutType *r_p = static_cast<const OutType *>(res_row);
    const GammaType *g_p = static_cast<const GammaType *>(gamma);
    const BetaType *b_p = static_cast<const BetaType *>(beta);
    ...
    if (use_scale) {
        for (; i < vec128; i += 128) {
            ...
            d0 = _mm512_add_ph(d0, f16x32_load_typed<OutType>(r_p + i + 0));
            d1 = _mm512_add_ph(d1, f16x32_load_typed<OutType>(r_p + i + 32));
            d2 = _mm512_add_ph(d2, f16x32_load_typed<OutType>(r_p + i + 64));
            d3 = _mm512_add_ph(d3, f16x32_load_typed<OutType>(r_p + i + 96));
        ...
        for (; i + 31 < norm_size; i += 32) {
            ...
            d0 = _mm512_add_ph(d0, f16x32_load_typed<OutType>(r_p + i));
            f16x32_store_typed<OutType>(out_p + i, d0);
        }
        if (i < norm_size) {
            const int tail = static_cast<int>(norm_size - i);
            ...
            d0 = _mm512_add_ph(
                    d0, f16x32_load_mask_typed<OutType>(r_p + i, mask));
            f16x32_store_tail_typed<OutType>(out_p + i, d0, mask, tail);
        }
    } else {
        for (; i < vec128; i += 128) {
...
// Row-kernel function pointers. The plain path takes (in, out, gamma, beta,
// ...); the fused path inserts a read-only residual: (in, out, res, gamma,
// beta, ...). Keeping them as distinct signatures mirrors the plain vs. fused
// split in rmsnorm_avx512_fp16_kernel.cpp.
using ln_row_fn_t = void (*)(const void *, void *, const void *, const void *,
        uint64_t, float, float, bool, bool);
using ln_fused_row_fn_t = void (*)(const void *, void *, const void *,
        const void *, const void *, uint64_t, float, float, bool, bool);

// Select the plain (gamma, beta) template specialization for a given (src, dst)
// tile. 4 combinations per (src, dst) tile; 12 total entries.
static ln_row_fn_t select_ln_row_fp16(data_type_t src_dt, data_type_t dst_dt,
        data_type_t gamma_dt, data_type_t beta_dt) {
    if (src_dt == data_type_t::f16 && dst_dt == data_type_t::f16) {
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f16) {
            return &layer_norm_row_fp16<float16_t, float16_t, float16_t,
                    float16_t>;
        }
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f32) {
            return &layer_norm_row_fp16<float16_t, float16_t, float16_t, float>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f16) {
            return &layer_norm_row_fp16<float16_t, float16_t, float, float16_t>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f32) {
            return &layer_norm_row_fp16<float16_t, float16_t, float, float>;
        }
    } else if (src_dt == data_type_t::f16 && dst_dt == data_type_t::f32) {
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f16) {
            return &layer_norm_row_fp16<float16_t, float, float16_t, float16_t>;
        }
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f32) {
            return &layer_norm_row_fp16<float16_t, float, float16_t, float>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f16) {
            return &layer_norm_row_fp16<float16_t, float, float, float16_t>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f32) {
            return &layer_norm_row_fp16<float16_t, float, float, float>;
        }
    } else if (src_dt == data_type_t::f32 && dst_dt == data_type_t::f16) {
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f16) {
            return &layer_norm_row_fp16<float, float16_t, float16_t, float16_t>;
        }
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f32) {
            return &layer_norm_row_fp16<float, float16_t, float16_t, float>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f16) {
            return &layer_norm_row_fp16<float, float16_t, float, float16_t>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f32) {
            return &layer_norm_row_fp16<float, float16_t, float, float>;
        }
    }
    return nullptr;
}

// Select the fused (LayerNorm + residual add) specialization. Same 12-entry
// tile as the plain selector; the residual element type is the output domain
// (OutType) so no extra type axis is needed.
static ln_fused_row_fn_t select_fused_ln_row_fp16(data_type_t src_dt,
        data_type_t dst_dt, data_type_t gamma_dt, data_type_t beta_dt) {
    if (src_dt == data_type_t::f16 && dst_dt == data_type_t::f16) {
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f16) {
            return &fused_layer_norm_add_row_fp16<float16_t, float16_t,
                    float16_t, float16_t>;
        }
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f32) {
            return &fused_layer_norm_add_row_fp16<float16_t, float16_t,
        ...
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f16) {
            return &fused_layer_norm_add_row_fp16<float16_t, float16_t, float,
                    float16_t>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f32) {
            return &fused_layer_norm_add_row_fp16<float16_t, float16_t, float,
        ...
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f16) {
            return &fused_layer_norm_add_row_fp16<float16_t, float, float16_t,
                    float16_t>;
        }
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f32) {
            return &fused_layer_norm_add_row_fp16<float16_t, float, float16_t,
        ...
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f16) {
            return &fused_layer_norm_add_row_fp16<float16_t, float, float,
                    float16_t>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f32) {
            return &fused_layer_norm_add_row_fp16<float16_t, float, float,
        ...
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f16) {
            return &fused_layer_norm_add_row_fp16<float, float16_t, float16_t,
                    float16_t>;
        }
        if (gamma_dt == data_type_t::f16 && beta_dt == data_type_t::f32) {
            return &fused_layer_norm_add_row_fp16<float, float16_t, float16_t,
        ...
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f16) {
            return &fused_layer_norm_add_row_fp16<float, float16_t, float,
                    float16_t>;
        }
        if (gamma_dt == data_type_t::f32 && beta_dt == data_type_t::f32) {
            return &fused_layer_norm_add_row_fp16<float, float16_t, float,
...
// =====================================================================
// Dispatches LAYER_NORM (with mixed-dtype f16/f32 boundaries) and
// FUSED_LAYER_NORM_ADD (residual is a read-only addend in the output domain)
// over the batch.
//
...
//
// FUSED_LAYER_NORM_ADD reuses the same 12 tiles: the residual (element type ==
// dst_dt) is added at the store boundary and advances with out_elem_sz. No
// build flag is needed because the add is a pure epilogue.
//
...
status_t layer_norm_avx512_fp16(const void *input, void *output,
    ...
    const bool is_fused = (params.norm_type == norm_type_t::FUSED_LAYER_NORM_ADD
            && residual);

    ln_row_fn_t row_fn = nullptr;
    ln_fused_row_fn_t fused_row_fn = nullptr;
    size_t in_elem_sz = 0;
    ...
    if (params.use_shift && params.beta_dt != data_type_t::f16
            && params.beta_dt != data_type_t::f32) {
        return status_t::unimplemented;
    }
    const data_type_t beta_dt
            = (params.use_shift && params.beta_dt == data_type_t::f32)
            ? data_type_t::f32
            : data_type_t::f16;
    ...
    if (is_fused) {
        fused_row_fn
                = select_fused_ln_row_fp16(src_dt, dst_dt, gamma_dt, beta_dt);
    } else {
        row_fn = select_ln_row_fp16(src_dt, dst_dt, gamma_dt, beta_dt);
    }

    if (row_fn == nullptr && fused_row_fn == nullptr) {
    ...
    auto row_loop = [&](int64_t b) {
        const char *in_byte_ptr
                = static_cast<const char *>(input) + b * N * in_elem_sz;
        char *out_byte_ptr = static_cast<char *>(output) + b * N * out_elem_sz;
        if (is_fused) {
            const char *res_byte_ptr
                    = static_cast<const char *>(residual) + b * N * out_elem_sz;
            fused_row_fn(in_byte_ptr, out_byte_ptr, res_byte_ptr, gamma, beta,
                    N, inv_n, params.epsilon, params.use_scale,
                    params.use_shift);
            return;
        }
        row_fn(in_byte_ptr, out_byte_ptr, gamma, beta, N, inv_n, params.epsilon,
...
status_t layer_norm_avx512_fp16(const void *, void *, const void *,
        const void *, const void *, norm_params &) {
