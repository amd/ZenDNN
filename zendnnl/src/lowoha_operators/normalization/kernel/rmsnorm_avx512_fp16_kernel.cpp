...
static inline void rms_norm_row_fp16(const void *__restrict in_row,
    ...
    float total_sq = reduce_add_ph_to_fp32(acc0) + reduce_add_ph_to_fp32(acc1)
            + reduce_add_ph_to_fp32(acc2) + reduce_add_ph_to_fp32(acc3);
    ...
    if (use_scale) {
        for (; i < vec128; i += 128) {
            __m512h g0 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i + 0), inv_rms_h);
            __m512h g1 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i + 32), inv_rms_h);
            __m512h g2 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i + 64), inv_rms_h);
            __m512h g3 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i + 96), inv_rms_h);
            ...
            f16x32_store_typed<OutType>(out_p + i + 32,
                    _mm512_mul_ph(
                            f16x32_load_typed<InType>(in_p + i + 32), g1));
            f16x32_store_typed<OutType>(out_p + i + 64,
                    _mm512_mul_ph(
                            f16x32_load_typed<InType>(in_p + i + 64), g2));
            f16x32_store_typed<OutType>(out_p + i + 96,
                    _mm512_mul_ph(
                            f16x32_load_typed<InType>(in_p + i + 96), g3));
        }
        for (; i + 31 < norm_size; i += 32) {
            __m512h g0 = _mm512_mul_ph(
                    f16x32_load_typed<GammaType>(g_p + i), inv_rms_h);
        ...
        if (i < norm_size) {
            ...
            f16x32_store_tail_typed<OutType>(out_p + i,
                    _mm512_mul_ph(f16x32_load_tail_typed<InType>(
                                          in_p + i, mask, tail),
                            g0),
                    mask, tail);
        }
        ...
        for (; i < vec128; i += 128) {
            f16x32_store_typed<OutType>(out_p + i + 0,
                    _mm512_mul_ph(f16x32_load_typed<InType>(in_p + i + 0),
                            inv_rms_h));
            f16x32_store_typed<OutType>(out_p + i + 32,
                    _mm512_mul_ph(f16x32_load_typed<InType>(in_p + i + 32),
                            inv_rms_h));
            f16x32_store_typed<OutType>(out_p + i + 64,
                    _mm512_mul_ph(f16x32_load_typed<InType>(in_p + i + 64),
                            inv_rms_h));
            f16x32_store_typed<OutType>(out_p + i + 96,
                    _mm512_mul_ph(f16x32_load_typed<InType>(in_p + i + 96),
                            inv_rms_h));
        }
        for (; i + 31 < norm_size; i += 32) {
            f16x32_store_typed<OutType>(out_p + i,
                    _mm512_mul_ph(
                            f16x32_load_typed<InType>(in_p + i), inv_rms_h));
        }
        if (i < norm_size) {
            const int tail = static_cast<int>(norm_size - i);
            __mmask32 mask = (__mmask32)((1ULL << tail) - 1ULL);
...
static inline void fused_add_rms_row_fp16(const void *__restrict in_row,
    ...
    for (; i < vec128; i += 128) {
        __m512h s0
                = _mm512_add_ph(f16x32_load_typed<float16_t>(res_u16 + i + 0),
                        f16x32_load_typed<float16_t>(in_u16 + i + 0));
        __m512h s1
                = _mm512_add_ph(f16x32_load_typed<float16_t>(res_u16 + i + 32),
                        f16x32_load_typed<float16_t>(in_u16 + i + 32));
        __m512h s2
                = _mm512_add_ph(f16x32_load_typed<float16_t>(res_u16 + i + 64),
                        f16x32_load_typed<float16_t>(in_u16 + i + 64));
        __m512h s3
                = _mm512_add_ph(f16x32_load_typed<float16_t>(res_u16 + i + 96),
    ...
    if (i < norm_size) {
        ...
        __m512h s0 = _mm512_add_ph(
                f16x32_load_mask_typed<float16_t>(res_u16 + i, mask),
    ...
    if (use_scale) {
        for (; i < vec128; i += 128) {
            __m512h g0 = _mm512_mul_ph(
                    f16x32_load_typed<float16_t>(g_u16 + i + 0), inv_rms_h);
            __m512h g1 = _mm512_mul_ph(
                    f16x32_load_typed<float16_t>(g_u16 + i + 32), inv_rms_h);
            __m512h g2 = _mm512_mul_ph(
                    f16x32_load_typed<float16_t>(g_u16 + i + 64), inv_rms_h);
            __m512h g3 = _mm512_mul_ph(
                    f16x32_load_typed<float16_t>(g_u16 + i + 96), inv_rms_h);

            f16x32_store_typed<float16_t>(out_u16 + i + 0,
                    _mm512_mul_ph(
                            f16x32_load_typed<float16_t>(res_u16 + i + 0), g0));
            f16x32_store_typed<float16_t>(out_u16 + i + 32,
                    _mm512_mul_ph(
                            f16x32_load_typed<float16_t>(res_u16 + i + 32),
                            g1));
            f16x32_store_typed<float16_t>(out_u16 + i + 64,
                    _mm512_mul_ph(
                            f16x32_load_typed<float16_t>(res_u16 + i + 64),
                            g2));
            f16x32_store_typed<float16_t>(out_u16 + i + 96,
                    _mm512_mul_ph(
                            f16x32_load_typed<float16_t>(res_u16 + i + 96),
                            g3));
        }
        for (; i + 31 < norm_size; i += 32) {
            __m512h g0 = _mm512_mul_ph(
                    f16x32_load_typed<float16_t>(g_u16 + i), inv_rms_h);
            f16x32_store_typed<float16_t>(out_u16 + i,
                    _mm512_mul_ph(
                            f16x32_load_typed<float16_t>(res_u16 + i), g0));
        }
        if (i < norm_size) {
            __mmask32 mask = (__mmask32)((1ULL << (norm_size - i)) - 1ULL);
            __m512h g0 = _mm512_mul_ph(
                    f16x32_load_mask_typed<float16_t>(g_u16 + i, mask),
                    inv_rms_h);
            f16x32_store_mask_typed<float16_t>(out_u16 + i,
                    _mm512_mul_ph(f16x32_load_mask_typed<float16_t>(
                                          res_u16 + i, mask),
                            g0),
                    mask);
        }
        ...
        for (; i < vec128; i += 128) {
            f16x32_store_typed<float16_t>(out_u16 + i + 0,
                    _mm512_mul_ph(f16x32_load_typed<float16_t>(res_u16 + i + 0),
                            inv_rms_h));
            f16x32_store_typed<float16_t>(out_u16 + i + 32,
                    _mm512_mul_ph(
                            f16x32_load_typed<float16_t>(res_u16 + i + 32),
        ...
        for (; i + 31 < norm_size; i += 32) {
            f16x32_store_typed<float16_t>(out_u16 + i,
                    _mm512_mul_ph(f16x32_load_typed<float16_t>(res_u16 + i),
                            inv_rms_h));
        }
        if (i < norm_size) {
            __mmask32 mask = (__mmask32)((1ULL << (norm_size - i)) - 1ULL);
...
status_t rms_norm_avx512_fp16(const void *input, void *output, void *residual,
    ...
    const bool is_fused
            = (params.norm_type == norm_type_t::FUSED_ADD_RMS_NORM && residual);
#else
    ...
    using rms_row_fn_t = void (*)(
            const void *, void *, const void *, uint64_t, float, float, bool);
    ...
    auto row_loop = [&](int64_t b) {
#if defined(ZENDNNL_FUSED_ADD_RMS_F16)
        if (is_fused) {
            fused_add_rms_row_fp16(static_cast<const uint16_t *>(input) + b * N,
                    static_cast<uint16_t *>(output) + b * N,
                    static_cast<uint16_t *>(residual) + b * N, gamma, N, inv_n,
                    params.epsilon, params.use_scale);
            return;
        }
#endif
        ...
        row_fn(in_byte_ptr, out_byte_ptr, gamma, N, inv_n, params.epsilon,
                params.use_scale);
    };
...
status_t rms_norm_avx512_fp16(
        const void *, void *, void *, const void *, norm_params &) {
