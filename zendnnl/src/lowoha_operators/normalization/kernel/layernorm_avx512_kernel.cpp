...
static inline void layer_norm_row_avx512(
  ...
  if (use_scale) {
    ...
    for (; i < vec64; i += 64) {
      ...
      __m512 d0 = _mm512_sub_ps(load16(np,                           src_dt),
                                mean_v);
      __m512 d1 = _mm512_sub_ps(load16((const char *)np + 16*src_sz, src_dt),
                                mean_v);
      __m512 d2 = _mm512_sub_ps(load16((const char *)np + 32*src_sz, src_dt),
                                mean_v);
      __m512 d3 = _mm512_sub_ps(load16((const char *)np + 48*src_sz, src_dt),
                                mean_v);
    ...
    for (; i + 15 < norm_size; i += 16) {
      __m512 g0 = _mm512_mul_ps(
                    load16(static_cast<const char *>(gamma) + i*g_sz, gamma_dt),
                    inv_std_v);
      __m512 d0 = _mm512_sub_ps(
                    load16(static_cast<const char *>(in_row) + i*src_sz, src_dt),
                    mean_v);
      if (use_shift) {
        __m512 b0 = load16(static_cast<const char *>(beta) + i*b_sz, beta_dt);
        d0 = _mm512_fmadd_ps(d0, g0, b0);
      }
      else {
        d0 = _mm512_mul_ps(d0, g0);
    ...
    if (i < norm_size) {
      ...
      store16_mask(static_cast<char *>(out_row) + i*dst_sz, d0, mask, dst_dt);
    }
  }
  else {
    ...
    for (; i < vec64; i += 64) {
      ...
      if (use_shift) {
        ...
        d1 = _mm512_add_ps(d1, load16((const char *)bp + 16*b_sz,   beta_dt));
        d2 = _mm512_add_ps(d2, load16((const char *)bp + 32*b_sz,   beta_dt));
        d3 = _mm512_add_ps(d3, load16((const char *)bp + 48*b_sz,   beta_dt));
      }
    ...
    for (; i + 15 < norm_size; i += 16) {
      ...
      if (use_shift) {
        d0 = _mm512_add_ps(d0,
                           load16(static_cast<const char *>(beta) + i*b_sz, beta_dt));
      }
      store16(static_cast<char *>(out_row) + i*dst_sz, d0, dst_dt);
    }
    if (i < norm_size) {
      ...
      if (use_shift) {
        d0 = _mm512_add_ps(d0,
                           load16_mask(static_cast<const char *>(beta) + i*b_sz, mask, beta_dt));
      }
      store16_mask(static_cast<char *>(out_row) + i*dst_sz, d0, mask, dst_dt);
    }
  }
...
status_t layer_norm_avx512(
  ...
  auto row_loop = [&](int64_t b) {
    layer_norm_row_avx512(
      ...
      src_dt, dst_dt, gamma_dt, beta_dt);
  };
