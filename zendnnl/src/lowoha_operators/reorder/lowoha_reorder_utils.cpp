...
status_t validate_reorder_inputs(const void *src, void *dst, size_t nelems,
    ...
    if (params.quant_params.scale.buff != nullptr) {
        ...
        if (params.quant_params.scale.dt != data_type_t::f32
            ...
            log_error(
                    "Invalid scale data type: f32, bf16, or f16 expected. "
                    "Got: ",
    ...
    if (params.quant_params.zero_point.buff != nullptr) {
        ...
        if (params.quant_params.zero_point.dt != data_type_t::s32) {
            log_error(
                    "Invalid zero_point data type: only s32 is currently "
                    "supported. Got: ",
                    reorder_data_type_to_string(
                            params.quant_params.zero_point.dt));
            return status_t::failure;
        }
    }
...
status_t validate_reorder_quant_params(const reorder_params_t &params) {
    ...
    if (is_float_only_conversion && scale.buff == nullptr
            && zp.buff == nullptr) {
    ...
    auto validate_quant_dims = [&](const std::vector<int64_t> &dims,
        ...
        for (size_t i = 0; i < dims.size(); ++i) {
            if (dims[i] <= 0) {
                log_error("Invalid ", param_name, " dims[", i, "]=", dims[i],
                        " (must be > 0)");
                return status_t::failure;
            }
        }
        ...
        if (dims.size() == 1) {
            ...
            if (dims[0] != 1 && dims[0] != N_dim) {
                log_error(param_name, " dims[0]=", dims[0],
                        " must be 1 (per-tensor) or N=", N_dim,
                        " (per-channel)");
                return status_t::failure;
            }
        } else if (dims.size() == 2) {
            ...
            if (!is_per_tensor && !is_per_channel_col && !is_per_channel_row
                    && !is_per_group_row && !is_per_group_col) {
                log_error(param_name, " invalid dims {", dims[0], ", ", dims[1],
                        ", ", dims[2], "} for shape {batch, ", M, ", ", N,
                        "}. "
                        "Expected: {1,1,1} (per-tensor), {1,1,",
                        N,
                        "} (per-channel-col), "
                        "{1,",
...
const char *reorder_algo_to_string(reorder_algo_t algo) {
    switch (algo) {
        case reorder_algo_t::none: return "none";
        case reorder_algo_t::native: return "native";
        case reorder_algo_t::reference: return "reference";
        case reorder_algo_t::DT: return "DT";
        default: return "unknown";
    }
}
...
reorder_algo_t select_reorder_algo(
    ...
    if (params.algo != reorder_algo_t::DT
            && params.algo != reorder_algo_t::none) {
        if (params.algo == reorder_algo_t::native && !has_native_isa) {
            log_info(
                    "Reorder native kernel requires AVX-512 "
                    "(avx512f + avx512bw + avx512vl); falling back to "
                    "reference "
    ...
    if (nelems >= 64 && has_native_isa) { return reorder_algo_t::native; }
...
status_t validate_reorder_shape(const reorder_params_t &params) {
    if (!params.is_shaped()) {
        log_error(
                "Invalid reorder shape: src_shape is empty or contains invalid "
                "dimensions");
        return status_t::failure;
    }
    ...
    if (!params.shapes_match()) {
        log_error(
                "Invalid reorder shape: src_shape and dst_shape must be "
                "identical. ",
    ...
    for (size_t i = 0; i < params.src_shape.size(); ++i) {
        if (params.src_shape[i] > max_dim) {
            log_error("Shape dimension too large at index ", i, ": ",
                    params.src_shape[i]);
            return status_t::failure;
        }
    }
...
status_t validate_reorder_strides(const reorder_params_t &params) {
    ...
    for (size_t i = 0; i < strides.size(); ++i) {
        if (strides[i] <= 0) {
            log_error("Invalid stride at index ", i, ": ", strides[i],
                    " (must be > 0)");
            return status_t::failure;
        }
    }
    ...
    if (strides.size() == 2) {
        ...
        if (strides[0] < params.N() && strides[1] == 1) {
            log_error("Warning: stride_M (", strides[0], ") is less than N (",
                    params.N(), "), may cause overlapping access");
        }
    } else if (strides.size() == 3) {
        // For 3D, stride_batch should be at least M*N
        const int64_t matrix_size = params.M() * params.N();
        if (strides[0] < matrix_size && strides[1] == params.N()
                && strides[2] == 1) {
            log_error("Warning: stride_batch (", strides[0],
                    ") is less than M*N (", matrix_size,
                    "), may cause overlapping access");
        }
    }
...
status_t validate_dynamic_quant_params(
    ...
    if (params.src_dtype != data_type_t::bf16
            ...
            && params.src_dtype != data_type_t::f16) {
        log_error(
                "Dynamic quantization only supports bf16, f32 or f16 source. "
                "Got: ",
                reorder_data_type_to_string(params.src_dtype));
        return status_t::failure;
    }
    ...
    if (params.dst_dtype != data_type_t::s8
        ...
        log_error(
                "Dynamic quantization only supports s8 or u8 destination. "
                "Got: ",
    ...
    if (params.quant_params.scale.dt != data_type_t::f32
            ...
            && params.quant_params.scale.dt != data_type_t::f16) {
        log_error(
                "Dynamic quantization scale output buffer must be f32, bf16, "
                "or f16. Got: ",
                reorder_data_type_to_string(params.quant_params.scale.dt));
        return status_t::failure;
    }
    ...
    if (is_symmetric) {
        ...
        if (params.dst_dtype != data_type_t::s8) {
            log_error(
                    "Symmetric dynamic quantization (zp=nullptr) requires s8 "
                    "destination. Got: ",
                    reorder_data_type_to_string(params.dst_dtype));
            return status_t::failure;
        }
        ...
        if (params.quant_params.zero_point.dims.empty()) {
            log_error(
                    "Asymmetric dynamic quantization requires zero_point dims "
                    "to be specified");
            return status_t::failure;
        }
        ...
        if (params.dst_dtype != data_type_t::u8) {
            log_error(
                    "Asymmetric dynamic quantization (zp provided) requires u8 "
                    "destination. Got: ",
                    reorder_data_type_to_string(params.dst_dtype));
            return status_t::failure;
        }
...
status_t compute_dynamic_quant_params(
    ...
    auto write_scale
            = [s_kind, scale_out_f32, scale_out_u16](int64_t idx, float val) {
        switch (s_kind) {
            case scale_kind_t::f32: scale_out_f32[idx] = val; break;
            case scale_kind_t::bf16:
                scale_out_u16[idx] = float_to_bf16(val);
                break;
    ...
    if (!is_symmetric) {
        ...
        if (scale_nelems < 0 || zp_nelems < 0) {
            log_error(
                    "Asymmetric dynamic quantization received invalid "
                    "scale/zero_point dims");
            return status_t::failure;
        }
        ...
        if (scale_nelems != zp_nelems || scale_dims != zp_dims) {
            log_error(
                    "Asymmetric dynamic quantization requires scale and "
                    "zero_point to have the same granularity and dims mapping");
            return status_t::failure;
        }
    }
    ...
    const src_kind_t skind = params.src_dtype == data_type_t::f32
            ? src_kind_t::f32
    ...
    auto read_src_value = [skind, src_u16, src_f32_p](int64_t idx) -> float {
        switch (skind) {
            ...
            case src_kind_t::f16:
                return common::float16_t::f16_to_f32_val(src_u16[idx]);
        }
        return 0.0f;
    };
    ...
    const int64_t stride_m = has_strides
            ? (params.src_strides.size() == 3
                              ...
                              : (params.src_strides.size() == 2
                                                ...
                                                : 0))
            : N;
    const int64_t stride_n = has_strides ? params.src_strides.back() : 1;
    ...
    auto compute_scale_zp
            = [is_symmetric](float min_val,
                      float max_val) -> std::pair<float, int32_t> {
        float scale;
        int32_t zp;
        ...
        if (is_symmetric) {
            ...
            // zp = round(-min / scale), with clamping to int32_t range to avoid UB
            {
                ...
                double int32_min = static_cast<double>(
                        std::numeric_limits<int32_t>::min());
                double int32_max = static_cast<double>(
    ...
    if (granularity == granularity_type_t::per_tensor) {
        ...
        const bool is_contiguous
                = (stride_n == 1 && stride_m == N && stride_b == M * N);
...
#pragma omp parallel for num_threads(nthreads) reduction(min : min_val) \
...
#pragma omp parallel for num_threads(nthreads) collapse(2) \
    ...
    if (granularity == granularity_type_t::per_channel
        ...
        if (is_per_row) {
            const bool contiguous_batch1 = (batch == 1 && stride_n == 1
                    && stride_m == N && (!has_strides || stride_b == M * N));
                ...
                if (is_symmetric) {
                    ...
                    if (is_bf16_src) {
                        ...
                        dynamic_per_token_compute_scales_f32_u8_asymmetric(
                                src_f32_p, dst_scale_f32, zp_out, M, N,
                                nthreads);
                    }
                    narrow_back();
                    return status_t::success;
...
#pragma omp parallel for num_threads(nthreads) collapse(2)
            for (int64_t g = 0; g < G; ++g) {
                for (int64_t n = 0; n < N; ++n) {
                    ...
                    for (int64_t b = 0; b < batch; ++b) {
                        for (int64_t m_local = 0; m_local < group_size;
                                ++m_local) {
                    ...
                    if (!is_symmetric) { zp_out[param_idx] = zp; }
                }
            }
        } else {
...
#pragma omp parallel for num_threads(nthreads) collapse(2)
            for (int64_t m = 0; m < M; ++m) {
                for (int64_t g = 0; g < G; ++g) {
                    ...
                    for (int64_t b = 0; b < batch; ++b) {
                        for (int64_t n_local = 0; n_local < group_size;
                                ++n_local) {
