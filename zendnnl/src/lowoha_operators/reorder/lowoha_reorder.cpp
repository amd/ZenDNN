...
status_t reorder_direct(const void *src, void *dst, reorder_params_t &params) {
    ...
    //============================================================================
    if (params.is_prepack) { return weight_prepack_into(src, params, dst); }
    ...
    if (params.dynamic_quant) {
        ...
        if (apilog_info_enabled() || is_profile) {
            ...
            ss_tmp << "M=" << params.M() << ", K=" << params.N()
                   ...
                   << ", granularity="
                   << granularity_to_string(get_single_granularity(
                              params.quant_params.scale.dims, params.src_shape))
                   << ", dst="
                   << (dst == nullptr ? "nullptr (compute only)" : "valid");
        ...
        if (dst != nullptr && params.is_2d()
            ...
            if (((dq_algo_override == 0 && algo_dq == reorder_algo_t::native)
                        || dq_algo_override == 1)
                    && dispatch_fused_per_token(
                            src, dst, params, params.M(), params.N())) {
                if (is_profile) {
                    std::string dq_log
                            = "LOWOHA reorder_direct (Dynamic_Quantize): "
                            + dq_params_str;
                    profiler.tbp_stop();
                    profilelog_verbose(dq_log,
                            ", kernel=native (fused per-token), time=",
                            profiler.tbp_elapsedtime(), profiler.get_res_str());
                }
        ...
        if (dst == nullptr) { return status_t::success; }
    ...
    if (apilog_info_enabled() || is_profile) {
        ...
        ss << "LOWOHA reorder_direct (Static_Quantize): M=" << params.M()
           ...
           << ", scale=" << scale_val << ", zero_point=" << zp_val
           << ", kernel=" << reorder_algo_to_string(algo) << ", granularity="
           << granularity_to_string(
                      (params.quant_params.zero_point.buff == nullptr)
                              ? get_single_granularity(
...
status_t group_dynamic_quant(const std::vector<const void *> &src,
    ...
    if (src.size() != num_ops || K.size() != num_ops || dst.size() != num_ops
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        ...
        if (M[i] < 0 || K[i] <= 0 || src_stride_m < K[i] || dst_stride_m < K[i]
        ...
        if (per_group && (K[i] % group_count != 0)) {
            log_error(
                    "group_dynamic_quant: per-group requires K % num_groups == "
                    "0 "
                    "at op ",
                    i, " (K=", K[i], ", num_groups=", group_count, ")");
            return status_t::failure;
        }
    ...
    if (per_channel) {
        ...
        if (!dispatch_group_dynamic_per_group(src, M, K, src_row_stride, dst,
                    dst_row_stride, scale, params)) {
            log_error(
                    "group_dynamic_quant: no native grouped per-group kernel "
                    "matched");
            return status_t::failure;
        }
    } else if (!dispatch_group_dynamic_per_token(src, M, K, src_row_stride, dst,
                       dst_row_stride, scale, params)) {
        log_error(
                "group_dynamic_quant: no native grouped per-token kernel "
                "matched");
        return status_t::failure;
    }
...
status_t group_reorder(const std::vector<const void *> &src,
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        ...
        if (st != status_t::success) {
            log_error("group_reorder: reorder_direct failed at op ", i, " of ",
                    num_ops);
            return st;
        }
    }
