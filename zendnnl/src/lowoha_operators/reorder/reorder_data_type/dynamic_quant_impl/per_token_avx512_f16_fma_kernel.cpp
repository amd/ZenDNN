...
static inline void compute_symmetric_scale_from_absmax_ph(
...
static inline void compute_asymmetric_scale_zp_ph(
...
void dynamic_per_token_quant_f16_s8_avx512fp16(
    ...
    auto row_loop = [&](int64_t m) ZENDNNL_TARGET(
        ...
        vam0 = _mm512_max_ph(
                _mm512_max_ph(vam0, vam1), _mm512_max_ph(vam2, vam3));
        ...
        if (common::fp16_inv_scale_is_finite(scale_f32)) {
            ...
            for (; j + 31 < N; j += 32) {
                ...
                __m256i s8 = _mm512_cvtepi16_epi8(
                        s16); // truncating narrow -> int8 (non-finite -> 0)
...
#pragma omp parallel for schedule(static)
    for (int64_t m = 0; m < M; ++m) {
        ...
        vmin0 = _mm512_min_ph(
                _mm512_min_ph(vmin0, vmin1), _mm512_min_ph(vmin2, vmin3));
        vmax0 = _mm512_max_ph(
                _mm512_max_ph(vmax0, vmax1), _mm512_max_ph(vmax2, vmax3));
        ...
        if (common::fp16_inv_scale_is_finite(scale_f32)
            ...
            for (; j + 31 < N; j += 32) {
                ...
                // unambiguous.
                __m512i s32_lo
                        = _mm512_cvtepi16_epi32(_mm512_castsi512_si256(s16));
                ...
                _mm_storeu_si128(
                        reinterpret_cast<__m128i *>(row_dst + j), u8_lo);
                _mm_storeu_si128(
                        reinterpret_cast<__m128i *>(row_dst + j + 16), u8_hi);
            }
        }
        ...
        for (; j < N; ++j) {
            ...
            int32_t q
                    = static_cast<int32_t>(std::nearbyint(v / scale_f32)) + zp;
