...
bool dispatch_fused_per_token(const void *src, void *dst,
    ...
    const bool scale_needs_narrow
            = (scale_dt == data_type_t::bf16 || scale_dt == data_type_t::f16);
    ...
    const bool f16_use_fp16fma = (params.src_dtype == data_type_t::f16)
...
bool dispatch_fused_per_group(const void *src, void *dst,
    ...
    if (is_symmetric) {
        ...
        if (params.quant_params.zero_point.dims
        ...
        if (params.src_dtype == data_type_t::bf16
            ...
            if (f16_use_fp16fma) {
                dynamic_per_group_quant_f16_u8_avx512fp16(
                        static_cast<const uint16_t *>(src),
                        static_cast<uint8_t *>(dst), scale_f32, zp_out, M, K,
                        G);
            } else {
...
bool dispatch_group_dynamic_per_group(const std::vector<const void *> &src,
    ...
    int64_t total_scales = 0;
    for (int m : M)
        total_scales += static_cast<int64_t>(std::max(0, m)) * G;
