...
inline void process_int8_row(const InType *row, __m512 *acc, int full_blocks,
    ...
    for (int b = 0; b < full_blocks; ++b) {
        __m128i qvals_i8 = _mm_loadu_si128(
                reinterpret_cast<const __m128i *>(row + b * simd_width));
        __m512i qvals_i32 = _mm512_cvtepi8_epi32(qvals_i8);
        __m512 val_f32 = _mm512_fmadd_ps(
                _mm512_cvtepi32_ps(qvals_i32), scale_vec, bias_vec);
    ...
    if (tail > 0) {
        ...
        __m128i qvals_i8
                = _mm_loadu_si128(reinterpret_cast<const __m128i *>(tmp));
        __m512i qvals_i32 = _mm512_cvtepi8_epi32(qvals_i8);
        __m512 val_f32 = _mm512_fmadd_ps(
                _mm512_cvtepi32_ps(qvals_i32), scale_vec, bias_vec);
...
inline void process_int4_row(const InType *row, __m512 *acc, int full_blocks,
    ...
    for (int b = 0; b < full_blocks; ++b) {
        ...
        __m128i packed_data = _mm_loadl_epi64(
        ...
        __m128i high_nibbles = _mm_and_si128(
                _mm_srli_epi16(packed_data, 4), _mm_set1_epi8(0x0F));
        ...
        if (table_dtype == data_type_t::s4) {
            ...
            __m128i needs_extend
                    = _mm_cmpeq_epi8(sign_bit, _mm_set1_epi8(0x08));
            interleaved = _mm_or_si128(interleaved,
                    _mm_and_si128(needs_extend,
                            _mm_set1_epi8(static_cast<char>(0xF0))));
        }
...
#define PROCESS_PAIR_U4(packed, acc_lo, acc_hi, scale_v, bias_v) \
    do { \
        ...
        __m512 v_lo \
                = _mm512_fmadd_ps(_mm512_cvtepi32_ps(q_lo), scale_v, bias_v); \
        __m512 v_hi \
                = _mm512_fmadd_ps(_mm512_cvtepi32_ps(q_hi), scale_v, bias_v); \
    ...
    for (int oi = 0; oi < outer_loop; ++oi) {
        ...
        for (; i + 1 < end; i += 2) {
            ...
            if (valid0) {
                ...
                float scale0, bias0;
                __m128i fp16_pair0 = _mm_cvtsi32_si128(
                        *reinterpret_cast<const int32_t *>(row0 + PACKED_SIZE));
                __m128 fp32_pair0 = _mm_cvtph_ps(fp16_pair0);
                scale0 = _mm_cvtss_f32(fp32_pair0);
                bias0 = _mm_cvtss_f32(
                        _mm_shuffle_ps(fp32_pair0, fp32_pair0, 1));
            ...
            if (valid1) {
                ...
                // Load scale/bias using hardware F16C
                float scale1, bias1;
                __m128i fp16_pair1 = _mm_cvtsi32_si128(
                        *reinterpret_cast<const int32_t *>(row1 + PACKED_SIZE));
                __m128 fp32_pair1 = _mm_cvtph_ps(fp16_pair1);
                scale1 = _mm_cvtss_f32(fp32_pair1);
                bias1 = _mm_cvtss_f32(
                        _mm_shuffle_ps(fp32_pair1, fp32_pair1, 1));
        ...
        for (; i < end; ++i) {
            ...
            [[unlikely]] if (idx == padidx) { continue; }

            const int8_t *row = input + idx * ROW_STRIDE;

            // Load scale/bias
            float scale, bias;
            __m128i fp16_pair = _mm_cvtsi32_si128(
                    *reinterpret_cast<const int32_t *>(row + PACKED_SIZE));
        ...
        if constexpr (std::is_same_v<OutType, float>) {
            ...
            // FP16 output path
            __m256i f16_0 = _mm512_cvtps_ph(
                    acc0, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
            __m256i f16_1 = _mm512_cvtps_ph(
                    acc1, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
            __m256i f16_2 = _mm512_cvtps_ph(
                    acc2, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
            __m256i f16_3 = _mm512_cvtps_ph(
                    acc3, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
            __m256i f16_4 = _mm512_cvtps_ph(
                    acc4, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
            __m256i f16_5 = _mm512_cvtps_ph(
                    acc5, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
            __m256i f16_6 = _mm512_cvtps_ph(
                    acc6, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
            __m256i f16_7 = _mm512_cvtps_ph(
                    acc7, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
            _mm256_storeu_si256(reinterpret_cast<__m256i *>(dst_ptr), f16_0);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 16), f16_1);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 32), f16_2);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 48), f16_3);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 64), f16_4);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 80), f16_5);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 96), f16_6);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 112), f16_7);
        } else {
            // BF16 output path
            ...
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr), (__m256i)bf0);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 16), (__m256i)bf1);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 32), (__m256i)bf2);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 48), (__m256i)bf3);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 64), (__m256i)bf4);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 80), (__m256i)bf5);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 96), (__m256i)bf6);
            _mm256_storeu_si256(
                    reinterpret_cast<__m256i *>(dst_ptr + 112), (__m256i)bf7);
        }
    }
...
ZENDNNL_HOT void embag_avx512_int8_int4_kernel(const InType *__restrict input,
    ...
    if constexpr (IsInt4) {
        ...
        if (width == 128 && algo == embag_algo_t::sum && !is_weights
            ...
            embag_int4_w128_sum_specialized<IndexType, OffsetType, OutType>(
                    reinterpret_cast<const int8_t *>(input), indices, offsets,
                    dst, indsz, offsz, padidx, dst_stride, include_last_offset);
            return;
        }
    }
    ...
    assert(acc_size <= MAX_ACC_BLOCKS
            && "Width exceeds maximum supported size");
...
#pragma omp parallel for schedule(static)
    for (int oi = 0; oi < outer_loop; ++oi) {
        ...
        for (int64_t i = start; i < end; ++i) {
            ...
            if (is_weights) { maybe_prefetch_weight(weights, pf_i, end); }
            ...
            if (fp16_scale_bias) {
                ...
                std::memcpy(
                        &scale_fp16, row + quantized_size, sizeof(uint16_t));
                std::memcpy(
                        &bias_fp16, row + quantized_size + 2, sizeof(uint16_t));
        ...
        for (int b = 0; b < full_blocks; ++b) {
            if constexpr (std::is_same_v<OutType, float>) {
                ...
                __m256i f16_vec = _mm512_cvtps_ph(
                        acc[b], _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
        ...
        if (tail > 0) {
            if constexpr (std::is_same_v<OutType, float>) {
                ...
                _mm512_mask_storeu_ps(
                        &dst[dst_offset + full_blocks * simd_width], tail_mask,
                        acc[full_blocks]);
            } else if constexpr (std::is_same_v<OutType, float16_t>) {
                ...
                _mm256_storeu_si256(
                        reinterpret_cast<__m256i *>(tmp_store), f16_vec);
                std::memcpy(&dst[dst_offset + full_blocks * simd_width],
                        tmp_store, tail * sizeof(uint16_t));
            } else {
                __m256bh bf16_vec = _mm512_cvtneps_pbh(acc[full_blocks]);
                uint16_t tmp_store[simd_width];
                _mm256_storeu_si256(reinterpret_cast<__m256i *>(tmp_store),
                        (__m256i)bf16_vec);
                std::memcpy(&dst[dst_offset + full_blocks * simd_width],
                        tmp_store, tail * sizeof(uint16_t));
            }
    ...
    if (tail > 0) {
        ...
        if (is_embedding) {
            ...
            if (algo == embag_algo_t::max) {
                ...
                acc[full_blocks]
                        = _mm512_fmadd_ph(val, wt_vec, acc[full_blocks]);
            }
        }
    }
}
...
inline void process_int4_row_f16_fma(const InType *row, __m512h *acc,
    ...
    for (int b = 0; b < full_blocks; ++b) {
        ...
        if (table_dtype == data_type_t::s4) {
            __m256i sign_bit
                    = _mm256_and_si256(combined, _mm256_set1_epi8(0x08));
            __m256i needs_extend
                    = _mm256_cmpeq_epi8(sign_bit, _mm256_set1_epi8(0x08));
...
#pragma omp parallel for schedule(static)
    for (int oi = 0; oi < outer_loop; ++oi) {
        ...
        for (int64_t i = start; i < end; ++i) {
            ...
            if constexpr (!IsInt4) {
                process_int8_row_f16_fma(row, acc, full_blocks, tail, scale_f16,
                        bias_f16, wt_vec, is_embedding, algo,
                        first_valid_index);
            } else {
                process_int4_row_f16_fma(row, acc, full_blocks, tail, scale_f16,
                        bias_f16, wt_vec, is_embedding, algo, first_valid_index,
                        table_dtype);
            }
            first_valid_index = false;
        }
        ...
        for (int b = 0; b < full_blocks; ++b) {
            f16x32_store_typed<OutType>(
                    &dst[dst_offset + b * simd_width], acc[b]);
        }
        if (tail > 0) {
            __mmask32 tail_mask = (1u << tail) - 1;
            f16x32_store_tail_typed<OutType>(
                    &dst[dst_offset + full_blocks * simd_width],
                    acc[full_blocks], tail_mask, tail);
        }
    }
}
