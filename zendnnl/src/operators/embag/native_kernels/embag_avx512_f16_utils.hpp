        ...
        for (int oi = 0; oi < outer_loop; ++oi) {
                    ...
                    if (is_embedding) {
                        ...
                        if (algo == embag_algo_t::max) {
                            acc[full_blocks] = first_valid_index
                                    ? in_vec
                                    : _mm512_mask_max_ph(acc[full_blocks],
                                              tail_mask, acc[full_blocks],
                                              in_vec);
                        } else {
                            acc[full_blocks] = _mm512_mask3_fmadd_ph(in_vec,
                                    wt_vec, acc[full_blocks], tail_mask);
                        }
                    }
                }
            ...
            if (!is_embedding) {
                if (algo == embag_algo_t::mean && wt_sum > 0.0f) {
                    __m512h recip_vec
                            = _mm512_set1_ph((_Float16)(1.0f / wt_sum));
                    for (int b = 0; b < full_blocks; ++b) {
                        acc[b] = _mm512_mul_ph(acc[b], recip_vec);
                    }
                    if (tail > 0) {
                        acc[full_blocks]
                                = _mm512_mul_ph(acc[full_blocks], recip_vec);
                    }
                }
            }
