...
template <typename T>
inline void maybe_prefetch_input(
        const T *input, int64_t idx, int64_t width, int64_t padidx) {
#ifdef ENABLE_PREFETCH
    if (idx != padidx) {
        _mm_prefetch(reinterpret_cast<const char *>(&input[idx * width]),
                _MM_HINT_T0);
    }
#endif
}
...
inline void maybe_prefetch_weight(
        const float *weights, int64_t i, int64_t end) {
#ifdef ENABLE_PREFETCH
...
#pragma omp for
        for (int oi = 0; oi < outer_loop; ++oi) {
            ...
            int64_t end = is_embedding
                    ? oi + 1
                    : (include_last_offset ? offsets[oi + 1]
                                           : (oi < offsz - 1 ? offsets[oi + 1]
                                                             : indsz));
            ...
            for (int i = start; i < end; ++i) {
                ...
                if (tail > 0) {
                    ...
                    if constexpr (std::is_same_v<InType, float>) {
                        ...
                        __m256i f16_data = _mm256_maskz_loadu_epi16(tail_mask,
                                reinterpret_cast<const __m256i *>(
                                        &input[input_offset
                                                + full_blocks * simd_width]));
                        in_vec = _mm512_cvtph_ps(f16_data);
                    } else {
                        __m256i bf16_data = _mm256_maskz_loadu_epi16(tail_mask,
                                reinterpret_cast<const __m256i *>(
                                        &input[input_offset
                                                + full_blocks * simd_width]));
                        in_vec = embag_bf16x16_to_fp32(bf16_data);
                    ...
                    if (is_embedding) {
                        ...
                        if (algo == embag_algo_t::max) {
                            if (first_valid_index) {
                                ...
                                acc[full_blocks] = _mm512_max_ps(
                                        acc[full_blocks], in_vec);
                            }
                        } else {
                            acc[full_blocks] = _mm512_fmadd_ps(
                                    in_vec, wt_vec, acc[full_blocks]);
                        }
                    }
                }
            ...
            if (!is_embedding) {
                if (algo == embag_algo_t::mean && wt_sum > 0.0f) {
                    ...
                    if (tail > 0) {
                        acc[full_blocks]
                                = _mm512_mul_ps(acc[full_blocks], recip_vec);
                    }
                }
            }
            ...
            if (tail > 0) {
                ...
                if constexpr (std::is_same_v<OutType, float>) {
                    _mm512_mask_storeu_ps(
                            ...
                            tail_mask, acc[full_blocks]);
                } else if constexpr (std::is_same_v<OutType, float16_t>) {
                    __m256i f16_vec = _mm512_cvtps_ph(acc[full_blocks],
                            _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
                    _mm256_mask_storeu_epi16(
                            reinterpret_cast<__m256i *>(&dst[dst_offset
                                    + full_blocks * simd_width]),
                            tail_mask, f16_vec);
                } else {
