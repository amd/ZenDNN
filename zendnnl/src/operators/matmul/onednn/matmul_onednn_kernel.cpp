...
status_t matmul_onednn_kernel_t::preprocess(const context_type &context_,
    ...
    params.weights.strides.assign(
            weights_stride_dims.begin(), weights_stride_dims.end());
    ...
    if (bias_tensor_) {
        ...
        params.bias.strides.assign(
                bias_stride_dims.begin(), bias_stride_dims.end());
    }

    if (input_tensor.is_quantized()) {
        params.src_quant.scales
        ...
        params.src_quant.scale_size.assign(
                src_scale_size.begin(), src_scale_size.end());
        matmul_attr.set_scales_mask(
                DNNL_ARG_SRC, src_scale_size.back() == 1 ? 0 : 1 << 1);

        if (input_tensor.get_quant_subtype() == quant_subtype_t::asymmetric) {
            params.src_quant.zero_points
            ...
            params.src_quant.zero_size.assign(
                    src_zero_size.begin(), src_zero_size.end());
            matmul_attr.set_zero_points_mask(
                    DNNL_ARG_SRC, src_zero_size.back() == 1 ? 0 : 1 << 1);
        }
    }

    if (weight_tensor.is_quantized()) {
        params.weights_quant.scales
        ...
        params.weights_quant.scale_size.assign(
                wei_scale_size.begin(), wei_scale_size.end());
        matmul_attr.set_scales_mask(
                DNNL_ARG_WEIGHTS, wei_scale_size.back() == 1 ? 0 : 1 << 1);
        ...
        if (weight_tensor.get_quant_subtype() == quant_subtype_t::asymmetric) {
            ...
            params.weights_quant.zero_size.assign(
                    wei_zero_size.begin(), wei_zero_size.end());
            matmul_attr.set_zero_points_mask(
                    DNNL_ARG_WEIGHTS, wei_zero_size.back() == 1 ? 0 : 1 << 1);
        }
    }
    ...
    if (output_tensor.is_quantized()) {
        ...
        if (output_tensor.get_quant_subtype() == quant_subtype_t::asymmetric) {
            ...
            params.dst_quant.zero_size.assign(
                    dst_zero_size.begin(), dst_zero_size.end());
            matmul_attr.set_zero_points_mask(
                    DNNL_ARG_DST, dst_zero_size.back() == 1 ? 0 : 1 << 1);
        }
    }
    ...
    if (params.alpha != 1.0f) {
        ...
        auto alpha_mem = dnnl::memory({{1}, dnnl::memory::data_type::f32, {1}},
                eng, const_cast<float *>(&params.alpha));
            ...
            switch (zen_po.type) {
                case post_op_type_t::elu: {
                    ...
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_elu, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::relu: {
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_relu, po_alpha, po_beta);
                    break;
                }
                ...
                case post_op_type_t::gelu_tanh: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_gelu_tanh, po_alpha,
                            po_beta);
                    break;
                }
                case post_op_type_t::gelu_erf: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_gelu_erf, po_alpha,
                            po_beta);
                    break;
                }
                case post_op_type_t::tanh: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_tanh, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::square: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_square, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::abs: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_abs, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::sqrt: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_sqrt, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::exp: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_exp, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::log: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_log, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::sigmoid: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_logistic, po_alpha,
                            po_beta);
                    break;
                }
                case post_op_type_t::swish: {
                    ...
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_swish, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::clip: {
                    ...
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_clip, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::mish: {
                    po_alpha = 1.0f;
                    matmul_pops.append_eltwise(
                            dnnl::algorithm::eltwise_mish, po_alpha, po_beta);
                    break;
                }
                case post_op_type_t::binary_add: {
                    std::string key_add = "binary_add_tensor_"
                    ...
                    binary_tensor.dims.assign(
                            buff_dims.begin(), buff_dims.end());
                ...
                case post_op_type_t::binary_mul: {
                    std::string key_mul = "binary_mul_tensor_"
                            + std::to_string(mul_index++);
...
void matmul_onednn_kernel_t::execute_matmul(
    ...
    if (params.dst_quant.scale_size.size()) {
        ...
        dnnl::memory dst_scale_mem
                = dnnl::memory(dst_scale_desc, eng, &dst_inv_scale);
...
status_t matmul_onednn_kernel_t::execute(const context_type &context_,
    ...
    onednn_utils_t::onednn_matmul_params params;
    status_t preprocess_status = preprocess(
            context_, inputs_, outputs_, params, matmul_args, matmul_attr, eng);
    if (preprocess_status != status_t::success) { return preprocess_status; }
