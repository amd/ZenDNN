...
#include "matmul_int8_avx512_kernel.hpp"

namespace zendnnl {
namespace ops {
using namespace zendnnl::error_handling;

status_t matmul_int8_avx512_kernel_t::execute(const context_type &context_,
    tensor_map_type &inputs_,
    tensor_map_type &outputs_) {

  LOG_DEBUG_INFO("Executing matmul_int8_avx512 kernel");
  log_info("Executing matmul_int8_avx512 kernel");
  ...
  void   *input_raw_handle    = input_tensor.get_raw_handle_unsafe();
  void   *output_raw_handle   = output_tensor.get_raw_handle_unsafe();
  int8_t *weights_raw_handle  = (int8_t *)weight_tensor.get_raw_handle_unsafe();
  ...
  for (auto bs=0; bs<batch_size; bs++) {
    if (input_tensor.get_data_type() == data_type_t::u8) {
      switch (output_tensor.get_data_type()) {
      case data_type_t::u8:
        LOG_DEBUG_INFO("Selected algorithm: u8 -> u8");
        // Call the appropriate algorithm for u8 -> u8
        aocl_gemm_u8s8s32ou8(order, trans_input, trans_weight,
                             ...
                             ldb, weight_format, beta, (uint8_t *)output_raw_handle + bs * offset_out, ldc,
                             aocl_dlp_po_ptr);
        break;
      case data_type_t::s8:
        LOG_DEBUG_INFO("Selected algorithm: u8 -> s8");
        // Call the appropriate algorithm for u8 -> s8
        aocl_gemm_u8s8s32os8(order, trans_input, trans_weight,
                             m, n, k, alpha, (uint8_t *)input_raw_handle + bs * offset_src, lda,
                             input_format,
                             (is_reordered_weights &&
                              weight_dim == 2) ? reorder_weights : weights_raw_handle + bs * offset_wei,
                             ldb, weight_format, beta, (int8_t *)output_raw_handle + bs * offset_out, ldc,
                             aocl_dlp_po_ptr);
        break;
      case data_type_t::s32:
        LOG_DEBUG_INFO("Selected algorithm: u8 -> s32");
        // Call the appropriate algorithm for u8 -> s32
        aocl_gemm_u8s8s32os32(order, trans_input, trans_weight,
                              m, n, k, alpha, (uint8_t *)input_raw_handle + bs * offset_src, lda,
                              input_format,
                              (is_reordered_weights &&
                               weight_dim == 2) ? reorder_weights : weights_raw_handle + bs * offset_wei,
                              ldb, weight_format,
                              beta,
                              (int32_t *)output_raw_handle + bs * offset_out, ldc,
                              aocl_dlp_po_ptr);
        break;
      case data_type_t::bf16:
        LOG_DEBUG_INFO("Selected algorithm: u8 -> bf16");
        // Call the appropriate algorithm for u8 -> bf16
        aocl_gemm_u8s8s32obf16(order, trans_input, trans_weight,
                               m, n, k, alpha, (uint8_t *)input_raw_handle + bs * offset_src, lda,
                               input_format,
                               (is_reordered_weights &&
                                weight_dim == 2) ? reorder_weights : weights_raw_handle + bs * offset_wei,
                               ldb, weight_format, beta, (int16_t *)output_raw_handle + bs * offset_out, ldc,
                               aocl_dlp_po_ptr);
        break;
      case data_type_t::f32:
        LOG_DEBUG_INFO("Selected algorithm: u8 -> f32");
        // Call the appropriate algorithm for u8 -> f32
        aocl_gemm_u8s8s32of32(order, trans_input, trans_weight,
                              m, n, k, alpha, (uint8_t *)input_raw_handle + bs * offset_src, lda,
                              input_format,
                              (is_reordered_weights &&
                               weight_dim == 2) ? reorder_weights : weights_raw_handle + bs * offset_wei,
      ...
      switch (output_tensor.get_data_type()) {
      case data_type_t::u8:
        LOG_DEBUG_INFO("Selected algorithm: s8 -> u8");
        // Call the appropriate algorithm for s8 -> u8
        aocl_gemm_s8s8s32ou8(order, trans_input, trans_weight,
                             m, n, k, alpha,
                             (int8_t *)input_raw_handle + bs * offset_src, lda, input_format,
                             (is_reordered_weights &&
                              weight_dim == 2) ? reorder_weights : weights_raw_handle + bs * offset_wei,
                             ldb, weight_format, beta, (uint8_t *)output_raw_handle + bs * offset_out, ldc,
                             aocl_dlp_po_ptr);
        break;
      case data_type_t::s8:
        LOG_DEBUG_INFO("Selected algorithm: s8 -> s8");
        // Call the appropriate algorithm for s8 -> s8
        aocl_gemm_s8s8s32os8(order, trans_input, trans_weight,
        ...
        break;
      case data_type_t::s32:
        LOG_DEBUG_INFO("Selected algorithm: s8 -> s32");
        // Call the appropriate algorithm for s8 -> s32
        aocl_gemm_s8s8s32os32(order, trans_input, trans_weight,
                              m, n, k, alpha, (int8_t *)input_raw_handle + bs * offset_src, lda, input_format,
                              (is_reordered_weights &&
                               weight_dim == 2) ? reorder_weights : weights_raw_handle + bs * offset_wei,
                              ldb, weight_format, beta, (int32_t *)output_raw_handle + bs * offset_out, ldc,
                              aocl_dlp_po_ptr);
        break;
      case data_type_t::bf16:
        LOG_DEBUG_INFO("Selected algorithm: s8 -> bf16");
        // Call the appropriate algorithm for s8 -> bf16
        aocl_gemm_s8s8s32obf16(order, trans_input, trans_weight,
                               m, n, k, alpha, (int8_t *)input_raw_handle + bs * offset_src, lda, input_format,
                               (is_reordered_weights &&
                                weight_dim == 2) ? reorder_weights : weights_raw_handle + bs * offset_wei,
                               ldb, weight_format, beta, (int16_t *)output_raw_handle + bs * offset_out, ldc,
        ...
        break;
      case data_type_t::f32:
        LOG_DEBUG_INFO("Selected algorithm: s8 -> f32");
        // Call the appropriate algorithm for s8 -> f32
        aocl_gemm_s8s8s32of32(order, trans_input, trans_weight,
                              m, n, k, alpha, (int8_t *)input_raw_handle + bs * offset_src, lda, input_format,
                              (is_reordered_weights &&
                               weight_dim == 2) ? reorder_weights : weights_raw_handle + bs * offset_wei,
                              ldb, weight_format, beta, (float *)output_raw_handle + bs * offset_out, ldc,
        ...
        break;
      default:
        apilog_error("Unsupported output data type for input s8");
        return status_t::unimplemented;
      }
    }
  }
