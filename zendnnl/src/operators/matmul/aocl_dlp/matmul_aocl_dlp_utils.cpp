...
inline uint64_t aocl_binary_row_vector_len_n(const tensor_t &t) {
    const auto &sz = t.get_size();
    if (sz.size() == 1) { return sz[0]; }
    if (sz.size() == 2 && sz[0] == 1) { return sz[1]; }
    return 0;
}
...
DLP_TYPE get_aocl_store_type(data_type_t dt) {
    switch (dt) {
        case data_type_t::f32: return DLP_TYPE::DLP_F32; break;
        case data_type_t::bf16: return DLP_TYPE::DLP_BF16; break;
        case data_type_t::f16: return DLP_TYPE::DLP_F16; break;
        case data_type_t::s32: return DLP_TYPE::DLP_S32; break;
...
template <typename T>
size_t aocl_dlp_utils_t::reorder_weights_execute(const void *weights,
        const int k, const int n, const int ldb, const char order,
        const char trans, get_reorder_buff_size_func_ptr get_reorder_buf_size,
...
status_t aocl_dlp_utils_t::set_runtime_post_op_buffer(
        tensor_map_type &inputs_, bool is_bias, tensor_t &output_tensor) {
    uint32_t max_matrix_mul_po
            = post_op_size["binary_mul_2d"] + post_op_size["binary_mul_1d"];
    ...
    if (inputs_.size() > max_matrix_mul_po + max_matrix_add_po) {
        ...
        for (size_t mul_idx = 0; mul_idx < max_matrix_mul_po; mul_idx++) {
            ...
            std::string key_mul
                    = "binary_mul_tensor_" + std::to_string(mul_idx);
            ...
            if (found_obj_mul != inputs_.end()) {
                ...
                if (aocl_binary_mul_uses_1d_scale_layout(found_obj_mul->second)
                    ...
                    (aocl_dlp_po_ptr->matrix_mul + mul_idx_2d)->matrix
                            = mul_buff_tensor.get_raw_handle_unsafe();
                    (aocl_dlp_po_ptr->matrix_mul + mul_idx_2d)->stor_type
                            = get_aocl_store_type(
                                    mul_buff_tensor.get_data_type());
                    (aocl_dlp_po_ptr->matrix_mul + mul_idx_2d)->ldm
        ...
        for (size_t add_idx = 0; add_idx < max_matrix_add_po; add_idx++) {
            ...
            std::string key_add
                    = "binary_add_tensor_" + std::to_string(add_idx);
            auto found_obj_add = inputs_.find(key_add);
            if (found_obj_add != inputs_.end()) {
                auto add_buff_tensor = inputs_[key_add];
                if (aocl_binary_add_uses_bias_tensor_layout(
                            found_obj_add->second)
                        && aocl_dlp_po_ptr->bias != nullptr) {
                    (aocl_dlp_po_ptr->bias + add_idx_1d)->bias
                            = (void *)add_buff_tensor.get_raw_handle_unsafe();
                    (aocl_dlp_po_ptr->bias + add_idx_1d)->stor_type
                            = get_aocl_store_type(
    ...
    if (is_dst_scale_zp) {
        ...
        if (dst_scale_ != nullptr || dst_zp_ != nullptr) {
            // Set dst scale
            (aocl_dlp_po_ptr->scale + mul_idx_1d)->sf->scale_factor
                    = dst_scale_ != nullptr ? const_cast<void *>(dst_scale_)
                                            : &dummy_scale;
            (aocl_dlp_po_ptr->scale + mul_idx_1d)->sf->scale_factor_type
                    = dst_scale_ != nullptr
                    ? get_aocl_store_type(
            ...
            (aocl_dlp_po_ptr->scale + mul_idx_1d)->sf->scale_factor_len
                    = dst_scale_ != nullptr
                    ? compute_product(output_tensor.get_quant_scale_size())
                    : 1;
            ...
            (aocl_dlp_po_ptr->scale + mul_idx_1d)->sf->scale_factor_dim
                    = ((aocl_dlp_po_ptr->scale + mul_idx_1d)
                                      ->sf->scale_factor_len
                              == 1)
            ...
            // Set dst zero-point
            (aocl_dlp_po_ptr->scale + mul_idx_1d)->zp->zero_point
                    = dst_zp_ != nullptr ? const_cast<void *>(dst_zp_)
                                         : &dummy_zp;
            (aocl_dlp_po_ptr->scale + mul_idx_1d)->zp->zero_point_type
                    = dst_zp_ != nullptr
                    ? get_aocl_store_type(
            ...
            (aocl_dlp_po_ptr->scale + mul_idx_1d)->zp->zero_point_len
                    = dst_zp_ != nullptr
                    ? compute_product(output_tensor.get_quant_zero_size())
                    : 1;
            mul_idx_1d++;
        }
    }
...
status_t aocl_dlp_utils_t::aocl_post_op_memory_alloc(
        const std::vector<post_op_t> &post_op_vec_, bool is_bias,
    ...
    if (alloc_aocl_po) {
        for (size_t i = 0; i < max_post_ops; ++i) {
            ...
            switch (zen_po.type) {
                case post_op_type_t::relu: num_post_ops_eltwise++; break;
                case post_op_type_t::leaky_relu: num_post_ops_eltwise++; break;
                case post_op_type_t::gelu_tanh: num_post_ops_eltwise++; break;
                case post_op_type_t::gelu_erf: num_post_ops_eltwise++; break;
                case post_op_type_t::tanh: num_post_ops_eltwise++; break;
                case post_op_type_t::swish: num_post_ops_eltwise++; break;
                case post_op_type_t::sigmoid: num_post_ops_eltwise++; break;
                case post_op_type_t::clip: num_post_ops_eltwise++; break;
                case post_op_type_t::mish: num_post_ops_eltwise++; break;
                case post_op_type_t::binary_add: {
                    auto it = inputs_.find(
                            zen_po.binary_add_params.tensor_name);
                    if (it != inputs_.end()
                            && aocl_binary_add_uses_bias_tensor_layout(
                                    it->second)) {
                        num_post_ops_1d_add++;
                    } else {
                ...
                case post_op_type_t::binary_mul: {
                    auto it = inputs_.find(
                            zen_po.binary_mul_params.tensor_name);
                    if (it != inputs_.end()
                            && aocl_binary_mul_uses_1d_scale_layout(
                                    it->second)) {
                        num_post_ops_1d_mul++;
                    } else {
        ...
        aocl_dlp_po_ptr->bias = (dlp_post_op_bias *)calloc(
                total_bias_mem, sizeof(dlp_post_op_bias));
        aocl_dlp_po_ptr->scale = (dlp_scale_t *)calloc(
                num_post_ops_1d_mul + num_post_ops_scale, sizeof(dlp_scale_t));
        aocl_dlp_po_ptr->eltwise = (dlp_post_op_eltwise *)calloc(
                num_post_ops_eltwise, sizeof(dlp_post_op_eltwise));
        aocl_dlp_po_ptr->matrix_add = (dlp_post_op_matrix_add *)calloc(
                num_post_ops_binary_add, sizeof(dlp_post_op_matrix_add));
        aocl_dlp_po_ptr->matrix_mul = (dlp_post_op_matrix_mul *)calloc(
                num_post_ops_binary_mul, sizeof(dlp_post_op_matrix_mul));
        ...
        for (int i = 0; i < num_post_ops_1d_mul + num_post_ops_scale; ++i) {
            aocl_dlp_po_ptr->scale[i].sf
                    = (dlp_sf_t *)calloc(1, sizeof(dlp_sf_t));
            aocl_dlp_po_ptr->scale[i].zp
                    = (dlp_zp_t *)calloc(1, sizeof(dlp_zp_t));
        }
        for (int i = 0; i < num_post_ops_binary_add; ++i) {
            aocl_dlp_po_ptr->matrix_add[i].sf
                    = (dlp_sf_t *)calloc(1, sizeof(dlp_sf_t));
            aocl_dlp_po_ptr->matrix_add[i].sf->scale_factor_type
                    = get_aocl_store_type(data_type_t::f32);
        }
        for (int i = 0; i < num_post_ops_binary_mul; ++i) {
            aocl_dlp_po_ptr->matrix_mul[i].sf
                    = (dlp_sf_t *)calloc(1, sizeof(dlp_sf_t));
            aocl_dlp_po_ptr->matrix_mul[i].sf->scale_factor_type
                    = get_aocl_store_type(data_type_t::f32);
        }
    }
    return status_t::success;
...
status_t aocl_dlp_utils_t::aocl_post_op_initialize(
        const std::vector<post_op_t> &post_op_vec_, int &post_op_count,
        bool is_bias, std::map<std::string, zendnnl::memory::tensor_t> &inputs_,
        tensor_t &output_tensor, size_t eltwise_index, size_t add_index_2d,
        size_t mul_index_1d, size_t mul_index_2d) {
    ...
    for (size_t i = 0; i < max_post_ops; ++i) {
        ...
        switch (zen_po.type) {
            case post_op_type_t::relu:
                ...
                eltwise_init(aocl_dlp_po_ptr, eltwise_index,
                        DLP_ELT_ALGO_TYPE::RELU);
                eltwise_index++;
                aocl_dlp_po_ptr->seq_vector[post_op_count++]
                        = DLP_POST_OP_TYPE::ELTWISE;
                break;
            case post_op_type_t::leaky_relu:
                log_info("Adding leaky_relu post-op");
                eltwise_init(aocl_dlp_po_ptr, eltwise_index,
                        DLP_ELT_ALGO_TYPE::PRELU);
                (aocl_dlp_po_ptr->eltwise[eltwise_index]).algo.stor_type
                        = DLP_F32;
                (aocl_dlp_po_ptr->eltwise[eltwise_index]).algo.alpha
                        = malloc(sizeof(float));
            ...
            case post_op_type_t::clip:
                ...
                *((float *)(aocl_dlp_po_ptr->eltwise[eltwise_index]).algo.alpha)
                ...
                (aocl_dlp_po_ptr->eltwise[eltwise_index]).algo.beta
                        = malloc(sizeof(float));
            ...
            case post_op_type_t::binary_add: {
                ...
                if (it != inputs_.end()
                        && aocl_binary_add_uses_bias_tensor_layout(
                                it->second)) {
                    aocl_dlp_po_ptr->seq_vector[post_op_count++]
                            = DLP_POST_OP_TYPE::BIAS;
                } else {
                    (aocl_dlp_po_ptr->matrix_add + add_index_2d)
                            ->sf->scale_factor
                            = malloc(sizeof(float));
                    *((float *)(aocl_dlp_po_ptr->matrix_add[add_index_2d])
                                    .sf->scale_factor)
            ...
            case post_op_type_t::binary_mul: {
                ...
                if (it != inputs_.end()
                        && aocl_binary_mul_uses_1d_scale_layout(it->second)) {
                    aocl_dlp_po_ptr->seq_vector[post_op_count++]
                            = DLP_POST_OP_TYPE::SCALE;
                    (aocl_dlp_po_ptr->scale + mul_index_1d)->sf->scale_factor
                            = NULL;
                    (aocl_dlp_po_ptr->scale + mul_index_1d)->zp->zero_point
                            = NULL;
                    (aocl_dlp_po_ptr->scale + mul_index_1d)
                            ->sf->scale_factor_len
                    ...
                    (aocl_dlp_po_ptr->scale + mul_index_1d)
                            ->sf->scale_factor_dim
                            = DLP_PARAM_DIM_PER_CHANNEL;
                    (aocl_dlp_po_ptr->scale + mul_index_1d)->zp->zero_point_len
                            = 1;
                    log_info("Adding done");
                    mul_index_1d++;
                } else {
                    (aocl_dlp_po_ptr->matrix_mul + mul_index_2d)
                            ->sf->scale_factor
                            = malloc(sizeof(float));
                    *((float *)(aocl_dlp_po_ptr->matrix_mul[mul_index_2d])
                                    .sf->scale_factor)
...
void aocl_dlp_utils_t::zero_point_compensation(int M, int N, int K,
        tensor_t &src, tensor_t &wei, int32_t src_zero_point,
...
status_t aocl_dlp_utils_t::alloc_post_op(
        const std::vector<post_op_t> &post_op_vec_,
        std::optional<tensor_t> optional_bias_tensor_, tensor_t &weight_tensor,
        std::map<std::string, zendnnl::memory::tensor_t> &inputs_,
    ...
    if (aocl_dlp_po_ptr != nullptr) { return status_t::success; }
    bool is_woq = false;
    auto weight_dtype = weight_tensor.get_data_type();
    if (weight_dtype == data_type_t::s4 || weight_dtype == data_type_t::u4) {
        is_woq = true;
    }
    ...
    bool is_quant
            = (src_it->second.get_data_type() == data_type_t::u8
    ...
    if (is_quant) {
        ...
        auto src_zp_ = is_src_scale_
                        && src_tensor.get_quant_subtype()
                                == quant_subtype_t::asymmetric
                ? src_tensor.get_quant_zero_raw_handle_const()
                : nullptr;
        auto wei_zp_ = is_wei_scale_
                        && weight_tensor.get_quant_subtype()
                                == quant_subtype_t::asymmetric
        ...
        dst_zp_ = is_dst_scale_
                        && output_tensor.get_quant_subtype()
                                == quant_subtype_t::asymmetric
                ? output_tensor.get_quant_zero_raw_handle_const()
                : nullptr;
        ...
        if (is_wei_scale_) {
            ...
            // Compute the product of all elements in the vector
            wei_scale_size
                    = compute_product(weight_tensor.get_quant_scale_size());
        ...
        int32_t src_zp_val = src_zp_ != nullptr
                ? read_and_cast<int32_t>(
                          src_zp_, src_tensor.get_quant_zero_data_type())
                : (int32_t)0;
        int32_t wei_zp_val = wei_zp_ != nullptr
                ? read_and_cast<int32_t>(
                          wei_zp_, weight_tensor.get_quant_zero_data_type())
                : (int32_t)0;
    ...
    // Add one for bias
    if (optional_bias_tensor_) { total_po++; }
    ...
    if (total_po > 0 || is_woq) {
        ...
        if (aocl_dlp_po_ptr == NULL) { return status_t::failure; }
        ...
        if (is_woq) {
            ...
            bool is_zero_point = weight_tensor.get_quant_subtype()
                    == quant_subtype_t::asymmetric;
        ...
        if (total_po > 0) {
            aocl_dlp_po_ptr->seq_vector = (DLP_POST_OP_TYPE *)calloc(
                    total_po, sizeof(DLP_POST_OP_TYPE));
            if (aocl_dlp_po_ptr->seq_vector == NULL) {
                free(aocl_dlp_po_ptr);
                return status_t::failure;
            }
            // Allocate memory for post-ops
            if (aocl_post_op_memory_alloc(post_op_vec_,
                        optional_bias_tensor_ ? true : false, inputs_)
                    != status_t::success) {
                return status_t::failure;
            }
            if (is_quant) {
                ...
                if (zp_comp_ndim == 1) {
                    ...
                    (aocl_dlp_po_ptr->bias + bias_index)->stor_type
                            = DLP_TYPE::DLP_S32;
                    ...
                    (aocl_dlp_po_ptr->matrix_add + add_index_2d)->stor_type
                            = DLP_TYPE::DLP_S32;
                    (aocl_dlp_po_ptr->matrix_add + add_index_2d)->ldm = dim_N;
                    (aocl_dlp_po_ptr->matrix_add + add_index_2d)->matrix
                            = zp_comp_acc;
                    (aocl_dlp_po_ptr->matrix_add + add_index_2d)
                            ->sf->scale_factor
                            = malloc(sizeof(float));
                ...
                if (wei_scale_) {
                    ...
                    (aocl_dlp_po_ptr->scale + mul_index_1d)
                            ->sf->scale_factor_dim
                            = (wei_scale_size == 1) ? DLP_PARAM_DIM_PER_TENSOR
                                                    : DLP_PARAM_DIM_PER_CHANNEL;
                    (aocl_dlp_po_ptr->scale + mul_index_1d)->zp->zero_point
                            = &dummy_zp;
                    (aocl_dlp_po_ptr->scale + mul_index_1d)->zp->zero_point_type
            ...
            if (optional_bias_tensor_) {
                ...
                aocl_dlp_po_ptr->seq_vector[post_op_count++]
                        = DLP_POST_OP_TYPE::BIAS;
                if (aocl_dlp_po_ptr->bias == NULL) { return status_t::failure; }
                (aocl_dlp_po_ptr->bias + bias_index)->bias
                        = (void *)optional_bias_tensor_
                                  ->get_raw_handle_unsafe();
                (aocl_dlp_po_ptr->bias + bias_index)->stor_type
                        = get_aocl_store_type(bias_type);
                (aocl_dlp_po_ptr->bias + bias_index)->bias_len = dim_N;
                bias_index++;
            }
            ...
            if (aocl_post_op_initialize(post_op_vec_, post_op_count,
                        optional_bias_tensor_ ? true : false, inputs_,
                        output_tensor, eltwise_index, add_index_2d,
                        mul_index_1d, mul_index_2d)
                    != status_t::success) {
                return status_t::failure;
            }
            if (is_dst_scale_ || dst_zp_ != nullptr) {
...
void aocl_dlp_utils_t::free_post_op() {
    ...
    for (int idx = 0; idx < aocl_dlp_po_ptr->seq_length; idx++) {
        if (aocl_dlp_po_ptr->seq_vector[idx] == DLP_POST_OP_TYPE::ELTWISE) {
            ...
            count_elt++;
        } else if (aocl_dlp_po_ptr->seq_vector[idx]
                == DLP_POST_OP_TYPE::MATRIX_ADD) {
            if (aocl_dlp_po_ptr->matrix_add[count_matrix_add_2d].sf
                    && aocl_dlp_po_ptr->matrix_add[count_matrix_add_2d]
                               .sf->scale_factor) {
                free(aocl_dlp_po_ptr->matrix_add[count_matrix_add_2d]
                                .sf->scale_factor);
            }
            if (aocl_dlp_po_ptr->matrix_add[count_matrix_add_2d].sf) {
            ...
            if (aocl_dlp_po_ptr->matrix_mul[count_matrix_mul_2d].sf
                    && aocl_dlp_po_ptr->matrix_mul[count_matrix_mul_2d]
                               .sf->scale_factor) {
                free(aocl_dlp_po_ptr->matrix_mul[count_matrix_mul_2d]
                                .sf->scale_factor);
            }
            if (aocl_dlp_po_ptr->matrix_mul[count_matrix_mul_2d].sf) {
    ...
    if (aocl_dlp_po_ptr->scale) { free(aocl_dlp_po_ptr->scale); }
    if (aocl_dlp_po_ptr->eltwise) { free(aocl_dlp_po_ptr->eltwise); }
    if (aocl_dlp_po_ptr->matrix_add) { free(aocl_dlp_po_ptr->matrix_add); }
    if (aocl_dlp_po_ptr->matrix_mul) { free(aocl_dlp_po_ptr->matrix_mul); }
    if (aocl_dlp_po_ptr->seq_vector) { free(aocl_dlp_po_ptr->seq_vector); }
    free(aocl_dlp_po_ptr);
}
...
status_t aocl_dlp_utils_t::reorder_weights(
        std::optional<tensor_t> weights, data_type_t src_dt) {
    ...
    int ldb = trans_weights ? weights->get_stride(1) : weights->get_stride(0);
    ...
    if (weight_data_type == data_type_t::f32) {
        ...
        reorder_weights_execute<float>(weights_ptr, // weights
                k, // k
                n, // n
                ldb, // ldb
                'r', // order
        ...
        reorder_weights_execute<int16_t>(weights_ptr, // weights
                k, // k
                n, // n
                ldb, // ldb
                'r', // order
        ...
        if (src_dt == data_type_t::s8) {
            ...
            reorder_weights_execute<int8_t>(weights_ptr, // weights
                    k, // k
                    n, // n
                    ldb, // ldb
                    'r', // order
            ...
            || weight_data_type == data_type_t::u4) {
        log_info("Reordering S4/U4 weights");
        // U4 uses the same reorder function as S4
        reorder_weights_execute<int8_t>(weights_ptr, // weights
                k, // k
                n, // n
                ldb, // ldb
        ...
        reorder_weights_execute<uint16_t>(weights_ptr, // weights
                k, // k
                n, // n
                ldb, // ldb
                'r', // order
...
void *aocl_dlp_utils_t::get_aocl_dlp_reordered_weights_ptr_unsafe() const {
    LOG_DEBUG_INFO(
            "Getting aocl dlp reordered weights ptr from aocl_dlp_utils_t");
