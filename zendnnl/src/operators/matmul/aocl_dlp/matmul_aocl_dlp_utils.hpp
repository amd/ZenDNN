...
public:
   ...
   *  only on LP64; on Windows (LLP64) `unsigned long` is 32-bit and mismatched. */
    ...
    using reorder_func_ptr
            = void (*)(const char, const char, const char, const T *, T *,
                    const md_t, const md_t, const md_t, dlp_metadata_t *);
    /** @brief entry function for tensor reordering for the AOCL */
    status_t reorder_weights(
            std::optional<tensor_t> weights, data_type_t src_dt);
    /** @brief weight reordering for the AOCL */
    template <typename T>
    size_t reorder_weights_execute(const void *weights, const int k,
            const int n, const int ldb, const char order, const char trans,
    ...
    void zero_point_compensation(int M, int N, int K, tensor_t &src,
            tensor_t &wei, int32_t src_zero_point, int32_t wei_zero_point);
    /** @brief allocate memory for the AOCL DLP post-ops */
    status_t aocl_post_op_memory_alloc(
    ...
    status_t aocl_post_op_initialize(const std::vector<post_op_t> &post_op_vec_,
            ...
            std::map<std::string, zendnnl::memory::tensor_t> &inputs_,
            zendnnl::memory::tensor_t &output_tensor, size_t eltwise_index,
            size_t add_index_2d, size_t mul_index_1d, size_t mul_index_2d);
    ...
    status_t set_runtime_post_op_buffer(
            tensor_map_type &inputs, bool is_bias, tensor_t &output_tensor);
...
protected:
    ...
    int32_t *zp_comp_acc;
    dlp_metadata_t
            *aocl_dlp_po_ptr; // Changed from aocl_post_op to dlp_metadata_t
