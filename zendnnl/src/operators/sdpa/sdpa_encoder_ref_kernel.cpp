...
status_t execute_typed(const sdpa_encoder_context_t &context_,
    ...
    if (!query_opt || !key_opt || !value_opt) { return status_t::failure; }
    ...
    if (has_mask && !mask_opt) {
        apilog_error(
                "SDPA ref kernel: has_mask=true but no \"mask\" tensor "
    ...
    const qkv_t *q_data
            = static_cast<const qkv_t *>(query.get_raw_handle_unsafe());
    const qkv_t *k_data
            = static_cast<const qkv_t *>(key.get_raw_handle_unsafe());
    const qkv_t *v_data
            = static_cast<const qkv_t *>(value.get_raw_handle_unsafe());
    qkv_t *out_data = static_cast<qkv_t *>(output.get_raw_handle_unsafe());
...
#pragma omp parallel for collapse(2)
        ...
        for (int64_t b = 0; b < batch; b++) {
            for (int64_t h = 0; h < num_heads; h++) {
                ...
                const mask_t *mask_for_bh = has_mask
                        ...
                        : nullptr;
                sdpa_encoder_ref::compute_sdpa_per_head<qkv_t, mask_t>(q_bh,
                        k_bh, v_bh, o_bh, mask_for_bh, seq_len_q, seq_len_kv,
                        head_dim, q_ss, k_ss, v_ss, o_ss, scale, is_causal,
                        has_mask);
            }
        }
    ...
    apilog_error("SDPA ref kernel: unsupported mask data_type = ",
            ...
            " for QKV data_type = ", static_cast<int>(query.get_data_type()));
    return status_t::failure;
}
...
status_t sdpa_encoder_ref_kernel_t::execute(const context_type &context_,
    ...
    auto query_opt = context_.get_param("query");
    if (!query_opt) { return status_t::failure; }
