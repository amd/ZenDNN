...
template<typename KEY_T, typename VALUE_T>
class lru_node_t {
  friend class lru_cche_t<KEY_T, VALUE_T>;
...
public:
  ...
  explicit lru_cche_t(size_type capacity_);
  ...
  /** @brief get a const iterator to end */
  void  set_config(const config_lru_cache_t& config_lru_cache_);
...
template<typename KEY_T, typename VALUE_T>
lru_cche_t<KEY_T, VALUE_T>::lru_cche_t():
...
template<typename KEY_T, typename VALUE_T>
lru_cche_t<KEY_T, VALUE_T>::lru_cche_t(size_type capacity_):
  lru_cche_t{} {
  capacity = capacity_;
}

template<typename KEY_T, typename VALUE_T>
status_t lru_cche_t<KEY_T, VALUE_T>::insert(const key_type& key_, const value_type& value_) {
  ...
  //insert new node
  [[maybe_unused]] bool insert_status = cache.insert({key_, lru_node_t{key_, value_}}).second;
...
template<typename KEY_T, typename VALUE_T>
status_t lru_cche_t<KEY_T, VALUE_T>::erase(const key_type& key_) {
  const std::lock_guard<std::mutex> lru_lock(lru_mutex);
...
template<typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::value_type
lru_cche_t<KEY_T, VALUE_T>::at(const key_type& key_) {
  const std::lock_guard<std::mutex> lru_lock(lru_mutex);
  return cache.at(key_).value;
}

template<typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::const_value_type&
lru_cche_t<KEY_T, VALUE_T>::at(const key_type& key_) const {
  const container_type& const_cache = cache;
  return const_cache.at(key_).value;
}

template<typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::optional_value_type
lru_cche_t<KEY_T, VALUE_T>::get_value(const key_type& key_) {
  const std::lock_guard<std::mutex> lru_lock(lru_mutex);
...
template<typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::const_iterator_type
lru_cche_t<KEY_T, VALUE_T>::find(const key_type& key_) const {
  const container_type& const_cache = cache;
  return const_cache.find(key_);
}

template<typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::size_type
lru_cche_t<KEY_T, VALUE_T>::get_size() const noexcept {
  return size;
}

template<typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::size_type
lru_cche_t<KEY_T, VALUE_T>::get_capacity() const noexcept {
  return capacity;
}

template<typename KEY_T, typename VALUE_T>
status_t lru_cche_t<KEY_T, VALUE_T>::set_capacity(size_type capacity_) {
  const std::lock_guard<std::mutex> lru_lock(lru_mutex);
  if (size >= capacity_) {
...
template<typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::const_iterator_type
lru_cche_t<KEY_T, VALUE_T>::cbegin() const noexcept {
  const container_type& const_cache = cache;
...
template<typename KEY_T, typename VALUE_T>
typename lru_cche_t<KEY_T, VALUE_T>::const_iterator_type
lru_cche_t<KEY_T, VALUE_T>::cend() const noexcept {
  const container_type& const_cache = cache;
  return const_cache.cend();
}

template<typename KEY_T, typename VALUE_T>
void lru_cche_t<KEY_T, VALUE_T>::set_config(const config_lru_cache_t& config_lru_cache_) {
  capacity = static_cast<size_type>(config_lru_cache_.capacity);
}

//some typedefs
using sptr_cache_value_type = std::shared_ptr<void>;
using sptr_lru_cache_t      = lru_cche_t<std::size_t, sptr_cache_value_type>;
}//common
