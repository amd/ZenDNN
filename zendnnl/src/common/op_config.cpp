    ...
    if (!matmul_json.empty()) {
        ...
        if (!matmul_algo_json.empty()) {
            ...
            if (!matmul_algo_str.empty()) {
                matmul_algo = static_cast<int32_t>(
                        str_to_matmul_algo(matmul_algo_str));
            }
        }
        ...
        if (!mm_partitioner_json.empty()) {
            auto mm_partitioner_str
                    = mm_partitioner_json.template get<std::string>();
            if (!mm_partitioner_str.empty()) {
                std::string value_str = mm_partitioner_str;
                std::transform(value_str.begin(), value_str.end(),
                        value_str.begin(),
                        [](unsigned char c) { return std::tolower(c); });
                mm_partitioner_enabled = (value_str == "true"
                        || value_str == "1" || value_str == "on");
            }
        }
        ...
        if (!bmm_algo_json.empty()) {
            ...
            if (!bmm_algo_str.empty()) {
                bmm_algo = static_cast<int32_t>(
                        str_to_matmul_algo(bmm_algo_str));
            }
        }
        ...
        if (!matmul_weight_cache_json.empty()) {
            ...
            if (!matmul_weight_cache_str.empty()) {
                ...
                if (matmul_weight_cache_str == "0") {
                    ...
                    apilog_warning(
                            "Unrecognized matmul weight_cache JSON value '",
        ...
        if (!zp_comp_cache_json.empty()) {
            auto zp_comp_cache_str
                    = zp_comp_cache_json.template get<std::string>();
...
void matmul_config_t::set_env_config() {
    ...
    if (bmm_algo_env) {
        ...
        std::transform(bmm_algoStr.begin(), bmm_algoStr.end(),
                bmm_algoStr.begin(),
                [](unsigned char c) { return std::tolower(c); });
        if (bmm_algoStr == "auto") {
    ...
    if (otf_bpack_env) {
        try {
            ...
            if (val == 0 || val == 1) { matmul_otf_bpack = val; }
        } catch (...) {}
    }
