...
const config_postop_cache_t &config_manager_t::get_postop_cache_config() const {
...
status_t config_manager_t::parse(std::string file_name_) {
    ...
    if (!json_file.is_open()) { return status_t::config_bad_json_file; }
...
void config_manager_t::set_user_config() {
    ...
    if (global_cache_off) { config_postop_cache.enable = false; }
...
status_t config_manager_t::set_user_logger_config() {
    ...
    if (logger_json.empty()) { return status_t::failure; }
...
status_t config_manager_t::set_env_logger_config() {
    {
        log_level_t level;
        if (parse_env_log_level(
                    std::getenv("ZENDNNL_COMMON_LOG_LEVEL"), level)) {
...

    {
        log_level_t level;
        if (parse_env_log_level(
                    std::getenv("ZENDNNL_PROFILE_LOG_LEVEL"), level)) {
...

    {
        log_level_t level;
        if (parse_env_log_level(
                    std::getenv("ZENDNNL_DEBUG_LOG_LEVEL"), level)) {
...
status_t config_manager_t::set_user_profiler_config() {
    ...
    if (profiler_json.empty()) { return status_t::failure; }
...
status_t config_manager_t::set_env_profiler_config() {
    {
        ...
        if (enable_profiler_str) {
            config_profiler.enable_profiler
                    = (std::string(enable_profiler_str) == "1");
        }
    }
...
status_t config_manager_t::set_user_lru_cache_config() {
    ...
    if (lru_cache_json.empty()) { return status_t::failure; }
...
status_t config_manager_t::set_env_lru_cache_config() {
    {
        char *lru_cache_capacity_str
                = std::getenv("ZENDNNL_LRU_CACHE_CAPACITY");
        char *endptr;
        if (lru_cache_capacity_str) {
            config_lru_cache.capacity
                    = std::strtoul(lru_cache_capacity_str, &endptr, 10);
        }
    }
...
status_t config_manager_t::set_user_global_cache_config() {
    ...
    if (global_cache_json.empty()) { return status_t::failure; }
...
status_t config_manager_t::set_user_postop_cache_config() {
    ...
    if (postop_cache_json.empty()) { return status_t::failure; }
...
status_t config_manager_t::set_env_postop_cache_config() {
    ...
    if (!enable_str) { return status_t::success; }
