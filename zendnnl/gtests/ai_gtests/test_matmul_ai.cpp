...
class TestMatmulAI : public ::testing::TestWithParam<MatmulParamsAI> {
    ...
    CreatedTensors create_test_tensors(const MatmulParamsAI &params,
            ...
            data_type_t output_dtype, bool use_boundary = false,
            bool create_reference_output = false,
            const std::string &name_suffix = "") {
        CreatedTensors tensors;
        auto tensor_creator = use_boundary
                ? &AITensorFactory::create_boundary_tensor
        ...
        tensors.input
                = tensor_creator({params.m, params.k}, input_dtype, input_name);
        ...
        if (weight_dtype == data_type_t::s4) {
            ...
            tensors.weights = tensor_creator(
                    {params.k, params.n}, weight_dtype, weights_name);
            tensors.is_woq = false;
        }

        tensors.bias = tensor_creator({1, params.n}, output_dtype, bias_name);
        tensors.output = AITensorFactory::create_zero_tensor(
                {params.m, params.n}, output_dtype, output_name);
        if (create_reference_output)
            tensors.reference_output = AITensorFactory::create_zero_tensor(
                    {params.m, params.n}, output_dtype, ref_output_name);
        ...
        for (const auto &post_op_type : params.post_op_config.post_ops) {
            if (post_op_type == post_op_type_t::binary_add) {
                std::string name = "binary_add_tensor_"
                        + std::to_string(add_count) + name_suffix;
                auto tensor = tensor_creator(
                        {params.m, params.n}, output_dtype, name);
                tensors.binary_post_op_tensors.emplace_back(name, tensor);
                ++add_count;
            } else if (post_op_type == post_op_type_t::binary_mul) {
                std::string name = "binary_mul_tensor_"
                        + std::to_string(mul_count) + name_suffix;
                auto tensor = tensor_creator(
                        {params.m, params.n}, output_dtype, name);
                tensors.binary_post_op_tensors.emplace_back(name, tensor);
                ++mul_count;
...
protected:
    ...
    void run_reference_kernel_test(const MatmulParamsAI &params) {
        ASSERT_TRUE(
                AITestUtils::validate_dimensions(params.m, params.n, params.k))
        ...
        status_t ref_status = AITestUtils::run_reference_matmul(tensors.input,
                tensors.weights, tensors.bias, tensors.output,
        ...
        if (AITestUtils::is_reference_implementation_supported(input_dtype,
                    ...
                    params.post_op_config.post_ops)) {
            EXPECT_EQ(ref_status, status_t::success)
                    << "Reference kernel must succeed for supported "
                       "dtype/post-op combinations";
        } else {
            EXPECT_NE(ref_status, status_t::success)
                    << "Reference kernel should fail for non-supported "
                       "post-ops";
        }
    }
    // -----------------------------------------------------------------------------
    ...
    float get_accuracy_tolerance(data_type_t output_dtype) const {
        switch (output_dtype) {
            case data_type_t::f32: return AI_MATMUL_TOLERANCE_F32;
            case data_type_t::bf16: return AI_MATMUL_TOLERANCE_BF16;
            case data_type_t::s8: return AI_MATMUL_TOLERANCE_S8;
            default: return AI_MATMUL_TOLERANCE_DEFAULT;
        }
    }

    float get_relative_tolerance(data_type_t output_dtype) const {
        switch (output_dtype) {
            case data_type_t::f32: return AI_MATMUL_REL_TOLERANCE_F32;
            case data_type_t::bf16: return AI_MATMUL_REL_TOLERANCE_BF16;
            case data_type_t::s8: return AI_MATMUL_REL_TOLERANCE_S8;
            default: return AI_MATMUL_REL_TOLERANCE_DEFAULT;
        }
    }
    ...
    float get_epsilon_value(data_type_t dtype) {
        switch (dtype) {
            case data_type_t::f32: return AI_MATMUL_EPSILON_F32;
            case data_type_t::bf16: return AI_MATMUL_EPSILON_BF16;
            case data_type_t::s8: return AI_MATMUL_EPSILON_S8;
            default: return AI_MATMUL_EPSILON_DEFAULT;
        }
    }
    ...
    void run_accuracy_test(const MatmulParamsAI &params) {
        ...
        auto input_dtype
                = ai_gtests::AITestUtils::get_input_dtype(params.data_types);
        auto weight_dtype
                = ai_gtests::AITestUtils::get_weight_dtype(params.data_types);
        auto output_dtype
                = ai_gtests::AITestUtils::get_output_dtype(params.data_types);

        auto tensors = create_test_tensors(params, input_dtype, weight_dtype,
                output_dtype, false, true, "_accuracy");
        if (!tensors.input.get_raw_handle_const()) {
            AITestUtils::debug_print(
                    "[AI_DEBUG][FATAL] Input tensor data pointer is null!");
            std::abort();
        }
        if (!tensors.weights.get_raw_handle_const()) {
            AITestUtils::debug_print(
                    "[AI_DEBUG][FATAL] Weights tensor data pointer is null!");
            std::abort();
        }
        if (!tensors.output.get_raw_handle_const()) {
            AITestUtils::debug_print(
                    "[AI_DEBUG][FATAL] Output tensor data pointer is null!");
            std::abort();
        }
        if (!tensors.reference_output.get_raw_handle_const()) {
            AITestUtils::debug_print(
                    "[AI_DEBUG][FATAL] Reference output tensor data pointer is "
                    "null!");
            std::abort();
        }
        ...
        status_t test_status
                = run_matmul_test(tensors.input, tensors.weights, tensors.bias,
                        tensors.output, params, tensors.binary_post_op_tensors);
        AITestUtils::debug_print("[AI_DEBUG] ZenDNNL kernel finished.");
        if (aocl_supported) {
            EXPECT_EQ(test_status, status_t::success)
                    << "ZenDNNL AOCL kernel must succeed for supported data "
                       "types";
        } else {
            EXPECT_NE(test_status, status_t::success)
                    << "ZenDNNL AOCL kernel should fail for unsupported data "
                       "types";
        }
        ...
        if (AITestUtils::is_reference_implementation_supported(input_dtype,
            ...
            if (test_status == status_t::success) {
                ...
                AITestUtils::debug_print(
                        "[AI_DEBUG] About to run reference implementation...");
                std::vector<tensor_t> ref_binary_post_op_tensors;
                for (const auto &pair : tensors.binary_post_op_tensors) {
                    ref_binary_post_op_tensors.push_back(pair.second);
                }
                status_t ref_status = AITestUtils::run_reference_matmul(
                        ...
                        tensors.reference_output, params.post_op_config,
                        ref_binary_post_op_tensors, mask_libxsmm_postops_,
                        skip_libxsmm_bf16_bias_);
                AITestUtils::debug_print(
                        "[AI_DEBUG] Reference implementation finished.");
                EXPECT_EQ(ref_status, status_t::success)
                        << "Reference implementation must succeed for "
                ...
                if (ref_status == status_t::success) {
                    ...
                    bool comparison_result
                            = AITestUtils::compare_sampled_tensors_matmul(
                                    tensors.output, tensors.reference_output,
                                    params.k, rel_tolerance, epsilon,
                                    mask_libxsmm_postops_);
                    EXPECT_TRUE(comparison_result)
                            << "ZenDNNL output must match reference within "
                               "abs_bound + rtol*|ref|, where abs_bound "
                               "(epsilon-based) = "
                            << epsilon << ", rel tolerance: " << rel_tolerance;
                }
            }
        } else {
            ...
            if (params.expect_success) {
                if (test_status == status_t::success) {
                    ...
                    EXPECT_TRUE(validate_output_tensor(
                            tensors.output, params, weight_dtype))
    ...
    void run_boundary_test(const MatmulParamsAI &params) {
        ...
        if (params.expect_success) {
            EXPECT_EQ(test_status, status_t::success)
                    << "Boundary test failed when success was expected for "
                    << params.test_name;
            ...
            EXPECT_TRUE(validate_boundary_output(tensors.output, params))
                    << "Boundary output validation failed for "
                    << params.test_name;

            // Check for precision-related issues
            EXPECT_TRUE(validate_numerical_stability(tensors, params))
                    << "Numerical stability check failed for "
                    << params.test_name;

        } else {
            EXPECT_NE(test_status, status_t::success)
                    << "Boundary test succeeded when failure was expected for "
                    << params.test_name;
        }

        std::cout << "[AI_BOUNDARY] " << params.test_name
                  << " completed with status: " << static_cast<int>(test_status)
                  << std::endl;
    }
   ...
   */
    void run_invalid_test(const MatmulParamsAI &params) {
        ...
        EXPECT_NE(test_status, status_t::success)
                << "Invalid test should fail but succeeded: "
                << params.test_name;
    }
...
private:
    ...
    status_t run_invalid_matmul_test(const MatmulParamsAI &params) {
        ...
        if (case_type.find("weights_null") != std::string::npos) {
            tensor_t input = AITensorFactory::create_uniform_tensor(
                    input_dims, input_dtype, "input_accuracy");
            tensor_t bias = AITensorFactory::create_uniform_tensor(
                    bias_dims, output_dtype, "bias_accuracy");
            tensor_t output = AITensorFactory::create_zero_tensor(
                    output_dims, output_dtype, "output_zero");
            try {
                matmul_context_t matmul_context
                        = matmul_context_t().set_param("bias", bias).create();
        ...
        if (case_type.find("weights_not_2d") != std::string::npos) {
            tensor_t input = AITensorFactory::create_uniform_tensor(
                    input_dims, input_dtype, "input_accuracy");
            tensor_t weights = AITensorFactory::create_uniform_tensor(
                    {params.k, params.n, 2}, weight_dtype,
                    "weights3d_accuracy");
            tensor_t bias = AITensorFactory::create_uniform_tensor(
                    bias_dims, output_dtype, "bias_accuracy");
            tensor_t output = AITensorFactory::create_zero_tensor(
        ...
        if (case_type.find("bias_size_mismatch") != std::string::npos) {
            tensor_t input = AITensorFactory::create_uniform_tensor(
                    input_dims, input_dtype, "input_accuracy");
            tensor_t weights = AITensorFactory::create_uniform_tensor(
                    weight_dims, weight_dtype, "weights_accuracy");
            tensor_t bias = AITensorFactory::create_uniform_tensor(
                    {1, params.n + 1}, output_dtype, "bias_bad_accuracy");
            tensor_t output = AITensorFactory::create_zero_tensor(
                    output_dims, output_dtype, "output_zero");
            try {
                auto matmul_context = matmul_context_t()
        ...
        if (case_type.find("input_not_2d") != std::string::npos) {
            tensor_t input = AITensorFactory::create_uniform_tensor(
                    {params.m, params.k, 2}, input_dtype, "input3d_accuracy");
            tensor_t weights = AITensorFactory::create_uniform_tensor(
                    weight_dims, weight_dtype, "weights_accuracy");
            tensor_t bias = AITensorFactory::create_uniform_tensor(
                    bias_dims, output_dtype, "bias_accuracy");
        ...
        if (case_type.find("output_not_2d") != std::string::npos) {
            ...
            tensor_t bias = AITensorFactory::create_uniform_tensor(
                    bias_dims, output_dtype, "bias_accuracy");
            tensor_t output = AITensorFactory::create_zero_tensor(
                    {params.m, params.n, 2}, output_dtype, "output3d_zero");
            try {
                auto matmul_context = matmul_context_t()
                                              .set_param("weights", weights)
                                              .set_param("bias", bias)
        ...
        if (case_type.find("forced_kernel_unsupported") != std::string::npos) {
            ...
            try {
                ...
                auto matmul_context = matmul_context_t()
                                              .set_param("weights", weights)
                                              .set_param("bias", bias)
                                              .set_param("forced_kernel",
                                                      forced_kernel_tensor)
                                              .create();
                if (!matmul_context.check()) { return status_t::failure; }
                matmul_operator_t matmul_operator
                        = matmul_operator_t()
        ...
        if (case_type.find("forced_kernel_unknown") != std::string::npos) {
            ...
            try {
                ...
                matmul_operator_t matmul_operator
                        = matmul_operator_t()
                                  .set_context(matmul_context)
                                  .set_forced_kernel("foobar_kernel")
                                  .create();
                matmul_operator.set_input("matmul_input", input)
                        .set_output("matmul_output", output)
                        .execute();
            ...
            try {
                ...
                matmul_context_t matmul_context
                        = matmul_context_t()
                                  ...
                                  .create();
                matmul_context
                        = matmul_context.set_post_op(bad_post_op).create();
                if (!matmul_context.check()) { return status_t::failure; }
                matmul_operator_t matmul_operator
                        = matmul_operator_t()
                                  .set_context(matmul_context)
            ...
            try {
                ...
                matmul_context_t matmul_context
                        = matmul_context_t()
                                  ...
                                  .set_param("bias", bias)
                                  .create();
                matmul_context = matmul_context.set_post_op(relu_post_op)
                                         .set_post_op(bad_post_op)
                                         .create();
                if (!matmul_context.check()) { return status_t::failure; }
                matmul_operator_t matmul_operator
                        = matmul_operator_t()
        ...
        if (case_type.find("binary_post_op_missing") != std::string::npos) {
            ...
            try {
                ...
                auto matmul_context = matmul_context_t()
                                              .set_param("weights", weights)
                                              .set_param("bias", bias)
                                              .set_post_op(binary_add_post_op)
                                              .create();
                if (!matmul_context.check()) { return status_t::failure; }
                matmul_operator_t matmul_operator
                        = matmul_operator_t()
        ...
        if (case_type.find("dim_mismatch") != std::string::npos) {
            tensor_t input = AITensorFactory::create_uniform_tensor(
                    {params.m, params.k + 1}, input_dtype,
                    "input_bad_accuracy");
            tensor_t weights = AITensorFactory::create_uniform_tensor(
                    weight_dims, weight_dtype, "weights_accuracy");
            tensor_t bias = AITensorFactory::create_uniform_tensor(
        ...
        if (case_type.find("unsupported_dtype") != std::string::npos) {
            tensor_t input = AITensorFactory::create_uniform_tensor(
                    input_dims, data_type_t::s4, "input_accuracy");
            tensor_t weights = AITensorFactory::create_uniform_tensor(
                    weight_dims, data_type_t::s4, "weights_accuracy");
            tensor_t bias = AITensorFactory::create_uniform_tensor(
                    bias_dims, data_type_t::s4, "bias_accuracy");
            tensor_t output = AITensorFactory::create_zero_tensor(
                    output_dims, data_type_t::s4, "output_zero");
            try {
                auto matmul_context = matmul_context_t()
                                              .set_param("weights", weights)
                                              .set_param("bias", bias)
        ...
        try {
            ...
            if (weight_dtype == data_type_t::s4) {
                ...
                weights = AITensorFactory::create_uniform_tensor(
                        weight_dims, weight_dtype, "weights_accuracy");
            }
            tensor_t bias = AITensorFactory::create_uniform_tensor(
                    bias_dims, output_dtype, "bias_accuracy");
            ...
            if (!matmul_context.check()) { return status_t::failure; }
            matmul_operator_t matmul_operator
                    = matmul_operator_t().set_context(matmul_context).create();
            matmul_operator.set_input("matmul_input", input)
                    .set_output("matmul_output", output)
                    .execute();
            return status_t::failure; // Invalid test should always fail
    ...
    // -----------------------------------------------------------------------------
    status_t run_matmul_test(tensor_t &input, tensor_t &weights, tensor_t &bias,
            tensor_t &output, const MatmulParamsAI &params,
            std::vector<std::pair<std::string, tensor_t>>
        ...
        try {
            ...
            if (is_lowoha_mode_enabled()) {
                // LOWOHA path - use matmul_direct API
                AITestUtils::debug_print(
                        "[AI_DEBUG] Running LOWOHA matmul_direct...");

                // Validate tensors
                if (!input.check() || !weights.check() || !output.check()) {
                    std::cout << "[AI_TEST] LOWOHA: Invalid tensor state for "
                ...
                if (!A_data || !B_data || !C_data) {
                    std::cout << "[AI_TEST] LOWOHA: Null data pointer for "
                              << params.test_name << std::endl;
                    return status_t::failure;
                }
                ...
                if (weights.get_data_type() == data_type_t::s4) {
                    ...
                    const void *scale_buff
                            = weights.get_quant_scale_raw_handle_const();
                    const void *zp_buff
                            = weights.get_quant_zero_raw_handle_const();

                    if (scale_buff) {
                        matmul_params_obj.quant_params.wei_scale.buff
                                = scale_buff;
                        matmul_params_obj.quant_params.wei_scale.dt
                                = weights.get_quant_scale_data_type();
                        auto scale_size = weights.get_quant_scale_size();
                        matmul_params_obj.quant_params.wei_scale.dims.assign(
                                scale_size.begin(), scale_size.end());
                        AITestUtils::debug_print(
                                "[AI_DEBUG] WoQ: Weight scale extracted for "
                                "LOWOHA");
                    }

                    if (zp_buff) {
                        matmul_params_obj.quant_params.wei_zp.buff = zp_buff;
                        matmul_params_obj.quant_params.wei_zp.dt
                                = weights.get_quant_zero_data_type();
                        auto zp_size = weights.get_quant_zero_size();
                        matmul_params_obj.quant_params.wei_zp.dims.assign(
                                zp_size.begin(), zp_size.end());
                        AITestUtils::debug_print(
                                "[AI_DEBUG] WoQ: Weight zero-point extracted "
                                "for LOWOHA");
                    }
                }
                ...
                const matmul_algo_t lowoha_algo = kernel_select(
                        matmul_params_obj, batch_params.Batch_A,
                        batch_params.Batch_B, 1, M, N, K,
                        matmul_params_obj.num_threads,
                        bias.get_raw_handle_unsafe(), is_weights_const, false);
                const bool is_libxsmm_kernel = (lowoha_algo
                                == matmul_algo_t::libxsmm
                ...
                const bool skip_bias = (is_libxsmm_kernel
                        && output.get_data_type() == data_type_t::bf16);
                skip_libxsmm_bf16_bias_ = skip_bias;
                void *bias_data
                        = skip_bias ? nullptr : bias.get_raw_handle_unsafe();
                matmul_dtypes.bias
                        = skip_bias ? data_type_t::f32 : bias.get_data_type();
                matmul_params_obj.dtypes = matmul_dtypes;
                ...
                for (size_t i = 0; i < params.post_op_config.post_ops.size();
                    ...
                    if (post_op_type == post_op_type_t::binary_add
                        ...
                        if (i < binary_post_op_tensors.size()) {
                            postop_item.buff
                                    = binary_post_op_tensors[i]
                                              .second.get_raw_handle_unsafe();
                            postop_item.dtype = binary_post_op_tensors[i]
                                                        .second.get_data_type();
                            auto dims = binary_post_op_tensors[i]
                                                .second.get_size();
                ...
                status_t status = matmul_direct('r', // layout: row-major
                        ...
                        M, N, K, 1.0f, A_data, lda, B_data, ldb, bias_data,
                        0.0f, C_data, ldc,
                        is_weights_const, // true for WoQ, false otherwise
                        batch_params, matmul_params_obj);

                if (status != status_t::success) {
                    std::cout << "[AI_TEST] LOWOHA matmul_direct failed for "
                              << params.test_name << std::endl;
                }
                return status;
            } else {
                ...
                AITestUtils::debug_print(
                        "[AI_DEBUG] Running primitive matmul operator...");

                matmul_context_t matmul_context
                        = matmul_context_t()
                                  .set_param("weights", weights)
                                  .set_param("bias", bias);
                for (const auto &post_op_type :
                        params.post_op_config.post_ops) {
                ...
                if (!matmul_context.check()) {
                    std::cout << "[AI_TEST] Context creation failed for "
                              << params.test_name << std::endl;
                    return status_t::failure;
                }
                auto matmul_operator
                        = matmul_operator_t()
                                  .set_name(AITestUtils::generate_unique_name(
                                          "matmul_ai_op"))
                                  .set_context(matmul_context)
                                  .create();
                if (matmul_operator.is_bad_object()) {
                    std::cout << "[AI_TEST] Operator creation failed for "
                              << params.test_name << std::endl;
                    return status_t::failure;
                }
                ...
                for (size_t i = 0; i < params.post_op_config.post_ops.size();
                    ...
                    if ((post_op_type == post_op_type_t::binary_add
                                || post_op_type == post_op_type_t::binary_mul)
                            && binary_tensor_idx
                                    < binary_post_op_tensors.size()) {
                        std::string tensor_name;
                        try {
                            if (post_op_type == post_op_type_t::binary_add) {
                                tensor_name = matmul_context.get_post_op(i)
                                                      .binary_add_params
                                                      .tensor_name;
                            } else {
                                tensor_name = matmul_context.get_post_op(i)
                            ...
                            tensor_name
                                    = binary_post_op_tensors[binary_tensor_idx]
                                              .first; // fallback
                        }
                ...
                auto status = matmul_operator.set_input("matmul_input", input)
                                      .set_output("matmul_output", output)
                                      .execute();
            ...
            std::cout << "[AI_TEST] Exception in " << params.test_name << ": "
                      << e.what() << std::endl;
            return status_t::failure;
        } catch (...) {
            std::cout << "[AI_TEST] Unknown exception in " << params.test_name
                      << std::endl;
            return status_t::failure;
        }
    }
    ...
    bool validate_output_tensor(const tensor_t &output,
        ...
        if (output.get_size() != expected_dims) { return false; }

        // Check data type
        auto expected_dtype = AITestUtils::get_output_dtype(params.data_types);
        if (output.get_data_type() != expected_dtype) { return false; }
        ...
        if (output.get_nelem() > 0) {
            ...
            auto sample_indices
                    = AITestUtils::get_sample_indices(output.get_nelem(), 100);
            if (dtype == data_type_t::f32) {
                ...
                const int8_t *data = static_cast<const int8_t *>(
                        output.get_raw_handle_const());
                for (size_t idx : sample_indices) {
                    if (data[idx] != 0) {
                        has_non_zero = true;
    ...
    // -----------------------------------------------------------------------------
    bool validate_boundary_output(
            const tensor_t &output, const MatmulParamsAI &params) {
        ...
        auto sample_indices
                = AITestUtils::get_sample_indices(num_elements, 1000);
        ...
        if (dtype == data_type_t::f32) {
            const float *data
                    = static_cast<const float *>(output.get_raw_handle_const());
            for (size_t idx : sample_indices) {
                float val = data[idx];
            ...
            for (size_t idx : sample_indices) {
                ...
                if (val != 0) { has_reasonable_values = true; }
            }
        } else {
            // For other types, fallback: check raw bytes
            const uint8_t *data = static_cast<const uint8_t *>(
        ...
        if (has_nan) {
            std::cout << "[AI_BOUNDARY_ERROR] Output contains NaN values"
                      << std::endl;
            return false;
        }
        if (has_inf) {
            std::cout << "[AI_BOUNDARY_ERROR] Output contains Inf values"
                      << std::endl;
            return false;
        }
        ...
        if (params.m * params.n <= 16 && !has_reasonable_values
            ...
            std::cout << "[AI_BOUNDARY_ERROR] Small boundary case produced "
                         "unreasonable values"
                      << std::endl;
            return false;
        }
    ...
    bool validate_numerical_stability(
            const CreatedTensors &tensors, const MatmulParamsAI &params) {
        // --- NUMERICAL STABILITY CHECKS FOR BOUNDARY CONDITIONS ---
        // If output is empty, nothing to check
        if (tensors.output.get_nelem() == 0) { return true; }
            ...
            switch (dtype) {
                ...
                case data_type_t::bf16:
                    return static_cast<float>(
                            static_cast<const bfloat16_t *>(ptr)[0]);
                case data_type_t::s8:
                case data_type_t::s4:
                    return static_cast<float>(
                            static_cast<const int8_t *>(ptr)[0]);
                case data_type_t::u8:
                    return static_cast<float>(
        ...
        float expected_max_magnitude
                = static_cast<float>(params.k) * max_single_product * 2.0f;
        ...
        float expected_max_range
                = static_cast<float>(params.k) * max_single_product * 4.0f;
        ...
        if (params.m == 1 && params.n == 1 && params.k == 1
                ...
                && weight_dtype != data_type_t::s4) {
            float input_val = get_val(
                    tensors.input.get_raw_handle_const(), input_dtype);
            float weight_val = get_val(
                    tensors.weights.get_raw_handle_const(), weight_dtype);
            float bias_val
                    = get_val(tensors.bias.get_raw_handle_const(), bias_dtype);
            float expected_val = input_val * weight_val + bias_val;
            float actual_val
                    = get_val(tensors.output.get_raw_handle_const(), dtype);
            float expected_magnitude = std::abs(expected_val);
            float actual_magnitude = std::abs(actual_val);
            float relative_error
                    = std::abs(actual_magnitude - expected_magnitude)
                    / (expected_magnitude + 1e-10f);
            if (relative_error > 0.01f) { return false; }
        }
        ...
        if (params.m == 1 || params.n == 1) {
            ...
            output_magnitude
                    = std::sqrt(output_magnitude / sample_indices.size());
            if (output_magnitude > magnitude_threshold) {
                std::cout << "[AI_BOUNDARY_ERROR] Vector operation produced "
                             "excessive magnitude: "
                          << output_magnitude
        ...
        if (params.m * params.n > 100) {
            ...
            if (output_range > range_threshold) {
                std::cout << "[AI_BOUNDARY_ERROR] Large output range detected: "
                          << output_range << " (threshold: " << range_threshold
                          << ")" << std::endl;
                return false;
            }
        }

        std::cout << "[AI_BOUNDARY_OK] Numerical stability validated"
                  << std::endl;
        return true;
    }
};
...
TEST_P(TestMatmulAI, ComprehensiveMatmulTest) {
    ...
    switch (params.category) {
        case TestCategory::ACCURACY: run_accuracy_test(params); break;
        case TestCategory::BOUNDARY: run_boundary_test(params); break;
        case TestCategory::EDGE_CASE: run_edge_case_test(params); break;
        case TestCategory::INVALID: run_invalid_test(params); break;
        case TestCategory::REFERENCE_KERNEL:
            run_reference_kernel_test(params);
            break;
        default:
            FAIL() << "Unknown test category: "
                   << static_cast<int>(params.category);
            break;
    }
}
...
INSTANTIATE_TEST_SUITE_P(AITests, TestMatmulAI,
        ::testing::ValuesIn(
