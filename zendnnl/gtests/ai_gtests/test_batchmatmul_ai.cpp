...
class TestBatchMatmulAI : public ::testing::TestWithParam<BatchMatmulParamsAI> {
 ...
 protected:
  ...
  void run_boundary_test(const BatchMatmulParamsAI &params) {
    ...
    std::cout << "[AI_BATCH_BOUNDARY] " << params.test_name << " completed with status: "
 ...
 private:
  ...
  bool validate_boundary_output(const tensor_t &output,
    ...
    if (has_nan) {
      std::cout << "[AI_BATCH_BOUNDARY_ERROR] Output contains NaN values" << std::endl;
      return false;
    }
    if (has_inf) {
      std::cout << "[AI_BATCH_BOUNDARY_ERROR] Output contains Inf values" << std::endl;
      return false;
    }
    
    if (params.batch_size * params.m * params.n <= 16 && !has_reasonable_values) {
      std::cout << "[AI_BATCH_BOUNDARY_ERROR] Small boundary case produced unreasonable values" 
                << std::endl;
      return false;
    }
  ...
  bool validate_numerical_stability(const CreatedTensors &tensors,
    ...
    std::cout << "[AI_BATCH_BOUNDARY_OK] Numerical stability validated" << std::endl;
    return true;
  }
};
...
INSTANTIATE_TEST_SUITE_P(
  AIMinimalTests,
  TestBatchMatmulAI,
  ::testing::ValuesIn(BatchMatmulParameterGenerator::generate_minimal_test_suite()),
...
INSTANTIATE_TEST_SUITE_P(
  AIAccuracyTests,
  TestBatchMatmulAI,
  ::testing::ValuesIn(BatchMatmulParameterGenerator::generate_category_specific_params(
