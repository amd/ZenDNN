...
GroupTensors build_group_embag_tensors(tensor_factory_t &factory,
        size_t group_size, uint64_t num_embeddings, uint64_t embedding_dim,
        uint64_t num_indices, uint64_t num_bags, embag_algo_t algo,
        int64_t padding_index, bool include_last_offset, bool is_weights,
        bool fp16_scale_bias_v, data_type_t table_dtype,
        data_type_t output_dtype, data_type_t indices_dtype,
        data_type_t offsets_dtype);
...
GroupTensors build_group_embag_quant_tensors(tensor_factory_t &factory,
        size_t group_size, uint64_t num_embeddings, uint64_t embedding_dim,
        uint64_t num_indices, uint64_t num_bags, embag_algo_t algo,
        int64_t padding_index, bool include_last_offset, bool is_weights,
        bool fp16_scale_bias_v, data_type_t table_dtype,
...
GroupTensors build_group_embedding_tensors(tensor_factory_t &factory,
        size_t group_size, uint64_t num_embeddings, uint64_t embedding_dim,
        uint64_t num_indices, int64_t padding_index, bool is_weights,
        bool fp16_scale_bias_v, data_type_t table_dtype,
        data_type_t output_dtype, data_type_t indices_dtype);
 ...
 *         Table buffers are heap-allocated; call free_quant_tables()
 *         before the GroupTensors goes out of scope. */
GroupTensors build_group_embedding_quant_tensors(tensor_factory_t &factory,
        size_t group_size, uint64_t num_embeddings, uint64_t embedding_dim,
        uint64_t num_indices, int64_t padding_index, bool is_weights,
        bool fp16_scale_bias_v, data_type_t table_dtype,
        data_type_t output_dtype, data_type_t indices_dtype);
 ...
 *  state.
 */
status_t group_embag_kernel_test(std::vector<tensor_t> &tables,
        std::vector<tensor_t> &indices, std::vector<tensor_t> &offsets,
        std::vector<tensor_t> &weights, std::vector<tensor_t> &outputs,
        const std::vector<embag_algo_t> &algos,
        const std::vector<int64_t> &padding_idxs,
        const std::vector<bool> &include_last_offsets,
        const std::vector<bool> &fp16_scale_bias, eb_thread_algo_t thread_algo);
 ...
 *         failure. */
status_t group_embag_forced_ref_kernel_test(std::vector<tensor_t> &tables,
        std::vector<tensor_t> &indices, std::vector<tensor_t> &offsets,
        std::vector<tensor_t> &weights, std::vector<tensor_t> &outputs,
        const std::vector<embag_algo_t> &algos,
        const std::vector<int64_t> &padding_idxs,
