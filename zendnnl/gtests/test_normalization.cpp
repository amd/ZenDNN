...
protected:
    virtual void SetUp() {
        ...
        gamma_size = (norm_type == norm_type_t::BATCH_NORM) ? num_channels
                                                            : norm_size;

        log_info("norm_type: ", norm_type_to_str(norm_type), " batch: ", batch,
                " norm_size: ", norm_size, " total_elements: ", total_elements,
                " epsilon: ", epsilon, " use_scale: ", use_scale,
                " use_shift: ", use_shift, " gamma_dt: ", dtype_info(gamma_dt),
   ...
   */
    void create_optional_tensors(data_type_t src_dt, data_type_t dst_dt) {
        ...
        if (norm_type == norm_type_t::FUSED_ADD_RMS_NORM) {
            residual_tensor
                    = tensor_factory.uniform_dist_tensor(shape, src_dt, 2.0f);
...
TEST_P(TestNormalization, F32_F32) {
    ...
    auto input_tensor = tensor_factory.uniform_dist_tensor(shape, src_dt, 2.0f);
    ...
    status_t status = normalization_kernel_test(input_tensor, output_tensor,
...
TEST(NormalizationFusedTailF16, MaskedTail) {
    ...
    for (uint64_t ns : tail_sizes) {
        ...
        auto residual_tensor
                = tensor_factory.uniform_dist_tensor(shape, dt, 2.0f);
        ...
        status_t status = normalization_kernel_test(input_tensor, output_tensor,
                gamma_tensor, beta_tensor, running_mean_tensor,
                running_var_tensor, residual_tensor, np);
        if (status == status_t::isa_unsupported) {
            GTEST_SKIP() << "F16 not supported: requires F16-capable ISA";
        }
        ...
        if (ok) {
            compare_norm_tensors(output_tensor, output_tensor_ref, shape,
                    total_elements, NORM_F16_TOL, ok);
        }
        EXPECT_TRUE(ok)
                << "FusedAddRMSNorm F16 masked-tail failed for norm_size="
                << ns;
    }
}
