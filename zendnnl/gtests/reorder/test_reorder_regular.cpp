...
/// @file test_reorder_regular.cpp
/// @brief Legacy "regular" (non-LOWOHA) reorder + matmul tests.
///
/// Every test in this file begins with `if (use_LOWOHA) GTEST_SKIP();` and is
/// therefore always SKIPPED on the LOWOHA-only suite (use_LOWOHA is always
/// true unless the user passes a non-true `--lowoha`, in which case SetUp()
/// masks the whole fixture).  Kept for reference and reactivation.
///
/// This TU also hosts the single `INSTANTIATE_TEST_SUITE_P(Reorder,
/// TestReorder, ...)` for the entire reorder suite (all behaviors share the
/// one `TestReorder` fixture defined in reorder_test_common.hpp).
...
TEST_P(TestReorder, F32_F32) {
    ...
    auto weights = tensor_factory.uniform_dist_tensor(
            {k, n}, data_type_t::f32, 1.0f);
    auto input_tensor = tensor_factory.uniform_dist_tensor(
            {m, k}, data_type_t::f32, 1.0f);
    auto bias_tensor = tensor_factory.uniform_dist_tensor(
            {1, n}, data_type_t::f32, 1.0f);
    auto binary_tensors = make_binary_postop_tensors(
            tensor_factory, po_types, {m, n}, data_type_t::f32, 1.0f);
    auto output_tensor_ref
            = tensor_factory.zero_tensor({m, n}, data_type_t::f32);
    ...
    status_t status
            = matmul_kernel_test(input_tensor, reorder_weights, bias_tensor,
                    output_tensor, po_types, binary_tensors, use_LOWOHA, algo);

    bool is_test_successful
            = (status == status_t::success && ref_status == status_t::success);

    if (is_test_successful) {
        compare_tensor_2D(output_tensor, output_tensor_ref, m, n,
...
TEST_P(TestReorder, BF16_F32) {
    if (use_LOWOHA) { GTEST_SKIP(); }
    auto weights = tensor_factory.uniform_dist_tensor(
            {k, n}, data_type_t::bf16, 1.0f);
    auto input_tensor = tensor_factory.uniform_dist_tensor(
            {m, k}, data_type_t::bf16, 1.0f);
    auto bias_tensor = tensor_factory.uniform_dist_tensor({1, n},
            rand() % 2 == 0 ? data_type_t::bf16 : data_type_t::f32, 1.0f);
    auto binary_tensors = make_binary_postop_tensors(
            tensor_factory, po_types, {m, n}, data_type_t::f32, 1.0f);
    auto output_tensor_ref
            = tensor_factory.zero_tensor({m, n}, data_type_t::f32);
...
TEST_P(TestReorder, BF16_BF16) {
    ...
    auto binary_tensors = make_binary_postop_tensors(
            tensor_factory, po_types, {m, n}, data_type_t::f32, 1.0f);
    auto output_tensor_ref
            = tensor_factory.zero_tensor({m, n}, data_type_t::bf16);
    status_t ref_status = matmul_kernel_test(input_tensor, weights, bias_tensor,
            output_tensor_ref, po_types, binary_tensors, use_LOWOHA, algo);
...
TEST_P(TestReorder, F32) {
    ...
    bool is_test_successful = (reorder_status == status_t::success
            && unreorder_status == status_t::success);
...
TEST_P(TestReorder, S8) {
    if (use_LOWOHA) { GTEST_SKIP(); }
    auto weights
            = tensor_factory.uniform_dist_tensor({k, n}, data_type_t::s8, 2.0f);
...
TEST_P(TestReorder, F32_F32_Stride) {
    ...
    auto weights = tensor_factory.uniform_dist_strided_tensor(
            {k, n}, stride_wt, data_type_t::f32, 1.0f, transB);
    auto input_tensor = tensor_factory.uniform_dist_strided_tensor(
            {m, k}, stride_in, data_type_t::f32, 1.0f, transA);
    auto bias_tensor = tensor_factory.uniform_dist_tensor(
            {1, n}, data_type_t::f32, 1.0f);
    auto binary_tensors = make_binary_postop_tensors(
            tensor_factory, po_types, {m, n}, data_type_t::f32, 1.0f);
...
TEST_P(TestReorder, BF16_F32_Stride) {
    ...
    auto weights = tensor_factory.uniform_dist_strided_tensor(
            {k, n}, stride_wt, data_type_t::bf16, 1.0f, transB);
    auto input_tensor = tensor_factory.uniform_dist_strided_tensor(
            {m, k}, stride_in, data_type_t::bf16, 1.0f, transA);
    auto bias_tensor = tensor_factory.uniform_dist_tensor({1, n},
            rand() % 2 == 0 ? data_type_t::bf16 : data_type_t::f32, 1.0f);
    auto binary_tensors = make_binary_postop_tensors(
            tensor_factory, po_types, {m, n}, data_type_t::f32, 1.0f);
 ...
 *
 *  This is the single instantiation for the entire reorder suite.  All
 *  reorder TEST_P bodies (across test_reorder_regular.cpp,
 *  test_static_quant_dequant.cpp, test_dynamic_quant.cpp,
 *  test_strided_cases.cpp, and test_conversion.cpp) share the one
 *  `TestReorder` fixture, so exactly one INSTANTIATE_TEST_SUITE_P is needed.
 */
INSTANTIATE_TEST_SUITE_P(
        Reorder, TestReorder, ::testing::ValuesIn(reorder_test));
