...
ReorderType::ReorderType(const ReorderInput &reorder_input, uint32_t test_index,
    ...
    if (!is_lowoha_test) {
        ...
        src_dtype = reorder_input.matmul_input.src_dtype
                ? *reorder_input.matmul_input.src_dtype
                : data_type_t::f32;
        dst_dtype = reorder_input.matmul_input.dst_dtype
                ? *reorder_input.matmul_input.dst_dtype
                : data_type_t::s8;
        ...
        if (reorder_input.matmul_input.weight_granularity) {
            ...
            if (granularity == quant_granularity_t::group) {
                ...
                if (!group_ok) {
                    if (batch == 0) {
                        log_info(
                                "LOWOHA: per-group quantization requires "
                                "batch>0 (2D/3D); "
            ...
            if (batch == 0) {
                granularity = (granularity_choice < 7)
                ...
                if (granularity_choice < 6) {
                    ...
                    if (valid_groups.empty()) {
                        ...
                        num_groups = valid_groups[std::rand()
                                % valid_groups.size()];
                    }
                }
            }
...
std::string lowoha_granularity_to_str(quant_granularity_t granularity) {
    switch (granularity) {
        case quant_granularity_t::tensor: return "per-tensor";
        case quant_granularity_t::channel: return "per-channel";
        case quant_granularity_t::group: return "per-group";
...
std::pair<tensor_t, status_t> reorder_kernel_test(tensor_t &input_tensor,
    ...
    try {
        ...
        reorder_context_t reorder_context
                = reorder_context_t().set_algo_format("aocl");

        // Set Input source dtype if the weights dtype is s8
        if ((dtype == data_type_t::s8)
        ...
        bool memory_reorder = (!(input_tensor.get_layout()
                                       | uint16_t(tensor_layout_t::contiguous))
                || (input_tensor.get_layout()
                        & uint16_t(tensor_layout_t::aligned)));
        bool memory_unreorder = (input_tensor.get_layout()
        ...
        if (memory_reorder) {
            ...
            if (reorder_operator.is_bad_object()) {
                log_error("operator ", reorder_operator.get_name(),
                        " creation failed.");
            ...
            if (reorder_operator.get_reorder_isa_status()
            ...
            if (inplace_reorder) {
                ...
                if (reorder_size != input_buffer_size) {
                    ...
                    output_tensor = tensor_factory.copy_tensor(
                            {rows, cols}, dtype, buffer_params, trans, true);
                ...
                *weights = zendnnl_aligned_alloc(alignment, reorder_size);
                ...
                StorageParam buffer_params
                        = std::make_pair(reorder_size, *weights);
            ...
            if (status != status_t::success) {
                log_info("operator ", reorder_operator.get_name(),
                        " execution failed.");
                return std::make_pair(tensor_t(), status_t::failure);
            } else {
                log_info("operator ", reorder_operator.get_name(),
            ...
            if (inplace_reorder) {
                ...
                output_tensor = tensor_factory.copy_tensor(
                        {rows, cols}, dtype, buffer_params, trans, false);
                output_tensor.set_name("reorder_output");
                ...
                if (reorder_operator.get_reorder_isa_status()
                    ...
                    return std::make_pair(
                            tensor_t(), status_t::isa_unsupported);
                }
...
status_t lowoha_reorder_kernel_test(tensor_t &src_tensor, tensor_t &dst_tensor,
        tensor_t &scale_tensor, tensor_t &zp_tensor, const ReorderType &params,
        bool dynamic_quant) {
    try {
        ...
        if (params.use_strided_src) {
            ...
            reorder_params.src_strides.assign(
                    src_strides.begin(), src_strides.end());
        }

        // Set quantization parameters
        if (scale_tensor.get_nelem() > 0) {
            reorder_params.quant_params.scale.buff
            ...
            reorder_params.quant_params.scale.dims.assign(
                    scale_size.begin(), scale_size.end());
        }

        if (zp_tensor.get_nelem() > 0) {
            reorder_params.quant_params.zero_point.buff
                    = zp_tensor.get_raw_handle_unsafe();
            reorder_params.quant_params.zero_point.dt
                    = zp_tensor.get_data_type();
            auto zp_size = zp_tensor.get_size();
            reorder_params.quant_params.zero_point.dims.assign(
...
void compare_lowoha_reorder_output(tensor_t &output_tensor,
        tensor_t &ref_tensor, const ReorderType &params,
...
void log_lowoha_test_info(const ReorderType &params, data_type_t src_dt,
    ...
    log_info("LOWOHA Reorder: batch=", params.batch, " M=", params.M,
            " N=", params.N, " src=", src_dt_str, " dst=", dst_dt_str,
...
std::vector<size_t> get_lowoha_shape(const ReorderType &params) {
    if (params.batch == 0)
        ...
        return {params.batch, params.M, params.N};
}
...
std::vector<size_t> get_lowoha_strided_shape(const ReorderType &params) {
    ...
    if (batch == 0) {
        // 1D: [N]
        log_info("Strided 1D, padding=", padding, ", strided_shape={",
                N + padding, "}");
        return {N + padding};
    } else if (batch == 1) {
        // 2D: [M, N] -> strides {N+padding, 1}
        log_info("Strided 2D, padding=", padding, ", strided_shape={", M, ",",
                N + padding, "}", " strides={", N + padding, ",1}");
        return {M, N + padding};
    } else {
        // 3D: [batch, M, N] -> strides {M*(N+padding), N+padding, 1}
        log_info("Strided 3D, padding=", padding, ", strided_shape={", batch,
                ",", M, ",", N + padding, "}", " strides={", M * (N + padding),
                ",", N + padding, ",1}");
...
std::vector<size_t> get_lowoha_quant_shape(const ReorderType &params) {
    ...
    if (granularity == quant_granularity_t::tensor) {
        ...
        if (use_col_variant && N % num_groups == 0) {
            ...
            if (use_col_variant) {
                log_info(
                        "Granularity: per-group-col fallback to per-group-row "
                        "(num_groups=",
                        num_groups, " does not divide N=", N, ")");
            }
            if (batch == 1) {
...
void compare_lowoha_quant_output(tensor_t &original_tensor,
        tensor_t &dequant_tensor, tensor_t &scale_tensor,
        const ReorderType &params, bool &is_comparison_successful) {
