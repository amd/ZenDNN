...
postop_cache_tols_t tols_for_dst(data_type_t dst_dt) {
    switch (dst_dt) {
        ...
        case data_type_t::bf16: return {rtol_bf16, epsilon_bf16, false};
        ...
        default: return {rtol_f32, epsilon_f32, false};
    }
}
...
void PrintTo(const PostopCacheParam &p, ::std::ostream *os) {
    *os << dtype_token(p.src_dt) << "_" << dtype_token(p.dst_dt) << "_m" << p.m
        << "_k" << p.k << "_n" << p.n << "_tA" << static_cast<int>(p.transA)
        << "_tB" << static_cast<int>(p.transB) << "_algo"
...
std::vector<PostopCacheParam> make_cache_params(
...
protected:
    void SetUp() override {
        ...
        log_info("PostopCache: src_dt=", dtype_token(src_dt),
                " dst_dt=", dtype_token(dst_dt), " m=", m, " k=", k, " n=", n,
    ...
    bool is_fp_path() const {
        return (src_dt == data_type_t::f32 || src_dt == data_type_t::bf16
    ...
    void run_binary_refresh_on_hit(post_op_type_t bin_po) {
        ...
        auto weight_tensor = tensor_factory.uniform_dist_tensor(
                {k, n}, src_dt, 2.0, transB);
        auto input_tensor = tensor_factory.uniform_dist_tensor(
                {m, k}, src_dt, 2.0, transA);
        auto bias_tensor
                = tensor_factory.uniform_dist_tensor({1, n}, dst_dt, 2.0);

        auto binary_1 = make_binary_postop_tensors(
                tensor_factory, po, {m, n}, dst_dt, 2.0);
        auto binary_2 = make_binary_postop_tensors(
                tensor_factory, po, {m, n}, dst_dt, 5.0);

        auto out_1 = tensor_factory.uniform_dist_tensor({m, n}, dst_dt, 2.0);
        auto ref_1 = tensor_factory.uniform_dist_tensor({m, n}, dst_dt, 2.0);
        status_t s1 = matmul_kernel_test(input_tensor, weight_tensor,
                bias_tensor, out_1, po, binary_1, true, algo, alpha, beta);
...
TEST_P(TestPostopCache, HitParity) {
    if (!is_fp_path()) { GTEST_SKIP() << "FP test (INT8 covered separately)"; }
    ...
    const std::vector<post_op_type_t> po = drop_postops_and_bias
            ...
            : std::vector<post_op_type_t> {post_op_type_t::relu};

    auto weight_tensor
            = tensor_factory.uniform_dist_tensor({k, n}, src_dt, 2.0, transB);
    auto input_tensor
            = tensor_factory.uniform_dist_tensor({m, k}, src_dt, 2.0, transA);
    auto bias_tensor = drop_postops_and_bias
            ? tensor_t()
            : tensor_factory.uniform_dist_tensor({1, n}, dst_dt, 2.0);
    auto binary_tensors
            = make_binary_postop_tensors(tensor_factory, po, {m, n}, dst_dt);
...
TEST_P(TestPostopCache, BinaryMulBcastRefreshOnHit) {
    ...
    const std::vector<post_op_type_t> po = {post_op_type_t::binary_mul,
            post_op_type_t::gelu_erf, post_op_type_t::mish};

    auto weight_tensor
            = tensor_factory.uniform_dist_tensor({k, n}, src_dt, 2.0, transB);
    auto input_tensor
            = tensor_factory.uniform_dist_tensor({m, k}, src_dt, 2.0, transA);
    auto bias_tensor = tensor_factory.uniform_dist_tensor({1, n}, dst_dt, 2.0);

    auto binary_1 = make_binary_postop_tensors(
            tensor_factory, po, {1, n}, dst_dt, 2.0);
    auto binary_2 = make_binary_postop_tensors(
            tensor_factory, po, {1, n}, dst_dt, 5.0);

    auto out_1 = tensor_factory.uniform_dist_tensor({m, n}, dst_dt, 2.0);
    auto ref_1 = tensor_factory.uniform_dist_tensor({m, n}, dst_dt, 2.0);
    status_t s1 = matmul_kernel_test(input_tensor, weight_tensor, bias_tensor,
            out_1, po, binary_1, true, algo, alpha, beta);
    if (s1 == status_t::isa_unsupported) {
        GTEST_SKIP() << dtype_token(src_dt) << " not supported on this ISA";
    }
    ASSERT_EQ(s1, status_t::success);
...
TEST_P(TestPostopCache, BiasDtypeKeysDistinct) {
    ...
    auto bias_bf16 = tensor_factory.uniform_dist_tensor(
            {1, n}, data_type_t::bf16, 2.0);
    auto bias_f32
    ...
    status_t s1 = matmul_kernel_test(input_tensor, weight_tensor, bias_bf16,
            out_1, po, no_binaries, true, algo, alpha, beta);
    if (s1 == status_t::isa_unsupported) {
        GTEST_SKIP() << dtype_token(src_dt) << " not supported on this ISA";
    }
    ASSERT_EQ(s1, status_t::success);
...
TEST_P(TestPostopCache, PostopOrderKeysDistinct) {
    ...
    const std::vector<post_op_type_t> chain_1
            = {post_op_type_t::relu, post_op_type_t::gelu_tanh};
    const std::vector<post_op_type_t> chain_2
            = {post_op_type_t::gelu_tanh, post_op_type_t::relu};
    ...
    auto ref_1 = tensor_factory.uniform_dist_tensor({m, n}, dst_dt, 2.0);
    status_t s1 = matmul_kernel_test(input_tensor, weight_tensor, bias_tensor,
            out_1, chain_1, no_binaries, true, algo, alpha, beta);
    if (s1 == status_t::isa_unsupported) {
        GTEST_SKIP() << dtype_token(src_dt) << " not supported on this ISA";
    }
    ASSERT_EQ(s1, status_t::success);
...
TEST_P(TestPostopCache, LifecycleClear) {
    ...
    auto ref_1 = tensor_factory.uniform_dist_tensor({m, n}, dst_dt, 2.0);
    status_t s1 = matmul_kernel_test(input_tensor, weight_tensor, bias_tensor,
            out_1, po, binary_tensors, true, algo, alpha, beta);
    if (s1 == status_t::isa_unsupported) {
        GTEST_SKIP() << dtype_token(src_dt) << " not supported on this ISA";
    }
    ASSERT_EQ(s1, status_t::success);
...
TEST_P(TestPostopCache, SymQuantPerTokenScaleRefreshOnHit) {
    ...
    const data_type_t ref_dt = (dst_dt == data_type_t::f32) ? data_type_t::f32
                                                            : data_type_t::bf16;
    ...
    auto wei_ref = tensor_factory.uniform_dist_tensor(
            {sym_k, n}, ref_dt, 25.0, transB);
    tensor_t weight_tensor, wei_scale, wei_zp;
    ASSERT_EQ(quant_params_compute(tensor_factory, wei_ref, ref_dt,
                      data_type_t::s8, wei_sd, scale_dt, wei_scale, wei_zp,
                      &weight_tensor),
            status_t::success)
            << "weight quantization failed";

    auto bias_tensor = tensor_factory.uniform_dist_tensor({1, n}, ref_dt, 2.0);
    std::vector<tensor_t> no_binaries;

    // Source 1: independently quantized => fresh src_scale_1 buffer.
    auto src_ref_1 = tensor_factory.uniform_dist_tensor(
            {m, sym_k}, ref_dt, 25.0, transA);
    tensor_t input_1, src_scale_1, src_zp_1;
    ASSERT_EQ(quant_params_compute(tensor_factory, src_ref_1, ref_dt,
                      data_type_t::s8, src_sd, scale_dt, src_scale_1, src_zp_1,
                      &input_1),
            status_t::success)
            << "source 1 quantization failed";

    // Source 2: different value range + independently quantized.
    auto src_ref_2 = tensor_factory.uniform_dist_tensor(
            {m, sym_k}, ref_dt, 15.0, transA);
    tensor_t input_2, src_scale_2, src_zp_2;
    ASSERT_EQ(quant_params_compute(tensor_factory, src_ref_2, ref_dt,
                      data_type_t::s8, src_sd, scale_dt, src_scale_2, src_zp_2,
                      &input_2),
            status_t::success)
            << "source 2 quantization failed";
...
TEST_P(TestPostopCache, DynQuantScaleRefreshOnHit) {
    ...
    const data_type_t test_dt = (dst_dt == data_type_t::f32)
            ...
            : data_type_t::bf16;
    const data_type_t scale_dt = data_type_t::f32;
    const std::vector<int64_t> wei_scale_dims = {1, static_cast<int64_t>(n)};
    ...
    auto input_1 = tensor_factory.uniform_dist_tensor(
            {m, sym_k}, test_dt, 2.0, transA, src_scale_1, tensor_t());
    auto input_1_ref = tensor_factory.uniform_dist_tensor(
            {m, sym_k}, test_dt, 2.0, transA);

    auto src_scale_2 = tensor_factory.zero_tensor(src_scale_shape, scale_dt);
    auto input_2 = tensor_factory.uniform_dist_tensor(
            {m, sym_k}, test_dt, 5.0, transA, src_scale_2, tensor_t());
    auto input_2_ref = tensor_factory.uniform_dist_tensor(
            {m, sym_k}, test_dt, 5.0, transA);
...
INSTANTIATE_TEST_SUITE_P(F32_F32, TestPostopCache,
        ::testing::ValuesIn(
                make_cache_params(data_type_t::f32, data_type_t::f32)));

INSTANTIATE_TEST_SUITE_P(BF16_BF16, TestPostopCache,
        ::testing::ValuesIn(
                make_cache_params(data_type_t::bf16, data_type_t::bf16)));

INSTANTIATE_TEST_SUITE_P(F16_F16, TestPostopCache,
        ::testing::ValuesIn(
                make_cache_params(data_type_t::f16, data_type_t::f16)));
...
INSTANTIATE_TEST_SUITE_P(INT8_BF16, TestPostopCache,
        ::testing::ValuesIn(
                make_cache_params(data_type_t::s8, data_type_t::bf16)));

INSTANTIATE_TEST_SUITE_P(INT8_F32, TestPostopCache,
        ::testing::ValuesIn(
                make_cache_params(data_type_t::s8, data_type_t::f32)));
...
TEST_F(TestPostopCacheZpCompNegative, NullAccCollapsesToNoZpCompKey) {
    ...
    dlp_metadata_t *md_zero
            = create_dlp_post_op(params, /*bias=*/nullptr, dtypes, N, K, M,
                    /*zp_comp_acc=*/nullptr,
                    /*zp_comp_ndim=*/0, algo, weight_ptr);
    ASSERT_NE(md_zero, nullptr)
            << "INT8 path always builds metadata even with zero ops";

    dlp_metadata_t *md_bad_1d
            = create_dlp_post_op(params, /*bias=*/nullptr, dtypes, N, K, M,
                    /*zp_comp_acc=*/nullptr,
                    /*zp_comp_ndim=*/1, algo, weight_ptr);
    ...
    dlp_metadata_t *md_bad_2d
            = create_dlp_post_op(params, /*bias=*/nullptr, dtypes, N, K, M,
                    /*zp_comp_acc=*/nullptr,
                    /*zp_comp_ndim=*/2, algo, weight_ptr);
...
TEST_F(TestPostopCacheZpCompNegative, NullAccDistinctFromRealAccKey) {
    ...
    ASSERT_EQ(md_real_2d->seq_length, 1)
            << "Real-pair 2D zp_comp call must wire exactly one MATRIX_ADD seq "
               "op";
...
TEST_F(TestPostopCacheEnableFlag, EnvSpellingsThatDisable) {
    for (const char *v :
            {"0", "false", "off", "no", "FALSE", "Off", "No", "fAlSe"}) {
...
TEST_F(TestPostopCacheEnableFlag, EnvSpellingsThatEnable) {
    for (const char *v :
            {"1", "true", "on", "yes", "TRUE", "On", "Yes", "tRuE"}) {
...
TEST_F(TestPostopCacheEnableFlag, GarbageLeavesDefault) {
    for (const char *v :
            {"ture", "enabled", "disabled", "2", "-1", "  ", "1.0", "yesno"}) {
        EXPECT_TRUE(enable_for(v)) << "Unrecognized env var spelling '" << v
                                   << "' must leave default (true) untouched, "
...
TEST_F(TestPostopCacheClearMechanism, ClearProducesFreshHolderForSameKey) {
    ...
    dlp_metadata_t *md_first = create_dlp_post_op(params, /*bias=*/nullptr,
            params.dtypes, N, K, M, /*zp_comp_acc=*/nullptr,
            /*zp_comp_ndim=*/0, algo, weight_ptr);
    ...
    dlp_metadata_t *md_hit = create_dlp_post_op(params, /*bias=*/nullptr,
            params.dtypes, N, K, M, /*zp_comp_acc=*/nullptr,
            /*zp_comp_ndim=*/0, algo, weight_ptr);
    ...
    dlp_metadata_t *md_after_clear = create_dlp_post_op(params,
            /*bias=*/nullptr, params.dtypes, N, K, M, /*zp_comp_acc=*/nullptr,
            /*zp_comp_ndim=*/0, algo, weight_ptr);
    ...
    dlp_metadata_t *md_hit_after_clear = create_dlp_post_op(params,
            /*bias=*/nullptr, params.dtypes, N, K, M, /*zp_comp_acc=*/nullptr,
            /*zp_comp_ndim=*/0, algo, weight_ptr);
