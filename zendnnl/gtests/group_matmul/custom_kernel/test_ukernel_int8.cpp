...
void scalar_ref_dq_int8(int M, int K, int N, const SrcByte *src, int lda,
        const int8_t *wei, int ldb, const float *src_scale,
        const int32_t *src_zp, // nullptr → sym
        const float *wei_scale, const void *bias, ck::BiasKind bias_kind,
        ck::ActKind act, bfloat16_t *dst, int ldc) {
    ...
    for (int m = 0; m < M; ++m) {
        ...
        const int32_t K_m = (src_zp != nullptr) ? src_zp[m] : 128;
        for (int v = 0; v < N; ++v) {
            int64_t acc = 0;
            int32_t sum_wei = 0;
            for (int k = 0; k < K; ++k) {
...
void apply_gated_ref(int M, int N, ck::ActKind act, const bfloat16_t *mm,
        int ldmm, bfloat16_t *dst, int ldc) {
    ...
    for (int NV : {2, 4}) {
        ...
        for (int MR = 1; MR <= max_mr; ++MR) {
            for (auto C : {ck::IntCompute::kS8_Sym, ck::IntCompute::kU8_Asym}) {
                for (auto act : {ck::ActKind::none, ck::ActKind::swiglu_oai_mul,
                             ck::ActKind::silu_and_mul,
                             ck::ActKind::gelu_and_mul}) {
                    auto *fn = ck::select_int8_ukernel(
                            MR, NV, C, act, ck::DstDt::kBf16);
                    EXPECT_NE(fn, nullptr) << "select_int8_ukernel returned "
                                              "null for valid tuple "
                                              "MR="
                                           << MR << " NV=" << NV
    ...
    for (int NV : {2, 4}) {
        ...
        for (int MR = 1; MR <= max_mr; ++MR) {
            for (auto C : {ck::IntCompute::kS8_Sym, ck::IntCompute::kU8_Asym}) {
                ...
                for (auto act :
                        {ck::ActKind::swiglu_oai_mul, ck::ActKind::silu_and_mul,
                                ck::ActKind::gelu_and_mul}) {
                    EXPECT_EQ(ck::select_int8_ukernel(
                                      MR, NV, C, act, ck::DstDt::kF32),
                            nullptr)
...
TEST(CkInt8UkernelSelect, NullForOutOfRangeMR) {
    ...
    for (int NV : {2, 4}) {
        const int max_mr = ck::max_mr_for_nv(NV);
        auto *fn = ck::select_int8_ukernel(max_mr + 1, NV,
                ck::IntCompute::kS8_Sym, ck::ActKind::none, ck::DstDt::kBf16);
        EXPECT_EQ(fn, nullptr)
                << "select_int8_ukernel must return null for MR > "
                   "max_mr_for_nv(NV)"
                << " (NV=" << NV << " MR=" << (max_mr + 1) << ")";
    }
}
...
TEST_P(CkInt8UkernelTest, MatchesScalarReference) {
    ...
    std::mt19937 rng(
            0xC0DECAFEu ^ static_cast<unsigned>(MR * 137 + NV * 11 + K));
    std::uniform_int_distribution<int> wd(
            -64, 63); // s8 wei in a moderate range
    ...
    ASSERT_EQ(
            ck::get_or_pack_weight_int8(wei.data(), K, NR, /*ldb=*/NR, pack_nr,
                    /*transB=*/false,
    ...
    if (c.bias == ck::BiasKind::bf16) {
        for (int v = 0; v < NR; ++v)
            bias_bf16[v] = bfloat16_t(0.02f * v);
        ...
        for (int v = 0; v < NR; ++v)
            bias_f16[v] = mt::float16_t(0.02f * v);
    ...
    std::vector<bfloat16_t> dst(
            static_cast<size_t>(MR) * out_cols, bfloat16_t(0.0f));

    // Resolve the kernel function pointer for this case (BF16 dst).
    auto *fn = ck::select_int8_ukernel(
            MR, NV, c.compute, c.act, ck::DstDt::kBf16);
    ...
    if (c.act == ck::ActKind::none) {
        ...
        apply_gated_ref(MR, NR, c.act, ref_mm.data(), NR, ref.data(), out_cols);
    }
    ...
    for (size_t i = 0; i < ref.size(); ++i) {
        ...
        if (diff > band) {
            if (n_bad++ < 8) {
                ADD_FAILURE() << "Mismatch at i=" << i << " kernel=" << a
                              << " ref=" << b << " diff=" << diff
                              << " band=" << band << " (" << c.label << ")";
            }
        }
    }
    EXPECT_EQ(n_bad, 0) << "Total mismatches: " << n_bad << " (" << c.label
                        << ")";
}
...
TEST(CkInt8UkernelBf16Scale, MatchesScalarReference) {
    ...
    const int8_t *packed_raw = nullptr;
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, NR, /*ldb=*/NR,
                      pack_nr, /*transB=*/false,
                      /*interleave_split_halves=*/false, &packed_raw, nullptr,
                      /*disable_cache=*/true),
            status_t::success);
    ASSERT_NE(packed_raw, nullptr);
    ...
    scalar_ref_dq_int8<uint8_t>(MR, K, NR, src.data(), K, wei.data(), NR,
            src_scale_ref.data(), /*src_zp=*/nullptr, wei_scale_ref.data(),
            /*bias=*/nullptr, ck::BiasKind::none, ck::ActKind::none, ref.data(),
            NR);
...
TEST(CkInt8UkernelExtremes, ZeroAndMaxValues) {
    ...
    for (auto compute : {ck::IntCompute::kS8_Sym, ck::IntCompute::kU8_Asym}) {
        for (int kind = 0; kind < 3; ++kind) {
            ...
            const int8_t wei_v = (kind == 0) ? 0
                    : (kind == 1) ? std::numeric_limits<int8_t>::max()
                                  : std::numeric_limits<int8_t>::min();
            const uint8_t src_v_sym = (kind == 0) ? 0
                    : (kind == 1)
                    ? static_cast<uint8_t>(std::numeric_limits<int8_t>::max())
                    : static_cast<uint8_t>(std::numeric_limits<int8_t>::min());
            const uint8_t src_v_asym = (kind == 0) ? 0u
                    : (kind == 1) ? std::numeric_limits<uint8_t>::max()
                                  : 0u;
            const uint8_t src_v = (compute == ck::IntCompute::kS8_Sym)
                    ? src_v_sym
                    : src_v_asym;
            ...
            const int32_t *src_zp_ptr = (compute == ck::IntCompute::kU8_Asym)
                    ? src_zp.data()
                    : nullptr;

            const int8_t *packed = nullptr;
            ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, NR, NR, NR,
                              /*transB=*/false,
                              /*interleave_split_halves=*/false, &packed,
                              nullptr, /*disable_cache=*/true),
                    status_t::success);
            ASSERT_NE(packed, nullptr);
            auto *fn = ck::select_int8_ukernel(
                    MR, NV, compute, ck::ActKind::none, ck::DstDt::kBf16);
            ASSERT_NE(fn, nullptr);
            std::vector<bfloat16_t> dst(
                    static_cast<size_t>(MR) * NR, bfloat16_t(0.0f));
            fn(src.data(), K, packed, src_scale.data(), src_zp_ptr,
                    wei_scale.data(), ck::ScaleKind::kF32,
                    /*bias=*/nullptr, ck::BiasKind::none,
                    /*Cout=*/dst.data(), /*ldc=*/NR,
                    /*Cout_tight=*/nullptr, /*ldc_tight=*/0,
                    /*K=*/K);
...
TEST(CkInt8UkernelExtremes, NegativeScales) {
    ...
    std::vector<uint8_t> src(static_cast<size_t>(MR) * K);
    for (auto &b : src)
        b = static_cast<uint8_t>(static_cast<int8_t>(sd(rng)));
    std::vector<float> src_scale(MR), wei_scale(NR);
    for (int m = 0; m < MR; ++m)
        src_scale[m] = -0.003f; // negative
    for (int v = 0; v < NR; ++v)
        wei_scale[v] = (v & 1) ? -0.004f : 0.004f;
    ...
    ASSERT_NE(fn, nullptr);
    std::vector<bfloat16_t> dst(static_cast<size_t>(MR) * NR, bfloat16_t(0.0f));
    fn(src.data(), K, packed, src_scale.data(), /*src_zp=*/nullptr,
            wei_scale.data(), ck::ScaleKind::kF32,
            /*bias=*/nullptr, ck::BiasKind::none, dst.data(), NR, nullptr, 0,
            K);
    std::vector<bfloat16_t> ref(static_cast<size_t>(MR) * NR, bfloat16_t(0.0f));
    scalar_ref_dq_int8<uint8_t>(MR, K, NR, src.data(), K, wei.data(), NR,
            src_scale.data(), nullptr, wei_scale.data(),
            /*bias=*/nullptr, ck::BiasKind::none, ck::ActKind::none, ref.data(),
            NR);
    int n_bad = 0;
    for (size_t i = 0; i < ref.size(); ++i) {
        ...
        if (std::fabs(a - b) > 0.05f + 0.05f * std::fabs(b)) {
            if (n_bad++ < 4) {
                ADD_FAILURE() << "neg-scale mismatch i=" << i << " kernel=" << a
                              << " ref=" << b;
            }
        }
    }
...
TEST(CkInt8UkernelF32Dst, MatchesScalarF32Reference) {
    ...
    for (const auto &c : cases) {
        ...
        std::mt19937 rng(
                0xF32D57u ^ static_cast<unsigned>(MR * 131 + NV * 7 + K));
        std::uniform_int_distribution<int> wd(-64, 63);
        std::uniform_int_distribution<int> sd_s8(-64, 63);
        std::uniform_int_distribution<int> sd_u8(0, 127);
        std::uniform_int_distribution<int> zpd(0, 96);
        ...
        struct Guard {
            const int8_t *p;
            ~Guard() { ck::free_owned_packed_weight_int8(p); }
        } pg {packed};
        ...
        auto *fn = ck::select_int8_ukernel(
                MR, NV, c.compute, ck::ActKind::none, ck::DstDt::kF32);
        ...
        for (int m = 0; m < MR && n_bad == 0; ++m) {
            ...
            for (int v = 0; v < NR; ++v) {
                ...
                const int32_t corrected = static_cast<int32_t>(
                        acc - static_cast<int64_t>(K_m) * sum_wei);
