...
TEST(CkInt8Pack, LayoutAndCompensation) {
    ...
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, /*ldb=*/N, pack_nr,
                      /*transB=*/false,
    ...
    for (int o = 0; o < num_oblks; ++o) {
        const std::byte *oblk_base = reinterpret_cast<const std::byte *>(packed)
        ...
        for (int kk : k_samples) {
            for (int vv : v_samples) {
                ...
                EXPECT_EQ(got, exp)
                        << "Pack byte mismatch at o=" << o << " k=" << kk
                        << " v=" << vv << " (off=" << off << ")";
            }
        }
    ...
    for (int o = 0; o < num_oblks; ++o) {
        ...
        for (int kk : {0, K / 2, K - 1}) {
            for (int vv : {0, pack_nr / 2, pack_nr - 1}) {
                const size_t off
                        = (static_cast<size_t>(kk / 4) * pack_nr + vv) * 4
                        + (kk % 4);
        ...
        for (int vv = 0; vv < pack_nr; ++vv) {
            ...
            for (int k = 0; k < K; ++k)
                expected += wei[k * N + v_full];
            EXPECT_EQ(comp[vv], expected)
                    << "pack_nr=64 compensation mismatch o=" << o
                    << " v=" << vv;
        }
    }
}
...
TEST(CkInt8Pack, DisableCacheReturnsFreshBuffer) {
    ...
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, N, pack_nr, false,
                      false, &p1, nullptr, /*disable_cache=*/true),
            status_t::success);
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, N, pack_nr, false,
                      false, &p2, nullptr, /*disable_cache=*/true),
            status_t::success);
    ASSERT_NE(p1, nullptr);
    ASSERT_NE(p2, nullptr);
    EXPECT_NE(p1, p2) << "disable_cache must allocate a fresh buffer per call";
...
TEST(CkInt8Pack, ClearInt8CacheDoesNotEvictBf16) {
    ...
    constexpr int K = 32, N = 32, pack_nr = 32;
    std::vector<bfloat16_t> wei_bf16(
            static_cast<size_t>(K) * N, bfloat16_t(0.0f));
    ...
    ASSERT_EQ(ck::get_or_pack_weight_bf16(wei_bf16.data(), K, N, N, pack_nr,
                      false, false, &pb, &bhit),
            status_t::success);
    EXPECT_FALSE(bhit);
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei_s8.data(), K, N, N, pack_nr,
                      false, false, &pi, &ihit),
            status_t::success);
    EXPECT_FALSE(ihit);
    ...
    bhit = false;
    ASSERT_EQ(ck::get_or_pack_weight_bf16(wei_bf16.data(), K, N, N, pack_nr,
                      false, false, &pb2, &bhit),
            status_t::success);
    EXPECT_TRUE(bhit) << "bf16 cache entry must survive an int8 cache clear";
    EXPECT_EQ(pb, pb2);
...
TEST(CkInt8Pack, KNotDivisibleByFourPadsToQuad) {
    ...
    for (int kk = K; kk < K_pad; ++kk) {
        const int kq = kk / 4;
        const int q_in = kk % 4;
        const size_t off = (static_cast<size_t>(kq) * pack_nr + 0) * 4 + q_in;
    ...
    const size_t weight_bytes_per_oblock
            = static_cast<size_t>(K_quad) * pack_nr * 4;
    const int32_t *comp = reinterpret_cast<const int32_t *>(
            packed + weight_bytes_per_oblock);
    for (int vv = 0; vv < pack_nr; ++vv) {
        int32_t expected = 0;
        for (int k = 0; k < K; ++k)
            expected += wei[k * N + vv];
...
TEST(CkInt8Pack, RejectsBadShape) {
    ...
    EXPECT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, /*N=*/33, /*ldb=*/64,
                      /*pack_nr=*/32, false, false, &p, nullptr, true),
            status_t::failure);
    // (b) pack_nr out of {32, 64}.
    EXPECT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, /*N=*/64, /*ldb=*/64,
                      /*pack_nr=*/16, false, false, &p, nullptr, true),
            status_t::failure);
    EXPECT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, /*N=*/64, /*ldb=*/64,
                      /*pack_nr=*/48, false, false, &p, nullptr, true),
            status_t::failure);
    // (c) ldb below minimum row stride for transB=false (= N).
    EXPECT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, /*N=*/64, /*ldb=*/32,
                      /*pack_nr=*/32,
...
TEST(CkInt8Pack, CacheKeyIncludesPackNR) {
    ...
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, N, /*pack_nr=*/32,
                      false, false, &p32, &h32, /*disable_cache=*/false),
            status_t::success);
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, N, /*pack_nr=*/64,
                      false, false, &p64, &h64, /*disable_cache=*/false),
            status_t::success);
    ...
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, N, 32, false, false,
                      &p32b, &h32b, false),
            status_t::success);
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, N, 64, false, false,
                      &p64b, &h64b, false),
            status_t::success);
    EXPECT_TRUE(h32b);
    EXPECT_TRUE(h64b);
...
TEST(CkInt8Pack, SiluGeluProduceIdenticalPacks) {
    ...
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, N, pack_nr, false,
                      /*interleave_split_halves=*/true, &p_silu, nullptr,
                      /*disable_cache=*/true),
            status_t::success);
    ASSERT_EQ(ck::get_or_pack_weight_int8(wei.data(), K, N, N, pack_nr, false,
                      /*interleave_split_halves=*/true, &p_gelu, nullptr,
                      /*disable_cache=*/true),
            status_t::success);
...
TEST(CkInt8Pack, SplitHalvesInterleaveMatchesSwigluLayout) {
    ...
    ASSERT_EQ(ck::get_or_pack_weight_int8(w_inter.data(), K, N, N, pack_nr,
                      /*transB=*/false,
    ...
    ASSERT_EQ(ck::get_or_pack_weight_int8(w_split.data(), K, N, N, pack_nr,
                      /*transB=*/false,
