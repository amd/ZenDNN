...
inline float make_value(
        size_t i, int e, int seed_step, int mod, int shift, float scale) {
    return scale
            * static_cast<float>(
...
struct TypedBuffers {
    ...
    void alloc(
            int num_ops, size_t elems, data_type_t fp, bool is_mixed = false) {
    ...
    float at(int e, size_t i, data_type_t fp) const {
        if (fp == data_type_t::bf16) { return static_cast<float>(bf16[e][i]); }
        if (fp == data_type_t::f16) { return static_cast<float>(f16[e][i]); }
        return f32[e][i];
    }
...
inline std::vector<matmul_params> make_uniform_params(
        int num_ops, data_type_t dt, data_type_t bias_dt = data_type_t::none) {
...
struct GemmVecs {
    ...
    // at all.
    static GemmVecs uniform(int num_ops, int M, int N, int K, float a = 1.0f,
            float b = 0.0f, bool wc = true, bool tA = false, bool tB = false) {
        GemmVecs v;
...
inline float ref_gated_act(
        grp_matmul_gated_act_t act, float g_or_even, float u_or_odd) {
    switch (act) {
...
template <typename T>
inline void apply_ref_gated_act(
        T *dst, int M, int N, int ldc, grp_matmul_gated_act_t act) {
...
inline void apply_ref_gated_act_tensor(
        tensor_t &t, int M, int N, int ldc, grp_matmul_gated_act_t act) {
    if (act == grp_matmul_gated_act_t::none) { return; }
    const data_type_t dtype = t.get_data_type();
    if (dtype == data_type_t::f32) {
        apply_ref_gated_act<float>(
                static_cast<float *>(t.get_raw_handle_unsafe()), M, N, ldc,
                act);
    } else if (dtype == data_type_t::bf16) {
        apply_ref_gated_act<bfloat16_t>(
                static_cast<bfloat16_t *>(t.get_raw_handle_unsafe()), M, N, ldc,
                act);
    } else if (dtype == data_type_t::f16) {
        apply_ref_gated_act<float16_t>(
                static_cast<float16_t *>(t.get_raw_handle_unsafe()), M, N, ldc,
                act);
    }
...
inline grp_matmul_gated_act_t pick_random_gated_act(
        uint64_t n, std::mt19937 &rng) {
    if (n % 2 != 0 || n < 2) { return grp_matmul_gated_act_t::none; }
...
struct CustomKernelOverride {
    ...
    ~CustomKernelOverride() {
        zendnnl::lowoha::matmul::test_api ::s_grp_matmul_custom_kernel_override
...
struct DecodeProportionalOverride {
    ...
    DecodeProportionalOverride &operator=(const DecodeProportionalOverride &)
            = delete;
    DecodeProportionalOverride(DecodeProportionalOverride &&) = delete;
    DecodeProportionalOverride &operator=(DecodeProportionalOverride &&)
            = delete;
};
...
struct CustomKernelInt8Override {
    ...
    CustomKernelInt8Override &operator=(const CustomKernelInt8Override &)
            = delete;
...
struct CustomKernelF16Override {
    ...
    CustomKernelF16Override &operator=(const CustomKernelF16Override &)
            = delete;
...
struct NRoundsModeOverride {
    ...
    explicit NRoundsModeOverride(int value) {
        prev = zendnnl::lowoha::matmul::test_api ::s_grp_n_rounds_mode_override
                       .exchange(value, std::memory_order_relaxed);
    }
    ~NRoundsModeOverride() {
        zendnnl::lowoha::matmul::test_api ::s_grp_n_rounds_mode_override.store(
                prev, std::memory_order_relaxed);
    }
...
struct NTileStrategyOverride {
    ...
    ~NTileStrategyOverride() {
        zendnnl::lowoha::matmul::test_api ::s_grp_n_tile_strategy_override
                .store(prev, std::memory_order_relaxed);
    }
...
struct NTileHeavyThresholdOverride {
    ...
    NTileHeavyThresholdOverride(const NTileHeavyThresholdOverride &) = delete;
    NTileHeavyThresholdOverride &operator=(const NTileHeavyThresholdOverride &)
            = delete;
    NTileHeavyThresholdOverride(NTileHeavyThresholdOverride &&) = delete;
    NTileHeavyThresholdOverride &operator=(NTileHeavyThresholdOverride &&)
            = delete;
};
...
struct MTileHybridOverride {
    ...
    ~MTileHybridOverride() {
        zendnnl::lowoha::matmul::test_api ::s_grp_matmul_m_tile_hybrid_override
                .store(prev, std::memory_order_relaxed);
    }
...
struct MTileSliceTargetOverride {
    ...
    explicit MTileSliceTargetOverride(int value) {
        prev = zendnnl::lowoha::matmul::test_api ::
                       s_grp_matmul_m_tile_slice_target_override.exchange(
                               value, std::memory_order_relaxed);
    }
    ~MTileSliceTargetOverride() {
        zendnnl::lowoha::matmul::test_api ::
                s_grp_matmul_m_tile_slice_target_override.store(
                        prev, std::memory_order_relaxed);
    }
    MTileSliceTargetOverride(const MTileSliceTargetOverride &) = delete;
    MTileSliceTargetOverride &operator=(const MTileSliceTargetOverride &)
            = delete;
...
struct MTileHybridMinMaxMOverride {
    ...
    explicit MTileHybridMinMaxMOverride(int value) {
        prev = zendnnl::lowoha::matmul::test_api ::
                       s_grp_matmul_m_tile_hybrid_min_max_m_override.exchange(
                               value, std::memory_order_relaxed);
    }
    ~MTileHybridMinMaxMOverride() {
        zendnnl::lowoha::matmul::test_api ::
                s_grp_matmul_m_tile_hybrid_min_max_m_override.store(
                        prev, std::memory_order_relaxed);
    }
    ...
    MTileHybridMinMaxMOverride &operator=(MTileHybridMinMaxMOverride &&)
            = delete;
};
...
struct MTileHybridMinSkewOverride {
    ...
    explicit MTileHybridMinSkewOverride(int value) {
        prev = zendnnl::lowoha::matmul::test_api ::
                       s_grp_matmul_m_tile_hybrid_min_skew_override.exchange(
                               value, std::memory_order_relaxed);
    }
    ~MTileHybridMinSkewOverride() {
        zendnnl::lowoha::matmul::test_api ::
                s_grp_matmul_m_tile_hybrid_min_skew_override.store(
                        prev, std::memory_order_relaxed);
    }
    ...
    MTileHybridMinSkewOverride &operator=(MTileHybridMinSkewOverride &&)
            = delete;
};
...
struct MTileHybridLightsPerThreadOverride {
    ...
    ~MTileHybridLightsPerThreadOverride() {
        zendnnl::lowoha::matmul::test_api ::
                s_grp_matmul_m_tile_hybrid_lights_per_thread_override.store(
                        prev, std::memory_order_relaxed);
    }
    MTileHybridLightsPerThreadOverride(
            const MTileHybridLightsPerThreadOverride &)
            = delete;
    MTileHybridLightsPerThreadOverride &operator=(
            const MTileHybridLightsPerThreadOverride &)
            = delete;
    MTileHybridLightsPerThreadOverride(MTileHybridLightsPerThreadOverride &&)
            = delete;
    MTileHybridLightsPerThreadOverride &operator=(
            MTileHybridLightsPerThreadOverride &&)
            = delete;
...
struct MoEVerticalFusionOverride {
    ...
    MoEVerticalFusionOverride &operator=(const MoEVerticalFusionOverride &)
            = delete;
...
struct MoEPipelineScratchKbOverride {
    ...
    explicit MoEPipelineScratchKbOverride(int value) {
        prev = zendnnl::lowoha::matmul::test_api ::
                       s_grp_matmul_m_tile_pipeline_scratch_kb_override
                               .exchange(value, std::memory_order_relaxed);
    }
    ...
    MoEPipelineScratchKbOverride(const MoEPipelineScratchKbOverride &) = delete;
    MoEPipelineScratchKbOverride &operator=(
            const MoEPipelineScratchKbOverride &)
            = delete;
    MoEPipelineScratchKbOverride(MoEPipelineScratchKbOverride &&) = delete;
    MoEPipelineScratchKbOverride &operator=(MoEPipelineScratchKbOverride &&)
            = delete;
};
...
struct CustomKernelSubtilePerExpertOverride {
    ...
    ~CustomKernelSubtilePerExpertOverride() {
        zendnnl::lowoha::matmul::test_api ::
                s_grp_matmul_custom_kernel_subtile_per_expert_override.store(
                        prev, std::memory_order_relaxed);
    }
    CustomKernelSubtilePerExpertOverride(
            const CustomKernelSubtilePerExpertOverride &)
            = delete;
...
struct CustomKernelNTileOverride {
    ...
    CustomKernelNTileOverride(const CustomKernelNTileOverride &) = delete;
    CustomKernelNTileOverride &operator=(const CustomKernelNTileOverride &)
            = delete;
    CustomKernelNTileOverride(CustomKernelNTileOverride &&) = delete;
    CustomKernelNTileOverride &operator=(CustomKernelNTileOverride &&) = delete;
};
...
struct LastInvocationCaptureGuard {
    ...
    LastInvocationCaptureGuard() {
        ...
        prev_capture = zendnnl::lowoha::matmul::group_matmul_prepack::
                               test_api ::s_capture_last_invocation.exchange(
                                       true, std::memory_order_release);
    }
    ~LastInvocationCaptureGuard() {
        zendnnl::lowoha::matmul::group_matmul_prepack::test_api ::
    ...
    LastInvocationCaptureGuard &operator=(const LastInvocationCaptureGuard &)
            = delete;
};
...
inline grp_matmul_fused_moe_params make_fused_moe_op2(int num_ops, int H,
...
inline void fill_moe_tensors(int num_ops, data_type_t fp, TypedBuffers *src,
        TypedBuffers *w1, TypedBuffers *w2) {
    for (int e = 0; e < num_ops; ++e) {
        if (fp == data_type_t::bf16) {
            ...
            if (src) { fill_src(src->f16[e], e); }
            if (w1) { fill_wei1(w1->f16[e], e); }
            if (w2) { fill_wei2(w2->f16[e], e); }
        } else {
...
inline void fill_moe_tensors(int num_ops, bool is_bf16, TypedBuffers *src,
        TypedBuffers *w1, TypedBuffers *w2) {
...
// delegates here.
inline status_t run_legacy_2call_ref(const std::vector<int> &Ms, int K_in,
        int N_gate_up, int K_down, int H, data_type_t dt,
        grp_matmul_gated_act_t act_type, const std::vector<const void *> &src,
        const std::vector<const void *> &wei1,
        const std::vector<const void *> &wei2,
        const std::vector<void *> &d1_ref, const std::vector<void *> &d2_ref) {
    ...
    auto pr1 = params;
    status_t s1 = group_matmul_direct(gv1.layout, gv1.transA, gv1.transB,
            gv1.Ms, gv1.Ns, gv1.Ks, gv1.alpha, src, gv1.lda, wei1, gv1.ldb,
            no_bias, gv1.beta, d1_ref, gv1.ldc, gv1.is_wc, pr1, nullptr,
            act_ptr);
    ...
    auto pr2 = params;
    return group_matmul_direct(gv2.layout, gv2.transA, gv2.transB, gv2.Ms,
            gv2.Ns, gv2.Ks, gv2.alpha, srcs2, gv2.lda, wei2, gv2.ldb, no_bias,
            gv2.beta, d2_ref, gv2.ldc, gv2.is_wc, pr2);
}

inline status_t run_legacy_2call_ref(int num_ops, int M, int K_in,
        int N_gate_up, int K_down, int H, data_type_t dt,
        grp_matmul_gated_act_t act_type, const std::vector<const void *> &src,
        const std::vector<const void *> &wei1,
        const std::vector<const void *> &wei2,
        const std::vector<void *> &d1_ref, const std::vector<void *> &d2_ref) {
    return run_legacy_2call_ref(std::vector<int>(num_ops, M), K_in, N_gate_up,
            K_down, H, dt, act_type, src, wei1, wei2, d1_ref, d2_ref);
}

// Legacy bool overloads.
inline status_t run_legacy_2call_ref(const std::vector<int> &Ms, int K_in,
        int N_gate_up, int K_down, int H, bool is_bf16,
        grp_matmul_gated_act_t act_type, const std::vector<const void *> &src,
        const std::vector<const void *> &wei1,
        const std::vector<const void *> &wei2,
        const std::vector<void *> &d1_ref, const std::vector<void *> &d2_ref) {
...
inline void verify_per_expert_2d(const TypedBuffers &got, size_t got_row_stride,
        const TypedBuffers &ref, size_t ref_row_stride,
        const std::vector<int> &Ms, int N, data_type_t fp, Tol tol,
        Label &&label) {
...
template <typename Label>
inline void verify_per_expert_2d(const TypedBuffers &got, size_t got_row_stride,
        const TypedBuffers &ref, size_t ref_row_stride, int num_ops, int M,
        int N, data_type_t fp, Tol tol, Label &&label) {
    verify_per_expert_2d(got, got_row_stride, ref, ref_row_stride,
            std::vector<int>(num_ops, M), N, fp, tol,
...
inline void verify_per_expert_2d(const TypedBuffers &got, size_t got_row_stride,
        const TypedBuffers &ref, size_t ref_row_stride,
        const std::vector<int> &Ms, int N, bool is_bf16, Tol tol,
        Label &&label) {
    verify_per_expert_2d(got, got_row_stride, ref, ref_row_stride, Ms, N,
            bool_to_fp_dt(is_bf16), tol, std::forward<Label>(label));
}
template <typename Label>
inline void verify_per_expert_2d(const TypedBuffers &got, size_t got_row_stride,
        const TypedBuffers &ref, size_t ref_row_stride, int num_ops, int M,
        int N, bool is_bf16, Tol tol, Label &&label) {
    verify_per_expert_2d(got, got_row_stride, ref, ref_row_stride, num_ops, M,
            N, bool_to_fp_dt(is_bf16), tol, std::forward<Label>(label));
}
