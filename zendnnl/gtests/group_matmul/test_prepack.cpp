...
TEST_F(TestFusedMoECacheStress, ColdThenWarm) {
    ...
    ASSERT_EQ(group_matmul_prepack::custom_kernel::
                      warm_pack_all_custom_kernel_experts(wei, K_vec, N_vec,
                              ldb_vec, transB,
                              /*is_weights_const=*/std::vector<bool> {}, E,
                              st_cold),
            status_t::success);
...
TEST_F(TestFusedMoEPointerChurn, ReallocOneExpertOnlyMisses) {
    ...
    ASSERT_EQ(
            group_matmul_prepack::custom_kernel::
                    warm_pack_all_custom_kernel_experts(wei, K_vec, N_vec,
                            ldb_vec, transB,
...
PrepackHarness make_harness(
        int total, int active, int K_v, int N_v, float fill_value) {
    ...
    for (int e = 0; e < total; ++e) {
        h.banks[e].assign(
                (size_t)K_v * N_v, bfloat16_t(fill_value + 0.001f * (float)e));
...
inline Int8WeightHarness make_int8_weights(
        int E, int K_v, int N_v, int8_t base) {
    Int8WeightHarness h;
    h.banks.resize(E);
    h.weight.resize(E);
    for (int e = 0; e < E; ++e) {
        h.banks[e].assign(
                static_cast<size_t>(K_v) * N_v, static_cast<int8_t>(base + e));
        h.weight[e] = h.banks[e].data();
    }
    h.K.assign(E, K_v);
    h.N.assign(E, N_v);
...
TEST_F(TestPrepackPerAlgoFunctions, AoclDlpSymQuantFullWeightPacks) {
    ...
    prepack::aocl_dlp::AoclDlpPackProbeStats st;
    ASSERT_EQ(prepack::aocl_dlp::warm_pack_all_aocl_dlp_experts_sym_quant(
                      h.weight, h.K, h.N, h.ldb, h.transB, h.is_weights_const,
...
TEST_F(TestPrepackPerAlgoFunctions, AoclDlpNTileTotalAttemptedCount) {
    ...
    prepack::aocl_dlp::AoclDlpPackProbeStats st;
    ASSERT_EQ(prepack::aocl_dlp::warm_pack_all_aocl_dlp_experts_n_tile(h.weight,
                      h.K, h.N, h.ldb, h.transB, h.is_weights_const,
                      /*total_count=*/4,
...
TEST_F(TestPrepackFusedMoEEndToEnd, BothPassesWarmAllExperts) {
    ...
    ASSERT_EQ(group_matmul_direct(gv.layout, gv.transA, gv.transB, gv.Ms, gv.Ns,
                      gv.Ks, gv.alpha, src_test_p, gv.lda, wei1_all, gv.ldb,
                      no_bias, gv.beta, dst_null, ldc_null, gv.is_wc, params,
                      nullptr, &act_params, &fused),
            status_t::success);
    ...
    prepack::custom_kernel::warm_pack_all_custom_kernel_experts(wei1_all,
            K_op1_vec, N_op1_vec, ldb_op1, transB_op1,
    ...
    prepack::custom_kernel::warm_pack_all_custom_kernel_experts(wei2_all,
            K_op2_vec, N_op2_vec, ldb_op2, transB_op2,
            /*is_weights_const=*/std::vector<bool> {},
...
TEST_P(TestPrepackKDownSynthesis, BothPassesWarmAllExperts) {
    ...
    act_params.act = act;
    auto act_ptr
            = (act != grp_matmul_gated_act_t::none) ? &act_params : nullptr;
    ...
    EXPECT_EQ(probe_op1.cache_hits, E)
            << "Op1 cache must be fully warmed (act=" << p.act_int << " E=" << E
            << " N1=" << N1 << ")";
    EXPECT_EQ(probe_op1.cache_misses, 0);
...
INSTANTIATE_TEST_SUITE_P(GroupMatmulPrepackKDownSynth,
        TestPrepackKDownSynthesis,
        ...
        [](const ::testing::TestParamInfo<KDownSynthParam> &info) {
            const char *act = (info.param.act_int == 0) ? "none"
                    ...
                    : (info.param.act_int == 2)         ? "gelu_and_mul"
                                                        : "swiglu_oai_mul";
            std::ostringstream s;
            s << "act_" << act << "_E" << info.param.total << "_N1_"
              << info.param.N1;
            return s.str();
        });
...
inline InvarianceHarness build_invariance_harness(const InvarianceParam &p) {
...
inline zendnnl::error_handling::status_t run_fused_moe_capture(
    ...
    auto params
            = make_uniform_params(num_ops, zendnnl::common::data_type_t::bf16);
    for (auto &pp : params) {
    ...
    return group_matmul_direct(gv.layout, gv.transA, gv.transB, gv.Ms, gv.Ns,
            ...
            dst_op1_null, ldc_null, gv.is_wc, params, nullptr, act_ptr, &fused);
}
...
TEST_P(TestPrepackResultInvariance, ConsistentAcrossIterations) {
    ...
    ASSERT_EQ(run_fused_moe_capture(h, p.num_ops, dst_a), status_t::success);
    ...
    ASSERT_EQ(run_fused_moe_capture(h, p.num_ops, dst_b), status_t::success);

    std::ostringstream lbl;
    lbl << "act=" << p.act_int << " E=" << p.num_ops << " dim=" << p.dim
        << " (cold→warm consistency)";
    ...
    verify_per_expert_2d(dst_a, h.H, dst_b, h.H, h.Ms, h.H, /*is_bf16=*/true,
...
TEST_P(TestPrepackResultInvariance, Algo1VsAlgo3) {
    ...
    // tile decomposition.
    {
        ...
        ASSERT_EQ(
                run_fused_moe_capture(h, p.num_ops, dst_a1), status_t::success);
    }
    ...
    reset_grp_matmul_caches();
    {
        AlgoEnvGuard algo_guard(3);
        ASSERT_EQ(
                run_fused_moe_capture(h, p.num_ops, dst_a3), status_t::success);
    }

    std::ostringstream lbl;
    lbl << "act=" << p.act_int << " E=" << p.num_ops << " dim=" << p.dim
        << " (ALGO 1 vs ALGO 3 result match)";
    ...
    verify_per_expert_2d(dst_a1, h.H, dst_a3, h.H, h.Ms, h.H, /*is_bf16=*/true,
...
INSTANTIATE_TEST_SUITE_P(GroupMatmulPrepackResultInvariance,
        TestPrepackResultInvariance,
        ...
        [](const ::testing::TestParamInfo<InvarianceParam> &info) {
            const char *act = (info.param.act_int == 0) ? "none"
                    ...
                    : (info.param.act_int == 2)         ? "gelu_and_mul"
                                                        : "swiglu_oai_mul";
            std::ostringstream s;
            s << "act_" << act << "_E" << info.param.num_ops << "_dim_"
              << info.param.dim;
            return s.str();
        });
...
TEST_F(TestPrepackClearCacheDirect, CustomKernelCacheClearEvictsEntries) {
    ...
    EXPECT_EQ(st_after_clear.cache_misses, 4)
            << "clear_custom_kernel_pack_cache() must evict all entries; "
               ...
               "(set_capacity(0) → set_capacity(MAX) which never iterates "
               "evict).";
...
TEST_F(TestPrepackVariableN, MixedNAcrossExperts) {
    ...
    prepack::custom_kernel::PackProbeStats probe_op1;
    prepack::custom_kernel::warm_pack_all_custom_kernel_experts(wei1_all,
            K_op1_vec, N_op1, /*ldb=*/N_op1, transB_vec,
            /*is_weights_const=*/std::vector<bool> {},
            /*total_count=*/E, probe_op1,
            /*interleave_split_halves=*/true);
    EXPECT_EQ(probe_op1.cache_hits, E)
            << "Op1 cache must hit at per-expert (K=" << K_in
            << ", N[e]) "
    ...
    prepack::custom_kernel::PackProbeStats probe_op2;
    prepack::custom_kernel::warm_pack_all_custom_kernel_experts(wei2_all, K_op2,
            N_op2_vec, ldb_op2_vec, transB_vec,
            /*is_weights_const=*/std::vector<bool> {},
            /*total_count=*/E, probe_op2);
    EXPECT_EQ(probe_op2.cache_hits, E)
            << "Op2 cache must hit at per-expert K_op2 = op2_k_for_act(N[e], "
               "act). "
...
inline StressHarness build_stress_harness(int E, int active) {
    ...
    for (int e = 0; e < E; ++e) {
        h.w1_banks[e].assign(
                (size_t)h.K_in * h.N1, bfloat16_t(0.005f + 1e-5f * (float)e));
        h.w2_banks[e].assign(
                (size_t)K_op2 * h.H, bfloat16_t(0.008f + 1e-5f * (float)e));
...
inline zendnnl::error_handling::status_t run_stress_call(
    ...
    return group_matmul_direct(gv.layout, gv.transA, gv.transB, gv.Ms, gv.Ns,
            gv.Ks, gv.alpha, srcs, gv.lda, h.wei1, gv.ldb, no_bias, gv.beta,
            dst_null, ldc_null, gv.is_wc, params, nullptr, &act_params, &fused);
}
...
TEST_F(TestPrepackStress, E64MultiIterationCacheStable) {
    ...
    std::vector<bool> transB_vec(E, false);
    prepack::custom_kernel::PackProbeStats probe_op1;
    prepack::custom_kernel::warm_pack_all_custom_kernel_experts(h.wei1,
            K_op1_vec, N_op1_vec, ldb_op1_vec, transB_vec,
            /*is_weights_const=*/std::vector<bool> {},
            /*total_count=*/E, probe_op1,
            /*interleave_split_halves=*/true);
    EXPECT_EQ(probe_op1.cache_hits, E)
    ...
    prepack::custom_kernel::PackProbeStats probe_op2;
    prepack::custom_kernel::warm_pack_all_custom_kernel_experts(h.wei2,
            K_op2_vec, N_op2_vec, ldb_op2_vec, transB_vec,
            /*is_weights_const=*/std::vector<bool> {},
            /*total_count=*/E, probe_op2);
    EXPECT_EQ(probe_op2.cache_hits, E)
...
inline EnvCase env_case(std::string label,
        std::vector<std::pair<const char *, const char *>> envs,
        bool check = true) {
    EnvCase c;
...
inline void run_fused_vs_ref_in_child(
        const EnvCase &p, const EnvMatrixShape &shape) {
    using namespace zendnnl::lowoha::matmul;
    using namespace moe_test_utils;
    using zendnnl::common::data_type_t;
    ...
    ASSERT_EQ(run_legacy_2call_ref(Ms, K_in, N1, K_op2, H,
                      /*is_bf16=*/true, act, srcs, wei1, wei2, dst1_ref_p,
                      dst2_r),
            status_t::success)
    ...
    ASSERT_EQ(group_matmul_direct(gv_op1.layout, gv_op1.transA, gv_op1.transB,
                      ...
                      gv_op1.lda, wei1, gv_op1.ldb, no_bias, gv_op1.beta,
                      dst_op1_null, ldc_null, gv_op1.is_wc, pf, nullptr,
                      act_ptr, &fused),
            status_t::success)
    ...
    if (p.check_correctness) {
        std::ostringstream lbl;
        lbl << "[env=" << p.label << "]";
        verify_per_expert_2d(d2_fused, H, d2_ref, H, Ms, H, /*is_bf16=*/true,
                tol_fused(/*is_bf16=*/true), lbl.str());
    }
}
...
static std::vector<EnvCase> make_bucket_a_cases() {
    ...
    return {
            ...
            env_case("prepack_explicit_on",
                    V {{"ZENDNNL_GRP_MATMUL_PREPACK", "1"}}),
            ...
            env_case("stable_ntile_off",
                    V {{"ZENDNNL_GRP_MATMUL_AOCL_STABLE_NTILE", "0"}}),
            env_case("stable_ntile_explicit_on",
                    V {{"ZENDNNL_GRP_MATMUL_AOCL_STABLE_NTILE", "1"}}),

            // ── ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL (BF16 fast path) ────────
            env_case("custom_kernel_on",
                    V {{"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL", "1"}}),
            env_case("custom_kernel_explicit_off",
                    V {{"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL", "0"}}),
            ...
            env_case("weight_cache_off",
                    V {{"ZENDNNL_MATMUL_WEIGHT_CACHE", "0"}}),
            ...
            env_case("fused_moe_tight_off",
                    V {{"ZENDNNL_GRP_MATMUL_FUSED_MOE_TIGHT", "0"}}),
            ...
            env_case("n_tile_fused_act_off",
                    V {{"ZENDNNL_GRP_MATMUL_N_TILE_FUSED_ACT", "0"}}),
            ...
            env_case("n_rounds_force_single",
                    V {{"ZENDNNL_GRP_MATMUL_ALGO", "3"},
                            {"ZENDNNL_GRP_MATMUL_N_ROUNDS", "1"}}),
            env_case("n_rounds_force_multi",
                    V {{"ZENDNNL_GRP_MATMUL_ALGO", "3"},
                            {"ZENDNNL_GRP_MATMUL_N_ROUNDS", "2"}}),
            env_case("n_rounds_force_balanced",
                    V {{"ZENDNNL_GRP_MATMUL_ALGO", "3"},
            ...
            env_case("custom_kernel_nr_32",
                    V {{"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL", "1"},
                            {"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL_NR", "32"}}),
            env_case("custom_kernel_nr_64",
                    V {{"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL", "1"},
...
INSTANTIATE_TEST_SUITE_P(GrpMatmulEnvBucketA, TestPrepackEnvBucketA,
        ::testing::ValuesIn(make_bucket_a_cases()), env_case_name);
...
static std::vector<EnvCase> make_bucket_b_cases() {
    ...
    return {
            ...
            env_case("target_slots_8",
                    V {{"ZENDNNL_GRP_MATMUL_AOCL_TARGET_SLOTS", "8"}}),
            env_case("target_slots_32",
                    V {{"ZENDNNL_GRP_MATMUL_AOCL_TARGET_SLOTS", "32"}}),

            // ── AOCL_BLIS_NC (narrow-N density check input) ──────────────
            env_case("blis_nc_256",
                    V {{"ZENDNNL_GRP_MATMUL_AOCL_BLIS_NC", "256"}}),
            ...
            env_case("subtile_per_expert_on",
                    V {{"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL", "1"},
                            {"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL_SUBTILE_PER_"
                             "EXPERT",
                                    "1"}}),
            ...
            env_case("custom_n_tile_128",
                    V {{"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL", "1"},
                            {"ZENDNNL_GRP_MATMUL_CUSTOM_KERNEL_N_TILE",
                                    "128"}}),
...
INSTANTIATE_TEST_SUITE_P(GrpMatmulEnvBucketB, TestPrepackEnvBucketB,
        ::testing::ValuesIn(make_bucket_b_cases()), env_case_name);
...
INSTANTIATE_TEST_SUITE_P(GrpMatmulEnvInteraction,
        ...
        ::testing::ValuesIn(make_interaction_matrix_cases()), env_case_name);
...
TEST_F(TestPrepackCrossWarmRegimes,
        CkOffAlgo1DoesNotCrossWarmCustomKernelPack) {
    using namespace zendnnl::lowoha::matmul;
    namespace prepack = zendnnl::lowoha::matmul::group_matmul_prepack;
...
TEST_F(TestPrepackCrossWarmRegimes,
    ...
    prepack::prepack_for_algo_3(
            h.pp); // primary regime 2 + 3 — regime 3 finds hits
...
inline group_matmul_prepack::PrepackParams build_grouped_s8_pp(
        ...
        const std::vector<bool> &transB, const std::vector<bool> &iwc,
        const std::vector<int> &M, std::vector<matmul_params> &params,
    ...
    return group_matmul_prepack::build_prepack_params(weight, K, N, ldb, transB,
            iwc, params, M,
            /*custom_kernel_on=*/1, /*num_threads=*/64, nr_align,
            grp_matmul_gated_act_t::none, data_type_t::bf16, &transA, &alpha,
            &beta);
}
} // namespace

TEST_F(TestPrepackCrossWarmRegimes,
        GroupedS8Algo3WarmsInt8CkAndCrossWarmsSymQuant) {
    using namespace zendnnl::lowoha::matmul;
    namespace prepack = zendnnl::lowoha::matmul::group_matmul_prepack;
    ...
    auto pp = build_grouped_s8_pp(h.weight, h.K, h.N, h.ldb, h.transB,
            h.is_weights_const, Ms, params, transA, alpha, beta,
            data_type_t::s8, /*nr_align=*/1);
...
TEST_F(TestPrepackCrossWarmRegimes,
        CrossWarmOnAlgo1NoCustomMatchesPrimaryOnly) {
    using namespace zendnnl::lowoha::matmul;
    namespace prepack = zendnnl::lowoha::matmul::group_matmul_prepack;
...
TEST_F(TestPrepackCrossWarmRegimes,
        FallbackClampPinnedAlgo3RunsAlgo1NoCrossWarm) {
    using namespace zendnnl::lowoha::matmul;
    using namespace moe_test_utils;
    namespace prepack = zendnnl::lowoha::matmul::group_matmul_prepack;
...
TEST_F(TestPrepackCkGateSymmetry, AllGatesPassEnablesCustomKernelWarm) {
    namespace prepack = zendnnl::lowoha::matmul::group_matmul_prepack;
    using zendnnl::lowoha::matmul::custom_kernel::
            clear_custom_kernel_pack_cache;
    clear_custom_kernel_pack_cache();
    prepack::clear_fingerprint_cache_for_test();
    prepack::test_api::clear_last_invocation_stats();
    ...
    EXPECT_GT(stats.ck.cache_misses + stats.ck.cache_hits, 0)
            << "All-pass gate must engage warm_custom (CK pack arena "
               "populated)";
}
...
TEST_F(TestPrepackCkGateSymmetry, BiasDtypeBf16AndF32Accepted) {
    ...
    for (data_type_t bias_dt :
            {data_type_t::none, data_type_t::bf16, data_type_t::f32}) {
        clear_custom_kernel_pack_cache();
        prepack::clear_fingerprint_cache_for_test();
        prepack::test_api::clear_last_invocation_stats();
        ...
        ASSERT_TRUE(stats.valid);
        EXPECT_GT(stats.ck.cache_misses + stats.ck.cache_hits, 0)
                << "bias_dtype " << static_cast<int>(bias_dt)
                << " is accepted by "
...
TEST_F(TestPrepackCkGateSymmetry, RefusalScopedToActiveExpertsOnly) {
    ...
    ASSERT_TRUE(stats.valid);
    EXPECT_GT(stats.ck.cache_misses + stats.ck.cache_hits, 0)
            << "Refusal triggers on inactive tail experts (i >= "
               "num_ops_active) "
...
TEST_F(TestPrepackBuildParamsContract, LegacyBothZeroResolvesToMSize) {
    ...
    auto pp = prepack::build_prepack_params(f.weight, f.K, f.N, f.ldb, f.transB,
            f.is_const, params, M, /*custom_kernel_on=*/false);
...
TEST_F(TestPrepackInt8WarmDtypeFamily, Int8WarmDoesNotPolluteBf16Cache) {
    ...
    for (int e = 0; e < E; ++e) {
        wei_s8[e].assign(
                static_cast<size_t>(K_in) * N1, static_cast<int8_t>(e));
        wei_ptrs[e] = wei_s8[e].data();
    }
...
TEST_F(TestPrepackInt8WarmDtypeFamily, ClearInt8CacheDoesNotEvictBf16) {
    ...
    for (int e = 0; e < E; ++e) {
        wei_s8[e].assign(
                static_cast<size_t>(K_in) * N1, static_cast<int8_t>(e));
        wei_bf16[e].assign(
                static_cast<size_t>(K_in) * N1, bfloat16_t(0.01f * e));
    ...
    }
    {
        ...
        ASSERT_EQ(prepack::custom_kernel::warm_pack_all_custom_kernel_experts(
                          ...
                          prepack::custom_kernel::WarmDtypeFamily::kINT8),
                zendnnl::error_handling::status_t::success);
        EXPECT_EQ(s.cache_misses, E) << "int8 entries must be evicted by "
                                        "`clear_custom_kernel_pack_cache_int8`";
...
TEST_F(TestPrepackF16WarmDtypeFamily, F16WarmHitsF16Probe) {
    ...
    for (int e = 0; e < E; ++e) {
        wei_f16[e].assign(
                static_cast<size_t>(K_in) * N1, float16_t(0.01f * (e + 1)));
    }
    std::vector<const void *> wei_ptrs(E);
    for (int e = 0; e < E; ++e) {
...
TEST_F(TestPrepackF16WarmDtypeFamily, ClearF16CacheDoesNotEvictBf16) {
    ...
    for (int e = 0; e < E; ++e) {
        ...
        wei_bf16[e].assign(
                static_cast<size_t>(K_in) * N1, bfloat16_t(0.02f * e));
    ...
    }
    {
        ...
        ASSERT_EQ(prepack::custom_kernel::warm_pack_all_custom_kernel_experts(
                          ...
                          prepack::custom_kernel::WarmDtypeFamily::kF16),
                zendnnl::error_handling::status_t::success);
        EXPECT_EQ(s.cache_misses, E) << "f16 entries must be evicted by "
                                        "`clear_custom_kernel_pack_cache_f16`";
        EXPECT_EQ(s.cache_hits, 0);
    }
}
...
TEST_F(TestPrepackInt8WarmDtypeFamily, FingerprintTrioDistinct) {
    ...
    for (int e = 0; e < E; ++e) {
        wei_bf16[e].assign(
                static_cast<size_t>(K_in) * N1, bfloat16_t(0.01f * (e + 1)));
        wei_ptrs_bf16[e] = wei_bf16[e].data();
    }
    std::vector<std::vector<int8_t>> wei_s8(E);
    ...
    auto make_params = [&](data_type_t wei_dt, bool dq, data_type_t cmp,
        ...
        return prepack::build_prepack_params(weights, Ks, Ns, ldbs, transBs,
                is_const, params, Ms,
                /*custom_kernel_env=*/1,
                /*num_threads=*/4,
                /*nr_align=*/1,
    ...
    prepack::prepack_for_algo_3(
            make_params(data_type_t::s8, true, data_type_t::s8, wei_ptrs_s8));
    prepack::prepack_for_algo_3(
            make_params(data_type_t::s8, true, data_type_t::u8, wei_ptrs_s8));
...
TEST_F(TestPrepackInt8WarmDtypeFamily, WarmCoversAllFourActivations) {
    ...
    for (bool interleave : {false, true}) {
        ...
        ASSERT_EQ(prepack::custom_kernel::warm_pack_all_custom_kernel_experts(
                          wei_ptrs, Ks, Ns, ldbs, transBs, std::vector<bool> {},
                          E, s_warm,
        ...
        EXPECT_EQ(s_probe.cache_hits, E)
                << "second probe at interleave="
                << (interleave ? "true" : "false") << " must hit every entry";
