...
protected:
    ...
    void run_basic_test(data_type_t src_dt, data_type_t wei_dt,
            data_type_t dst_dt, data_type_t bias_dt, float rtol_pref,
            float eps_pref, bool use_stride = false) {
        if (use_stride) {
            ...
            for (size_t i = 0; i < num_ops; ++i) {
                inp[i] = tensor_factory.uniform_dist_strided_tensor(
                        {m, k}, stride_in, src_dt, 2.0, transA);
                wt[i] = tensor_factory.uniform_dist_strided_tensor(
                        {k, n}, stride_wt, wei_dt, 2.0, transB);
                bias[i] = skip_bias ? tensor_t {}
                                    : tensor_factory.uniform_dist_tensor(
                                              {1, n}, bias_dt, 2.0);
                out[i] = tensor_factory.uniform_dist_strided_tensor(
                        {m, n}, stride_dst, dst_dt, 2.0);
                out_ref[i] = tensor_factory.uniform_dist_strided_tensor(
                        {m, n}, stride_dst, dst_dt, 2.0);
            }

            status_t status = group_matmul_kernel_test(
                    inp, wt, bias, out, algo, alpha, beta);
            ...
            for (size_t i = 0; i < num_ops && ref_status == status_t::success;
                    ++i) {
        ...
        const bool enable_moe = moe_dst_ok && ((m + n + k + num_ops) % 2 == 1)
        ...
        std::vector<int> Ms(num_ops, (int)m), Ns(num_ops, (int)n),
                Ks(num_ops, (int)k);
        ...
        for (size_t i = 0; i < num_ops; ++i) {
            in_t[i] = tensor_factory.uniform_dist_tensor(
                    {m, k}, src_dt, 2.0, transA);
            wei_t[i] = tensor_factory.uniform_dist_tensor(
                    {k, n}, wei_dt, 2.0, transB);
            bias_t[i]
                    = tensor_factory.uniform_dist_tensor({1, n}, bias_dt, 2.0);
            out_t[i] = tensor_factory.uniform_dist_tensor({m, n}, dst_dt, 2.0);
            out_ref_t[i]
                    = tensor_factory.uniform_dist_tensor({m, n}, dst_dt, 2.0);
        ...
        if (enable_moe) {
            ...
            for (int t = 0; t < num_tokens; ++t) {
                for (int kk = 0; kk < topk; ++kk) {
                    ...
                    row_ptrs[t * topk + kk]
                            = base + (size_t)t * ldcs[expert] * dst_elem_sz;
                }
            }
        ...
        if (ok && enable_moe) {
            ...
            const float moe_abs_bound = (src_dt == data_type_t::f32)
                    ? std::fabs(alpha)
                            * ((20 + std::log2((float)k) / 4) * k + 15)
                            * eps_pref
                    : (std::fabs(alpha) * (float)k + std::fabs(beta)) * topk
                            * eps_pref;
            ...
            auto load_dst_f32 = [&](const char *base, size_t idx) -> float {
                if (is_bf16_dst) {
                    uint16_t bits;
                    std::memcpy(
                            &bits, base + idx * sizeof(uint16_t), sizeof(bits));
            ...
            for (int t = 0; t < num_tokens && ok; ++t) {
                for (int d = 0; d < D && ok; ++d) {
                    ...
                    for (int kk = 0; kk < topk; ++kk) {
                        ...
                        const float w = moe.skip_weighted
                                ? 1.0f
                                : moe_weights[t * topk + kk];
                        acc += w * v;
                    }
                    const float got = load_dst_f32(
                            ...
                            (size_t)t * D + d);
                    if (std::abs(acc - got)
                            > moe_abs_bound + effective_rtol * std::abs(acc)) {
                        log_error("MoE mismatch t=", t, " d=", d,
...
INSTANTIATE_TEST_SUITE_P(
        GroupMatmul, TestGroupMatmul, ::testing::ValuesIn(matmul_test));
...
static std::string GatedActParamName(
    ...
    const char *dt_name = (p.dt == data_type_t::bf16) ? "bf16"
            : (p.dt == data_type_t::f16)              ? "f16"
                                                      : "f32";
    return std::string(act_names[p.act_int]) + "_" + dt_name + "_d"
            + std::to_string(p.dim) + "_M" + std::to_string(p.M) + "_E"
...
TEST_P(TestGatedAct, Correctness) {
    ...
    {
        auto pr = params;
        const status_t st_ref = group_matmul_direct(gv.layout, gv.transA,
                gv.transB, gv.Ms, gv.Ns, gv.Ks, gv.alpha, srcs, gv.lda, weis,
                gv.ldb, biases, gv.beta, dsts_ref, gv.ldc, gv.is_wc, pr,
                nullptr, nullptr);
    ...
    act.act = act_type;
    {
        ...
        ASSERT_EQ(group_matmul_direct(gv.layout, gv.transA, gv.transB, gv.Ms,
                          ...
                          biases, gv.beta, dsts, gv.ldc, gv.is_wc, pa, nullptr,
                          (act_type != grp_matmul_gated_act_t::none) ? &act
                                                                     : nullptr),
                status_t::success);
    }
    ...
    if (act_type == grp_matmul_gated_act_t::none) {
        for (int e = 0; e < num_ops; ++e) {
            ...
            if (is_bf16)
                ASSERT_EQ(std::memcmp(dst_act.bf16[e].data(),
                                  dst_ref.bf16[e].data(),
                                  sz * sizeof(bfloat16_t)),
                        0)
                        << "expert=" << e;
            else if (is_f16)
                ASSERT_EQ(
                        std::memcmp(dst_act.f16[e].data(),
                                dst_ref.f16[e].data(), sz * sizeof(float16_t)),
                        0)
                        << "expert=" << e;
            else
                ASSERT_EQ(std::memcmp(dst_act.f32[e].data(),
                                  dst_ref.f32[e].data(), sz * sizeof(float)),
                        0)
                        << "expert=" << e;
        }
        return;
        ...
        for (int m = 0; m < M; ++m) {
            for (int n = 0; n < dim; ++n) {
                ...
                ASSERT_NEAR(actual, expected,
                        std::abs(expected) * tol.rel + tol.abs)
...
static std::string MoEPostopParamName(
    ...
    const char *dt_name = (p.dt == data_type_t::bf16) ? "bf16"
            : (p.dt == data_type_t::f16)              ? "f16"
                                                      : "f32";
    return std::string(dt_name) + "_E" + std::to_string(p.num_ops) + "_M"
            + std::to_string(p.M) + "_N" + std::to_string(p.N) + "_K"
            + std::to_string(p.K) + "_topk" + std::to_string(p.topk)
...
TEST_P(TestMoEPostop, Correctness) {
    ...
    auto pa_run = params;
    const status_t st_run = group_matmul_direct(gv.layout, gv.transA, gv.transB,
            gv.Ms, gv.Ns, gv.Ks, gv.alpha, srcs, gv.lda, weis, gv.ldb, biases,
            gv.beta, dsts, gv.ldc, gv.is_wc, pa_run, &moe);
    ...
    auto pa_ref = params;
    const status_t st_ref = group_matmul_direct(gv.layout, gv.transA, gv.transB,
            gv.Ms, gv.Ns, gv.Ks, gv.alpha, srcs, gv.lda, weis, gv.ldb, biases,
            gv.beta, dsts_ref, gv.ldc, gv.is_wc, pa_ref);
    ASSERT_EQ(st_ref, status_t::success);
    ...
    for (int t = 0; t < num_tokens; ++t) {
        for (int d = 0; d < p.N; ++d) {
            ...
            if (is_bf16) {
                int16_t raw;
                std::memcpy(&raw,
                        moe_out.data() + (size_t)t * p.N * elem_sz
                                + d * sizeof(int16_t),
                        sizeof(raw));
            ...
            ASSERT_NEAR(got, acc, std::abs(acc) * tol.rel + tol.abs)
                    << dt_label << " E=" << p.num_ops << " M=" << p.M
                    << " N=" << p.N << " topk=" << p.topk
                    << (p.skip_weighted ? " skip" : "") << " t=" << t
                    << " d=" << d;
        }
    }
}
    ...
    for (data_type_t dt :
        ...
        for (int e : {2, 4, 8})
            for (int m : {2, 4, 8, 16})
                for (int topk : {1, 2})
                    if ((e * m) % topk == 0)
                        out.push_back({e, m, 64, 32, topk, false, dt});
...
TEST(TestSingleExpertParallelRouting, SingleExpertRoutesToParallel) {
    ...
    EXPECT_STRNE(mode, "sequential")
            << "single-expert (num_ops==1) non-fused call with src.size()==1 "
               "must "
    ...
    for (int m = 0; m < M; ++m) {
        for (int n = 0; n < N; ++n) {
            float acc = 0.0f;
            for (int k = 0; k < K; ++k) {
                acc += src.f32[0][(size_t)m * K + k]
                        * wei.f32[0][(size_t)k * N + n];
            }
            EXPECT_NEAR(dst.f32[0][(size_t)m * N + n], acc,
                    std::abs(acc) * rtol_f32 + epsilon_f32)
                    << "single-expert parallel mismatch at m=" << m
                    << " n=" << n;
        }
    }
}
...
TEST(TestSingleExpertParallelRouting, MultiOpChainStaysSequential) {
    ...
    EXPECT_STREQ(mode, "sequential")
            << "a genuine multi-op sequential chain (src.size()==1, num_ops>1) "
               "must NOT be rerouted to the parallel dispatch; got mode="
            << mode;
}
...
TEST(TestFusedMoEPipeline, Op1ActOp2VerticalFusion) {
    ...
    ASSERT_EQ(run_legacy_2call_ref(E, M, K_in, N_gate_up, K_down, H, dt,
                      act_type, srcs, wei1, wei2, dst1_r, dst2_r),
            status_t::success)
    ...
    for (auto &p : params)
        p.num_threads = 32; // deterministic planner rig
    ...
    status_t st = status_t::success;
    {
        ...
        st = group_matmul_direct(gv1.layout, gv1.transA, gv1.transB, gv1.Ms,
                gv1.Ns, gv1.Ks, gv1.alpha, srcs, gv1.lda, wei1, gv1.ldb,
                no_bias, gv1.beta, dst1, gv1.ldc, gv1.is_wc, params,
    ...
    verify_per_expert_2d(d2, H, d2_ref, H, E, M, H, dt, tol_fused(dt),
            "vertical_fusion Op1+act+Op2");
}
...
TEST(TestFusedMoEPipeline, SingleExpertOp1ActOp2) {
    ...
    const char *mode = nullptr;
    {
        GemmModeCaptureGuard guard;
        ASSERT_EQ(
                group_matmul_direct(gv1.layout, gv1.transA, gv1.transB, gv1.Ms,
                        gv1.Ns, gv1.Ks, gv1.alpha, srcs, gv1.lda, wei1, gv1.ldb,
                        no_bias, gv1.beta, dst1, gv1.ldc, gv1.is_wc, params,
                        /*moe_postop=*/nullptr, /*gated_act=*/&act,
                        /*fused_moe=*/&fused),
    ...
    for (int m = 0; m < M; ++m) {
        ...
        for (int h = 0; h < H; ++h) {
            ...
            ASSERT_NEAR(got, acc, std::abs(acc) * tol.rel + tol.abs)
                    << "single-expert fused pipeline mismatch m=" << m
                    << " h=" << h;
        }
    }
}
...
TEST_F(TestGroupMatmulHelperParity, QuickO1_EmptyFusedPresent_IsInternal) {
    ...
    ASSERT_EQ(
            detect_internal_alloc(v, /*num_ops=*/4, /*fused_moe_present=*/true,
...
TEST_F(TestGroupMatmulHelperParity, Sweep_MixedFusedPresent_ReturnsFailure) {
    ...
    EXPECT_EQ(
            detect_internal_alloc(v, /*num_ops=*/4, /*fused_moe_present=*/true,
                    internal_alloc_mode::sweep_active, &out),
            status_t::failure)
            << "sweep_active must report failure on mixed null/non-null active "
               "range";
}
...
TEST_F(TestGroupMatmulHelperParity, Sweep_NumOpsExceedsVSize_ClampsSafely) {
    ...
    ASSERT_EQ(
            detect_internal_alloc(v, /*num_ops=*/8, /*fused_moe_present=*/true,
                    internal_alloc_mode::sweep_active, &out),
            status_t::success);
...
TEST_F(TestGroupMatmulHelperParity,
    ...
    auto run_new = [](const std::vector<void *> &v, bool fmp) -> bool {
        ...
        detect_internal_alloc(
                v, /*num_ops=*/4, fmp, internal_alloc_mode::quick_o1, &out);
        return out;
    };
    for (bool fmp : {false, true}) {
        ...
        EXPECT_EQ(
                run_old(v_all_nonnull(4), fmp), run_new(v_all_nonnull(4), fmp));
    }
}
