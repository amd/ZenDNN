...
TEST_P(TestGroupMatmulQuant, WOQ_BF16_S4) {
    if (algo == matmul_algo_t::onednn
        ...
        GTEST_SKIP()
                << "WOQ_BF16_S4 is not supported for oneDNN-based algorithms.";
    }
    ...
    if (!has_valid_group_size && quant_combo == 2) { quant_combo = 1; }

    switch (quant_combo) {
        case 0: scale_size = {1, 1}; break;
        case 1: scale_size = {1, n}; break;
        case 2: {
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        wei_scale[i] = tensor_factory.uniform_dist_tensor(
                scale_size, scale_dtype, 2.0);
        wt[i] = tensor_factory.uniform_dist_tensor(
                {k, n}, data_type_t::s4, 7.0, transB, wei_scale[i]);
        inp[i] = tensor_factory.uniform_dist_tensor(
    ...
    if (ok) {
        ...
        if (act_type != grp_matmul_gated_act_t::none) {
            const float eps = (out_dt == data_type_t::bf16) ? epsilon_bf16
                                                            : epsilon_woq;
            for (size_t i = 0; i < num_ops; ++i) {
                moe_test_utils::apply_ref_gated_act_tensor(out_ref[i],
                        static_cast<int>(m), static_cast<int>(n),
                        static_cast<int>(n), act_type);
            }
            for (size_t i = 0; i < num_ops && ok; ++i)
                moe_test_utils::compare_activated_2D(
                        out[i], out_ref[i], m, n / 2, k, alpha, eps, ok);
        } else {
            for (size_t i = 0; i < num_ops && ok; ++i)
                compare_tensor_2D_matrix(out[i], out_ref[i], m, n, k,
                        out_dt == data_type_t::bf16 ? rtol_bf16 : rtol_woq,
                        out_dt == data_type_t::bf16 ? epsilon_bf16
                                                    : epsilon_woq,
                        ok, false, alpha, true);
        }
    }
...
TEST_P(TestGroupMatmulQuant, WOQ_BF16_U4) {
    ...
    std::vector<tensor_t> inp(num_ops), wt(num_ops), bias(num_ops),
            out(num_ops), out_ref(num_ops), w_scale(num_ops), w_zp(num_ops);

    // Bias dtype must be uniform across experts (m_tile_safe gate); pick once.
    auto bias_dt = (rand() % 2 == 0) ? data_type_t::bf16 : data_type_t::f32;

    for (size_t i = 0; i < num_ops; ++i) {
        w_scale[i] = tensor_factory.uniform_dist_tensor(
                scale_size, scale_dtype, 2.0);
        ...
        wt[i] = tensor_factory.uniform_dist_tensor(
                {k, n}, data_type_t::u4, 15.0, transB, w_scale[i], w_zp[i]);
        inp[i] = tensor_factory.uniform_dist_tensor(
                {m, k}, data_type_t::bf16, 2.0, transA);
        bias[i] = tensor_factory.uniform_dist_tensor({1, n}, bias_dt, 2.0);
        out[i] = tensor_factory.uniform_dist_tensor({m, n}, out_dt, 2.0);
...
TEST_P(TestGroupMatmulQuant, INT8) {
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        ...
        if (output_dtype != data_type_t::f32
                && output_dtype != data_type_t::bf16) {
            auto dst_ref
                    = tensor_factory.uniform_dist_tensor({m, n}, ref_dt, 2.0);
            if (quant_params_compute(tensor_factory, dst_ref, ref_dt,
                        output_dtype, {1, 1}, data_type_t::f32, dst_scale,
                        dst_zp)
                    != status_t::success) {
        ...
        wt[i] = weight_tensor;
        bias[i] = tensor_factory.uniform_dist_tensor({1, n}, bias_dt, 2.0);
        out[i] = tensor_factory.uniform_dist_tensor(
                {m, n}, output_dtype, 2.0, false, dst_scale, dst_zp);
        out_ref[i] = tensor_factory.uniform_dist_tensor(
                {m, n}, output_dtype, 2.0, false, dst_scale, dst_zp);
    }
    ...
    if (ok) {
        if (act_type != grp_matmul_gated_act_t::none) {
            ...
            for (size_t i = 0; i < num_ops && ok; ++i)
                moe_test_utils::compare_activated_2D(out[i], out_ref[i], m,
                        n / 2, k, 1.0f, epsilon_bf16, ok);
        } else {
            for (size_t i = 0; i < num_ops && ok; ++i)
                compare_tensor_2D_matrix(out[i], out_ref[i], m, n, k, rtol_bf16,
                        epsilon_bf16, ok, false, 1.0f, true);
        }
    }
    EXPECT_TRUE(ok);
}
...
TEST_P(TestGroupMatmulQuant, INT8_SYM_QUANT_PER_GROUP_BF16) {
    ...
    // Bias dtype must be uniform across experts (m_tile_safe gate); pick once.
    data_type_t bias_dt
            = (local_rng() % 2 == 0) ? data_type_t::bf16 : data_type_t::f32;
    uint64_t ngroups = sym_k / group_size;
    std::vector<int64_t> wei_sd
            = {static_cast<int64_t>(ngroups), static_cast<int64_t>(n)};
    std::vector<int64_t> src_sd
            = {static_cast<int64_t>(m), static_cast<int64_t>(ngroups)};
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        ...
        if (quant_params_compute(tensor_factory, src_ref, ref_dt,
                    data_type_t::s8, src_sd, scale_dt, src_scale, src_zp,
                    &input_tensor)
                != status_t::success) {
            FAIL() << "source dynamic quantization failed";
        }
    ...
    if (ok) {
        if (act_type != grp_matmul_gated_act_t::none) {
            ...
            for (size_t i = 0; i < num_ops && ok; ++i)
                moe_test_utils::compare_activated_2D(out[i], out_ref[i], m,
                        n / 2, sym_k, 1.0f, epsilon_bf16, ok);
        } else {
            for (size_t i = 0; i < num_ops && ok; ++i)
...
TEST_P(TestGroupMatmulQuant, INT8_DYNAMIC_GEMM_BF16) {
    ...
    std::vector<tensor_t> inp(num_ops), inp_ref(num_ops), inp_quant(num_ops),
            wt_s8(num_ops), wt_ref(num_ops), bias(num_ops), out(num_ops),
            out_ref(num_ops);
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        ...
        if (quant_params_compute(tensor_factory, wt_ref_t, test_dt,
        ...
        inp_ref[i] = tensor_factory.uniform_dist_tensor(
                {m, sym_k}, test_dt, 2.0, transA);
        inp[i] = tensor_factory.zero_tensor({m, sym_k}, test_dt, src_scale,
                tensor_t(), /*strided=*/false, transA);
        std::memcpy(inp[i].get_raw_handle_unsafe(),
                inp_ref[i].get_raw_handle_unsafe(),
                inp_ref[i].get_buffer_sz_bytes());
        tensor_t inp_quant_t, ref_src_scale, ref_src_zp;
        if (quant_params_compute(tensor_factory, inp_ref[i], test_dt,
                    data_type_t::s8, src_scale_dims, scale_dt, ref_src_scale,
                    ref_src_zp, &inp_quant_t)
                != status_t::success) {
            FAIL() << "source dynamic quantization failed";
        }
    ...
    status_t status = group_matmul_kernel_test(inp, wt_s8, bias, out, algo,
            1.0f, 0.0f, /*moe_postop=*/nullptr, act_ptr);
...
TEST(TestGroupMatmulWeightCacheInt8, SymQuantWc2StaysOutOfPlace) {
    ...
    tensor_factory_t tensor_factory {};
    constexpr uint64_t num_ops = 3, m = 8, sym_k = 256, n = 256,
                       group_size = 64;
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        ...
        inp[i] = input_tensor;
        wt[i] = weight_tensor;
        bias[i] = tensor_factory.uniform_dist_tensor(
    ...
    for (size_t i = 0; i < num_ops; ++i) {
        ...
        EXPECT_EQ(std::memcmp(p, wsnap[i].data(), wsnap[i].size()), 0)
                << "INT8 sym-quant weight expert " << i
                << " was mutated in place "
                   "under WC=2 (must stay out-of-place: compensation row > raw "
                   "size)";
    }
...
GroupDqProbe make_group_dq_probe(const std::vector<int> &Ms, int K_v,
...
TEST(TestGroupDynamicQuant, M1PerTensorTakesGroupedFastPath) {
    ...
    ASSERT_EQ(group_reorder_quantization_wrapper(pr.src, pr.lda, pr.transA,
                      pr.M, pr.K, /*num_threads=*/8, pr.params, qsrc, qlda,
                      buffers, quantized),
    ...
    EXPECT_TRUE(buffers.fallback_buf.empty())
            << "grouped fast path must not populate the per-expert fallback "
               "buffers";
    ...
    for (size_t e = 0; e < pr.params.size(); ++e) {
        ...
        if (pr.M[e] == 0) {
            EXPECT_EQ(qsrc[e], pr.src[e]) << "inactive e=" << e
                                          << " must forward the original src "
                                             "pointer, not the null slice";
            EXPECT_EQ(
                    pr.params[e].dtypes.src, zendnnl::common::data_type_t::bf16)
...
TEST(TestGroupDynamicQuant, FallbackSkipsInactiveExperts) {
    ...
    EXPECT_EQ(buffers.src_arena, nullptr)
            << "per_channel scale must use per-expert fallback, not grouped "
               "arena";
