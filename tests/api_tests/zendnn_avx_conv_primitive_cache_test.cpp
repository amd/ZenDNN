...
void convolution_param(engine eng, zendnn::memory user_src_memory, int batch,
                       int channel,
                       int height, int width, zendnn::memory user_weights_memory, int no_of_filter,
                       int kernel_h, int kernel_w, int pad_h, int pad_w, int stride_h,
                       int stride_w, zendnn::memory conv1_user_bias_memory,
                       zendnn::memory conv1_dst_memory, int out_height, int out_width,
                       bool reluFused, bool batchNormFused,
                       zendnn::memory conv1_batchNormScale_memory,
                       zendnn::memory conv1_batchNormMean_memory,
                       zendnn::memory conv1_batchNormOffset_memory) {
    ...
    //FIXME:correct BN dims
    memory::dims conv1_batchNormScale_tz  = {no_of_filter};
    memory::dims conv1_batchNormMean_tz   = {no_of_filter};
    memory::dims conv1_batchNormOffset_tz = {no_of_filter};
    ...
    //FIXME: correct BN dims and tgs
    auto conv1_batchNormScale_md  = memory::desc({conv1_batchNormScale_tz}, dt::f32,
                                    tag::any);
    auto conv1_batchNormMean_md   = memory::desc({conv1_batchNormMean_tz}, dt::f32,
                                    tag::any);
    auto conv1_batchNormOffset_md = memory::desc({conv1_batchNormOffset_tz},
                                    dt::f32, tag::any);
    ...
    //FIXME: This fusion calls ReLU and BatchNorm
    auto conv1_desc = convolution_forward::desc(prop_kind::forward_inference,
                      ...
                      conv1_padding, reluFused, batchNormFused,
                      conv1_batchNormScale_md, conv1_batchNormMean_md,
                      conv1_batchNormOffset_md);
    //[Create convolution descriptor]
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, user_src_memory},
        ...
        {ZENDNN_ARG_BN_SCALE, conv1_batchNormScale_memory},
        {ZENDNN_ARG_BN_MEAN, conv1_batchNormMean_memory},
        {ZENDNN_ARG_BN_OFFSET, conv1_batchNormOffset_memory}
    });
    //[Create convolution primitive]
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        for (int val=0; val <1 ; val++) {
            ...
            memory::dims conv1_bias_tz = {no_of_filter};
            memory::dims conv1_dst_tz = {batch, no_of_filter, out_height, out_width};
            //memory for Relu + BN Fusion
            //FIXEME: correct the dimension
            memory::dims conv1_batchNormScale_tz = {no_of_filter};
            memory::dims conv1_batchNormMean_tz = {no_of_filter};
            memory::dims conv1_batchNormOffset_tz = {no_of_filter};
            ...
            auto user_weights_memory = memory({{conv1_weights_tz}, dt::f32, tag::hwcn},
            ...
            auto conv1_dst_memory_1 = memory({{conv1_dst_tz}, dt::f32, tag::aBcd8b }, eng);
            auto conv1_dst_memory_2 = memory({{conv1_dst_tz}, dt::f32, tag::aBcd8b }, eng);
            auto conv1_dst_memory_3 = memory({{conv1_dst_tz}, dt::f32, tag::aBcd8b }, eng);
            auto conv1_batchNormScale_memory = memory({{conv1_batchNormScale_tz}, dt::f32, tag::x},
            eng);
            auto conv1_batchNormMean_memory = memory({{conv1_batchNormMean_tz}, dt::f32, tag::x},
            eng);
            auto conv1_batchNormOffset_memory = memory({{conv1_batchNormOffset_tz}, dt::f32, tag::x},
            eng);
            ...
            init_data(conv1_batchNormScale_memory);
            init_data(conv1_batchNormMean_memory);
            init_data(conv1_batchNormOffset_memory);

            //Fused convolution with relu
            convolution_param(eng, user_src_memory, batch, channel, height, width,
                              user_weights_memory, no_of_filter, kernel_h, kernel_w,
                              pad_h, pad_w, stride_h, stride_w, conv1_user_bias_memory,
                              conv1_dst_memory, out_height, out_width, 1,
                              0, conv1_batchNormScale_memory,
                              conv1_batchNormMean_memory, conv1_batchNormOffset_memory);
            float *dataHandle = (float *)conv1_dst_memory.get_data_handle();

            //Fused convolution with batchnorm
            convolution_param(eng, user_src_memory, batch, channel, height, width,
                              user_weights_memory, no_of_filter, kernel_h, kernel_w,
                              pad_h, pad_w, stride_h, stride_w, conv1_user_bias_memory,
                              conv1_dst_memory_1, out_height, out_width, 0,
                              1, conv1_batchNormScale_memory,
                              conv1_batchNormMean_memory, conv1_batchNormOffset_memory);
            float *dataHandle_1 = (float *)conv1_dst_memory_1.get_data_handle();

            //Fused convolution with relu and batchnorm
            convolution_param(eng, user_src_memory, batch, channel, height, width,
                              user_weights_memory, no_of_filter, kernel_h, kernel_w,
                              pad_h, pad_w, stride_h, stride_w, conv1_user_bias_memory,
                              conv1_dst_memory_2, out_height, out_width, 1,
                              1, conv1_batchNormScale_memory,
                              conv1_batchNormMean_memory, conv1_batchNormOffset_memory);
            float *dataHandle_2 = (float *)conv1_dst_memory_2.get_data_handle();

            //Convolution
            convolution_param(eng, user_src_memory, batch, channel, height, width,
                              user_weights_memory, no_of_filter, kernel_h, kernel_w,
                              pad_h, pad_w, stride_h, stride_w, conv1_user_bias_memory,
                              conv1_dst_memory_3, out_height, out_width, 0,
                              0, conv1_batchNormScale_memory,
                              conv1_batchNormMean_memory, conv1_batchNormOffset_memory);
            float *dataHandle_3= (float *)conv1_dst_memory_3.get_data_handle();

            //Output verification
            bool pass_flag=1;
            for (int i=0; i<batch*no_of_filter*out_height*out_width; i++) {
                if ((dataHandle_3[i] == dataHandle[i]) ||
                        (dataHandle_3[i] == dataHandle_1[i]) ||
                        (dataHandle_3[i] == dataHandle_2[i])) {
                    pass_flag = 0;
                    break;
                }
            }
            if (pass_flag) {
                zendnnInfo(ZENDNN_TESTLOG, "**********TEST PASSED********\n");
            }
            else {
                zendnnInfo(ZENDNN_TESTLOG, "**********TEST FAILED********\n");
            }
        }
    }
    catch (error &e) {
