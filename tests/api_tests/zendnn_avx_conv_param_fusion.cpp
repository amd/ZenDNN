...
void convolution_param(engine eng, zendnn::memory user_src_memory, int batch, int channel,
    ...
    auto conv1_batchNormScale_md  = memory::desc({conv1_batchNormScale_tz}, dt::f32, tag::any);
    auto conv1_batchNormMean_md   = memory::desc({conv1_batchNormMean_tz}, dt::f32, tag::any);
    auto conv1_batchNormOffset_md = memory::desc({conv1_batchNormOffset_tz}, dt::f32, tag::any);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        for (int val=0; val <4 ; val++) {
            ...
            auto user_weights_memory = memory({{conv1_weights_tz}, dt::f32, tag::hwcn}, eng); //cdba is hwcn for zendnn lib
            ...
            auto conv1_batchNormScale_memory = memory({{conv1_batchNormScale_tz}, dt::f32, tag::x}, eng);
            auto conv1_batchNormMean_memory = memory({{conv1_batchNormMean_tz}, dt::f32, tag::x}, eng);
            auto conv1_batchNormOffset_memory = memory({{conv1_batchNormOffset_tz}, dt::f32, tag::x}, eng);
            ...
            convolution_param(eng, user_src_memory, batch, channel, height, width,
                              ...
                              conv1_dst_memory, out_height, out_width, reluFused,
                              batchNormFused, conv1_batchNormScale_memory,
            ...
            file.open(zenDnnRootPath + std::string("/_out/tests/zendnn_avx_conv_param_output"));
            ...
            string str = std::string("sha1sum ") + zenDnnRootPath +
                         std::string("/_out/tests/zendnn_avx_conv_param_output >") + zenDnnRootPath +
                         std::string("/_out/tests/zendnn_avx_conv_param_output.sha1");
            ...
            ifstream zenDnnSha1(zenDnnRootPath + std::string("/_out/tests/zendnn_avx_conv_param_output.sha1"));
            string firstWordZen;
            ...
            if (firstWordZen != firstWordRef) {
                zendnnInfo(ZENDNN_TESTLOG, "As expected firstWordZen and firstWordRef dont match when fusion is enabled");
