...
void convolution_param(engine eng, zendnn::memory user_src_memory, int batch,
    ...
    auto conv1_desc = convolution_forward::desc(prop_kind::forward_inference,
                      s16_downscaling_val!=0? algorithm::convolution_gemm_u8s8s16os8:
                      algorithm::convolution_gemm_u8s8s16os16, conv1_src_md, conv1_weights_md,
...
int main(int argc, char **argv) {
    ...
    const char *s16_downscaling = std::getenv("S16_DOWNSCALING");
    //int s16_downscaling_val = 1;
    if (s16_downscaling) {
        s16_downscaling_val = atoi(s16_downscaling);
    }
    if (s16_downscaling_val != 0) {
        s16_downscaling_val = 1;
    }
    ...
    std::cout << "S16_DOWNSCALING: " << s16_downscaling_val << std::endl;
    ...
    try {
        ...
        for (int test_num = 0; test_num < num_test_cases; ++test_num) {
            ...
            eng);
            if (s16_downscaling_val == 0)
                conv1_dst_memory[test_num] = memory({{conv1_dst_tz}, dt::s16, tag::aBcd8b },
            eng);
