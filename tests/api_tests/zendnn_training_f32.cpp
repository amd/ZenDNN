...
void simple_net(engine::kind engine_kind) {
    ...
    write_to_zendnn_memory(net_src.data(), conv_user_src_memory);
    auto conv_user_weights_memory
    = memory({{conv_weights_tz}, dt::f32, tag::oihw}, eng);
    write_to_zendnn_memory((void *)conv_weights.data(), conv_user_weights_memory);
    auto conv_user_bias_memory = memory({{conv_bias_tz}, dt::f32, tag::x}, eng);
    write_to_zendnn_memory(conv_bias.data(), conv_user_bias_memory);
    ...
    if (conv_pd.src_desc() != conv_user_src_memory.get_desc()) {
        ...
        net_fwd_args.push_back({{ZENDNN_ARG_FROM, conv_user_src_memory},
            {ZENDNN_ARG_TO, conv_src_memory}});
    }
    ...
    if (conv_pd.weights_desc() != conv_user_weights_memory.get_desc()) {
        ...
        net_fwd_args.push_back({{ZENDNN_ARG_FROM, conv_user_weights_memory},
            {ZENDNN_ARG_TO, conv_weights_memory}});
    }
    ...
    net_fwd_args.push_back({{ZENDNN_ARG_SRC, conv_src_memory},
        {ZENDNN_ARG_WEIGHTS, conv_weights_memory},
        {ZENDNN_ARG_BIAS, conv_user_bias_memory},
    ...
    {{ZENDNN_ARG_SRC, conv_dst_memory}, {ZENDNN_ARG_DST, relu_dst_memory}});
    ...
    net_fwd_args.push_back({
        {ZENDNN_ARG_SRC, relu_dst_memory}, {ZENDNN_ARG_DST, lrn_dst_memory},
        {ZENDNN_ARG_WORKSPACE, lrn_workspace_memory}});
    ...
    write_to_zendnn_memory(net_dst.data(), pool_user_dst_memory);
    ...
    net_fwd_args.push_back({{ZENDNN_ARG_SRC, lrn_dst_memory},
        // delay putting DST until reorder (if needed)
        {ZENDNN_ARG_WORKSPACE, pool_workspace_memory}});
    ...
    if (pool_pd.dst_desc() != pool_user_dst_memory.get_desc()) {
        ...
        net_fwd_args.back().insert({ZENDNN_ARG_DST, pool_dst_memory});
        net_fwd.push_back(reorder(pool_dst_memory, pool_user_dst_memory));
        net_fwd_args.push_back({{ZENDNN_ARG_FROM, pool_dst_memory},
            {ZENDNN_ARG_TO, pool_user_dst_memory}});
    }
    else {
        net_fwd_args.back().insert({ZENDNN_ARG_DST, pool_dst_memory});
    }
    ...
    = memory({{pool_dst_tz}, dt::f32, tag::nchw}, eng);
    write_to_zendnn_memory(net_diff_dst.data(), pool_user_diff_dst_memory);
    ...
    if (pool_dst_memory.get_desc() != pool_user_diff_dst_memory.get_desc()) {
        ...
        net_bwd_args.push_back({{ZENDNN_ARG_FROM, pool_user_diff_dst_memory},
            {ZENDNN_ARG_TO, pool_diff_dst_memory}});
    }
    ...
    net_bwd_args.push_back({{ZENDNN_ARG_DIFF_DST, pool_diff_dst_memory},
        {ZENDNN_ARG_DIFF_SRC, pool_diff_src_memory},
        {ZENDNN_ARG_WORKSPACE, pool_workspace_memory}});
    ...
    if (lrn_diff_dst_memory.get_desc() != lrn_bwd_pd.diff_dst_desc()) {
        ...
        net_bwd_args.push_back({{ZENDNN_ARG_FROM, pool_diff_src_memory},
            {ZENDNN_ARG_TO, lrn_diff_dst_memory}});
    }
    ...
    net_bwd_args.push_back({{ZENDNN_ARG_SRC, relu_dst_memory},
        {ZENDNN_ARG_DIFF_DST, lrn_diff_dst_memory},
        {ZENDNN_ARG_DIFF_SRC, lrn_diff_src_memory},
        {ZENDNN_ARG_WORKSPACE, lrn_workspace_memory}});
    ...
    if (relu_diff_dst_memory.get_desc() != relu_bwd_pd.diff_dst_desc()) {
        ...
        net_bwd_args.push_back({{ZENDNN_ARG_FROM, lrn_diff_src_memory},
            {ZENDNN_ARG_TO, relu_diff_dst_memory}});
    }
    ...
    net_bwd_args.push_back({{ZENDNN_ARG_SRC, conv_dst_memory},
        {ZENDNN_ARG_DIFF_DST, relu_diff_dst_memory},
        {ZENDNN_ARG_DIFF_SRC, relu_diff_src_memory}});
    ...
    write_to_zendnn_memory(conv_user_diff_weights_buffer.data(),
                           conv_user_diff_weights_memory);
    auto conv_diff_bias_memory = memory({{conv_bias_tz}, dt::f32, tag::x}, eng);
    write_to_zendnn_memory(conv_diff_bias_buffer.data(), conv_diff_bias_memory);
    ...
    if (conv_bwd_weights_pd.src_desc() != conv_src_memory.get_desc()) {
        ...
        net_bwd_args.push_back({{ZENDNN_ARG_FROM, conv_src_memory},
            {ZENDNN_ARG_TO, conv_bwd_src_memory}});
    }
    ...
    if (conv_bwd_weights_pd.diff_dst_desc()
        ...
        net_bwd_args.push_back({{ZENDNN_ARG_FROM, relu_diff_src_memory},
            {ZENDNN_ARG_TO, conv_diff_dst_memory}});
    }

    // create backward convolution primitive
    net_bwd.push_back(convolution_backward_weights(conv_bwd_weights_pd));
    net_bwd_args.push_back({{ZENDNN_ARG_SRC, conv_bwd_src_memory},
        {ZENDNN_ARG_DIFF_DST, conv_diff_dst_memory},
        // delay putting DIFF_WEIGHTS until reorder (if needed)
        {ZENDNN_ARG_DIFF_BIAS, conv_diff_bias_memory}});
    ...
    if (conv_bwd_weights_pd.diff_weights_desc()
        ...
        {ZENDNN_ARG_DIFF_WEIGHTS, conv_diff_weights_memory});

        net_bwd.push_back(reorder(
                              conv_diff_weights_memory, conv_user_diff_weights_memory));
        net_bwd_args.push_back({{ZENDNN_ARG_FROM, conv_diff_weights_memory},
            {ZENDNN_ARG_TO, conv_user_diff_weights_memory}});
    }
    else {
        net_bwd_args.back().insert(
        {ZENDNN_ARG_DIFF_WEIGHTS, conv_diff_weights_memory});
...
int main(int argc, char **argv) {
    cout << "zendnn_training_f32 API test starts\n";
    try {
    ...
    cout << "zendnn_training_f32 API test end\n";
    return 0;
}
