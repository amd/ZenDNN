...
void simple_net(engine::kind engine_kind, int times = 100) {
    ...
    file.open(zenDnnRootPath + std::string("/_out/tests/ref_avx_conv_output"));
#else
    float *dataHandle= (float *)conv1_dst_memory.get_data_handle();
    file.open(zenDnnRootPath + std::string("/_out/tests/zendnn_avx_conv_output"));
#endif
    ...
    string str = std::string("sha1sum ") + zenDnnRootPath +
        std::string("/_out/tests/zendnn_avx_conv_output >") + zenDnnRootPath +
        std::string("/_out/tests/zendnn_avx_conv_output.sha1");
#else
    zendnnInfo(ZENDNN_TESTLOG, "Ref API test: SUM: ", sum);
    string str = std::string("sha1sum ") + zenDnnRootPath +
        std::string("/_out/tests/ref_avx_conv_output >") + zenDnnRootPath +
        std::string("/tests/api_tests/sha_out_NHWC/ref_avx_conv_output.sha1");
#endif
    ...
    ifstream zenDnnSha1(zenDnnRootPath + std::string("/_out/tests/zendnn_avx_conv_output.sha1"));
    string firstWordZen;
