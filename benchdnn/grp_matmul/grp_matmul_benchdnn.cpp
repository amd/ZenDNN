...
///   `wei_scale` buffer (dims = {1, N}, f32) on every active expert,
///   or per-group `wei_scale` ({G, N}) when group_size > 0.  The
///   `src_scale` is left null (buff=nullptr, dims = {M, 1} or {M, G});
///   the library's pre-OMP source-reorder hoist allocates and fills it
///   at runtime — same contract as production callers.
///   Requires src:wei:dst == bf16:{s8|s4}:bf16 and K%4==0; the parser
...
static bool run_config(const GrpMatmulConfig &cfg, std::ostream &csv,
        const global_options &options, [[maybe_unused]] size_t cache_size,
    ...
    const int lda_val = internal_alloc ? std::max(cfg.K, cfg.N_down) : cfg.K;
    ...
    // Weight buffer byte size per expert.  s4 is nibble-packed (2
    // elements per byte), so K*N elements need K*N/2 bytes; all other
    // dtypes use size_of(dt) bytes per element.
    const size_t wei_buf_bytes = (cfg.wei_dt == data_type_t::s4)
            ? (static_cast<size_t>(cfg.K) * cfg.N + 1) / 2
            : static_cast<size_t>(cfg.K) * cfg.N * wei_elem;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!internal_alloc) {
            ...
            std::memset(
                    C[i].ptr, 0, static_cast<size_t>(Mi) * cfg.N * dst_elem);
        }
    }
    ...
    if (cfg.dynamic_quant) {
        for (int e = 0; e < n; ++e) {
            wei_scale_buf[e].alloc(wei_scale_nelems * sizeof(float));
            fill_quant_scale_f32(static_cast<float *>(wei_scale_buf[e].ptr),
                    wei_scale_nelems, 311 + e * 13);
        }
        for (int i = 0; i < n; ++i) {
            ...
            params[i].quant_params.wei_scale.dims
                    = {wei_scale_G, static_cast<int64_t>(cfg.N)};
    ...
    if (moe_enabled) {
        ...
        if (internal_alloc && cfg.N_down <= 0) {
            std::cerr
                    << "ERROR: internal_alloc + moe_postop requires N_down>0\n";
            return false;
        }

        moe_weights.assign(
                static_cast<size_t>(num_slots), 1.f / static_cast<float>(topk));
        moe_output_buf.alloc(
                static_cast<size_t>(num_tokens) * cfg.N * dst_elem);
        ...
        int slot = 0;
        for (int e = 0; e < n; ++e) {
            auto *base = !internal_alloc ? static_cast<const char *>(C[e].ptr)
    ...
    grp_matmul_gated_act_params *act_ptr = (cfg.gated_act > 0) ? &act : nullptr;
    ...
    if (fused_enabled && !act_halves_k) {
        std::cerr << "WARNING: N_down=" << cfg.N_down
                  << " with gated_act=0 — Op2 will read the FULL N=" << cfg.N
                  << " columns of Op1's output (K_down=N, "
    ...
    // Op2 weight buffer byte size per expert (same nibble-pack as Op1).
    const size_t wei_down_buf_bytes = (cfg.wei_dt == data_type_t::s4)
            ? (static_cast<size_t>(K_down) * cfg.N_down + 1) / 2
            : static_cast<size_t>(K_down) * cfg.N_down * wei_elem;
    ...
    if (fused_enabled) {
        ...
        if (w4a8_fused) {
            ...
            for (int e = 0; e < n; ++e) {
                wei_down_scale_buf[e].alloc(
                        wei_down_scale_nelems * sizeof(float));
                fill_quant_scale_f32(
                        static_cast<float *>(wei_down_scale_buf[e].ptr),
                        wei_down_scale_nelems, 500 + e * 17);
            }
        }

        for (int i = 0; i < n; ++i) {
            B_down[i].alloc(wei_down_buf_bytes);
            ...
            if (w4a8_fused) {
                ...
                ds.dims = {wei_down_scale_G, static_cast<int64_t>(cfg.N_down)};
            ...
            if (!internal_alloc) {
                ...
                std::memset(C_down[i].ptr, 0,
                        static_cast<size_t>(cfg.M_per_op[i]) * cfg.N_down
                                * dst_elem);
        ...
        if (moe_enabled) {
            ...
            moe_output_buf.alloc(
                    static_cast<size_t>(num_tokens) * cfg.N_down * dst_elem);
            moe.output = moe_output_buf.ptr;
            int slot = 0;
            for (int e = 0; e < n; ++e) {
                ...
                const int row_stride = !internal_alloc ? cfg.N_down : lda_val;
                for (int j = 0; j < cfg.M_per_op[e]; ++j) {
                    moe_row_ptrs_vec[static_cast<size_t>(slot)] = base
                            + static_cast<size_t>(j) * row_stride * dst_elem;
                    ++slot;
                }
            }
        }
    ...
    for (int w = 0; w < cfg.warmup; ++w) {
        ...
        auto st = group_matmul_direct(layout, transA, transB, Mv, Nv, Kv, alpha,
                src_ptrs, lda, wei_ptrs, ldb, bias_ptrs, beta, dst_ptrs, ldc,
                wconst, params, moe_ptr, act_ptr, fused_ptr);
    ...
    for (int it = 0; it < cfg.iters; ++it) {
        ...
        restore_src();
        if (options.cache_mode == CacheMode::COLD) { flush_cache(cache_size); }
    ...
    for (int i = 0; i < n; ++i) {
        total_flops += 2.0 * cfg.M_per_op[i] * cfg.K
                * cfg.N; // Stage 1: Op1 gate+up GEMM
        if (fused_enabled) {
            total_flops += 2.0 * cfg.M_per_op[i] * K_down
                    * cfg.N_down; // Stage 3: Op2 down_proj GEMM
        }
    }
    if (moe_enabled) {
        int D_out = fused_enabled ? cfg.N_down : cfg.N;
        total_flops
                += 2.0 * num_tokens * D_out * topk; // Stage 4: weighted reduce
    }
    double gflops_avg = (total_flops / avg_ms) * 1e-6; // mean throughput
    double gflops_peak = (total_flops / min_ms) * 1e-6; // best-of-N throughput

    std::string dtypes = datatypeToStr(cfg.src_dt) + ":"
            + datatypeToStr(cfg.wei_dt) + ":" + datatypeToStr(cfg.dst_dt);
    ...
    // Console
    std::cout << std::setw(4) << n << "  " << std::setw(12) << m_str << "  "
              << std::setw(6) << cfg.K << "  " << std::setw(6) << cfg.N << "  "
              << std::setw(5) << cfg.iters << "  " << std::setw(6) << cfg.warmup
              << "  " << std::setw(14) << dtypes << "  " << std::setw(5)
              << quant_str << "  " << std::setw(10) << moe_str << "  "
              << std::setw(13) << fused_str << "  " << std::fixed
              << std::setprecision(3) << std::setw(10) << avg_ms << "  "
              << std::setw(10) << min_ms << "  " << std::setprecision(1)
              << std::setw(9) << gflops_avg << "  " << std::setw(9)
              << gflops_peak << std::endl;
    ...
    csv << n << "," << m_str << "," << cfg.K << "," << cfg.N << "," << cfg.iters
        << "," << cfg.warmup << "," << dtypes << "," << quant_str << ","
        << (cfg.is_weights_const ? "true" : "false") << "," << cfg.moe_topk
        << "," << cfg.gated_act << "," << cfg.N_down << ","
        << cfg.use_internal_alloc << "," << std::fixed << std::setprecision(6)
        << total_ms << "," << sum_iter_ms << "," << avg_ms << "," << min_ms
...
int bench(const std::string &in_filename, const std::string &out_filename,
    ...
    while (std::getline(infile, line)) {
        GrpMatmulConfig cfg;
        if (parse_config(line, cfg)) { configs.push_back(cfg); }
    }
    infile.close();
    ...
    std::cout << "  MATMUL_ALGO: " << (algo_env ? algo_env : "default(1)")
              << std::endl;
    std::cout << "  Threads    : " << (omp_env ? omp_env : "default")
              << std::endl;
    std::cout << "============================================================="
                 "==="
              << std::endl;
    std::cout << " ops             M       K       N  iters warmup          "
                 "dtypes"
                 "  quant         moe          fused      avg_ms      min_ms  "
                 "GFLOPS_a  GFLOPS_p"
              << std::endl;
    ...
    PerfProfile perf_profile
            = parse_perf_profile(options.perf_profile_str.c_str());
    ...
    if (options.perf_counters && perf_ctrs.is_available()) {
        if (perf_ctrs.open()) {
            ...
            commonlog_warning(
                    "HW perf counters unavailable; continuing without them.");
        }
    }
