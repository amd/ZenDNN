...
bool parse_config(const std::string &line, GrpMatmulConfig &cfg) {
    ...
    try {
        ...
        if (!ialloc_str.empty()) {
            cfg.use_internal_alloc = (ialloc_str == "1" || ialloc_str == "true"
                                             || ialloc_str == "internal_alloc")
                    ? 1
                    : 0;
        }
    ...
    // DQ-INT8 / W4A8 contract — dynamic_quant=true is accepted for:
    //   * (src=bf16, wei=s8, dst=bf16) — DQ-INT8 (per-token or per-group)
    //   * (src=bf16, wei=s4, dst=bf16) — W4A8 (per-group s4 weights)
    // with compute ∈ {s8, u8}.  Refuse anything else so the driver
    // fails fast with a precise message instead of dispatching a config
    // the library will silently fall back for.  Also enforce the K%4
    // microkernel alignment (mirrors the `sym_k = (k/4)*4` clamp in
    // test_quant.cpp::INT8_DYNAMIC_GEMM_BF16).
    if (cfg.dynamic_quant) {
        const bool valid_wei = (cfg.wei_dt == data_type_t::s8
                || cfg.wei_dt == data_type_t::s4);
        if (cfg.src_dt != data_type_t::bf16 || !valid_wei
                || cfg.dst_dt != data_type_t::bf16) {
            std::cerr << "parse_config: dynamic_quant=1 requires "
                         "src:wei:dst = bf16:{s8|s4}:bf16; got "
                      << datatypeToStr(cfg.src_dt) << ":"
                      << datatypeToStr(cfg.wei_dt) << ":"
                      << datatypeToStr(cfg.dst_dt) << "\n";
            return false;
        }
        // W4A8 (s4 weights) is symmetric-only; reject asym (u8).
        if (cfg.wei_dt == data_type_t::s4
                && cfg.compute_dt == data_type_t::u8) {
            std::cerr << "parse_config: W4A8 (wei=s4) is symmetric-only; "
                         "compute_dt must be s8, not u8\n";
            return false;
        }
        // W4A8 requires per-group scales (group_size > 0, K divisible).
        if (cfg.wei_dt == data_type_t::s4 && cfg.group_size <= 0) {
            std::cerr << "parse_config: W4A8 (wei=s4) requires "
                         "group_size > 0 for per-group weight scales\n";
            return false;
        }
        if (cfg.wei_dt == data_type_t::s4 && cfg.group_size > 0
                && cfg.K % cfg.group_size != 0) {
            std::cerr << "parse_config: W4A8 group_size=" << cfg.group_size
                      << " must divide K=" << cfg.K << "\n";
            return false;
        }
        if (cfg.wei_dt == data_type_t::s4 && cfg.group_size > 0
        ...
        // s8).  Per-group requires symmetric compute (s8) and K divisible
        // by group_size — same constraints the original per-group contract
        // imposed.  W4A8 (s4) is already validated above.
        if (cfg.group_size > 0 && cfg.wei_dt != data_type_t::s4) {
            ...
            if (cfg.compute_dt != data_type_t::s8) {
                std::cerr << "parse_config: per-group quant (group_size > 0) "
                             "requires compute_dt=s8 (symmetric-only); got "
...
std::string format_M(const GrpMatmulConfig &cfg) {
    if (cfg.is_uniform_M()) { return std::to_string(cfg.M_per_op[0]); }
...
void fill_bf16_random(void *buf, size_t elems, uint32_t seed) {
    ...
    for (size_t i = 0; i < elems; ++i) {
        ...
        float val
                = static_cast<float>(static_cast<int>(state >> 16) % 200 - 100)
                * 0.01f;
        uint32_t bits;
...
void fill_buffer(void *buf, size_t elems, data_type_t dt, uint32_t seed) {
    ...
    } else if (dt == data_type_t::s4) {
        // Nibble-packed: 2 s4 values per byte (low nibble first).
        // Each nibble is in [-7, 7] (symmetric s4 range).
        // `elems` is the logical element count (K*N); packed bytes = elems/2.
        auto *p = static_cast<uint8_t *>(buf);
        const size_t packed_bytes = (elems + 1) / 2;
        uint32_t state = seed;
        for (size_t i = 0; i < packed_bytes; ++i) {
            state = state * 1103515245u + 12345u;
            int lo = static_cast<int>((state >> 16) % 15) - 7; // [-7, 7]
            state = state * 1103515245u + 12345u;
            int hi = static_cast<int>((state >> 16) % 15) - 7; // [-7, 7]
            p[i] = static_cast<uint8_t>((lo & 0xF) | ((hi & 0xF) << 4));
        }
    } else {
