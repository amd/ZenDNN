...
int run_matmul(tensor_t output_tensor, tensor_t input_tensor, tensor_t weights,
    ...
    if (!isNotWarmup) {
        ...
        auto matmul_context = matmul_context_t()
                                      ...
                                      .set_alpha(cfg.alpha)
                                      .set_beta(cfg.beta);
        if (cfg.isBiasEnabled) { matmul_context.set_param("bias", bias); }
        ...
        for (size_t i = 0; i < binary_post_ops_tensors.size(); i++) {
            if (postOpsToStr(cfg.post_ops[cfg.binary_post_ops_pos[i]])
                    == "binary_mul") {
                ...
                matmul_operator.set_input(
                        matmul_context.get_post_op(cfg.binary_post_ops_pos[i])
                                .binary_mul_params.tensor_name,
                        binary_post_ops_tensors[i]);
            } else if (postOpsToStr(cfg.post_ops[cfg.binary_post_ops_pos[i]])
                    == "binary_add") {
        ...
        stats.context_creation_ms += (std::chrono::duration<double, std::milli>(
                end_context_creation - start_context_creation)
                                              .count());
        stats.operator_creation_ms
                += (std::chrono::duration<double, std::milli>(
                        end_operator_creation - start_operator_creation)
                                .count());
        stats.operator_execution_ms
                += (std::chrono::duration<double, std::milli>(
                        end_operator_execution - start_operator_execution)
                                .count());
        if (status == status_t::success) {
            testlog_info("<", matmul_operator.get_name(), ">",
                    " operator execution successful.");
...
int matmul_benchdnn(std::vector<MatmulConfig> configs,
    ...
    for (auto &cfg : configs) {
        try {
            ...
            ret = create_weights_tensor(
                    tensor_factory, cfg, weights_buffer_pool[0], options);
            if (ret != OK) {
            ...
            ret = create_input_tensor(
                    tensor_factory, cfg, input_tensor, options);
            if (ret != OK) {
            ...
            for (auto j = 0; j < cfg.warmup_iters && !skip; j++) {
                for (auto i = 0; i < cfg.n_values.size(); i++) {
                    int ret = run_matmul(output_tensor[i],
                            ...
                            (bias.empty() ? tensor_t() : bias[i]), cfg,
                            binary_post_ops_tensors[i], time_stats);
                    if (ret != OK) {
                ...
                for (auto i = 0; i < cfg.n_values.size(); i++) {
                    ...
                    int ret = run_matmul(output_tensor[i],
                            ...
                            (cfg.cache_mode == CacheMode::WARM)
                                    ? weights_buffer_pool[(1 + j)
                                              % num_weight_buffers][i]
                                    : weights_buffer_pool[0][i],
                            (bias.empty() ? tensor_t() : bias[i]), cfg,
                            binary_post_ops_tensors[i], time_stats, true);
                    if (ret != OK) {
                        testlog_error("run_matmul execution failed.");
                        skip = true;
                        break;
                    ...
                    time_stats_layer[i].context_creation_ms
                            += time_stats.context_creation_ms;
                    time_stats_layer[i].operator_creation_ms
                            += time_stats.operator_creation_ms;
                    time_stats_layer[i].operator_execution_ms
                            += time_stats.operator_execution_ms;
                    time_stats_layer[i].total_time_ms
                            += time_stats.context_creation_ms
                            + time_stats.operator_creation_ms
                            + time_stats.operator_execution_ms;
#else
...
int bench(const std::string &in_filename, const std::string &out_filename,
        ...
        if (inputMode != InputMode::MODEL && inputMode != InputMode::FILE) {
            ...
            try {
                ...
                if (matmulConfig.empty()) {
                    testlog_error(
                            "Sweep expansion produced no benchmark "
                            "configurations.");
                    return NOT_OK;
                }
    ...
    if (!isLOWOHA) {
        ...
        int status = matmul_benchdnn(
                matmulConfig, matmul_results, options, cache_size);
        if (status != OK) {
        ...
        int status = matmul_lowoha_benchdnn(
                matmulConfig, matmul_results, options, cache_size);
        if (status != OK) {
    ...
    if (isPipeline) {
        ...
        if (!outfile.is_open()) {
            testlog_error(
                    "Error: Cannot write to output file ", out_filename, "\n");
            return NOT_OK;
        }
