    ...
    for (auto i = 0; i < cfg.n_values.size(); i++) {
        ...
        if (dt == data_type_t::s4 || dt == data_type_t::u4) {
            if (dt == data_type_t::s4 && cfg.src_dynamic_quant
                ...
                cfg.scale_granularity = "group";
                commonlog_warning(
                        "W4A8 requires per-group weight scales; using "
                        "per-group.");
            }
            ...
            if (scale_granularity == "tensor") {
                ...
                if (cfg.group_size == 0 || k % cfg.group_size != 0) {
                    commonlog_warning("weight group_size=", cfg.group_size,
                            " does not divide K=", k,
        ...
        if (cfg.kernel_name == "aocl_dlp_blocked" && !isLOWOHA) {
            ...
            if (reorder_size == input_buffer_size) {
                ...
                weights_tensor = tensor_factory.copy_tensor({k, n}, dt,
            ...
            if (options.ndims > 2) {
                weights_tensor = tensor_factory.uniform_dist_tensor(
                        {cfg.bs, k, n}, dt, 1.0, "weights_" + std::to_string(i),
                        cfg.isTransB);
            } else {
                auto wei_scale = (dt == data_type_t::s8 || dt == data_type_t::s4
                                         || dt == data_type_t::u4)
                        ? tensor_factory.uniform_dist_tensor(
                                  scale_size, scale_dtype, 0.2)
                        : tensor_t();
                auto wei_zp = dt == data_type_t::u4
                        ? tensor_factory.uniform_dist_tensor(
                                  scale_size, data_type_t::bf16, 0.2)
                        : tensor_t();
                weights_tensor = tensor_factory.uniform_dist_tensor({k, n}, dt,
...
int create_bias_tensor(tensor_factory_t tensor_factory, const MatmulConfig &cfg,
    ...
    if (cfg.isBiasEnabled) {
        ...
        for (auto i = 0; i < cfg.n_values.size(); i++) {
            ...
            if (options.ndims > 2) {
                bias_tensor = tensor_factory.uniform_dist_tensor(
                        {1, 1, cfg.n_values[i]}, dt, -10.0,
                        "bias_" + std::to_string(i));
            } else {
                bias_tensor = tensor_factory.uniform_dist_tensor(
                        {1, cfg.n_values[i]}, dt, -10.0,
                        "bias_" + std::to_string(i));
            }
...
int create_input_tensor(tensor_factory_t &tensor_factory, MatmulConfig &cfg,
        tensor_t &input, const global_options &options, bool isLOWOHA) {
    ...
    if (options.ndims > 2) {
        if (cfg.src_dynamic_quant) {
            commonlog_warning(
                    "src_dynamic_quant is not supported for BMM (ndims > 2). "
                    "Disabling.");
        ...
        input = tensor_factory.uniform_dist_tensor({cfg.bs, cfg.m, cfg.k},
                cfg.dt[0], 1.0, "matmul_input", cfg.isTransA);
    } else {
        tensor_t src_scale;
        tensor_t src_zp;
        ...
        if (cfg.src_dynamic_quant) {
            ...
            const bool is_w4a8_dynamic = cfg.dt[0] == data_type_t::bf16
            ...
            if (is_w8a8_dynamic || is_w4a8_dynamic) {
                ...
                if (gran == "per-tensor") {
                    ...
                    if (gs == 0) {
                        commonlog_warning(
                                "src_group_size=0 is not valid for per-group "
                                "src-scale "
                                "granularity; falling back to per-token.");
                        src_scale_dims = {M, 1};
                    } else if (K % gs != 0) {
                        commonlog_warning("src_group_size=", gs,
                                " does not divide K=", K,
                                "; falling back to per-token src-scale "
                                "granularity.");
                        src_scale_dims = {M, 1};
                    } else {
                    ...
                    commonlog_warning("Unknown src_scale_granularity '", gran,
                            "'. Falling back to per-tensor.");
                    src_scale_dims = {1, 1};
                }
                src_scale = tensor_factory.uniform_tensor(src_scale_dims,
                        cfg.src_scale_dt, 0.0f, "matmul_src_scale");
            } else {
                commonlog_warning(
                        "src_dynamic_quant=true requires W8A8 (src in {bf16, "
                        "f32}, wei=s8) "
        ...
        if (cfg.dt[0] == data_type_t::s8 || cfg.dt[0] == data_type_t::u8) {
            ...
            if (gran == "per-token") {
                ...
                const uint64_t gs = cfg.src_group_size;
                if (gs == 0 || K % gs != 0) {
                    commonlog_warning("static src_group_size=", gs,
                            " is invalid for K=", K,
                            "; falling back to per-token static src-scale "
                            "granularity.");
                    src_scale_dims = {M, 1};
                } else {
                    src_scale_dims = {M, K / gs};
            ...
            const data_type_t src_scale_dt
                    = (cfg.src_scale_dt != data_type_t::none)
                    ? cfg.src_scale_dt
                    : data_type_t::f32;
            src_scale = tensor_factory.uniform_dist_tensor(
                    src_scale_dims, src_scale_dt, 0.3);
            if (cfg.dt[0] == data_type_t::u8) {
                src_zp = tensor_factory.uniform_tensor(
                        {1, 1}, data_type_t::u8, 16);
            }
        }

        input = tensor_factory.uniform_dist_tensor({cfg.m, cfg.k}, cfg.dt[0],
...
int create_output_tensor(tensor_factory_t &tensor_factory,
    ...
    auto dst_scale = !(dt == data_type_t::f32 || dt == data_type_t::bf16
                             || dt == data_type_t::f16)
            ? tensor_factory.uniform_dist_tensor({1, 1}, data_type_t::f32, 1.2)
            : tensor_t();
    auto dst_zp = dt == data_type_t::u8
            ? tensor_factory.uniform_tensor({1, 1}, data_type_t::u8, 53)
            : tensor_t();
    for (auto i = 0; i < cfg.n_values.size(); i++) {
        size_t n = cfg.n_values[i];
        tensor_t output_tensor;
        if (options.ndims > 2) {
            output_tensor = tensor_factory.zero_tensor(
                    {cfg.bs, m, n}, dt, "matmul_output_" + std::to_string(i));
        } else {
            output_tensor = tensor_factory.zero_tensor({m, n}, dt,
                    "matmul_output_" + std::to_string(i), dst_scale, dst_zp);
        }
...
int create_binary_post_ops_tensors(tensor_factory_t &tensor_factory,
    ...
    for (auto i = 0; i < cfg.n_values.size(); i++) {
        ...
        for (const auto &post_op : cfg.binary_post_ops_pos) {
            ...
            auto binary_tensor = tensor_factory.uniform_dist_tensor(
                    {cfg.m, cfg.n_values[i]}, cfg.post_op_dt, 2.0,
                    "binary_post_op_" + std::to_string(post_op));
