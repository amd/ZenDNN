...
static void set_src_int8_params(
        matmul_params &params, const tensor_t &input_tensor) {
    if (!input_tensor.is_quantized()) { return; }
...
static void set_wei_int8_params(
        matmul_params &params, const tensor_t &weight_tensor) {
    if (!weight_tensor.is_quantized()) { return; }
...
// tensor into params. No-op when the tensor is not quantized.
static void set_dst_int8_params(
        matmul_params &params, const tensor_t &output_tensor) {
    if (!output_tensor.is_quantized()) { return; }
...
static bool is_dynamic_quant_dtype(const tensor_t &input_tensor,
    ...
    bool w4a8 = wei_dt == data_type_t::s4 && src_dt == data_type_t::bf16
            && dst_dt == data_type_t::bf16;
    return w8a8 || w4a8;
}

void set_lowoha_matmul_params(matmul_params &params, int &lda, int &ldb,
        int &ldc, const tensor_t &input_tensor, const tensor_t &weight_tensor,
        const tensor_t &output_tensor, const tensor_t &bias,
        const std::vector<tensor_t> &binary_post_ops_tensors,
    ...
    lda = cfg.isTransA ? input_tensor.get_stride(input_dim - 1)
                       : input_tensor.get_stride(input_dim - 2);
    ldb = cfg.isTransB ? weight_tensor.get_stride(weight_dim - 1)
    ...
    if (cfg.isBiasEnabled) { matmul_dtypes.bias = bias.get_data_type(); }
    ...
    const bool dyn_quant_src_dtype_match = is_dynamic_quant_dtype(
    ...
    const bool dyn_quant_src_has_scale = input_tensor.is_quantized()
            && input_tensor.get_quant_scale_raw_handle_const() != nullptr;
    bool is_dyn_quant_src = cfg.src_dynamic_quant && dyn_quant_src_dtype_match
            && dyn_quant_src_is_2d && dyn_quant_src_has_scale;
    ...
    if (cfg.src_dynamic_quant && dyn_quant_src_dtype_match
            && !is_dyn_quant_src) {
        commonlog_warning(
                "LOWOHA: disabling dynamic source quantization for this layer; "
                "source tensor is missing quant-scale metadata or has "
                "unsupported "
                "shape (ndims=",
                input_tensor.get_dim(),
                ", has_src_scale=", dyn_quant_src_has_scale ? 1 : 0, ").");
    }

    if (is_woq) { set_woq_params(params, weight_tensor); }
...
int matmul_lowoha_benchdnn(std::vector<MatmulConfig> configs,
    ...
    for (auto &cfg : configs) {
        try {
            ...
            std::vector<std::vector<tensor_t>> weights_buffer_pool(
                    num_weight_buffers);
            ...
            int ret = create_weights_tensor(
                    tensor_factory, cfg, weights_buffer_pool[0], options, true);
            if (ret != OK) {
            ...
            if (cfg.cache_mode == CacheMode::WARM) {
                ...
                if (options.num_weight_buffers == -1) {
                    int num_weight_buffers_ = (cfg.iters > 0)
                            ? cfg.iters
                            : MIN_NUM_WEIGHT_BUFFERS;
                    ...
                    if (weight_bytes > 0 && cache_size_ > 0) {
                        ...
                        num_weight_buffers = (cache_size_ + weight_bytes - 1)
                                        / weight_bytes
                                + 1;
                        num_weight_buffers = std::max(
                                num_weight_buffers, MIN_NUM_WEIGHT_BUFFERS);
                    } else {
                        num_weight_buffers = num_weight_buffers_;
                    }
                    num_weight_buffers
                            = std::min(num_weight_buffers, num_weight_buffers_);
                } else {
                    // User-specified count; cannot use more buffers than benchmark iterations.
                    if (cfg.iters > 0
                            && options.num_weight_buffers > cfg.iters) {
                        num_weight_buffers = cfg.iters;
                        commonlog_warning("num_weight_buffers (",
                                options.num_weight_buffers, ") exceeds iters (",
                                cfg.iters, "); using ", num_weight_buffers);
                    } else {
                ...
                for (size_t j = 1; j < weights_buffer_pool.size(); j++) {
                    ret = create_weights_tensor(tensor_factory, cfg,
                            weights_buffer_pool[j], options, true);
                    if (ret != OK) {
                        testlog_error("create_weights_tensor failed");
                        log_benchmark_failure(cfg);
                        skip = true;
            ...
            ret = create_output_tensor(
                    tensor_factory, cfg, output_tensor, options);
            if (ret != OK) {
            ...
            ret = create_binary_post_ops_tensors(
                    tensor_factory, cfg, binary_post_ops_tensors);
            if (ret != OK) {
            ...
            const bool use_perf
                    = options.perf_counters && perf_ctrs.is_available();
            if (use_perf) {
                if (!perf_ctrs.open()) {
                    commonlog_warning(
                            "HW perf counters unavailable; continuing without "
                            "them.");
                }
            }

            if (cfg.cache_mode == CacheMode::WARM) { flush_cache(cache_size); }
            // warm-up iterations
            for (auto j = 0; j < cfg.warmup_iters && !skip; j++) {
                for (auto i = 0; i < cfg.n_values.size(); i++) {
                    ...
                    set_lowoha_matmul_params(params, lda, ldb, ldc,
                            input_tensor_, weight_tensor_, output_tensor_,
                            bias[i], binary_post_ops_tensors[i], cfg);
                    ...
                    status_t status = matmul_direct('r', // layout: row-major
                            cfg.isTransA, cfg.isTransB, static_cast<int>(M),
                            static_cast<int>(N), static_cast<int>(K), alpha,
                            A_data, lda, B_data, ldb, bias_data, beta, C_data,
                            ldc, is_weights_const, batch_params, params);
            ...
            for (auto j = 0; j < cfg.iters && !skip; j++) {
                ...
                for (auto i = 0; i < cfg.n_values.size(); i++) {
                    ...
                    auto weight_tensor_ = (cfg.cache_mode == CacheMode::WARM)
                            ? weights_buffer_pool[(1 + j) % num_weight_buffers]
                                                 [i]
                    ...
                    TimingStats time_stats;
                    status_t status = matmul_direct('r', cfg.isTransA,
                            cfg.isTransB, static_cast<int>(M),
                            static_cast<int>(N), static_cast<int>(K), alpha,
                            A_data, lda, B_data, ldb, bias_data, beta, C_data,
                            ldc, is_weights_const, batch_params, params);
                    if (status != status_t::success) {
