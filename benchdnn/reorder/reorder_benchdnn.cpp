...
int run_reorder(tensor_t input_tensor, const ReorderConfig &cfg,
    ...
    if (!isNotWarmup) {
#endif
        auto reorder_context
                = reorder_context_t().set_algo_format(cfg.kernel_name).create();
        ...
        if (cfg.isInplace) {
            ...
            if (reorder_size == input_buffer_size) {
                ...
                auto output_tensor = tensor_factory.copy_tensor(
                        {cfg.rows, cfg.cols}, cfg.dt, buffer_params, false,
                        true, "reorder_output");
                ...
                testlog_error(
                        "Inplace reorder is not possible for given input");
                return NOT_OK;
            }
        } else {
            ...
            auto output_tensor
                    = tensor_factory.copy_tensor({cfg.rows, cfg.cols}, cfg.dt,
                            buffer_params, false, true, "reorder_output");
        ...
        double context_creation_time
                = (std::chrono::duration<double, std::milli>(
                        end_context_creation - start_context_creation)
                                .count());
        double operator_creation_time
                = (std::chrono::duration<double, std::milli>(
...
int reorder_benchdnn(const std::vector<ReorderConfig> &configs,
    ...
    for (const auto &cfg : configs) {
        try {
            ...
            int ret = create_src_tensor(
                    tensor_factory, cfg, input_tensor, false);
            if (ret != OK) {
            ...
            elapsed_ms = time_stats.context_creation_ms
                    ...
                    + time_stats.operator_execution_ms + time_stats.other_ms;
#endif
...
int bench(const std::string &in_filename, const std::string &out_filename,
    ...
    if (!isLOWOHA) {
        status = reorder_benchdnn(
                reorderConfig, reorder_results, options, cache_size);
        if (status != OK) {
            testlog_error("Reorder benchmark failed.");
            return NOT_OK;
        }
    } else {
        status = reorder_lowoha_benchdnn(
                reorderConfig, reorder_results, options, cache_size);
        if (status != OK) {
