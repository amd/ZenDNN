...
std::vector<int64_t> compute_quant_dims(const ReorderConfig &cfg) {
    ...
    if (cfg.scale_granularity == "per_tensor") {
        ...
        commonlog_warning("Invalid group_size ", group_size,
    ...
    return is_3d ? std::vector<int64_t> {1, 1, 1} : std::vector<int64_t> {1, 1};
}

int create_src_tensor(tensor_factory_t &tensor_factory,
        const ReorderConfig &cfg, tensor_t &src, bool is_lowoha) {
    if (!is_lowoha) {
        src = tensor_factory.uniform_tensor(
                {cfg.rows, cfg.cols}, cfg.dt, 1.0, "reorder_input");
    } else {
        if (cfg.batch_size > 1) {
            src = tensor_factory.uniform_dist_tensor(
                    {cfg.batch_size, cfg.rows, cfg.cols}, cfg.src_dtype, 1.0,
                    "reorder_src");
        } else {
            src = tensor_factory.uniform_dist_tensor(
                    {cfg.rows, cfg.cols}, cfg.src_dtype, 1.0, "reorder_src");
        }
    }
    if (!src.check()) {
...
int create_dst_tensor(tensor_factory_t &tensor_factory,
    ...
    if (cfg.batch_size > 1) {
        dst = tensor_factory.uniform_tensor(
                {cfg.batch_size, cfg.rows, cfg.cols}, cfg.dst_dtype, 0,
                "reorder_dst");
    } else {
        dst = tensor_factory.uniform_tensor(
                {cfg.rows, cfg.cols}, cfg.dst_dtype, 0, "reorder_dst");
    }
    if (!dst.check()) {
