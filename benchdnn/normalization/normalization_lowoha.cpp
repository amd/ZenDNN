...
int normalization_lowoha_benchdnn(std::vector<NormalizationConfig> configs,
    ...
    for (const auto &cfg : configs) {
        try {
            ...
            ret = create_running_mean_tensor(
                    tensor_factory, cfg, running_mean_tensor);
            if (ret != OK) {
            ...
            ret = create_running_var_tensor(
                    tensor_factory, cfg, running_var_tensor);
            if (ret != OK) {
            ...
            const void *gamma_data = cfg.use_scale
                    ? gamma_tensor.get_raw_handle_const()
                    : nullptr;
            const void *beta_data
                    = (cfg.use_shift && cfg.norm_type != "rms_norm"
                              && cfg.norm_type != "fused_add_rms_norm")
                    ? beta_tensor.get_raw_handle_const()
                    : nullptr;
            const void *running_mean_data = (cfg.norm_type == "batch_norm")
                    ? running_mean_tensor.get_raw_handle_const()
                    : nullptr;
            ...
            for (int i = 0; i < cfg.warmup_iters && !skip; ++i) {
                status_t status = normalization_direct(input_data, output_data,
                        gamma_data, beta_data, running_mean_data,
                        running_var_data, residual_data, params);
                if (status != status_t::success) {
                    testlog_error(
                            "LOWOHA: Normalization execution failed during "
                            "warm-up.");
                    skip = true;
                    break;
                }
