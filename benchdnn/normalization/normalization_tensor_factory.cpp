...
int create_input_tensor(tensor_factory_t &tensor_factory,
    ...
    input = tensor_factory.uniform_dist_tensor(
            flat_shape(cfg), cfg.src_dt, 2.0f, "input_tensor");
    return OK;
}
...
int create_output_tensor(tensor_factory_t &tensor_factory,
    ...
    output = tensor_factory.zero_tensor(
            flat_shape(cfg), cfg.dst_dt, "output_tensor");
    return OK;
}
...
int create_gamma_tensor(tensor_factory_t &tensor_factory,
    ...
    uint64_t param_size = (cfg.norm_type == "batch_norm") ? cfg.num_channels
                                                          : cfg.norm_size;
    gamma = tensor_factory.uniform_dist_tensor(
            {param_size}, cfg.gamma_dt, 1.0f, "gamma_tensor");
    return OK;
}

int create_beta_tensor(tensor_factory_t &tensor_factory,
        const NormalizationConfig &cfg, tensor_t &beta) {
    if (!cfg.use_shift || cfg.norm_type == "rms_norm"
    ...
    uint64_t param_size = (cfg.norm_type == "batch_norm") ? cfg.num_channels
                                                          : cfg.norm_size;
    beta = tensor_factory.uniform_dist_tensor(
            {param_size}, cfg.beta_dt, 1.0f, "beta_tensor");
    return OK;
}
...
int create_running_mean_tensor(tensor_factory_t &tensor_factory,
    ...
    running_mean = tensor_factory.uniform_dist_tensor(
            {cfg.num_channels}, data_type_t::f32, 2.0f, "running_mean_tensor");
    return OK;
}
...
int create_running_var_tensor(tensor_factory_t &tensor_factory,
    ...
    running_var = tensor_factory.uniform_dist_tensor(
            {cfg.num_channels}, data_type_t::f32, 1.0f, "running_var_tensor");

    float *var_ptr = static_cast<float *>(running_var.get_raw_handle_unsafe());
    if (!var_ptr) {
