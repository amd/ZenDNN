...
if (ZENDNNL_DEPENDS_FBGEMM)
  ...
  if (NOT ZENDNNL_FBGEMM_INJECTED)
    ...
    if (ZENDNNL_LOCAL_FBGEMM)
      ...
      message(WARNING "${ZENDNNL_MSG_PREFIX}FBGEMM will be built from local source at ${FBGEMM_ROOT_DIR}. This version may not be fully compatible with ZenDNN. If unsure, it is recommended to use the standard ZenDNN build.")
      message(DEBUG "${ZENDNNL_MSG_PREFIX}Will use local FBGEMM from ${FBGEMM_ROOT_DIR}")

      set(FBGEMM_DEPS_LINK "${ZENDNNL_DEPS_DIR}/fbgemm")
      if(NOT "${FBGEMM_ROOT_DIR}" STREQUAL "${FBGEMM_DEPS_LINK}")
        if(EXISTS "${FBGEMM_DEPS_LINK}")
          if(IS_SYMLINK "${FBGEMM_DEPS_LINK}")
            file(REMOVE "${FBGEMM_DEPS_LINK}")
          else()
            message(WARNING "${ZENDNNL_MSG_PREFIX}${FBGEMM_DEPS_LINK} exists and is not a symlink. Skipping symlink creation. Remove it manually if needed.")
          endif()
        endif()
        if(NOT EXISTS "${FBGEMM_DEPS_LINK}")
          execute_process(COMMAND ${CMAKE_COMMAND} -E create_symlink
            "${FBGEMM_ROOT_DIR}" "${FBGEMM_DEPS_LINK}")
          message(STATUS "${ZENDNNL_MSG_PREFIX}Created symlink ${FBGEMM_DEPS_LINK} -> ${FBGEMM_ROOT_DIR}")
        endif()
      endif()
    ...
    message(WARNING "${ZENDNNL_MSG_PREFIX}FBGEMM will be injected from ${ZENDNNL_FBGEMM_INJECT_DIR}. This version may not be fully compatible with ZenDNN. If unsure, it is recommended to use the standard ZenDNN build.")
    set(SYMLNK_DST "${CMAKE_INSTALL_PREFIX}/deps/fbgemm")
    set(SYMLNK_SRC "${ZENDNNL_FBGEMM_INJECT_DIR}")
