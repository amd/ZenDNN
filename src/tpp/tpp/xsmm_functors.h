...
using bfloat16 = zendnn::tpp::bfloat16;
using half     = zendnn::tpp::half;
using bfloat8  = zendnn::tpp::bfloat8;
...
inline bfloat16 downconvert_to_bfloat16(float val) {
  libxsmm_bfloat16_f32 fc;
  bfloat16             r;
...
/* debug-ss - converting from matrix_eqn to meqn */
...
class BrgemmTPP {
 public:
  ...
  void operator()(
    ...
    /* debug-ss calling refernce implementation */
    if (!no_tile_cfg) {
  ...
  class BrgemmKernel : public BaseTPP {
   public:
    ...
    void operator()(libxsmm_gemm_param* gemm_param) {
      ...
      /* debug-ss - NULL is not accepted by libxsmm */
      if(NULL == gemm_param)
          return;
 ...
 public:
  ...
  void ref(
    ...
    for (int64_t i = 0; i < sz; i++) {
      ...
      /* debug-ss type conversion */
      // float avg_i = exp_avg[i];
      // float avg_sq_i = exp_avg_sq[i];
      // float grad_i = grad[i];

      float avg_i = upconvert_to_float(exp_avg[i]);
      float avg_sq_i = upconvert_to_float(exp_avg_sq[i]);
      float grad_i = upconvert_to_float(grad[i]);
      ...
      data_hp.i[0] = lo[i].data;
      data_hp.i[1] = hi[i].data;
      ...
      if (weight_decay > 0.0)
        data_i = data_i - data_i * lr * weight_decay;
      exp_avg[i] = downconvert_to_bfloat16(avg_i);
      exp_avg_sq[i] = downconvert_to_bfloat16(avg_sq_i);
      data_hp.f = data_i;
      lo[i].data = data_hp.i[0];
      hi[i].data = data_hp.i[1];
    }
#else
    auto vbeta1 = _mm512_set1_ps(beta1);
    auto vbeta1_1 = _mm512_set1_ps(beta1_1);
