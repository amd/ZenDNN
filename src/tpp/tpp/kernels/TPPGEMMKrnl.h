...
template <typename T>
inline zendnn::tpp::ZenTensor wt_tensor_for_first_token(zendnn::tpp::ZenTensor& t) {
...
inline void tpp_linear_bias(
  ...
  auto t_wt_V = zendnn::tpp::wt_tensor_for_fwd(Nk, Hk, Nc, Hc, t_wt_);
...

  {
    ...
    auto ogemm_loop = zendnn::tpp::ThreadedLoop<3>(
...
inline void tpp_linear_no_bias(
...

  {
    //RECORD_SCOPE(tpp_linear_krnl, {t_in, t_wt_V});
    auto loop_scheme = large_cache_opt ? GEMM_LOOP_SCHEME : "aCb";
    auto gemm_loop = zendnn::tpp::ThreadedLoop<3>(
    ...
    gemm_loop(
        [&](int* ind) {
          ...
          if (!is_rem) {
            ...
            brgemm_tpp(in[s1][nc], wt_V[nk][nc], out[s1][nk], count, false);
          } else {
            if (nc == 0) {
              zero_tpp_rem(out[s1][nk]);
            }
...
inline void tpp_linear_gelu(
...

  {
    ...
    auto loop_scheme = large_cache_opt ? GEMM_LOOP_SCHEME : "aCb";
    auto igemm_loop = zendnn::tpp::ThreadedLoop<3>(
        {{0, Nc, Ncb, false}, {0, BS, BSb}, {Nk}}, loop_scheme);
    igemm_loop(
        [&](int* ind) {
          int nc = ind[0], s1 = ind[1], nk = ind[2];
...
inline void tpp_fused_gate_up_proj(
  ...
  auto t_wt_gate_V =
      zendnn::tpp::wt_tensor_for_fwd(Nk, Hk, Nc, Hc, t_wt_gate_);
  auto t_wt_up_V = zendnn::tpp::wt_tensor_for_fwd(Nk, Hk, Nc, Hc, t_wt_up_);

  // This is used to store the intermediate result of the up_proj layer
  auto t_out_tmp = zendnn::tpp::empty_like(t_out);
