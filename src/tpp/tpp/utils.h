...
template <typename T>
inline T* pt_get_data_ptr(zendnn::tpp::ZenTensor t) {
  if (!t.is_contiguous()) {
    zendnnInfo(ZENDNN_ALGOLOG, "tensor is not contiguous");
  }
  return t.data_ptr<T>();
}

template <typename T, std::size_t N> //, typename index_t = int64_t>
VLAPtr<T, N, index_t> GetVLAPtr(zendnn::tpp::ZenTensor t, const index_t (&sizes)[N]) {
  if (!t.is_defined()) {
...
template <typename T>
T* GetVLAPtr(zendnn::tpp::ZenTensor t) {
