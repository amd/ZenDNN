...
struct sum_pd_t : public primitive_desc_t {
    ...
    arg_usage_t arg_usage(int arg) const override {
        if (arg >= ZENDNN_ARG_MULTIPLE_SRC
                && arg < ZENDNN_ARG_MULTIPLE_SRC + n_inputs())
    ...
    const memory_desc_t *arg_md(int arg) const override {
        int src_index = arg - ZENDNN_ARG_MULTIPLE_SRC;
        if (src_index >= 0 && src_index < n_inputs()) return src_md(src_index);
        if (arg == ZENDNN_ARG_DST) return dst_md(0);
    ...
    bool need_output_reorder() const { return dst_md()->data_type != zendnn_f32; }
...
protected:
    ...
    virtual void define_dst_acc_md() {
        dst_acc_md_ = dst_md_;
        dst_acc_md_.data_type = zendnn_f32;
    }
