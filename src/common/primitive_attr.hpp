...
struct arg_scales_t : public c_compatible {
    arg_scales_t() {
        for (const auto &sa : {ZENDNN_ARG_SRC_0, ZENDNN_ARG_SRC_1}) {
            set(sa, 1.f);
        }
    }
...
struct zero_points_t : public c_compatible {
    ...
    const int *get(int arg) const {
        ...
        switch (arg) {
            case ZENDNN_ARG_SRC: return &zero_point_src;
            case ZENDNN_ARG_WEIGHTS: return &zero_point_wei;
            case ZENDNN_ARG_DST: return &zero_point_dst;
        }
        static int zero = 0;
        return &zero;
    }
...
private:
    ...
    int get_mask(int arg) const {
        int mask = 0;
        switch (arg) {
            case ZENDNN_ARG_SRC: mask = mask_src; break;
            case ZENDNN_ARG_WEIGHTS: mask = mask_wei; break;
            case ZENDNN_ARG_DST: mask = mask_dst; break;
            default: mask = 0;
        }
        return mask;
    }
