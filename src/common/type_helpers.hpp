...
extern memory_desc_t ZENDNN_API glob_zero_md;
...
inline bool blocking_desc_is_equal(const memory_desc_t &lhs_md,
    ...
    using zendnn::impl::utils::array_cmp;
...
inline bool operator==(
    ...
    bool ret = COMPARE_DESC_MEMBERS(primitive_kind)
            ...
            && COMPARE_DESC_ARRAY_MEMBERS(strides, ZENDNN_MAX_NDIMS)
            && COMPARE_DESC_ARRAY_MEMBERS(dilates, ZENDNN_MAX_NDIMS)
            && COMPARE_DESC_ARRAY_MEMBERS(padding[0], ZENDNN_MAX_NDIMS)
            && COMPARE_DESC_ARRAY_MEMBERS(padding[1], ZENDNN_MAX_NDIMS)
...
inline bool operator==(const pooling_desc_t &lhs, const pooling_desc_t &rhs) {
    bool ret = COMPARE_DESC_MEMBERS(primitive_kind)
            ...
            && COMPARE_DESC_MEMBERS(diff_dst_desc)
            && COMPARE_DESC_ARRAY_MEMBERS(strides, ZENDNN_MAX_NDIMS)
            && COMPARE_DESC_ARRAY_MEMBERS(kernel, ZENDNN_MAX_NDIMS)
            && COMPARE_DESC_ARRAY_MEMBERS(padding[0], ZENDNN_MAX_NDIMS)
            && COMPARE_DESC_ARRAY_MEMBERS(padding[1], ZENDNN_MAX_NDIMS)
            && COMPARE_DESC_MEMBERS(accum_data_type);
    return ret;
...
inline bool operator==(
    ...
    bool ret = v1_desc_lhs == v1_desc_rhs
            && COMPARE_DESC_ARRAY_MEMBERS(dilation, ZENDNN_MAX_NDIMS);
     return ret;
}
...
inline bool operator==(
    ...
    bool ret = COMPARE_DESC_MEMBERS(primitive_kind)
            ...
            && COMPARE_DESC_MEMBERS(dst_desc)
            && COMPARE_DESC_MEMBERS(diff_dst_desc)
            && COMPARE_FLOAT_DESC_ARRAY_MEMBERS(factors, ZENDNN_MAX_NDIMS);
    return ret;
}
...
inline bool memory_desc_strides_check(
    ...
    for (int d = 0; d < md.ndims; ++d) {
        ...
        const bool has_runtime_dim = utils::one_of(
                ZENDNN_RUNTIME_DIM_VAL, strides[d], md.padded_dims[d]);
...
inline status_t memory_desc_init_by_strides(
    ...
    return zendnn_memory_desc_init_by_strides(
...
inline status_t memory_desc_init_by_tag(
    ...
    status_t status = zendnn_memory_desc_init_by_tag(
...
inline status_t memory_desc_init_by_blocking_desc(
    ...
    const int ndims = nstl::min(ZENDNN_MAX_NDIMS, md.ndims); // make GCC 5 happy
    ...
    utils::simultaneous_sort(
            mblk.strides, ou_blocks, perm, ndims, [](stride_t a, stride_t b) {
                if (utils::one_of(ZENDNN_RUNTIME_DIM_VAL, a, b))
 ...
 * assumed (i.e. the one that zendnn_memory_desc_init_by_tag() returns).
 * Strides might contain `0` value, indicating the stride must match the one
 * that zendnn_memory_desc_init_by_tag() returns.
...
/** returns true if fp32 value denotes ZENDNN_RUNTIME_F32_VAL */
inline bool is_runtime_value(float val) {
    return utils::bit_cast<unsigned>(val) == ZENDNN_RUNTIME_F32_VAL_REP.u;
}

/** returns true if s32 value denotes ZENDNN_RUNTIME_S32_VAL */
inline bool is_runtime_value(int val) {
    return val == ZENDNN_RUNTIME_S32_VAL;
}

/** returns true if dim_t value denotes ZENDNN_RUNTIME_DIM_VAL */
inline bool is_runtime_value(dim_t val) {
    return val == ZENDNN_RUNTIME_DIM_VAL;
}
...
inline bool memory_desc_sanity_check(int ndims, const dims_t dims,
    ...
    for (int d = 0; d < ndims; ++d) {
        if (dims[d] != ZENDNN_RUNTIME_DIM_VAL && dims[d] < 0) return false;
        if (dims[d] == ZENDNN_RUNTIME_DIM_VAL) has_runtime_dims = true;
    }
