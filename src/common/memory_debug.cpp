...
static inline int num_protect_pages() {
    if (is_mem_debug())
        return ZENDNN_MEM_DEBUG_PROTECT_SIZE;
    else
        return 0;
}
...
void *malloc(size_t size, int alignment) {
    ...
    if (rc == 0) {
        ...
        protect_buffer(ptr, buffer_size, engine_kind_t::zendnn_cpu);
    }
...
void free(void *p) {
    if (p != nullptr) {
        ...
        assert(status == 0);
        unprotect_buffer(p, tag->buffer_size, engine_kind_t::zendnn_cpu);
...
void protect_buffer(void *addr, size_t size, engine_kind_t engine_kind) {
    if (engine_kind != engine_kind_t::zendnn_cpu)
