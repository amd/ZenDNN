...
void serialize_desc(
    ...
    sstream.write(desc.strides, ZENDNN_MAX_NDIMS);
    sstream.write(desc.dilates, ZENDNN_MAX_NDIMS);
    sstream.write(desc.padding[0], ZENDNN_MAX_NDIMS);
    sstream.write(desc.padding[1], ZENDNN_MAX_NDIMS);
    // Accumulator type
...
void serialize_desc(
    ...
    // Strides, dilates, padding
    sstream.write(desc.strides, ZENDNN_MAX_NDIMS);
    sstream.write(desc.kernel, ZENDNN_MAX_NDIMS);
    sstream.write(desc.padding[0], ZENDNN_MAX_NDIMS);
    sstream.write(desc.padding[1], ZENDNN_MAX_NDIMS);
    // Accumulator type
    sstream.write(&desc.accum_data_type);
...
void serialize_desc(
    ...
    sstream.write(desc.dilation, ZENDNN_MAX_NDIMS);
}
...
void serialize_desc(
    ...
    serialize_md(sstream, desc.diff_dst_desc);
    // Factors
    sstream.write(desc.factors, ZENDNN_MAX_NDIMS);
}
