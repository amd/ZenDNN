...
bool is_per_mb_w_bcast(const std::bitset<ZENDNN_MAX_NDIMS> mask,
...
bool is_per_w_bcast(const std::bitset<ZENDNN_MAX_NDIMS> mask,
        const memory_desc_wrapper &dst_d) {
    const auto ndims = dst_d.ndims();
    const int last_dim = ndims - 1;
...
bool is_channel_bcast(const std::bitset<ZENDNN_MAX_NDIMS> mask,
        const memory_desc_wrapper &dst_d) {
...
bool is_per_oc_bcast(const std::bitset<ZENDNN_MAX_NDIMS> mask,
...
broadcasting_strategy_t get_rhs_arg_broadcasting_strategy(
    ...
    bool all_ones = true;
    bool all_equal = true;
    std::bitset<ZENDNN_MAX_NDIMS> mask(0);
