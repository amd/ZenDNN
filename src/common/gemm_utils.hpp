...
static inline void init_2d_desc(memory_desc_t *md_2d,
    ...
    if (transpose_dims) {
        zendnn_dims_t dims_2d = {p_dim1, p_dims[0]};
        zendnn_dims_t strides_2d = {stride1, strides[0]};
        zendnn_memory_desc_init_by_strides(
                md_2d, 2, dims_2d, md_nd->data_type, strides_2d);
    } else {
        zendnn_dims_t dims_2d = {p_dims[0], p_dim1};
        zendnn_dims_t strides_2d = {strides[0], stride1};
        zendnn_memory_desc_init_by_strides(
                md_2d, 2, dims_2d, md_nd->data_type, strides_2d);
    }
}
...
static inline void create_2d_desc(memory_desc_t *md_2d, int d0, int d1,
    ...
    zendnn_dims_t dims_2d = {d0, d1};
    if (trans == transpose::notrans) {
        zendnn_dims_t strides_2d = {ld, 1};
        zendnn_memory_desc_init_by_strides(md_2d, 2, dims_2d, dt, strides_2d);
    } else {
        zendnn_dims_t strides_2d = {1, ld};
        zendnn_memory_desc_init_by_strides(md_2d, 2, dims_2d, dt, strides_2d);
    }
}
