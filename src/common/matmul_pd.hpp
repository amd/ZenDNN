...
struct matmul_pd_t : public primitive_desc_t {
    ...
    const matmul_desc_t *desc() const {
        return &desc_;
    }
    ...
    dim_t M() const {
        return dst_md_.dims[ndims() - 2];
    }
    dim_t N() const {
        return dst_md_.dims[ndims() - 1];
    }
    dim_t K() const {
        return src_md_.dims[ndims() - 1];
    }
    ...
    virtual bool attr_scales_ok() const {
        const std::vector<int> &supported_args
            = {ZENDNN_ARG_SRC, ZENDNN_ARG_WEIGHTS, ZENDNN_ARG_DST};
        if (attr()->static_scales_.has_default_values()) {
            return true;
        }
