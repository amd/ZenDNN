...
//    hence return ZENDNN_RUNTIME_SIZE_VAL
...
size_t memory_desc_map_size(const memory_desc_t *md) {
    ...
    if (mdw.has_runtime_dims_or_strides()) return ZENDNN_RUNTIME_SIZE_VAL;
...
zendnn_memory::zendnn_memory(zendnn::impl::engine_t *engine,
...
status_t zendnn_memory_desc_init_by_strides(memory_desc_t *memory_desc, int ndims,
    ...
    if (strides == nullptr) {
        ...
        for (int d = md.ndims - 2; d >= 0; --d) {
            if (md.padded_dims[d] == ZENDNN_RUNTIME_DIM_VAL)
                has_runtime_strides = true;
            default_strides[d] = has_runtime_strides
                    ? ZENDNN_RUNTIME_DIM_VAL
...
status_t zendnn_memory_desc_init_submemory(memory_desc_t *md,
    ...
    for (int d = 0; d < src_d.ndims(); ++d) {
        if (utils::one_of(ZENDNN_RUNTIME_DIM_VAL, dims[d], offsets[d]))
    ...
    zendnnInfo(ZENDNN_CORELOG, "Memory desc init submemory [memory]");
    *md = dst_d;

    return success;
}

int zendnn_memory_desc_equal(const memory_desc_t *lhs, const memory_desc_t *rhs) {
    if (lhs == rhs) return 1;
    if (any_null(lhs, rhs)) return 0;
    return memory_desc_wrapper(*lhs) == memory_desc_wrapper(*rhs);
}

status_t zendnn_memory_desc_reshape(memory_desc_t *out_md,
        const memory_desc_t *in_md, int ndims, const dims_t dims) {
    auto volume = [](const dim_t *dims, int ndims) -> dim_t {
        dim_t prod = 1;
        for (int i = 0; i < ndims; ++i) {
            if (dims[i] == ZENDNN_RUNTIME_DIM_VAL) return ZENDNN_RUNTIME_DIM_VAL;
    ...
    if (in_md->format_kind == format_kind::any)
        return zendnn_memory_desc_init_by_tag(
...
status_t zendnn_memory_desc_permute_axes(
...
size_t zendnn_memory_desc_get_size(const memory_desc_t *md) {
    if (md == nullptr) return 0;
    return memory_desc_wrapper(*md).size();
}

size_t zendnn_data_type_size(zendnn_data_type_t data_type) {
...
status_t zendnn_memory_create(memory_t **memory, const memory_desc_t *md,
    ...
    unsigned flags = (handle == ZENDNN_MEMORY_ALLOCATE)
            ? memory_flags_t::alloc
            : memory_flags_t::use_runtime_ptr;
    void *handle_ptr = (handle == ZENDNN_MEMORY_ALLOCATE) ? nullptr : handle;
...
status_t zendnn_memory_get_engine(const memory_t *memory, engine_t **engine) {
    if (any_null(memory, engine)) return invalid_arguments;
    *engine = memory->engine();
    return success;
}

status_t zendnn_memory_get_data_handle(const memory_t *memory, void **handle) {
...
status_t zendnn_memory_set_data_handle(memory_t *memory, void *handle) {
    return zendnn_memory_set_data_handle_v2(memory, handle, nullptr);
}

status_t zendnn_memory_set_data_handle_v2(
...
status_t zendnn_memory_map_data(const memory_t *memory, void **mapped_ptr) {
    ...
    if (map_size == 0) {
        *mapped_ptr = nullptr;
        return success;
    } else if (map_size == ZENDNN_RUNTIME_SIZE_VAL) {
        return invalid_arguments;
    }
...
status_t zendnn_memory_unmap_data(const memory_t *memory, void *mapped_ptr) {
...
status_t zendnn_memory_destroy(memory_t *memory) {
    delete memory;
    return success;
}
