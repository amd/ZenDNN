...
int zendnn_md2fmt_str(
        char *str, size_t str_len, const zendnn_memory_desc_t *mdesc) {
    ...
    if (mdesc == nullptr) {
        DPRINT("%s::%s::", zendnn_dt2str(data_type::undef),
                zendnn_fmt_kind2str(format_kind::undef));
        return written_len;
    }

    memory_desc_wrapper md(mdesc);

    DPRINT("%s:", zendnn_dt2str(md.data_type()));
    ...
    DPRINT("%s:", zendnn_fmt_kind2str(md.format_kind()));
