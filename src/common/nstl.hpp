...
// time.
void ZENDNN_WEAK *malloc(size_t size, int alignment);
// Use glibc malloc (and, consequently, free) when ZENDNN_ENABLE_MEM_DEBUG is
...
// Rationale: ZENDNN needs numeric limits implementation that does not
...
// Rationale: ZENDNN needs container implementations that do not generate
...
// burn: this is a bug in ZENDNN and throwing an exception would not have been
// recoverable.
//
...
// caller. The situation is probably also not recoverable also, but ZENDNN
