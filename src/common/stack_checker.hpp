...
#if defined(ZENDNN_ENABLE_STACK_CHECKER)
 ...
 * for `zendnn_primitive_create`, `zendnn_primitive_execute` and GEMM APIs that
 ...
 * - ZENDNN_SC_STACK_SIZE: specifies the size of the stack in bytes for the thread
 *   that runs a function that needs to be checked.
 *   The default is 8388608 bytes (8 MiB).
 *
 * - ZENDNN_SC_SOFT_STACK_LIMIT: specifies a soft limit in memory pages. When
 ...
 *   pages (20480 bytes).
 *
 * - ZENDNN_SC_HARD_STACK_LIMIT: specifies a hard limit in memory pages. When
 ...
 *   for debug purposes.
 *
 * - ZENDNN_SC_TRACE: enables tracing. If the soft limit is exceeded and the
 ...
 * be enabled via CMake option `ZENDNN_ENABLE_STACK_CHECKER=ON` at the build time.
 *
 * Usage example
 *
 * ```cpp
...
private:
    ...
    static size_t get_stack_size() {
        ...
        if (stack_size % get_page_size() != 0) {
            printf("Stack checker: ZENDNN_SC_STACK_SIZE is expected to be "
