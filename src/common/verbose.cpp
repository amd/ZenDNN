...
std::ostream &operator<<(std::ostream &ss, engine_kind_t eng_kind) {
    ss << zendnn_engine_kind2str(eng_kind);
    return ss;
}
...
std::string md2fmt_tag_str(const zendnn_memory_desc_t *md) {
    ...
    char dim_chars[ZENDNN_MAX_NDIMS + 1];
...
std::ostream &operator<<(std::ostream &ss, const primitive_attr_t *attr) {
    ...
    if (spm != scratchpad_mode_t::zendnn_scratchpad_mode_library) {
        ss << "attr-scratchpad:" << zendnn_scratchpad_mode2str(spm) << " ";
    }
    const fpmath_mode_t &fpm = attr->fpmath_mode_;
    if (fpm != fpmath_mode_t::zendnn_fpmath_mode_strict) {
...
static std::string init_info_rnn(const engine_t *e, const pd_t *pd) {
    ...
    // output
    print_tensor(true, ZENDNN_ARG_SRC_LAYER, "src_layer");
    print_tensor(pd->with_src_iter(), ZENDNN_ARG_SRC_ITER, "src_iter");
    ...
    ss << pd->attr() << ",";
    ss << "alg:" << pd->cell_kind()
       << " direction:" << zendnn_rnn_direction2str(pd->direction())
