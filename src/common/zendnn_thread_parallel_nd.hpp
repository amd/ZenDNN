...
#ifndef COMMON_ZENDNN_THREAD_PARALLEL_ND_HPP
#define COMMON_ZENDNN_THREAD_PARALLEL_ND_HPP

/* This header must be included by zendnn_thread.hpp only */

/* Functions:
 *  - parallel(nthr, f)                  - executes f in parallel using at
 *                                         most nthr threads. If nthr equals
 *                                         0 zendnn_get_max_threads() threads
 *                                         is used
...
inline int adjust_num_threads(int nthr, size_t work_amount) {
    if (nthr == 0) nthr = zendnn_get_current_num_threads();
...
#pragma omp parallel num_threads(nthr)
    ...
    if (!tp || zendnn_in_parallel()) {
        ...
        bool async = tp->get_flags()
                & zendnn::threadpool_interop::threadpool_iface::ASYNCHRONOUS;
...
void parallel_nd(const T0 &D0, F f) {
    const size_t work_amount = (size_t)D0;
    int nthr = adjust_num_threads(zendnn_get_current_num_threads(), work_amount);
    if (nthr)
