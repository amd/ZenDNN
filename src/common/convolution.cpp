...
status_t conv_desc_init(convolution_desc_t *conv_desc, prop_kind_t prop_kind,
    ...
    cd.reluFused = reluFused;
    cd.batchNormFused = batchNormFused;
    ...
    cd.batchNormScale_desc = zero_md();
    cd.batchNormMean_desc = zero_md();
    cd.batchNormOffset_desc = zero_md();
    ...
    const bool with_batchNormScale =
        batchNormScale_desc && batchNormScale_desc->format_kind != format_kind::undef;
    const bool with_batchNormMean =
        batchNormMean_desc && batchNormMean_desc->format_kind != format_kind::undef;
    const bool with_batchNormOffset =
        batchNormOffset_desc && batchNormOffset_desc->format_kind != format_kind::undef;
    ...
    //Conv Fusion
    if (with_batchNormScale) {
        cd.batchNormScale_desc = *batchNormScale_desc;
    }
    if (with_batchNormMean) {
        cd.batchNormMean_desc = *batchNormMean_desc;
    }
    if (with_batchNormOffset) {
        cd.batchNormOffset_desc = *batchNormOffset_desc;
    }
...
status_t zendnn_convolution_forward_desc_init(convolution_desc_t *conv_desc,
...
status_t zendnn_fused_convolution_forward_desc_init(convolution_desc_t *conv_desc,
        prop_kind_t prop_kind, alg_kind_t alg_kind,
        const memory_desc_t *src_desc, const memory_desc_t *weights_desc,
        const memory_desc_t *bias_desc, const memory_desc_t *dst_desc,
        const dims_t strides, const dims_t padding_l, const dims_t padding_r,
    ...
    return zendnn::impl::conv_desc_init(conv_desc, prop_kind, alg_kind, src_desc,
                                        ...
                                        reluFused, batchNormFused,
                                        batchNormScale_desc, batchNormMean_desc,
                                        batchNormOffset_desc);
}

status_t zendnn_dilated_convolution_forward_desc_init(
...
status_t zendnn_convolution_backward_data_desc_init(convolution_desc_t *conv_desc,
        alg_kind_t alg_kind, const memory_desc_t *diff_src_desc,
        const memory_desc_t *weights_desc, const memory_desc_t *diff_dst_desc,
        const dims_t strides, const dims_t padding_l, const dims_t padding_r) {
    return zendnn::impl::conv_desc_init(conv_desc, backward_data, alg_kind,
            diff_src_desc, weights_desc, nullptr, diff_dst_desc, strides,
            nullptr, padding_l, padding_r, false, false, nullptr, nullptr, nullptr);
}

status_t zendnn_dilated_convolution_backward_data_desc_init(
    ...
    return zendnn::impl::conv_desc_init(conv_desc, backward_data, alg_kind,
            diff_src_desc, weights_desc, nullptr, diff_dst_desc, strides,
            dilates, padding_l, padding_r, false, false, nullptr, nullptr, nullptr);
}

status_t zendnn_convolution_backward_weights_desc_init(
        convolution_desc_t *conv_desc, alg_kind_t alg_kind,
        ...
        const dims_t strides, const dims_t padding_l, const dims_t padding_r) {
    return zendnn::impl::conv_desc_init(conv_desc, backward_weights, alg_kind,
            src_desc, diff_weights_desc, diff_bias_desc, diff_dst_desc, strides,
            nullptr, padding_l, padding_r, false, false, nullptr, nullptr, nullptr);
}

status_t zendnn_dilated_convolution_backward_weights_desc_init(
        convolution_desc_t *conv_desc, alg_kind_t alg_kind,
        const memory_desc_t *src_desc, const memory_desc_t *diff_weights_desc,
        const memory_desc_t *diff_bias_desc, const memory_desc_t *diff_dst_desc,
