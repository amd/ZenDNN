...
status_t zendnn_matmul_desc_init(matmul_desc_t *matmul_desc,
    ...
    bool ok = ndims >= 2 && ndims <= ZENDNN_MAX_NDIMS
    ...
    for (int d = 0; d < ndims - 2; ++d) {
        ...
        if (one_of(ZENDNN_RUNTIME_DIM_VAL, s_dim, w_dim, d_dim, b_dim)) {

            if (!(everyone_is(ZENDNN_RUNTIME_DIM_VAL, s_dim, w_dim, d_dim)
                        && IMPLICATION(
                                with_bias, b_dim == ZENDNN_RUNTIME_DIM_VAL)))
    ...
    zendnnInfo(ZENDNN_CORELOG, "matmul desc init [matmul]");
    *matmul_desc = op_d;
