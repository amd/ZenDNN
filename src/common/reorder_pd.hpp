...
struct reorder_primitive_desc_iface_t : public zendnn_primitive_desc {
    reorder_primitive_desc_iface_t(const std::shared_ptr<primitive_desc_t> &pd,
            engine_t *engine, engine_t *src_engine, engine_t *dst_engine)
        : zendnn_primitive_desc(pd, engine)
        , src_engine_(src_engine)
        , dst_engine_(dst_engine)
        , scratchpad_engine_(nullptr) {}

    zendnn::impl::engine_t *src_engine() const override { return src_engine_; }
    zendnn::impl::engine_t *dst_engine() const override { return dst_engine_; }

    zendnn::impl::engine_t *scratchpad_engine() const override {
        return scratchpad_engine_;
    }

    zendnn::impl::status_t query(
            zendnn::impl::query_t what, int idx, void *result) const override {
        auto status = zendnn::impl::status::success;
        switch (what) {
            case zendnn::impl::query::reorder_src_engine:
                *(zendnn::impl::engine_t **)result = src_engine();
                break;
            case zendnn::impl::query::reorder_dst_engine:
                *(zendnn::impl::engine_t **)result = dst_engine();
                break;
            default: status = zendnn_primitive_desc::query(what, idx, result);
        }
        return status;
    }
...
private:
    zendnn::impl::engine_t *src_engine_;
    zendnn::impl::engine_t *dst_engine_;
    zendnn::impl::engine_t *scratchpad_engine_;
};
...
struct reorder_pd_t : public primitive_desc_t {
    ...
    arg_usage_t arg_usage(int arg) const override {
        if (arg == ZENDNN_ARG_FROM) return arg_usage_t::input;

        if (arg == ZENDNN_ARG_TO) return arg_usage_t::output;
    ...
    const memory_desc_t *arg_md(int arg) const override {
        switch (arg) {
            case ZENDNN_ARG_FROM: return src_md(0);
            case ZENDNN_ARG_TO: return dst_md(0);
