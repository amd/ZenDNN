...
private:
    primitive_t() = delete;
    ZENDNN_DISALLOW_COPY_AND_ASSIGN(primitive_t);
};
...
struct nested_scratchpad_t {
    ...
    ZENDNN_DISALLOW_COPY_AND_ASSIGN(nested_scratchpad_t);
...
struct resource_mapper_t {
    ...
    ZENDNN_DISALLOW_COPY_AND_ASSIGN(resource_mapper_t);
...
// zendnn_primitive is a user facing entity that has an alias primitive_iface_t
// for internal use.
...
// 3. primitive_desc_iface_t - an alias for zendnn_primitive_desc and is
...
struct zendnn_primitive : public zendnn::impl::c_compatible {
    zendnn_primitive(const std::shared_ptr<zendnn::impl::primitive_t> &primitive,
            zendnn::impl::engine_t *engine);

    // This is a ctor for reorder
    zendnn_primitive(const std::shared_ptr<zendnn::impl::primitive_t> &primitive,
            zendnn::impl::engine_t *engine, zendnn::impl::engine_t *src_engine,
...
private:
    ...
    std::shared_ptr<zendnn::impl::primitive_t> primitive_;
    std::unique_ptr<zendnn::impl::scratchpad_t> scratchpad_;
    std::unique_ptr<primitive_desc_iface_t> pd_;
    zendnn::impl::resource_mapper_t resource_mapper_;

    zendnn_primitive() = delete;
    ZENDNN_DISALLOW_COPY_AND_ASSIGN(zendnn_primitive);
};
