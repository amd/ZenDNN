...
struct rnn_pd_t : public primitive_desc_t {
    ...
    zendnn::impl::alg_kind_t cell_kind() const { return desc_.cell_kind; }
    zendnn::impl::alg_kind_t activation_kind() const {
    ...
    zendnn_rnn_direction_t direction() const { return desc_.direction; }
...
struct rnn_fwd_pd_t : public rnn_pd_t {
    ...
    arg_usage_t arg_usage(int arg) const override {
        ...
        if (arg == ZENDNN_ARG_SRC_ITER_C && with_src_iter_c())
            return arg_usage_t::input;

        if (utils::one_of(arg, ZENDNN_ARG_WEIGHTS_LAYER, ZENDNN_ARG_WEIGHTS_ITER))
            return arg_usage_t::input;

        if (arg == ZENDNN_ARG_WEIGHTS_PEEPHOLE && is_lstm_peephole())
            return arg_usage_t::input;

        if (arg == ZENDNN_ARG_WEIGHTS_PROJECTION && is_lstm_projection())
            return arg_usage_t::input;

        if (arg == ZENDNN_ARG_BIAS && with_bias()) return arg_usage_t::input;

        if (arg == ZENDNN_ARG_DST_LAYER) return arg_usage_t::output;

        if (arg == ZENDNN_ARG_DST_ITER && with_dst_iter())
            return arg_usage_t::output;

        if (arg == ZENDNN_ARG_DST_ITER_C && with_dst_iter() && is_lstm())
            return arg_usage_t::output;

        if (arg == ZENDNN_ARG_WORKSPACE && is_training())
            return arg_usage_t::output;
    ...
    const memory_desc_t *arg_md(int arg) const override {
        switch (arg) {
            ...
            case ZENDNN_ARG_WEIGHTS_ITER: return weights_md(1);
            case ZENDNN_ARG_WEIGHTS_PEEPHOLE:
                return is_lstm_peephole() ? weights_md(2) : &glob_zero_md;
            case ZENDNN_ARG_WEIGHTS_PROJECTION:
                return is_lstm_projection() ? weights_md(2 + is_lstm_peephole())
                                            : &glob_zero_md;
            case ZENDNN_ARG_BIAS:
                return weights_md(
                        2 + is_lstm_peephole() + is_lstm_projection());
            case ZENDNN_ARG_DST_LAYER: return dst_md(0);
            case ZENDNN_ARG_DST_ITER: return dst_md(1);
            case ZENDNN_ARG_DST_ITER_C: return dst_md(2);
...
struct rnn_bwd_pd_t : public rnn_pd_t {
    ...
    arg_usage_t arg_usage(int arg) const override {
        if (utils::one_of(arg, ZENDNN_ARG_SRC_LAYER, ZENDNN_ARG_DST_LAYER,
                    ZENDNN_ARG_DIFF_DST_LAYER, ZENDNN_ARG_WEIGHTS_LAYER,
                    ZENDNN_ARG_WEIGHTS_ITER))
            return arg_usage_t::input;
        ...
        if (is_lstm_peephole()) {
            ...
            if (arg == ZENDNN_ARG_DIFF_WEIGHTS_PEEPHOLE)
                return arg_usage_t::output;
        }

        if (is_lstm_projection()) {
            if (arg == ZENDNN_ARG_WEIGHTS_PROJECTION) return arg_usage_t::input;

            if (arg == ZENDNN_ARG_DIFF_WEIGHTS_PROJECTION)
                return arg_usage_t::output;
        }

        if (with_bias()) {
            if (arg == ZENDNN_ARG_BIAS) return arg_usage_t::input;

            if (arg == ZENDNN_ARG_DIFF_BIAS) return arg_usage_t::output;
        }

        if (with_src_iter()) {
            if (arg == ZENDNN_ARG_SRC_ITER) return arg_usage_t::input;

            if (arg == ZENDNN_ARG_DIFF_SRC_ITER) return arg_usage_t::output;
        }

        if (with_src_iter_c()) {
            if (arg == ZENDNN_ARG_SRC_ITER_C) return arg_usage_t::input;

            if (arg == ZENDNN_ARG_DIFF_SRC_ITER_C) return arg_usage_t::output;
        }

        if (with_dst_iter()
                && utils::one_of(
                        arg, ZENDNN_ARG_DST_ITER, ZENDNN_ARG_DIFF_DST_ITER))
            return arg_usage_t::input;

        if (with_dst_iter_c()
                && utils::one_of(
                        arg, ZENDNN_ARG_DST_ITER_C, ZENDNN_ARG_DIFF_DST_ITER_C))
            return arg_usage_t::input;

        if (arg == ZENDNN_ARG_WORKSPACE) return arg_usage_t::input;
    ...
    const memory_desc_t *arg_md(int arg) const override {
        switch (arg) {
            ...
            case ZENDNN_ARG_BIAS:
                return weights_md(
                        2 + is_lstm_peephole() + is_lstm_projection());
            case ZENDNN_ARG_DIFF_WEIGHTS_LAYER: return diff_weights_md(0);
            case ZENDNN_ARG_DIFF_WEIGHTS_ITER: return diff_weights_md(1);
            case ZENDNN_ARG_DIFF_WEIGHTS_PEEPHOLE:
                return is_lstm_peephole() ? diff_weights_md(2) : &glob_zero_md;
            case ZENDNN_ARG_DIFF_WEIGHTS_PROJECTION:
                return is_lstm_projection()
                        ? diff_weights_md(2 + is_lstm_peephole())
                        : &glob_zero_md;
            case ZENDNN_ARG_DIFF_BIAS:
                return diff_weights_md(
                        2 + is_lstm_peephole() + is_lstm_projection());
            case ZENDNN_ARG_DST_LAYER: return dst_md(0);
            case ZENDNN_ARG_DST_ITER: return dst_md(1);
            case ZENDNN_ARG_DST_ITER_C: return dst_md(2);
            case ZENDNN_ARG_DIFF_DST_LAYER: return diff_dst_md(0);
            case ZENDNN_ARG_DIFF_DST_ITER: return diff_dst_md(1);
            case ZENDNN_ARG_DIFF_DST_ITER_C: return diff_dst_md(2);
            default: return rnn_pd_t::arg_md(arg);
        }
    }
