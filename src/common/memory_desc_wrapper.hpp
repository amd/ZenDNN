...
struct memory_desc_wrapper : public c_compatible {
    ...
    dim_t nelems(bool with_padding = false) const {
        if (is_zero()) return 0;
        if (has_runtime_dims()) return ZENDNN_RUNTIME_DIM_VAL;
    ...
    size_t size() const {
        ...
        if (has_runtime_dims_or_strides()) return ZENDNN_RUNTIME_SIZE_VAL;
    ...
    bool has_runtime_dims() const {
        for (int d = 0; d < ndims(); ++d)
            if (dims()[d] == ZENDNN_RUNTIME_DIM_VAL) return true;
        return false;
    }
    ...
    bool has_runtime_strides() const {
        ...
        for (int d = 0; d < ndims(); ++d)
            if (blocking_desc().strides[d] == ZENDNN_RUNTIME_DIM_VAL) return true;
        return false;
    }
