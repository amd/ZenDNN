...
status_t ncsp_batch_normalization_fwd_t<d_type>::execute_forward(
    ...
    if (!calculate_stats) {
        ...
        if (save_stats) {
            mean = CTX_OUT_MEM(acc_data_t *, ZENDNN_ARG_MEAN);
            variance = CTX_OUT_MEM(acc_data_t *, ZENDNN_ARG_VARIANCE);
        } else {
    ...
    parallel(nthr, [&](const int ithr, const int nthr) {
        ...
        for (int64_t it = 0; it < iters; ++it) {
            ...
            if (it == iters - 1 && iters > 1) {
                ...
                if (SP_N_nthr == 1 && zendnn_thr_syncable()) zendnn_thr_barrier();
            ...
            size_t ws_iter_off = (zendnn_thr_syncable() ? 0 : 1) * C_off;
            ...
            if (calculate_stats) {
                ...
                for (dim_t c = C_blk_s; c < C_blk_e; c++) {
                    ...
                    for (dim_t n = N_s; n < N_e; ++n) {
                        ...
                        ZENDNN_PRAGMA_OMP_SIMD(reduction(+ : sum))
                ...
                if (zendnn_thr_syncable()) zendnn_thr_barrier();
...
status_t ncsp_batch_normalization_bwd_t<d_type>::execute_backward(
    ...
    auto diff_shift = use_shift
            ? CTX_OUT_MEM(acc_data_t *, ZENDNN_ARG_DIFF_SHIFT)
    ...
    parallel(nthr, [&](const int ithr, const int nthr) {
        ...
        for (int64_t it = 0; it < iters; ++it) {
            ...
            // possible (in case of TBB) use different parts of ws for each
            // iteration if threads are not synced by the algorithm.
            size_t ws_iter_off = (zendnn_thr_syncable() ? 0 : 1) * 2 * C_off;
...
#if CLANG_WA_02_SAFE_TO_USE_OMP_SIMD
                    ZENDNN_PRAGMA_OMP_SIMD(reduction(+ : diff_gamma, diff_beta))
#endif
                    for (dim_t sp = S_s; sp < S_e; ++sp) {
