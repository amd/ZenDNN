...
#define DECLARE_COMMON_PARAMS() \
    ...
    auto input = CTX_IN_MEM(const data_t<type_i> *, ZENDNN_ARG_FROM); \
    auto output = CTX_OUT_CLEAN_MEM(data_t<type_o> *, ZENDNN_ARG_TO, status); \
    CHECK(status); \
    const auto &scratchpad = ctx.get_scratchpad_grantor(); \
    MAYBE_UNUSED(scratchpad); \
    const auto input_d = ctx.memory_mdw(ZENDNN_ARG_FROM, pd->src_md()); \
    const auto output_d = ctx.memory_mdw(ZENDNN_ARG_TO, pd->dst_md()); \
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    ...
    static size_t get_scratchpad_size(const memory_desc_wrapper &input_d,
        ...
        return sizeof(float) * blksize * blksize * zendnn_get_max_threads();
    }
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    ...
    static size_t get_scratchpad_size(const memory_desc_wrapper &input_d,
        ...
        return sizeof(float) * blksize * W * zendnn_get_max_threads();
    }
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
    ...
    static bool is_applicable(const memory_desc_wrapper &input_d,
        ...
        return input_d.is_blocking_desc() && output_d.is_blocking_desc()
                ...
                && attr->has_default_values(
                        zendnn_primitive_attr::skip_mask_t::oscale_runtime
                        | zendnn_primitive_attr::skip_mask_t::zero_points_runtime
                        | zendnn_primitive_attr::skip_mask_t::post_ops)
    ...
    static status_t execute(
        ...
        DEFINE_ZERO_POINT_VALUE(i0, ZENDNN_ARG_FROM);
        DEFINE_ZERO_POINT_VALUE(o0, ZENDNN_ARG_TO);
    ...
    struct pd_t : public cpu_reorder_pd_t {
        ...
        static status_t create(reorder_pd_t **reorder_pd, engine_t *engine,
            ...
            bool args_ok = true && src_md->data_type == type_i
                    ...
                    && attr->has_default_values(
                            zendnn_primitive_attr::skip_mask_t::oscale_runtime
                            | zendnn_primitive_attr::skip_mask_t::zero_points
                            | zendnn_primitive_attr::skip_mask_t::
                                    zero_points_runtime
                            | zendnn_primitive_attr::skip_mask_t::post_ops)
