...
status_t nchw_pooling_fwd_t<d_type>::execute_forward(
    ...
    CHECK(status);
    auto ws = CTX_OUT_CLEAN_MEM(unsigned char *, ZENDNN_ARG_WORKSPACE, status);
    CHECK(status);
    ...
    const auto ker_avg = [=](data_t *d, int mb, int c, int od, int oh, int ow) {
...
status_t nchw_pooling_fwd_t<data_type::bf16>::execute_forward(
    ...
    status_t status = status::success;
    auto src = CTX_IN_MEM(const bfloat16_t *, ZENDNN_ARG_SRC);
    auto dst = CTX_OUT_CLEAN_MEM(bfloat16_t *, ZENDNN_ARG_DST, status);
    CHECK(status);
    auto ws = CTX_OUT_CLEAN_MEM(unsigned char *, ZENDNN_ARG_WORKSPACE, status);
    CHECK(status);
...
status_t nchw_pooling_bwd_t<d_type>::execute_backward(
    ...
    auto ws = CTX_IN_MEM(const unsigned char *, ZENDNN_ARG_WORKSPACE);
...
status_t nchw_pooling_bwd_t<data_type::bf16>::execute_backward(
    ...
    status_t status = status::success;
    auto diff_src = CTX_OUT_CLEAN_MEM(bfloat16_t *, ZENDNN_ARG_DIFF_SRC, status);
    CHECK(status);
    auto diff_dst = CTX_IN_MEM(const bfloat16_t *, ZENDNN_ARG_DIFF_DST);
    auto ws = CTX_IN_MEM(const unsigned char *, ZENDNN_ARG_WORKSPACE);
