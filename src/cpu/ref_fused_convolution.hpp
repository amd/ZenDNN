...
struct ref_fused_convolution_fwd_t : public primitive_t {
    ...
    private:
        ...
        status_t append_op(primitive_desc_t *op_pd, size_t &sp_begin,
            ...
            if (*from_md != *to_md) {
                ...
                arg_cache.append_inout_arg(
                        ZENDNN_ARG_FROM, sp_begin, from_md, true);
                arg_cache.append_inout_arg(ZENDNN_ARG_TO, sp_end, to_md, false);
        ...
        status_t init_ops(engine_t *engine) {
            ...
            arg_cache_t arg_cache;
            arg_cache.append_ctx_arg(ZENDNN_ARG_SRC);
            arg_cache.append_ctx_arg(ZENDNN_ARG_WEIGHTS);
            if (desc()->bias_desc.data_type != data_type::undef)
                arg_cache.append_ctx_arg(ZENDNN_ARG_BIAS);
            arg_cache.append_inout_arg(ZENDNN_ARG_DST, inout_sp_offset_end,
                    root_pd->dst_md(), false);
            for (int idx = 0; idx < attr_1x1.post_ops_.len(); ++idx) {
                if (attr_1x1.post_ops_.contain(primitive_kind::binary, idx))
                    arg_cache.append_ctx_arg(ZENDNN_ARG_ATTR_MULTIPLE_POST_OP(idx)
                            | ZENDNN_ARG_SRC_1);
            }
            args_.push_back(arg_cache);
            ...
            while (po_op_iter < end) {
                ...
                arg_cache_t arg_cache;
                arg_cache.append_inout_arg(ZENDNN_ARG_SRC, inout_sp_offset_begin,
                        op->src_md(), true);
                arg_cache.append_ctx_arg(ZENDNN_ARG_DST);
                arg_cache.append_ctx_arg(ZENDNN_ARG_WEIGHTS,
                        ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_WEIGHTS);
                if (op->weights_md(1)->data_type != data_type::undef)
                    arg_cache.append_ctx_arg(ZENDNN_ARG_BIAS,
                            ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_BIAS);
                    ...
                    if (attr_dw.post_ops_.contain(primitive_kind::binary, idx))
                        arg_cache.append_ctx_arg(
                                (ZENDNN_ARG_ATTR_MULTIPLE_POST_OP(idx)
                                        | ZENDNN_ARG_SRC_1),
                                (ZENDNN_ARG_ATTR_MULTIPLE_POST_OP(
                                         idx + po_op_iter + 1)
                                        | ZENDNN_ARG_SRC_1));
                }
