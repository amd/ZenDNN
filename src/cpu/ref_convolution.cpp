...
status_t ref_convolution_fwd_t::execute_forward(const exec_ctx_t &ctx) const {
    ...
    auto weights = CTX_IN_MEM(const void *, ZENDNN_ARG_WEIGHTS);
...
status_t ref_convolution_bwd_weights_t::execute_backward_weights(
    ...
    auto src = CTX_IN_MEM(const void *, ZENDNN_ARG_SRC);
    auto diff_weights
            = CTX_OUT_CLEAN_MEM(void *, ZENDNN_ARG_DIFF_WEIGHTS, status);
    CHECK(status);
    auto diff_bias = CTX_OUT_CLEAN_MEM(void *, ZENDNN_ARG_DIFF_BIAS, status);
    CHECK(status);
