...
ref_convolution_fwd_t<src_type, wei_type, dst_type, acc_type>::execute_forward(
    ...
    const zendnn_dims_t &src_str = src_d.blocking_desc().strides;
    ...
    const zendnn_dims_t &weights_str = weights_d.blocking_desc().strides;
...
status_t ref_convolution_bwd_data_t<diff_src_type, wei_type, diff_dst_type,
    ...
    auto bias = CTX_IN_MEM(const char *, ZENDNN_ARG_BIAS);
    auto diff_src
            = CTX_OUT_CLEAN_MEM(diff_src_data_t *, ZENDNN_ARG_DIFF_SRC, status);
    CHECK(status);
    ...
    // help compiler optimize the code
    // constants for plain layouts kernel
    const zendnn_dims_t &diff_dst_str = diff_dst_d.blocking_desc().strides;
...
status_t ref_convolution_bwd_weights_t<src_type, diff_wei_type, diff_dst_type,
    ...
    auto diff_weights = CTX_OUT_CLEAN_MEM(
            diff_wei_data_t *, ZENDNN_ARG_DIFF_WEIGHTS, status);
    CHECK(status);
    auto diff_bias
            = CTX_OUT_CLEAN_MEM(diff_wei_data_t *, ZENDNN_ARG_DIFF_BIAS, status);
    CHECK(status);
