...
status_t acl_init_conf(acl_conv_conf_t &acp, memory_desc_t &src_md,
    ...
    const bool is_fwd = (prop_kind == zendnn_forward_training)
            || (prop_kind == zendnn_forward_inference);
    ...
    // ZENDNN dilations:          dk = 1 + (k_size - 1) * (dilate_size + 1)
    // Compute Library dilations: dk = dilate_size * (k_size - 1) + 1
    // thus acl_dilation = ZENDNN_dilation + 1
    dilate_h += 1;
    dilate_w += 1;
    ...
    // equal to -1 in ZENDNN for some cases, when the very right (bottom)
...
arm_compute::DataType get_acl_data_t(const zendnn_data_type_t dt) {
    switch (dt) {
...
arm_compute::ActivationLayerInfo get_acl_act(const primitive_attr_t &attr) {
    ...
    switch (eltwise_alg) {
        case eltwise_relu:
            // ZENDNN defines RELU: f(x) = (x > 0) ? x : a*x
            ...
            break;
        case eltwise_tanh:
            // ZENDNN defines TANH activation as:          f(x) = tanh(x)
