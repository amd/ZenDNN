...
void gates_reduction(const rnn_utils::rnn_conf_t &rnn, const gates_t *ws_gates_,
    ...
    // parallel_nd path
#if ZENDNN_CPU_RUNTIME == ZENDNN_RUNTIME_OMP && _OPENMP >= 201307 \
    ...
    struct pd_t : public base_pd_t {
        ...
        status_t init_ref(engine_t *engine) {
            ...
            ok = init_conf<class_name>(rnn_, *this->desc(), this->src_md(0),
                    ...
                    this->weights_md(1),
                    this->arg_md(ZENDNN_ARG_WEIGHTS_PROJECTION), this->dst_md(0),
                    this->dst_md(1), this->dst_md(2),
                    this->arg_md(ZENDNN_ARG_BIAS));
            if (!ok) return status::unimplemented;
            ...
            if (rnn_.is_lstm_projection) {
                memory_desc_t new_weights_projection_md
                        = *this->arg_md(ZENDNN_ARG_WEIGHTS_PROJECTION);
            ...
            set_conf<class_name>(rnn_, *this->desc(), this->weights_md(0),
                    ...
                    this->arg_md(ZENDNN_ARG_DIFF_WEIGHTS_PROJECTION));
        ...
        status_t init(engine_t *engine) {
            ...
            if (st == status::success) {
                ...
                if (rnn_.is_training) {
                    ...
                    zendnn_memory_desc_init_by_tag(&this->ws_md_, 1, ws_dims,
    ...
    private:
        void init_scratchpad(size_t scratchpad_sz) {
            ...
            const auto bias_dt_size = types::data_type_size(
                    this->arg_md(ZENDNN_ARG_BIAS)->data_type);
