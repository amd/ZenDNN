...
status_t ref_eltwise_bwd_t<data_type>::execute_backward_dense(
        const exec_ctx_t &ctx) const {
    status_t status = status::success;
    const void *src = pd()->use_dst() ? CTX_IN_MEM(const void *, ZENDNN_ARG_DST)
                                      : CTX_IN_MEM(const void *, ZENDNN_ARG_SRC);
    const void *diff_dst = CTX_IN_MEM(const void *, ZENDNN_ARG_DIFF_DST);
    void *diff_src = CTX_OUT_CLEAN_MEM(void *, ZENDNN_ARG_DIFF_SRC, status);
    CHECK(status);
