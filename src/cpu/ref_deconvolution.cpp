...
status_t ref_deconvolution_fwd_t::compute_ref_attrs(const exec_ctx_t &ctx,
    ...
    return status_t::zendnn_success;
}
...
static status_t apply_src_zero_point(const exec_ctx_t &ctx,
    ...
    const auto wei = CTX_OUT_MEM(wei_data_t *, ZENDNN_ARG_WEIGHTS);
    DEFINE_ZERO_POINTS_BUFFER(src_zero_point, ZENDNN_ARG_SRC);
    const bool is_src_zp_common
            = deconv_pd->attr()->zero_points_.common(ZENDNN_ARG_SRC);
...
status_t ref_deconvolution_fwd_t::execute(const exec_ctx_t &ctx) const {
    ...
    conv_args[ZENDNN_ARG_DIFF_DST] = args.at(ZENDNN_ARG_SRC);
    conv_args[ZENDNN_ARG_WEIGHTS] = args.at(ZENDNN_ARG_WEIGHTS);
    if (pd()->with_bias() && pd()->conv_supports_bias_)
        conv_args[ZENDNN_ARG_BIAS] = args.at(ZENDNN_ARG_BIAS);

    // Create intermediate memory for f32 output if needed.
    auto dst = args.at(ZENDNN_ARG_DST);
    ...
    if (pd()->attr()->post_ops_.find(primitive_kind::sum) != -1) {
        ...
        const memory_desc_wrapper dst_d(pd()->dst_md());
        void *dst = CTX_OUT_MEM(void *, ZENDNN_ARG_DST);
    ...
    if (!pd()->attr()->zero_points_.has_default_values(ZENDNN_ARG_SRC)) {
...
#define CASE(DT) \
    case (DT): { \
        ...
        dst_data_t *dst = CTX_OUT_MEM(dst_data_t *, ZENDNN_ARG_DST); \
...
status_t ref_deconvolution_bwd_data_t::execute(const exec_ctx_t &ctx) const {
    ...
    const auto &args = ctx.args();
    exec_args_t conv_args;
    conv_args[ZENDNN_ARG_SRC] = args.at(ZENDNN_ARG_DIFF_DST);
    conv_args[ZENDNN_ARG_WEIGHTS] = args.at(ZENDNN_ARG_WEIGHTS);
    conv_args[ZENDNN_ARG_DST] = args.at(ZENDNN_ARG_DIFF_SRC);
    exec_ctx_t conv_ctx(ctx, std::move(conv_args));
...
void ref_deconvolution_bwd_weights_t::compute_bias(
    ...
    auto diff_bias = CTX_OUT_MEM(dbia_data_t *, ZENDNN_ARG_DIFF_BIAS);
    auto diff_dst = CTX_IN_MEM(const ddst_data_t *, ZENDNN_ARG_DIFF_DST);
...
status_t ref_deconvolution_bwd_weights_t::execute(const exec_ctx_t &ctx) const {
    ...
    conv_args[ZENDNN_ARG_DIFF_DST] = args.at(ZENDNN_ARG_SRC);
    conv_args[ZENDNN_ARG_SRC] = args.at(ZENDNN_ARG_DIFF_DST);
    conv_args[ZENDNN_ARG_DIFF_WEIGHTS] = args.at(ZENDNN_ARG_DIFF_WEIGHTS);
    exec_ctx_t conv_ctx(ctx, std::move(conv_args));
