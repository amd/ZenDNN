...
void simple_layer_normalization_fwd_t<data_type>::execute_forward(
    ...
    if (pd()->use_tmp_stats()) {
        ...
        variance = pd()->stats_are_src()
                ? const_cast<float *>(
                        CTX_IN_MEM(const float *, ZENDNN_ARG_VARIANCE))
...
void simple_layer_normalization_bwd_t<data_type>::execute_backward(
    ...
    if (pd()->use_tmp_stats()) {
        ...
        mean = CTX_IN_MEM(const float *, ZENDNN_ARG_MEAN);
        variance = CTX_IN_MEM(const float *, ZENDNN_ARG_VARIANCE);
    }
