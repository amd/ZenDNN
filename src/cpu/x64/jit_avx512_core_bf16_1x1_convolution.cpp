...
namespace {
/*TODO: investigate why common balance2D defined in common/zendnn_thread.hpp
 * not used here ?*/
...
void jit_avx512_core_bf16_1x1_convolution_fwd_t<dst_type>::execute_forward(
    ...
    auto dst = CTX_OUT_MEM(const char *, ZENDNN_ARG_DST);
    auto weights_dw = CTX_IN_MEM(
            const dw_wei_data_t *, ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_WEIGHTS);
    ...
    if (pd()->arg_md(ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_BIAS)->data_type
            == data_type::bf16) {
        ...
        auto bias_in = CTX_IN_MEM(
                const src_data_t *, ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_BIAS);
        ...
        auto bias_in = CTX_IN_MEM(
                const float *, ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_BIAS);
...
void jit_avx512_core_bf16_1x1_convolution_fwd_t<dst_type>::execute_forward_thr(
    ...
    const memory_desc_wrapper dw_weights_d(
            pd()->arg_md(ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_WEIGHTS));
    const memory_desc_wrapper dw_bias_d(
            pd()->arg_md(ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_BIAS));
...
void jit_avx512_core_bf16_1x1_convolution_bwd_weights_t<diff_weights_type>::
    ...
    if (jcp.with_bias && pd()->jcp_.bia_dt == data_type::f32) {
        diff_bias = pd()->with_bias() && jcp.oc_without_padding % jcp.oc_block
                ...
                : CTX_OUT_MEM(float *, ZENDNN_ARG_DIFF_BIAS);
    }
    const memory_desc_wrapper diff_dst_d(pd()->diff_dst_md());
    const memory_desc_wrapper src_d(pd()->src_md());
    ...
    if (zendnn_thr_syncable()) simple_barrier::ctx_init(&reduction_barrier);
                    ...
                    for (int g = g_start; g < g_end; g++) {
                        ...
                        if (is_bf16_bias && thr_mb == jcp.nthr_mb - 1) {
                            ...
                            bfloat16_t *diff_bias_result
                                    = CTX_OUT_MEM(
                                              bfloat16_t *, ZENDNN_ARG_DIFF_BIAS)
                                    + diff_bias_idx;
            ...
            if (is_bf16_bias && ithr_ic_b == 0 && ic_b_work > 0) {
                for (int g = g_start; g < g_end; g++) {
                    ...
                    const size_t acc_size = nstl::min(jcp.oc_without_padding,
                            ...
                            - oc_b_start * jcp.oc_block;
                    bfloat16_t *diff_bias_result
                            = CTX_OUT_MEM(bfloat16_t *, ZENDNN_ARG_DIFF_BIAS)
                            + result_start_idx;
    ...
    if (pd()->jcp_.bia_dt == data_type::f32
        ...
        auto diff_bias_in = CTX_OUT_MEM(float *, ZENDNN_ARG_DIFF_BIAS);
