...
void jit_avx512_common_convolution_winograd_bwd_weights_t::
        _execute_backward_weights_S_D_G_W(const exec_ctx_t &ctx,
                const memory_tracking::grantor_t &scratchpad) const {
    auto ptr_diff_dst = CTX_IN_MEM(const float *, ZENDNN_ARG_DIFF_DST);
    auto ptr_src = CTX_IN_MEM(const float *, ZENDNN_ARG_SRC);
    auto ptr_diff_weights = CTX_OUT_MEM(float *, ZENDNN_ARG_DIFF_WEIGHTS);
    auto ptr_diff_bias = CTX_OUT_MEM(float *, ZENDNN_ARG_DIFF_BIAS);
