...
using namespace zendnn::impl::cpu::x64::bf16_support;
...
status_t gemm_bf16_convolution_fwd_t<dst_data_type>::execute_forward_nspc(
    ...
    auto src_base = CTX_IN_MEM(const src_data_t *, ZENDNN_ARG_SRC);
    auto wei_base = CTX_IN_MEM(const wei_data_t *, ZENDNN_ARG_WEIGHTS);
    auto dst_base = CTX_OUT_MEM(dst_data_t *, ZENDNN_ARG_DST);
    ...
    if (jcp.with_bias) {
        if (pd()->desc()->bias_desc.data_type == data_type::bf16) {
            auto bias_in = CTX_IN_MEM(const bfloat16_t *, ZENDNN_ARG_BIAS);
            ...
            auto bias_in = CTX_IN_MEM(const float *, ZENDNN_ARG_BIAS);
...
status_t gemm_bf16_convolution_fwd_t<dst_data_type>::execute_forward_ncsp(
    ...
    auto weights = CTX_IN_MEM(const wei_data_t *, ZENDNN_ARG_WEIGHTS);
...
status_t gemm_bf16_convolution_bwd_data_t<diff_src_data_type>::
    ...
    auto diff_dst_base = CTX_IN_MEM(const diff_dst_data_t *, ZENDNN_ARG_DIFF_DST);
    auto wei_base = CTX_IN_MEM(const wei_data_t *, ZENDNN_ARG_WEIGHTS);
    auto diff_src_base = CTX_OUT_MEM(diff_src_data_t *, ZENDNN_ARG_DIFF_SRC);
...
status_t gemm_bf16_convolution_bwd_data_t<diff_src_data_type>::
    ...
    auto diff_dst = CTX_IN_MEM(const diff_dst_data_t *, ZENDNN_ARG_DIFF_DST);
    auto weights = CTX_IN_MEM(const wei_data_t *, ZENDNN_ARG_WEIGHTS);
    auto diff_src = CTX_OUT_MEM(diff_src_data_t *, ZENDNN_ARG_DIFF_SRC);
...
status_t gemm_bf16_convolution_bwd_weights_t<diff_wei_data_type>::
    ...
    auto diff_dst = CTX_IN_MEM(const diff_dst_data_t *, ZENDNN_ARG_DIFF_DST);
    auto src = CTX_IN_MEM(const src_data_t *, ZENDNN_ARG_SRC);
    auto diff_weights = CTX_OUT_MEM(diff_wei_data_t *, ZENDNN_ARG_DIFF_WEIGHTS);
    ...
    if (jcp.with_bias) {
        if (pd()->desc()->diff_bias_desc.data_type == data_type::bf16)
            ...
            diff_bias = CTX_OUT_MEM(float *, ZENDNN_ARG_DIFF_BIAS);
    }
    ...
    parallel(jcp.nthr, [&](const int ithr, const int nthr) {
        ...
        if (ithr_g != -1 && ithr_mb != -1) {
            ...
            if (need_reduction && zendnn_thr_syncable()) {
                zendnn_thr_barrier();
            ...
            if (need_reduction && zendnn_thr_syncable()) zendnn_thr_barrier();
        }
    });

    if (jcp.need_wei_reduction && !zendnn_thr_syncable()) {
        parallel(jcp.nthr, [&](const int ithr, const int nthr) {
            int ithr_g, nthr_g, ithr_mb, nthr_mb;
            size_t g_start {0}, g_end {0}, mb_start {0}, mb_end {0};
    ...
    if (jcp.with_bias) {
        parallel_nd(jcp.ngroups, jcp.oc, [&](dim_t g, dim_t oc) {
            ...
            for (dim_t oh = 0; oh < jcp.oh; ++oh) {
                ...
                ZENDNN_PRAGMA_OMP_SIMD(reduction(+ : db))
        ...
        if (pd()->desc()->diff_bias_desc.data_type == data_type::bf16) {
            auto diff_bias_in = CTX_OUT_MEM(bfloat16_t *, ZENDNN_ARG_DIFF_BIAS);
