...
void brgemm_inner_product_fwd_t<isa>::execute_forward(
    ...
    // for 1 < work_amont < zendnn_get_max_threads() case to avoid potential
...
void brgemm_inner_product_bwd_data_t<isa, src_type, wei_type,
    ...
    auto diff_dst_ = CTX_IN_MEM(const diff_dst_data_t *, ZENDNN_ARG_DIFF_DST);
    auto weights_ = CTX_IN_MEM(const wei_data_t *, ZENDNN_ARG_WEIGHTS);
    auto diff_src_ = CTX_OUT_MEM(diff_src_data_t *, ZENDNN_ARG_DIFF_SRC);
...
struct brgemm_inner_product_bwd_weights_t<isa, src_type, diff_wei_type,
    ...
    thread_info_t(const brgemm_inner_product_bwd_weights_t *self,
        ...
        diff_weights = CTX_OUT_MEM(diff_wei_data_t *, ZENDNN_ARG_DIFF_WEIGHTS);
        diff_bias = CTX_OUT_MEM(char *, ZENDNN_ARG_DIFF_BIAS);
...
void brgemm_inner_product_bwd_weights_t<isa, src_type, diff_wei_type,
    ...
    if (zendnn_thr_syncable() && jbgp.nthr > 1)
