...
status_t brgemm_inner_product_fwd_t<isa>::execute_forward(
    ...
    // for 1 < work_amount < zendnn_get_max_threads() case to avoid potential
...
void brgemm_inner_product_bwd_data_t<isa>::execute_backward_data(
    ...
    auto diff_dst_ = CTX_IN_MEM(const char *, ZENDNN_ARG_DIFF_DST);
    auto weights_ = CTX_IN_MEM(const char *, ZENDNN_ARG_WEIGHTS);
    auto diff_src_ = CTX_OUT_MEM(char *, ZENDNN_ARG_DIFF_SRC);
...
struct brgemm_inner_product_bwd_weights_t<isa>::thread_info_t {
    ...
    thread_info_t(const brgemm_inner_product_bwd_weights_t *self,
        ...
        diff_dst = CTX_IN_MEM(const char *, ZENDNN_ARG_DIFF_DST);
        diff_weights = CTX_OUT_MEM(char *, ZENDNN_ARG_DIFF_WEIGHTS);
