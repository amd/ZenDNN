...
status_t jit_uni_x8s8s32x_1x1_conv_kernel<isa>::init_conf(
    ...
    const int SMALL_SPATIAL = 28 * 28;
    const int BIG_REDUCE_DIM = 256;

    // To be Tuned for Optimal performance
    int load_blocking = 0;
    int load_blocking_max = 4;
    int bcast_blocking = 0;
    int bcast_blocking_max = 4;
    int reduce_blocking = 0;
    int reduce_blocking_max = 4;
    ...
    // Size Threshold corresponds to thresholding specific to input sizes
    int size_threshold = 28;

    // min and max registers are used in identifying the unrolling width
    int min_regs = 4;
    int max_regs = 4;
