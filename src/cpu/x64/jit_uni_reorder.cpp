...
#define DEBUg(...)
#endif
#define DEBUG_JIT(...) DEBUg(__VA_ARGS__)
...
status_t jit_uni_reorder_t::execute(const exec_ctx_t &ctx) const {
    auto in = CTX_IN_MEM(const char *, ZENDNN_ARG_FROM);
    auto out = CTX_OUT_MEM(char *, ZENDNN_ARG_TO);
    DEFINE_SCALES_BUFFER(scales);
    DEFINE_ZERO_POINT_VALUE(src_zp, ZENDNN_ARG_FROM);
    DEFINE_ZERO_POINT_VALUE(dst_zp, ZENDNN_ARG_TO);
...
status_t jit_blk_reorder_t::execute(const exec_ctx_t &ctx) const {
    const auto in = CTX_IN_MEM(const char *, ZENDNN_ARG_FROM);
    auto out = CTX_OUT_MEM(char *, ZENDNN_ARG_TO);
    DEFINE_ZERO_POINT_VALUE(src_zp, ZENDNN_ARG_FROM);
    DEFINE_ZERO_POINT_VALUE(dst_zp, ZENDNN_ARG_TO);
