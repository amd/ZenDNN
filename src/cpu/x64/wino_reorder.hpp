...
struct wino_reorder_t : public primitive_t {
        ...
        static status_t create(reorder_pd_t **reorder_pd, engine_t *engine,
            ...
            bool args_ok = true && id.data_type() == type_i
                    ...
                    && utils::one_of(od.wino_desc().wino_format,
                            zendnn_wino_wei_aaOIoi, zendnn_wino_wei_aaOio,
                            zendnn_wino_wei_aaOBiOo, zendnn_wino_wei_OBaaIBOIio)
    ...
    status_t init(engine_t *engine) override {
        ...
        ic2_block_ = 1;
        if (wino_format_ == zendnn_wino_wei_OBaaIBOIio)
...
private:
    ...
    void transform(out_data_t *__restrict tmp_wei,
        ...
        float *__restrict g;
        if (utils::one_of(wino_format_, zendnn_wino_wei_aaOIoi,
                    zendnn_wino_wei_aaOio, zendnn_wino_wei_aaOBiOo))
            g = (float *)G_2x2_3x3;
        else if (wino_format_ == zendnn_wino_wei_OBaaIBOIio)
            g = (float *)G_4x4_3x3;
        else {
        ...
        parallel_nd_ext(
                0, ic_, nb_oc_, [&](int ithr, int nthr, int iic, int ob) {
                    ...
                    for (int ioc = 0; ioc < oc_block_; ++ioc) {
                        float res = 0;
                        ZENDNN_PRAGMA_OMP_SIMD(reduction(+ : res))
                        for (int k = 0; k < r_; ++k)
                            res += g[i * r_ + k]
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        ...
        switch (wino_format_) {
            case zendnn_wino_wei_aaOIoi:
                reorder_to_aaOIoi(output, tmp_wei);
                break;
            case zendnn_wino_wei_aaOio: reorder_to_aaOio(output, tmp_wei); break;
            case zendnn_wino_wei_aaOBiOo:
                reorder_to_aaOBiOo(output, tmp_wei);
                break;
            case zendnn_wino_wei_OBaaIBOIio:
