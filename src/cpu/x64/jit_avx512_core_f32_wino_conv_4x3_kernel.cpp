...
bool is_winograd_faster_than_direct(const jit_conv_winograd_conf_t &jcp) {
    ...
    unsigned int nthreads = zendnn_get_max_threads();
...
bool check_L2_block_per_thread(jit_conv_winograd_conf_t &jcp, int dimN_block,
    ...
    float block_size = alpha * alpha
            * (2 * (jcp.oc + jcp.ic) * dimN_block * jcp.dimN_reg_block
                    + div_up(jcp.ic * jcp.oc, zendnn_get_max_threads()))
            * (float)sizeof(float);
...
void _jit_avx512_core_f32_wino_conv_4x3_data_kernel ::
    ...
    bool is_fwd = one_of(
            jcp.prop_kind, zendnn_forward_training, zendnn_forward_inference);
    int kh = jcp.kh;
    int kw = jcp.kw;
...
status_t set_wsched_DATA_W_SGD_avx512_core(jit_conv_winograd_conf_t &jcp) {
    ...
    if (check_L2_block_per_thread(jcp, jcp.dimN_block, 0.1, 3.2)
            && (jcp.dimN_nb_block >= 1.5 * zendnn_get_max_threads())) {
...
status_t jit_avx512_core_f32_wino_conv_4x3_fwd_kernel::init_conf(
    ...
    if (cd.prop_kind == zendnn_forward_inference) {
        memory_desc_t expect_wei_md = weights_md;

        expect_wei_md.format_kind = format_kind::wino;
        expect_wei_md.data_type = data_type::f32;
        zendnn_wino_desc_t &wd = expect_wei_md.format_desc.wino_desc;
        wd.wino_format = zendnn_wino_wei_OBaaIBOIio;
        wd.r = 3;
        wd.alpha = 6;
...
status_t set_wsched_WEI_SDGtWo(jit_conv_winograd_conf_t &jcp) {
    ...
    auto test_MV_large_enough = [](jit_conv_winograd_conf_t &jcp) {
        ...
        size_t nthreads = zendnn_get_max_threads();
                    ...
                    if (!(jcp.dimN_block % N_blk)) {
                        for (M_blk = jcp.dimM_block; M_blk >= 1; --M_blk) {
                            if (!(jcp.dimM_block % M_blk) && blocking_ok()) {
                                ...
                                jcp.nthr = nstl::min(
                                        zendnn_get_max_threads(), jcp.tile_block);
                                return status::success;
                            }
                        }
                    }
