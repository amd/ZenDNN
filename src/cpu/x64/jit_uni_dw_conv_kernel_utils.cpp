...
status_t jit_uni_dw_conv_fwd_kernel<isa, kernel_dt>::init_conf(
    ...
    if (jcp.with_binary) {
        using namespace zendnn::impl::cpu::binary_injector_utils;
...
status_t jit_uni_dw_conv_bwd_weights_kernel<isa, kernel_dt>::init_conf(
    ...
    const auto dat_tag_blocked
            = isa == avx512_core ? nChw16c : nChw8c; // zendnn_aBcd16b
    const auto wei_tag = isa == avx512_core ? Goihw16g : Goihw8g;
...
void jit_uni_dw_conv_bwd_weights_kernel<isa, kernel_dt>::partition_nthr_nxc(
    ...
    // if necessary
    const int env_max_nthr_g = nthreads; // ZENDNN_MAX_NTHR_G
    const int env_max_nthr_mb = nthreads; // ZENDNN_MAX_NTHR_MB
    const int env_max_nthr_oh = nthreads; // ZENDNN_MAX_NTHR_OH
    const int env_min_oh_block = 1; // ZENDNN_MIN_OH_BLOCK
