...
struct reduce_balancer_t {
    ...
    reduce_balancer_t &init(int nthr, int job_size, int njobs,
        ...
        allow_nthr_in_group_ = lock_free ? true : zendnn_thr_syncable();
        nthr_ = nthr;
        job_size_ = job_size;
        njobs_ = njobs;
...
struct cpu_reducer_t {
    ...
    void init(const memory_tracking::grantor_t &scratchpad) const {
        if (balancer().nthr_per_group_ == 1 || !zendnn_thr_syncable()) return;
...
private:
    ...
    ZENDNN_DISALLOW_COPY_AND_ASSIGN(cpu_reducer_t);
};
...
private:
    ...
    ZENDNN_DISALLOW_COPY_AND_ASSIGN(cpu_reducer_2d_t);
};
...
struct cpu_accumulator_1d_t {
    ...
    ZENDNN_DISALLOW_COPY_AND_ASSIGN(cpu_accumulator_1d_t);
};
