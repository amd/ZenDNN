...
struct jit_uni_i8i8_binary_kernel_t : public i8i8_binary_kernel_t {
    ...
    void init() {
        ...
        scales[0].copy_from(pd_->attr()->scales_.get(ZENDNN_ARG_SRC_0));
        scales[1].copy_from(pd_->attr()->scales_.get(ZENDNN_ARG_SRC_1));
        ...
        for (size_t i = 0; i < tail_size_; i++) {
            switch (arg_num) {
                ...
                case ZENDNN_ARG_SRC_1:
                    ...
                    break;
                case ZENDNN_ARG_DST: uni_vpinsrb(xmm, xmm, dst_ptr(i), i); break;
    ...
    void compute_dst(int unroll, bool tail) override {
        for (int i = 0; i < unroll; i++) {
            ...
            load(vreg_tmp_src0,
                    ...
                    ZENDNN_ARG_SRC_0, src0_type, tail);
            if (!broadcast_src1_value_)
                load(vreg_tmp_src1,
                        src1_ptr(offt * types::data_type_size(src1_type)),
                        ZENDNN_ARG_SRC_1, src1_type, tail);
            ...
            perform_op(vreg_tmp_src0, vreg_tmp, vreg_scales_src0,
                    vreg_scales_src1);
            if (do_sum_) {
                load(vreg_tmp, dst_ptr(offt), ZENDNN_ARG_DST, dst_type, tail);
...
status_t jit_uni_i8i8_binary_t<src0_type, src1_type, dst_type>::execute(
    ...
    const auto src0 = CTX_IN_MEM(const char *, ZENDNN_ARG_SRC_0);
    const auto src1 = CTX_IN_MEM(const char *, ZENDNN_ARG_SRC_1);
    auto dst = CTX_OUT_CLEAN_MEM(char *, ZENDNN_ARG_DST, status);
    CHECK(status);
    ...
    scales_t scales[nargs];
    CHECK(scales[0].copy_from(pd()->attr()->scales_.get(ZENDNN_ARG_SRC_0)));
    CHECK(scales[1].copy_from(pd()->attr()->scales_.get(ZENDNN_ARG_SRC_1)));
