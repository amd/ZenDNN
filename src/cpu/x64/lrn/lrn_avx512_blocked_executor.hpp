...
template <::zendnn::impl::data_type_t d_type, typename PD_T>
...
public:
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        ...
        const auto dst = CTX_OUT_CLEAN_MEM(data_t *, ZENDNN_ARG_DST, status);
        CHECK(status);
        const auto ws = CTX_OUT_CLEAN_MEM(data_t *, ZENDNN_ARG_WORKSPACE, status);
        CHECK(status);
...
public:
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        ...
        const auto ws = CTX_IN_MEM(const data_t *, ZENDNN_ARG_WORKSPACE);
        const auto diff_src
                = CTX_OUT_CLEAN_MEM(data_t *, ZENDNN_ARG_DIFF_SRC, status);
        CHECK(status);
