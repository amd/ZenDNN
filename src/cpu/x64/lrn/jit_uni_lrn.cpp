...
static zendnn_dim_t compute_n_summands(
        zendnn_dim_t size, int ndims, const zendnn_alg_kind_t &alg_kind) {
...
status_t jit_uni_lrn_fwd_t<isa, d_type>::execute_forward(
    ...
    CHECK(status);
    auto ws = CTX_OUT_CLEAN_MEM(data_t *, ZENDNN_ARG_WORKSPACE, status);
    CHECK(status);
...
status_t jit_uni_lrn_fwd_t<isa, d_type>::pd_t::init(engine_t *engine) {
    ...
    if (desc()->prop_kind == forward_training && status == success) {
        ...
        zendnn_memory_desc_init_by_tag(&ws_md_, 4, ws_dims, d_type, dat_tag_);
    }
...
status_t jit_uni_lrn_bwd_t<isa, d_type>::execute_backward(
    ...
    auto ws = CTX_IN_MEM(const data_t *, ZENDNN_ARG_WORKSPACE);
