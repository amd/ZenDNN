...
template <typename a_type, typename b_type, typename c_type>
static zendnn_status_t pack_no_copy(gemm_info_t<a_type, b_type, c_type> *arg) {
...
template <typename a_type, typename b_type, typename c_type>
static zendnn_status_t gemm_packing_driver(int ithr, dim_t m, dim_t n, dim_t k,
        const a_type *a, const b_type *b,
        const gemm_info_t<a_type, b_type, c_type> *arg) {

    if (m <= 0 || n <= 0) return zendnn_success;

    gemm_pack_storage_t *pack_dst = arg->pack_dst;

    if (!pack_dst->is_first_thread_in_slice(ithr)) return zendnn_success;
...
template <typename a_type, typename b_type, typename c_type>
static zendnn_status_t gemm_kernel_driver(int ithr, dim_t m, dim_t n, dim_t k,
    ...
    if (mem_size > 0) {
        ...
        if (!mem) return zendnn_out_of_memory;
    }
...
template <typename a_type, typename b_type, typename c_type>
static zendnn_status_t kernel_driver_parallel_acopiedbcopy(int ithr, dim_t m,
...
static inline void set_thread_opts_nopack(int nthrs, int nthrs_spawn,
    ...
    } else if (condition_1D_copya && zendnn_thr_syncable()) {
...
template <typename a_type, typename b_type, typename c_type>
static zendnn_status_t parallel_a_copy(const int ithr, const int nthrs,
    ...
    zendnn_status_t result = zendnn_success; // Return status
    ...
    for (dim_t Bk = 0; Bk < k; Bk += sizeK, blk_k++) {
        ...
        for (dim_t Bm = 0; Bm < m; Bm += sizeM) {
            ...
            if (!a_packed)
                zendnn_thr_barrier(); // Wait for finishing parallel copy.
            ...
            if (this_result != zendnn_success) result = this_result;

            if (!a_packed)
                zendnn_thr_barrier(); // Wait for kernel computations to finish.
        }
    }
...
template <typename a_type, typename b_type, typename c_type>
static zendnn_status_t call_no_copy_sgemm(
...
template <typename a_type, typename b_type, typename c_type>
static zendnn_status_t gemm_threading_driver(
    ...
    if ((arg->m <= 0) || (arg->n <= 0)) return zendnn_success;

    if (!is_a_packed && !is_b_packed && jump_to_gemv_s8x8s32(arg))
        return zendnn_success;

    if (!is_a_packed && !is_b_packed
            && jump_to_gemm_smalln_tn(arg) == zendnn_success)
        return zendnn_success;

    if (!is_a_packed && !is_b_packed && jump_to_gemv(arg) == zendnn_success)
        return zendnn_success;

    if (is_a_packed && arg->bo != 0)
        if (!arg->a_packed->has_row_sums()) return zendnn_invalid_arguments;

    if (is_b_packed && arg->ao != 0)
        if (!arg->b_packed->has_col_sums()) return zendnn_invalid_arguments;

    auto nthr_max = zendnn_get_current_num_threads();
    ...
    if (!packing) {
        ...
        if (arg->measure_only) return zendnn_success;
    }
    ...
    if (!thread_arg) return zendnn_out_of_memory;
    ...
    int nthr_spawn = zendnn_thr_syncable() ? nthr_max : nthr_goal;
    ...
    zendnn_status_t result = zendnn_success; // Initialize to success
    for (int ithr = 0; ithr < nthr_max; ithr++) {
        if (thread_arg[ithr].result != zendnn_success) {
            result = static_cast<zendnn_status_t>(thread_arg[ithr].result);
            break;
        }
    }
    ...
    if (c_local_storage) zendnn::impl::free(c_local_storage);
    zendnn::impl::free(thread_arg);
...
zendnn_status_t gemm_driver(const char *transA, const char *transB,
    ...
    if (!args.hasKernels()) return zendnn_unimplemented;
