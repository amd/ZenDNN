...
static zendnn_status_t check_pack_get_size_input(const char *identifier,
    ...
    if (!ok) return zendnn_invalid_arguments;

    return zendnn_success;
}

static zendnn_status_t check_pack_input(const char *identifier,
        const char *transa, const char *transb, const dim_t *M, const dim_t *N,
        const dim_t *K, const float *alpha, const dim_t *lda, const dim_t *ldb,
        const void *src, void *dst) {
    if (utils::any_null(src, dst, alpha)) return zendnn_invalid_arguments;
...
static zendnn_status_t gemm_pack_driver(const char *identifier,
        const char *transa, const char *transb, const dim_t *M, const dim_t *N,
        const dim_t *K, const float *alpha, const dim_t *lda, const dim_t *ldb,
...
zendnn_status_t sgemm_pack_get_size(const char *identifier, const char *transa,
    ...
    if (!pack_sgemm_supported()) return zendnn_unimplemented;

    zendnn_status_t result;
    *size = 0;
    if (pack) *pack = true;
    ...
    *size = cblas_sgemm_pack_get_size(cblas_identifier(identifier), *M, *N, *K);
    ...
    float alpha = 1.0f;
    gemm_pack_storage_shell_t shell {zendnn_get_max_threads()};
    if (!shell.get()) return zendnn_out_of_memory;
...
zendnn_status_t gemm_bf16bf16f32_pack_get_size(const char *identifier,
    ...
    if (!pack_gemm_bf16bf16f32_supported()) return zendnn_unimplemented;
...
template <typename a_dt, typename b_dt>
zendnn_status_t gemm_x8x8s32_pack_get_size(const char *identifier,
        const char *transa, const char *transb, const dim_t *M, const dim_t *N,
        const dim_t *K, const dim_t *lda, const dim_t *ldb, size_t *size,
        bool *pack) {
    ...
    *size = 0;
    ...
    bool do_a = utils::one_of(*identifier, 'a', 'A');
    float alpha = 1.0f;
    gemm_pack_storage_shell_t shell {zendnn_get_max_threads(), do_a, !do_a};
    if (!shell.get()) return zendnn_out_of_memory;
...
template <typename a_dt, typename b_dt>
zendnn_status_t gemm_x8x8s32_pack(const char *identifier, const char *transa,
        const char *transb, const dim_t *M, const dim_t *N, const dim_t *K,
...
template <typename a_dt, typename b_dt>
zendnn_status_t gemm_x8x8s32_compute(const char *transa, const char *transb,
