...
template <>
zendnn_status_t jump_to_gemm_smalln_tn(
...
zendnn_status_t sgemm_smalln_tn(const dim_t m, const dim_t n, const dim_t k,
    ...
    if (st != zendnn_success) return st;
...
zendnn_status_t jit_avx512_core_gemm_smalln_tn_f32(const char *transa,
    ...
    int max_num_threads = zendnn_get_current_num_threads();
    ...
    if (n <= 0 || m <= 0) return zendnn_success;
    ...
    parallel(max_num_threads, [&](int ithr, int nthr) {
        ...
        if (st_thr != zendnn_success) st = st_thr;
    });
    return st;
}
