...
zendnn_status_t sgemm_nocopy_driver(const char *transa, const char *transb,
    ...
    if (utils::any_null(ker_bn, ker_b1, ker_b0)) return zendnn_runtime_error;
...
zendnn_status_t jit_avx512_common_gemm_f32(int nthrs, const char *transa,
    ...
    int nthr_max = zendnn_get_current_num_threads();
    ...
    assert(IMPLICATION(!zendnn_thr_syncable(), nthr_k == 1));
    ...
    if (nthr_k > 1) {
        ...
        if (!ompstatus_) return zendnn_out_of_memory;
    ...
    int nthr_spawn = zendnn_thr_syncable() ? nthr_max : nthr_to_use;

    std::atomic<zendnn_status_t> st(zendnn_success);
