    ...
    if (B_any_layout) {
        ...
        if (transposed_tensor_layout_tag == bgmmc.wei_tag) {
            ...
            const status_t status
                    = zendnn_memory_desc_init_by_tag(&B_md_plain, B_md.ndims,
...
status_t init_brgemm_matmul_conf(cpu_isa_t isa, brgemm_matmul_conf_t &bgmmc,
    ...
    bgmmc.isa = isa;
    bgmmc.nthr = zendnn_get_max_threads();
    ...
    bgmmc.src_zp_type = get_zp_type(attr, ZENDNN_ARG_SRC);
    bgmmc.wei_zp_type = get_zp_type(attr, ZENDNN_ARG_WEIGHTS);
    bgmmc.dst_zp_type = get_zp_type(attr, ZENDNN_ARG_DST);
...
void init_aux_values(brgemm_matmul_conf_t &bgmmc,
        ...
        const memory_desc_wrapper &dst_d) {
    bgmmc.wsp_tile_per_thr_bytes = 4 * 1024;
