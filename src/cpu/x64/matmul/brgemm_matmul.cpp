...
struct brgemm_matmul_t<isa>::brg_matmul_exec_ctx_t {
    brg_matmul_exec_ctx_t(const exec_ctx_t &ctx, const pd_t *pd, int32_t src_zp,
        ...
        data_A_ptr_ = CTX_IN_MEM(const char *, ZENDNN_ARG_SRC);
        data_B_ptr_ = CTX_IN_MEM(const char *, ZENDNN_ARG_WEIGHTS);
        data_C_ptr_ = CTX_OUT_MEM(char *, ZENDNN_ARG_DST);

        bias_ptr_ = CTX_IN_MEM(const char *, ZENDNN_ARG_BIAS);
        ...
        nthr_ = nstl::min(zendnn_get_current_num_threads(), bgmmc.nthr);
        ...
        // 1 < parallel_work_amount_ < zendnn_get_max_threads() to avoid potential
    ...
    int32_t *get_s8s8_comp_ptr(int ithr, int b, int n_blk_idx) const {
        if (!bgmmc_.s8s8_compensation_required) {
            return nullptr;
        }
    ...
    int get_base_brgemm_kernel_idx() const {
        return base_brg_ker_idx_;
    }
    ...
    int get_parallel_work_amount() const {
        return parallel_work_amount_;
    }
    int get_num_threads_for_k() const {
        return nthr_k_;
    }
    bool parallel_reduction_is_used() const {
        return nthr_k_ > 1 && bgmmc_.K_chunks > 1;
    }
    int get_num_threads_for_bmn() const {
        return nthr_bmn_;
    }
    // ithr = ithr_k * nthr_bmn + ithr_bmn
    int get_thread_idx_for_k(int ithr) const {
        if (ithr >= num_threads_used_) {
            return -1;
        }
