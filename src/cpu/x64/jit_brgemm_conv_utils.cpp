...
status_t init_jcp(jit_brgemm_conv_conf_t &jcp, cpu_isa_t isa,
    ...
    jcp.src_zero_point
            = get_zp_type(attr, ZENDNN_ARG_SRC) != brgemm_broadcast_t::none;
    jcp.dst_zero_point
            = get_zp_type(attr, ZENDNN_ARG_DST) != brgemm_broadcast_t::none;
    ...
    if (has_zero_points || jcp.s8s8_avx512) {
        const bool params_ok = IMPLICATION(has_zero_points, !is_amx(jcp.isa))
                ...
                && IMPLICATION(jcp.src_zero_point,
                        attr.zero_points_.common(ZENDNN_ARG_SRC))
                && IMPLICATION(jcp.dst_zero_point,
                        attr.zero_points_.common(ZENDNN_ARG_DST));
