...
private:
    ZENDNN_DISALLOW_COPY_AND_ASSIGN(_jit_avx512_core_f32_wino_conv_4x3_t);
};
...
struct jit_avx512_core_f32_wino_conv_4x3_fwd_t
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        auto src = CTX_IN_MEM(const float *, ZENDNN_ARG_SRC);
        auto weights = CTX_IN_MEM(const float *, ZENDNN_ARG_WEIGHTS);
...
struct jit_avx512_core_f32_wino_conv_4x3_bwd_data_t
    ...
    struct pd_t : public cpu_convolution_bwd_data_pd_t {
        ...
        status_t init(engine_t *engine) {
            bool ok = true && zendnn_thr_syncable()
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        auto diff_dst = CTX_IN_MEM(const float *, ZENDNN_ARG_DIFF_DST);
        auto weights = CTX_IN_MEM(const float *, ZENDNN_ARG_WEIGHTS);
        auto diff_src = CTX_OUT_MEM(float *, ZENDNN_ARG_DIFF_SRC);
...
struct jit_avx512_core_f32_wino_conv_4x3_bwd_weights_t : public primitive_t {
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        auto diff_dst = CTX_IN_MEM(const float *, ZENDNN_ARG_DIFF_DST);
        auto src = CTX_IN_MEM(const float *, ZENDNN_ARG_SRC);
        auto diff_weights = CTX_OUT_MEM(float *, ZENDNN_ARG_DIFF_WEIGHTS);
