...
zendnn_cpu_isa_hints_t ZENDNN_API get_cpu_isa_hints(bool soft = false);
...
static unsigned cvt2mask(zendnn_cpu_isa_hints_t hints) {
    static const std::unordered_map<zendnn_cpu_isa_hints_t, unsigned,
            std::hash<int>>
            hints_map = {{zendnn_cpu_isa_no_hints, 0},
                    {zendnn_cpu_isa_prefer_ymm, prefer_ymm_bit}};
...
static bool is_hints_bit_set(cpu_isa_bit_t hint_bit, bool soft) {
    const zendnn_cpu_isa_hints_t hints = get_cpu_isa_hints(soft);
