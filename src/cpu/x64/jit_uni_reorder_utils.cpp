...
status_t prb_init(prb_t &p, const memory_desc_t &imd, const memory_desc_t &omd,
    ...
    p.req_src_zp = *attr->zero_points_.get(ZENDNN_ARG_SRC);
    p.req_dst_zp = *attr->zero_points_.get(ZENDNN_ARG_DST);
...
void prb_dump(const prb_t &p) {
    printf("@@@ type:%s:%s ndims:%d ", zendnn_dt2str(p.itype),
            zendnn_dt2str(p.otype), p.ndims);
    for (int d = 0; d < p.ndims; ++d)
