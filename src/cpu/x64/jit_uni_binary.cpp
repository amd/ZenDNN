...
status_t jit_uni_binary_t::pd_t::init(engine_t *engine) {
    ...
    conf_.do_scale_src0 = !attr()->scales_.get(ZENDNN_ARG_SRC_0).defined()
            || !attr()->scales_.get(ZENDNN_ARG_SRC_0).has_default_values();
    conf_.do_scale_src1 = !attr()->scales_.get(ZENDNN_ARG_SRC_1).defined()
            || !attr()->scales_.get(ZENDNN_ARG_SRC_1).has_default_values();
...
void jit_uni_binary_t::execute_no_bcast_strategy(const data_t *src0,
    ...
    if (is_src_different_layouts) {
        ...
        const int nthr = zendnn_get_current_num_threads();
...
void jit_uni_binary_t::execute_bcast_per_batch_strategy(const data_t *src0,
    ...
    const dim_t nthr = nstl::min(
            nelems0_simd + has_tail, (dim_t)zendnn_get_current_num_threads());
