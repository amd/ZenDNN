...
struct jit_uni_binary_kernel_t : public binary_kernel_t {
    ...
    void init() {
        ...
        do_scale_src0_ = !pd_->attr()
                                  ->scales_.get(ZENDNN_ARG_SRC_0)
                                  .has_default_values();
        do_scale_src1_ = !pd_->attr()
                                  ->scales_.get(ZENDNN_ARG_SRC_1)
                                  .has_default_values();
...
struct jit_uni_binary_subkernel_t<sse41, src_type>
    ...
    Address get_address(const int off, const int arg_num) {
        switch (arg_num) {
            case ZENDNN_ARG_SRC_0: return src0_ptr(off);
            case ZENDNN_ARG_SRC_1: return src1_ptr(off);
            case ZENDNN_ARG_DST: return dst_ptr(off);
    ...
    void store(const Vmm &src, const int off, bool tail) {
        if (!tail)
            movups(get_address(off, ZENDNN_ARG_DST), src);
        else
            for (size_t i = 0; i < tail_size_; i++)
                pextrd(get_address(i * data_type_size_ + off, ZENDNN_ARG_DST),
                        src, i);
    }
    ...
    void compute_bcast(bool tail) override {
        if (broadcast_src1_value_)
            ...
            load(vbcast_src1_, 0, ZENDNN_ARG_SRC_1, tail);
    }
    ...
    void compute_dst(int unroll, bool tail) override {
        for (int i = 0; i < unroll; i++) {
            ...
            const Vmm vreg_tmp = Vmm(unroll + i + 1);
            const Vmm vreg_tmp_src1 = offt_src1_ ? vreg_tmp : vbcast_src1_;
            load(vreg_tmp_src0, i * offt_src0_, ZENDNN_ARG_SRC_0, tail);
            if (offt_src1_)
                load(vreg_tmp_src1, i * offt_src1_, ZENDNN_ARG_SRC_1, tail);

            perform_op(vreg_tmp_src0, vreg_tmp_src1, vreg_scales_src0_,
                    vreg_scales_src1_);
            if (do_sum_) {
                load(vreg_tmp, i * offt_src0_, ZENDNN_ARG_DST, tail);
...
status_t jit_uni_binary_t<src_type>::execute(const exec_ctx_t &ctx) const {
    ...
    const auto src0 = CTX_IN_MEM(const data_t *, ZENDNN_ARG_SRC_0);
    const auto src1 = CTX_IN_MEM(const data_t *, ZENDNN_ARG_SRC_1);
    auto dst = CTX_OUT_CLEAN_MEM(data_t *, ZENDNN_ARG_DST, status);
    CHECK(status);
    const auto &post_ops = pd()->attr()->post_ops_;
    const auto &post_ops_binary_rhs_arg_vec
            = binary_injector::prepare_binary_args(post_ops, ctx);
    const auto scales_src0 = pd()->attr()->scales_.get(ZENDNN_ARG_SRC_0).scales_;
    const auto scales_src1 = pd()->attr()->scales_.get(ZENDNN_ARG_SRC_1).scales_;
