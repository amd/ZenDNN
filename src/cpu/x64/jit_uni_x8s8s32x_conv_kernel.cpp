...
status_t jit_uni_x8s8s32x_fwd_kernel<isa>::init_conf(jit_conv_conf_t &jcp,
    ...
    jcp.dst_zero_point = !zp.has_default_values(ZENDNN_ARG_DST);
    jcp.src_zero_point = !zp.has_default_values(ZENDNN_ARG_SRC);
    jcp.zp_src_is_common
            = zp.common(ZENDNN_ARG_SRC); // otherwise, it's per-channel
    ...
    if (jcp.is_depthwise) {
        ...
        if (jcp.ngroups == 1) {
            ...
            /* For grouped convolutions, ZENDNN doesn't support padding.
    ...
    // This tuning improves performance of specific layers by upto 10% on MILAN
    // TODO: Do detailed analysis
    // FIXME: Tuned for 64C server. To be tuned for other Server with different no of Cores
    if (jcp.nb_ic ==  jcp.nb_oc) {
        max_threading_nb_oc_chunk = 3;
    }
