...
status_t jit_prelu_bwd_t::pd_t::init(engine_t *engine) {
    ...
    if (ok) {
        if (utils::one_of(bcast, prelu::bcast::per_oc_blocked,
            ...
            const auto max_num_threads = zendnn_get_max_threads();
...
status_t jit_prelu_bwd_t::execute(const exec_ctx_t &ctx) const {
    ...
    const byte *const dst_diff = CTX_IN_MEM(const byte *, ZENDNN_ARG_DIFF_DST);
    byte *const weights_diff
            = CTX_OUT_CLEAN_MEM(const byte *, ZENDNN_ARG_DIFF_WEIGHTS, status);
    CHECK(status);
    byte *const src_diff = CTX_OUT_CLEAN_MEM(byte *, ZENDNN_ARG_DIFF_SRC, status);
    CHECK(status);
    ...
    if (bcast == prelu::bcast::full) {
        ...
        const size_t max_threads = zendnn_get_max_threads();
