...
Xbyak::Address jit_prelu_forward_kernel_t::data_ptr(int arg_num, size_t offt) {
    ...
    switch (arg_num) {
        ...
        case ZENDNN_ARG_DST: return get_addr(reg_dst_, dst_dt_);
...
void jit_uni_prelu_forward_kernel_t<Vmm>::compute_dst(
    ...
    for (size_t unroll_group = 0; unroll_group < unrolling_factor;
        ...
        const Xbyak::Address weights_addr = data_ptr(ZENDNN_ARG_WEIGHTS, offset);
        ...
        io_.at(dst_dt_)->store(dst_vmm, data_ptr(ZENDNN_ARG_DST, offset), tail);
    }
}
