...
Xbyak::Address jit_prelu_backward_kernel_t::data_ptr(int arg_num, size_t offt) {
    ...
    switch (arg_num) {
        case ZENDNN_ARG_SRC: return get_addr(reg_src_, src_dt_);
        case ZENDNN_ARG_WEIGHTS: return get_addr(reg_weights_, wei_dt_);
        case ZENDNN_ARG_DIFF_SRC: return get_addr(reg_src_diff_, diff_src_dt_);
        case ZENDNN_ARG_DIFF_WEIGHTS:
            return get_addr(reg_weights_diff_, diff_wei_dt_);
        case ZENDNN_ARG_DIFF_DST: return get_addr(reg_dst_diff_, diff_dst_dt_);
...
void jit_uni_prelu_backward_kernel_t<Vmm>::compute_dst(
    ...
    for (size_t unroll_group = 0; unroll_group < unrolling_factor;
        ...
        io_.at(diff_dst_dt_)
                ->load(data_ptr(ZENDNN_ARG_DIFF_DST, offset), dst_diff_vmm, tail);
        io_.at(src_dt_)->load(data_ptr(ZENDNN_ARG_SRC, offset), src_vmm, tail);
        ...
        const auto weights_operand = get_or_load_weights(
                data_ptr(ZENDNN_ARG_WEIGHTS, offset), weights_vmm, tail);
