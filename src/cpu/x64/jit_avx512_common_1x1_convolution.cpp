...
void jit_avx512_common_1x1_convolution_fwd_t<src_type, wei_type,
    ...
    auto bias = CTX_IN_MEM(const dst_data_t *, ZENDNN_ARG_BIAS);
    auto dst = CTX_OUT_MEM(dst_data_t *, ZENDNN_ARG_DST);
    auto weights_dw = CTX_IN_MEM(
            const wei_data_t *, ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_WEIGHTS);
    auto bias_dw = CTX_IN_MEM(
            const dst_data_t *, ZENDNN_ARG_ATTR_POST_OP_DW | ZENDNN_ARG_BIAS);
...
void jit_avx512_common_1x1_convolution_bwd_weights_t::execute_backward_weights(
    ...
    auto diff_bias_in = CTX_OUT_MEM(data_t *, ZENDNN_ARG_DIFF_BIAS);
    ...
    auto ker = [&](const int ithr, const int nthr) {
        ...
        assert(IMPLICATION(
                !ready_for_async && !zendnn_thr_syncable(), jcp.nthr_mb == 1));
        ...
        if (zendnn_thr_syncable() && jcp.nthr_mb > 1) {
