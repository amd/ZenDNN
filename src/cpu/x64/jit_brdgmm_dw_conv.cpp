...
status_t brdgmm_dw_convolution_fwd_t::execute(const exec_ctx_t &ctx) const {
    ...
    const char *const __restrict src = CTX_IN_MEM(const char *, ZENDNN_ARG_SRC);
    const char *const __restrict weights
            = CTX_IN_MEM(const char *, ZENDNN_ARG_WEIGHTS);
    const char *const __restrict bias = CTX_IN_MEM(const char *, ZENDNN_ARG_BIAS);
    char *const __restrict dst = CTX_OUT_MEM(const char *, ZENDNN_ARG_DST);
