...
static status_t weights_axes_permutation(
    ...
    int perm[ZENDNN_MAX_NDIMS] {}; // deconv to conv weight permutation
    for (int d = 0; d < ZENDNN_MAX_NDIMS; ++d)
        perm[d] = d;
    nstl::swap(perm[0 + with_groups], perm[1 + with_groups]);

    return zendnn_memory_desc_permute_axes(o_md, i_md, perm);
}
...
static status_t conv_descr_create(const deconvolution_desc_t *dd,
    ...
    return conv_desc_init(cd, prop_kind, alg_kind, src_md, &c_weights_d,
            ...
            dd->padding[1], false, false, nullptr, nullptr, nullptr);
}
...
struct ref_deconvolution_fwd_t : public primitive_t {
    ...
    private:
        void init_scratchpad() {
            ...
            if (!attr()->zero_points_.has_default_values(ZENDNN_ARG_SRC)) {
        ...
        bool zero_points_ok() const {
            ...
            return IMPLICATION(!utils::one_of(src_md()->data_type, s8, u8),
                    ...
                    && attr()->zero_points_.has_default_values(ZENDNN_ARG_WEIGHTS)
