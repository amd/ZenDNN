...
struct simple_layer_normalization_fwd_t : public primitive_t {
    ...
    void reorder_stat(const exec_ctx_t &ctx, engine_t *engine,
        ...
        using namespace memory_tracking::names;
        exec_args_t r_args;
        r_args[ZENDNN_ARG_SRC] = in;
        r_args[ZENDNN_ARG_DST] = out;
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        ...
        if (pd()->stats_are_src() && reorder_) {
            reorder_stat(
                    ctx, engine, ctx.args().at(ZENDNN_ARG_MEAN), {&mean, false});
        ...
        if (!pd()->stats_are_src() && reorder_) {
            reorder_stat(
                    ctx, engine, {&mean, true}, ctx.args().at(ZENDNN_ARG_MEAN));
            reorder_stat(ctx, engine, {&variance, true},
                    ctx.args().at(ZENDNN_ARG_VARIANCE));
        }
