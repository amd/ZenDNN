...
struct ref_concat_t : public primitive_t {
    struct pd_t : public cpu_concat_pd_t {
        ...
        status_t init(engine_t *engine) {
            ...
            if (status != status::success) {
                ...
                status = zendnn_memory_desc_init_by_strides(&tent_dst_md_,
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        ...
        auto execute_reorder = [&](const std::shared_ptr<primitive_t> &reorder,
            ...
            exec_args_t r_args;
            r_args[ZENDNN_ARG_SRC] = src;
            r_args[ZENDNN_ARG_DST] = dst;
        ...
        if (pd()->use_tent_dst()) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                execute_reorder(reorders_[i],
                        ctx.args().at(ZENDNN_ARG_MULTIPLE_SRC + i),
                        {&tent_dst_i, false}, i);
            }
            ...
            execute_reorder(reorders_[n], {&tent_dst, true},
                    ctx.args().at(ZENDNN_ARG_DST), n);
        } else {
            auto &dst_mem_storage = CTX_OUT_STORAGE(ZENDNN_ARG_DST);
            for (int i = 0; i < n; ++i) {
                memory_t tent_dst_i(
