...
void compute_tile_scalar(const float *A, const float *B, float *C,
    ...
    for (int ii = 0; ii < MR && i + ii < M; ++ii) {
        for (int jj = 0; jj < NR && j + jj < N; ++jj) {
            ...
            float result = alpha * (sum + bias[j + jj]) + beta * C[(i + ii) * N + (j + jj)];
            C[(i + ii) * N + (j + jj)] = apply_post_op_scalar(result, post_op);
        }
    }
}
...
2. B Registers (Unrolled)
    ...
    Live during each k += 4 iteration

3. A Broadcast Registers
    a0 to a3: 4 registers
    Only 4 live at a time
...
5. Total: 1 (accumulator) + 4 (B) + 4 (A) + 4 (temporaries) = 13 registers
*/
__attribute__((target("avx512f")))
void compute_tile_1x16(const float *A, const float *B, float *C,
    ...
    // Main loop: process K dimension in chunks of 4 (loop unrolled for performance)
    int k = 0;
    for (; k <= K - 4; k += 4) {
        // Load 4 rows of B (each 16-wide) using masked loads to avoid out-of-bounds
        ...
        // Broadcast 4 A values for the row
        ...
        // Zen 5 tuned prefetch: prefetch A data 3 iterations ahead (4 * 3 = 12 elements)
        _mm_prefetch((const char *)&A[i * K + k + 12], _MM_HINT_T0);

        // Zen 5 tuned prefetch: prefetch B data 4 iterations ahead (4 * 4 = 16 elements)
        _mm_prefetch((const char *)&B[(k + 16) * N + j], _MM_HINT_T0);
    }

    // Remainder loop: handle leftover K values when K is not divisible by 4
    for (; k < K; ++k) {
...
3. A Broadcast Registers (Per Row)
    For each row r, we broadcast 4 values: a0 to a3
    These are reused per row, so only 4 A registers live at a time
    Since we loop over 4 rows and reuse the same names, we don’t accumulate 16 A registers — just 4 reused per row
...
5. Total: 4 (accumulators) + 4 (B) + 4 (A reused) + 4 (temporaries) = 16 registers
*/
__attribute__((target("avx512f")))
void compute_tile_4x16(const float *A, const float *B, float *C,
    ...
    for (; k <= K - 4; k += 4) {
        ...
        for (int r = 0; r < 4; ++r) {
            ...
            // Zen 5 tuned prefetch: prefetch A data 3 iterations ahead (4 * 3 = 12 elements)
            _mm_prefetch((const char *)&A[(i + r) * K + k + 12], _MM_HINT_T0);
        }
...
3. A Broadcast Registers (Per Row)
    For each row r, we broadcast 4 values: a0 to a3
    These are reused per row, so only 4 A registers live at a time
    Since we loop over 6 rows and reuse the same names, we don’t accumulate 24 A registers — just 4 reused per row
...
5. Total: 6 (accumulators) + 4 (B) + 4 (A reused) + 4 (temporaries) = 18 registers
*/
__attribute__((target("avx512f")))
...
3. A Broadcast Registers (Per Row)
    For each row r, we broadcast 4 values: a0 to a3
    These are reused per row, so only 4 A registers live at a time
    Since we loop over 8 rows and reuse the same names, we don’t accumulate 32 A registers — just 4 reused per row
...
5. Total: 8 (accumulators) + 4 (B) + 4 (A reused) + 4 (temporaries) = 20 registers
*/
__attribute__((target("avx512f")))
...
2. B Registers (Unrolled by 2)
    b0 to b1: 2 registers
    Live during each kk iteration

3. A Broadcast Registers
    a0 to a1: 2 registers
...
5. Total: 12 (accumulators) + 2 (B) + 2 (A) + 4 (temporaries) = 20 registers
*/
__attribute__((target("avx512f")))
void compute_tile_12x16(const float *A, const float *B, float *C,
    ...
    // Main loop: process K dimension in chunks of 8, with inner loop unrolled in steps of 2
    int k = 0;
    for (; k <= K - 8; k += 8) {
        for (int kk = 0; kk < 8; kk += 2) {
            // Load 2 rows of B (each 16-wide) using masked loads
            ...
            for (int r = 0; r < 12; ++r) {
                ...
                // Broadcast 2 A values for row r
                __m512 a0 = _mm512_set1_ps(a_ptr[0]);
                __m512 a1 = _mm512_set1_ps(a_ptr[1]);
...
void matmul_avx512_fp32_registerBlocking(const float *A, const float *B,
    ...
    zendnnInfo(ZENDNN_CORELOG,
               "Runing Reference matmul_avx512_fp32_registerBlocking with MR x NR = : ", MR,
               " x ", NR,
               " kernel");
...
void matmul_avx512_fp32_registerBlocking_auto(const float *A, const float *B,
    ...
    zendnnInfo(ZENDNN_CORELOG,
               "Running Reference matmul_avx512_fp32_registerBlocking with dynamic MR x NR  kernel");
...
void matmul_avx512_fp32_registerBlocking_batch(const float *A, const float *B,
    ...
    zendnnInfo(ZENDNN_CORELOG,
               "Running Reference batched matmul_avx512_fp32_registerBlocking_batch with MR x NR = ",
               MR,
               " x ", NR,
...
void matmul_avx512_fp32_registerBlocking_auto_batch(const float *A,
    ...
    zendnnInfo(ZENDNN_CORELOG,
               "Running Reference batched matmul_avx512_fp32_registerBlocking_auto with dynamic MR x NR kernel, BATCH = ",
               BATCH);
