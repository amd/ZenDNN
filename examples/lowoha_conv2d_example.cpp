...
int run_lowoha_conv2d_fp32_test() {
    try {
        ...
        status_t status = conv_direct(input.data(), filter.data(), bias.data(),
                output.data(),
...
int run_lowoha_conv2d_bf16_test() {
    try {
        ...
        std::fill(
                filter_bf16.begin(), filter_bf16.end(), 0x3F00); // 0.5f in BF16
        std::fill(
                add_tensor.begin(), add_tensor.end(), 0x3E80); // 0.25f in BF16
        ...
        add_op.dims = {static_cast<int64_t>(batch), static_cast<int64_t>(out_h),
                static_cast<int64_t>(out_w), static_cast<int64_t>(out_c)};
        ...
        status_t status = conv_direct(input_bf16.data(), filter_bf16.data(),
                bias.data(), output_bf16.data(),
                true, // is_weights_const (enables caching)
                params);
...
int run_lowoha_depthwise_conv2d_test() {
    try {
        ...
        log_info("Input: [", batch, ", ", in_h, ", ", in_w, ", ", in_c, "]");
        log_info("Filter: [", filter_h, ", ", filter_w, ", ", in_c, ", ",
                depth_multiplier, "]");
        ...
        std::vector<float> filter(
                filter_h * filter_w * in_c * depth_multiplier, 0.5f);
        std::vector<float> bias(out_c, 0.0f);
        std::vector<float> output(batch * out_h * out_w * out_c, 0.0f);
        ...
        log_info("Output[0] = ", output[0]);
        log_info("Depthwise reduces parameters by ~",
                (out_c / depth_multiplier), "x");
...
int run_lowoha_strided_conv2d_test() {
    try {
        ...
        log_info("Spatial dimensions downsampled from ", in_h, "x", in_w,
                " to ", out_h, "x", out_w);
...
int run_lowoha_dilated_conv2d_test() {
    try {
        ...
        log_info("Filter: [", filter_h, ", ", filter_w, ", ", in_c, ", ", out_c,
                "]");
        log_info("Dilation: ", dilation,
                " (effective kernel: ", effective_filter_h, "x",
                effective_filter_w, ")");
        log_info(
                "Output: [", batch, ", ", out_h, ", ", out_w, ", ", out_c, "]");
