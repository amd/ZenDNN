...
/** @fn run_lowoha_matmul_w4a8_static_test
 *  @brief Demonstrates W4A8 static (pre-quantized s8 activations + s4 weights).
 *
 *  Static A8 + W4: s8 source with @c dynamic_quant=false and populated
 *  @c src_scale.buff / @c wei_scale.buff. Do not set @c dynamic_quant=true
 *  on an s8 source.
 *
 *  Configuration:
 *    - Input: S8 [M, K], @c dynamic_quant = false, @c dtypes.compute = s8
 *    - Weights: S4 packed [K, N], @c is_weights_const = true
 *    - Output: BF16 [M, N]
 *    - @c src_scale: per-group {M, G} (logical dims, even if transA)
 *    - @c wei_scale: per-group {G, N} with K divisible by G and (K/G) % 4 == 0
 *    - No @c src_zp / @c wei_zp
 */
 ...
 *  @brief Demonstrates sequential GEMM using group_matmul_direct API.
 *
