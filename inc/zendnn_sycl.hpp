...
#ifndef ZENDNN_SYCL_HPP
#define ZENDNN_SYCL_HPP
...
#include "zendnn.hpp"
#include "zendnn_sycl.h"

#if defined(__INTEL_LLVM_COMPILER)
#if (__INTEL_LLVM_COMPILER < 20220000)
#define ZENDNN_SYCL_INTEROP_USE_SYCL121
#endif
...
#ifdef ZENDNN_SYCL_INTEROP_USE_SYCL121
...
/// @addtogroup zendnn_api_sycl_interop SYCL interoperability API
...
enum class memory_kind {
    ...
    usm = zendnn_sycl_interop_usm,
    /// Buffer memory allocation kind.
    buffer = zendnn_sycl_interop_buffer,
};
...
inline zendnn_sycl_interop_memory_kind_t convert_to_c(memory_kind akind) {
    return static_cast<zendnn_sycl_interop_memory_kind_t>(akind);
}
...
inline engine make_engine(
    ...
    zendnn_engine_t aengine;
    error::wrap_c_api(zendnn_sycl_interop_engine_create(&aengine,
...
inline cl::sycl::context get_context(const engine &aengine) {
    void *ctx_ptr;
    error::wrap_c_api(
            zendnn_sycl_interop_engine_get_context(aengine.get(), &ctx_ptr),
...
inline cl::sycl::device get_device(const engine &aengine) {
    void *dev_ptr;
    error::wrap_c_api(
            zendnn_sycl_interop_engine_get_device(aengine.get(), &dev_ptr),
...
inline stream make_stream(const engine &aengine, cl::sycl::queue &aqueue) {
    zendnn_stream_t astream;
    error::wrap_c_api(
            zendnn_sycl_interop_stream_create(&astream, aengine.get(), &aqueue),
...
inline cl::sycl::queue get_queue(const stream &astream) {
    void *queue_ptr;
    error::wrap_c_api(
            zendnn_sycl_interop_stream_get_queue(astream.get(), &queue_ptr),
...
/// memory object is not equal to zendnn::sycl_interop::memory_kind::buffer.
///
...
cl::sycl::buffer<T, ndims> get_buffer(const memory &amemory) {
    ...
    // implement the check.
    zendnn_sycl_interop_memory_kind_t ckind;
    error::wrap_c_api(
            zendnn_sycl_interop_memory_get_memory_kind(amemory.get(), &ckind),
            "could not get SYCL buffer object");

    void *handle_ptr;
    error::wrap_c_api(zendnn_memory_get_data_handle(amemory.get(), &handle_ptr),
            "could not get SYCL buffer object");
...
void set_buffer(memory &amemory, cl::sycl::buffer<T, ndims> &abuffer) {
    ...
    error::wrap_c_api(zendnn_sycl_interop_memory_set_buffer(amemory.get(),
...
///   equal to zendnn::sycl_interop::memory_kind::usm, or
/// - zendnn::sycl_interop::set_buffer() has been called, if @p memory_kind is
///   equal to zendnn::sycl_interop::memory_kind::buffer.
///
...
inline memory make_memory(const memory::desc &memory_desc,
    ...
    error::wrap_c_api(
            zendnn_sycl_interop_memory_create(&c_memory, &memory_desc.data,
...
/// <index, memory object> pairs. The index must be one of the `ZENDNN_ARG_*`
/// values such as `ZENDNN_ARG_SRC`, and the memory must have a memory descriptor
/// matching the one returned by
/// #zendnn::primitive_desc::query_md(#query::exec_arg_md, index) unless using
...
inline cl::sycl::event execute(const zendnn::primitive &aprimitive,
    ...
    error::wrap_c_api(
            zendnn_sycl_interop_primitive_execute(aprimitive.get(), astream.get(),
...
#ifdef ZENDNN_SYCL_INTEROP_USE_SYCL121
#undef ZENDNN_SYCL_INTEROP_USE_SYCL121
#pragma clang diagnostic pop
#endif

#endif // ZENDNN_SYCL_HPP
