...
## Whole-call selection (`ZENDNNL_GRP_MATMUL_ALGO`)
...
| **4** | W8A8 fused MoE | Attempts the private symmetric per-channel W8A8 gate/up → SiLU → down fast path for either BF16-dynamic or caller-prequantized S8 input. Global 4 applies to both phases; under global AUTO, a matching phase setting of 4 applies only to that phase. | Flat |
| **5** | Per-expert | Parallel-for over experts, 1 thread each. Best when experts ≥ threads. | Flat |
| **6** | Multilevel | CCD-aware nested OMP. Multi-CCD for large M, round-based for small M. | Nested |
...
num_ops > 256                          → ALGO 1 (N-tile planner capacity ceiling)
...
Each phase knob accepts `{0,1,2,3,4,5,6}`. Values `{1,2,3,5,6}` pin that generic ALGO for the phase; `0` defers to the legacy 3-rule cascade. Value `4` is an AUTO-only W8A8 request for the matching phase. If the W8A8 attempt declines, the setting behaves as the inherited default—not as an explicit generic pin—so default-policy refinements still run for both BF16 and caller-prequantized S8.
...
**Safety clamps:** when the chosen tiled algo is not legal for the call, auto-select falls back to ALGO 1 — ALGO 2 requires `m_tile_safe`, ALGO 3 requires `n_tile_safe`. The inherited prompt default starts at ALGO 2 and the inherited decode default starts at ALGO 3, but existing default-only regime refinements may choose another generic scheduler before those clamps. Any declined phase-4 request follows exactly this inherited path; main's generic fused-MoE implementation validates BF16 and caller-prequantized-S8 calls under its own contract. The per-phase knobs (`ZENDNNL_GRP_MATMUL_AUTO_PROMPT_ALGO`, `ZENDNNL_GRP_MATMUL_AUTO_DECODE_ALGO`) are internal tuning overrides; production deployments normally leave them unset.

Tiled algos (2/3) require row-major layout, uniform per-expert dtypes, and standard unpacked A and B (`mem_format_a=='n'`, `mem_format_b=='n'`, `pack_format_b==0`). ALGO 2 (M-tile) additionally supports the full quantization stack (WOQ, static W8A8, and dynamic INT8 with row-local src granularity — `{M, 1}` per-token or `{M, G}` per-group) and most post-ops; it rejects packed B, softmax/pooling, and dynamic-quant with non-row-local src granularity (per-tensor / per-column / per-channel). ALGO 3 (N-tile) supports dynamic or caller-prequantized INT8 when the source scale is row-local (`{M, 1}` per-token or `{M, G}` per-group) and the weight scale resolves the K axis (per-channel `{N}` / `{1, N}` or per-group `{G, N}`). `check_n_tile_extra` validates the pairing: per-token source works with per-channel or per-group weight scales, while per-group source requires a matching per-group weight scale. Per-group execution uses the AOCL per-tile path rather than the custom INT8 microkernel. Unsupported scale pairings, incompatible zero-points, pre-reordered weights, and unsupported buffer-bearing post-ops fall back safely. See the detailed quantization matrix and `check_n_tile_extra` for the exact boundaries. Auto-select routes prompt-phase shapes to ALGO 2 by default (when `m_tile_safe` holds) and falls back to ALGO 1 when the safety gate fails.

Set the whole-call mode via environment variable: `ZENDNNL_GRP_MATMUL_ALGO=0|1|2|3|4|5|6`.
...
|---|---|---|
| `ZENDNNL_GRP_MATMUL_ALGO` | `0` (auto) | Select group matmul execution. `0` = auto, `{1,2,3,5,6}` = global generic scheduler pins, `4` = attempt the two-mode W8A8 fused-MoE fast path in both phases. Any eligibility decline falls back to generic AUTO; main's generic fused-MoE path supports BF16 and caller-prequantized S8. |
| `ZENDNNL_GRP_MATMUL_AUTO_DECODE_ALGO` | `3` | AUTO-only decode setting (`max active M <= 32`). `4` requests W8A8 for decode. After a decline it inherits the full decode default policy. `{1,2,3,5,6}` are generic pins and `0` selects the legacy cascade. |
| `ZENDNNL_GRP_MATMUL_AUTO_PROMPT_ALGO` | `2` | AUTO-only prompt setting (`max active M > 32`). `4` requests W8A8 for prompt. After a decline it inherits the full prompt default policy. `{1,2,3,5,6}` are generic pins and `0` selects the legacy cascade. |
...
8. **Tiled algos (2/3)**: Both require row-major layout, uniform per-expert dtypes, and standard unpacked A/B. ALGO 2 (M-tile, `m_tile_safe`) additionally supports the full quantization stack — weight-only (S4 sym, U4 asym), static W8A8 (per-tensor / per-channel / per-group / per-token), and **dynamic INT8** (BF16/F32 source quantised at runtime; per-token `{M, 1}` and per-group `{M, G}` activation scales only) — and most post-ops. It blocks: packed B, softmax/pooling, and dynamic-quant with non-row-local source granularity because the per-thread reorder would race on shared scale/zero-point buffers and use slice-local statistics. ALGO 3 (N-tile, `n_tile_safe`) supports buffer-free element-wise post-ops plus the documented row-local INT8 quantization pairings, including supported per-token/per-group source and per-channel/per-group weight scales. It rejects incoherent granularity pairings, incompatible zero-points, pre-reordered weights, and other buffer-bearing post-ops, then falls back safely according to the selected policy.
